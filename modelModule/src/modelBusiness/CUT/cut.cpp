...
namespace CUT
{
	...
	// inputMesh 是输入Mesh，是总数居
	// inputFace 是输入面数据索引
	// newMesh的顶点是按照面的顺序每三个一组排列
	// newMesh有问题，数据是完全开放的
...
#define min_value 1e-4
		...
		for (unsigned int i = 0; i < inputMesh->faces.size(); i++)
		{
			...
			if (Point0 == 0.0f && Point1 == 0.0f && Point2 == 0.0f)
			{
				...
				GEOM::Vector vNow(v3.x, v3.y, v3.z);
				vNow.normalize();
				GEOM::Vector vUp(normal.x, normal.y, normal.z);
				vUp.normalize();
				if (vNow.dot(vUp) > 0)
				{
