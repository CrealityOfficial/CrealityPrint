...
{
	Read3MF::Read3MF(const std::string& file_name,const std::string& backup_path)
		:impl(new Read3MFImpl(file_name))
	{
		impl->set_backup_path(backup_path);
	}
	...
	bool Write3MF::write_scene_2_3mf(const Scene3MF& scene, ccglobal::Tracer* tracer,bool is_tmp_project)
	{
		return impl->write_scene_2_3mf(scene, tracer,is_tmp_project);
	}
	void Write3MF::set_backup_path(const std::string path)
	{
		impl->set_backup_path(path);
	}
