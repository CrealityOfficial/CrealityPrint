...
Item {
    ...
    Popup {
        id: menu
        width: 235 * screenScaleFactor + (textLength > 8 ? 100 * screenScaleFactor : 0) 
        height: (64 + (root.contextList.length / 2) * 18)* screenScaleFactor 
        ...
        function trackTo(target) {
            ...
            let globalPos = target.mapToItem(target.parent, 0, 0)
            x = globalPos.x - 11
