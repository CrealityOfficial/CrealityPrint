...
namespace creative_kernel
{
	...
	PrintMachine::PrintMachine(QObject* parent, const MachineData& data, PrintType pt)
		...
		, m_data(data)
	{
        m_PrintMaterialModel = new PrintMaterialModel(this);
        if (!m_ModelProxy)
        {
    ...
    QList<QString> PrintMachine::topMaterial()
    {
        return m_data.top_material;
    }
    ...
    QString PrintMachine::inheritBaseName()
    {
        return m_inherits_from.mid(0, m_inherits_from.lastIndexOf("-"));
    }
    ...
    void PrintMachine::setCurrBedType(const QString& curr_bed_type)
    {
        ...
        if (m_oldBedType == curr_bed_type)
        {
            return;
        }
        m_PlateSetting->add("curr_bed_type", curr_bed_type, true);
        m_oldBedType = m_PlateSetting->value("curr_bed_type", "");
        writeCacheCurrentBedType(uniqueName(), curr_bed_type);
    ...
    bool PrintMachine::canUpload3mf()
    {
        QStringList machine_list{"F008"};
        if (!machine_list.contains(m_data.codeName))
        {
            return false;
        }
        for (int i = 0; i < m_extruders.count(); i++)
        {
            if (m_extruders[i]->curMaterial()->isUserDef())
            {
                return false;
            }
        }
    ...
    void PrintMachine::added(bool isInitialize)
    {
        ...
        m_oldBedType = readCacheCurrentBedType(machine_unique_name);

        const auto default_bed_type = m_PlateSetting->value("curr_bed_type", "");
        if (m_oldBedType.isEmpty())
        {
            m_oldBedType = m_settings->value("curr_bed_type", default_bed_type);
        }

        if (m_oldBedType != default_bed_type)
        {
            m_PlateSetting->add("curr_bed_type", m_oldBedType, true);
        }
        ...
        //materials
        //1.读取注册表
        QStringList storedMaterialNames = readCacheSelectMaterials(uniqueName());
        if(storedMaterialNames.empty())
        {
            storedMaterialNames = m_data.top_material;
        }
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            for (const QString& selectMaterialName : selectMaterialNames)
            {
               ...
               if (!meta.isUserDef && !storedMaterialNames.isEmpty() && !storedMaterialNames.contains(meta.id))
               {
                   continue;
               }
        ...
        std::sort(defaultFileNames.begin(), defaultFileNames.end(), [](QStringView left, QStringView right) {
            static const QRegularExpression EXPRESSION{ QStringLiteral(R"(^\d+(\.\d+)?)") };

            const auto match_left = EXPRESSION.match(left);
            const auto match_right = EXPRESSION.match(right);
            if (match_left.hasMatch() && match_right.hasMatch()) {
                const auto diameter_left = match_left.capturedView(0).toFloat();
                const auto diameter_right = match_right.capturedView(0).toFloat();
    ...
    PrintExtruder* PrintMachine::getExtruder(int index)
    {
        ...
        if (index < 0 || index >= m_extruders.size())
        {
            return nullptr;
        }
    ...
    void PrintMachine::resyncExtruders(const QStringList& rfid, const QVariantList& color, const QStringList& syncIdList, const QStringList& stateList)
    {
        QList<int> indexes;
        for (int index = 0; index < stateList.size(); ++index)
        {
            QString state = stateList.at(index);
            if (state == "2" && !rfid.at(index).contains("RFID"))
            {
                indexes << index;
            }
        }

        m_SyncExtruderInfos.clear();
        isSyncedExtruderInfo = true;

        //int synExtruderCount = rfid.count();
        int synExtruderCount = indexes.count();
        ...
        for (int index = 0; index < rfid.count(); ++index)
        {
            QString materialName = findMaterialByRFID(rfid.at(index));
            QString curState = stateList.at(index);

            if (rfid.at(index).contains("RFID"))
            {
                curState = "1";
            }
            if (curState == "0")
            ...
            {
                materialName = findMaterialByRFID(rfid.at(index));
            }

            if (indexes.contains(index))
            {
                int syncIndex = indexes.indexOf(index);

                QList<PrintExtruder*> extruders = this->extruders();
                auto new_extruder = extruders.at(syncIndex);

                new_extruder->setMaterial(materialName);
                new_extruder->setSyncId(syncIdList.at(index));
                new_extruder->setMaterialIndex(index);
                writeCacheExtruderMaterialIndex(uniqueName(),syncIndex,materialName);
                setExtruderColor(syncIndex, color.at(index).toString());
            }

            SyncExtruderInfo extruderInfo;
            extruderInfo.materialName = materialName;
            extruderInfo.materialRFID = rfid.at(index);
            extruderInfo.color = color.at(index).toString();
            QString tempId = syncIdList.at(index);
            QString num = tempId.at(0);
            QString letter = tempId.at(1);
            QVector<QString> array = { "A", "B", "C", "D" };
            extruderInfo.boxIndex = syncIdList.at(index) == "00" ? "EXT" : num + array.at(letter.toInt());
            m_SyncExtruderInfos.append(extruderInfo);
        }

        const QList<MaterialData>& allMetas = materialMetas();

        for (MaterialData md : allMetas)
        {
            QString strID = md.id;
        }
    ...
    void PrintMachine::savePrinterSlot(QList<QVariantMap> data)
    {
        m_resyncMap = data;
    }


    void PrintMachine::syncExtruders(const QStringList& rfid, const QVariantList& color, QList<QVariantMap> data)
    {
        if (!data.count())
            return;
        QList<QVariantMap> filteredData;
        std::copy_if(data.begin(), data.end(), std::back_inserter(filteredData), [this](const QVariantMap& map) {
            return map["color"] != "#00000000"&& this->findMaterialByRFID(map["name"].toString())!="";
            });
        while (m_extruders.size()> filteredData.size())
        {
            this->removeExtruder();
        }
         m_SyncExtruderInfos.clear();
        ...
        {
            auto item = filteredData.at(index);
            QString id = item.value("syncId").toString();
            QString name = item.value("name").toString();
            QString materialName = findMaterialByRFID(name);
            QString color = item.value("color").toString();
            bool isMapId = false;
            if (m_extruders.size()<index+1)
            {
                this->addExtruder();
                auto new_extruder = m_extruders.at(index);
                new_extruder->setMaterial(materialName);
                new_extruder->setSyncId(id);
            }
            setExtruderColor(index, color);
            for (const QVariantMap& item1 : m_resyncMap)
            {
                QString id1 = item1.value("syncId").toString();
                QString name1 = item1.value("name").toString();
                QString materialName1 = findMaterialByRFID(name1);
                QString color1 = item1.value("color").toString();
                if (id == id1)
                {
                    isMapId = true;
                    for (auto ex : m_extruders)
                    {
                        QString exId = ex->syncId();
                        QString exColor = ex->color().name();
                        if (exId == id)
                        {
                            if (materialName != materialName1)
                                ex->setMaterial(materialName);
                            if(color!= color1)
                                ex->setColor(color);
                        }
                    }
            ...
            SyncExtruderInfo extruderInfo;
            extruderInfo.materialName = materialName;
            extruderInfo.materialRFID = name;
            extruderInfo.color = color;
            QString tempId = id;
            QString num = tempId.at(0);
            QString letter = tempId.at(1);
            QVector<QString> array = { "A", "B", "C", "D" };
            extruderInfo.boxIndex = num + array.at(letter.toInt());
            m_SyncExtruderInfos.append(extruderInfo);
        }
        setFilamentColor(color);
        m_extrudersModel->refresh();
    ...
    void PrintMachine::addMachineMaterial(const QStringList& rfid, const QVariantList& color)
    {
        ...
        for (int i = 0; i < rfid.count(); i++)
        {
            ...
            for (PrintMaterial* material : m_materials)
            {
                if (material->rfid() == rfid.at(i))
                {
                    pMaterial = material;
                    materialName = findMaterialByRFID(rfid.at(i));
                    break;
                }
            }
            MaterialData data;
            ...
            data.isMachineDef = true;

            PrintMaterial* materialObj = new PrintMaterial(inheritsFrom(), data, this);
            m_materials.push_front(materialObj);
            if (m_materialsModel) {
                m_materialsModel->insertMaterial(materialObj, m_materials.indexOf(materialObj));
            }
            emit materialsNameChanged();
        }
    ...
    int PrintMachine::extruderMaterialIndex(int extruderIndex)
    {
        ...
        /*    QString materialName = m_extruders.at(extruderIndex)->materialName();
            ...
            int eIndex = m_extruders.at(extruderIndex)->materialIndex();
            return eIndex;
        }
        return -1;
    }
    ...
    void PrintMachine::setExtruderMaterial(int extruderIndex, int materialIndex)
    {
        ...
        if (extruder)
        {
            extruder->setMaterialIndex(materialIndex);
        }

        if (isSyncedExtruderInfo && materialIndex < m_SyncExtruderInfos.size())
        {
            //同步的
            auto new_extruder = extruders.at(extruderIndex);
            QVector<PrintMachine::SyncExtruderInfo> infos = syncedExtrudersInfo();
            SyncExtruderInfo info = infos[materialIndex];
            materialName = info.materialName;
            setExtruderColor(extruderIndex, info.color);
            new_extruder->setSyncId(info.materialRFID);
        }
        else
        {//非同步的
            materialIndex = isSyncedExtruderInfo ? materialIndex - m_SyncExtruderInfos.size() : materialIndex;
            materialName = m_materials.at(materialIndex)->uniqueName();
            //setExtruderMaterial(extruderIndex, materialName);
        }
    ...
    void PrintMachine::setExtruderColor(int extruderIndex, const QString& extruderColor)
    {
        ...
        m_extruders[extruderIndex]->setColor(color);
         QString str = m_extruders[extruderIndex]->color().name();
    ...
    void PrintMachine::addExtruder()
    {
        if (m_extruders.count() >= MATERIAL_COLORS_NUM)
            return;
        PrintExtruder* extruder = new PrintExtruder(uniqueName(), m_extruders.count()-1, this, false, getNextExtruderColor(), m_extruders.isEmpty()? nullptr : m_extruders[0]->settings()->copy());
        ...
        if (!isContant)
        {
            for (auto data : m_SupportMeterialsDataList)
            {
                if (data->name == rfid)
                {
                    ...
                    QList<MaterialData*> dataList = m_PrintMaterialModel->materialList();
                    int index = 0;
                    for (auto data : dataList)
                    {
                        if (materialName == data->uniqueName())
                        {
                            break;
                        }
                        ++index;
                    }
                    //begin同步的耗材每天加的话要自动添加
                    MaterialCenter* mc = getKernel()->materialCenter();
                    QStringList types = mc->types();
                    if (types.contains(data->type))
                    {
                        m_ModelProxy->filterMaterialType(data->type, true);
                    }
                    m_ModelProxy->setChecked(true, index);
                    finishMeterialSelect(0);
                    m_ModelProxy->confirm();
                    //end
                }
            }
        }
    ...
    void PrintMachine::removeExtruder()
    {
        ...
        this->_update_model_filament(m_extruders.size() + 1);
    }
    ...
    {
        if (isSyncedExtruderInfo)
        {
            SyncExtruderInfo sei = m_SyncExtruderInfos.at(index);
            QString name = sei.materialName;
    ...
    int PrintMachine::addUserMaterial(const QString& materialModel, const QString& userMaterialName, int extruderNum)
    {
        ...
        data.diameter = pMaterial->diameter();
        data.isUserDef = true;
        m_UserMaterialsData.append(data);
        ...
        if (isSyncedExtruderInfo)
        {
            material_index += m_SyncExtruderInfos.size();
        }
        return material_index;
    ...
    Q_INVOKABLE void creative_kernel::PrintMachine::deleteUserMaterial(const QString& materialName, int index)
    {
        ...
        for (auto materialObj : m_materials)
        {
            if (materialObj && materialObj->uniqueName() == materialName)
            {
                ...
                materialObj = nullptr;*/
                int dIndex = 0;
                for (int dIndex = 0; dIndex < m_UserMaterialsData.count(); ++dIndex)
                {
                    if (m_UserMaterialsData.at(dIndex).uniqueName() == materialName)
                    {
                        m_UserMaterialsData.removeAt(dIndex);
                        break;
                    }
                }
    ...
    void PrintMachine::handleLegacy()
    {
        QString legacy_key = "ensure_vertical_shell_thickness";
        if (m_globalSetting->hasKey(legacy_key))
        {
            auto value = m_globalSetting->value(legacy_key, "");
            if (value == "1")
            {
                m_globalSetting->add(legacy_key, "ensure_all", true);
            }
            if (value == "0")
            {
                m_globalSetting->add(legacy_key, "ensure_moderate", true);
            }
        }
    }
    ...
    QList<QSharedPointer<us::USettings>> PrintMachine::createCurrentExtruderSettings()
    {
        ...
        us::USettings* user_settings;
        if (!m_extruders.isEmpty())
        {
            settings = m_extruders[0]->settings();
            user_settings = m_extruders[0]->userSettings();
        }
        for (PrintExtruder* extruder : m_extruders)
        {
            ...
            extruderSetting->merge(user_settings->copy());
	...
	void PrintMachine::_update_model_filament(int numFilaments)
	{
		ModelSpace* space = getModelSpace();
		QList<ModelGroup*> mGroup = space->modelGroups();
		for (auto& group : mGroup)
		{
			int group_mat_index = group->defaultColorIndex();
            if(group_mat_index >= numFilaments-1)//update group
            {
                group->discardDefaultColor();
                group_mat_index = group->defaultColorIndex();
            }

			QList<ModelN*> models = group->models();
			for (auto& model : models)
			{
				int mat_index = model->defaultColorIndex();
                if(mat_index >= numFilaments-1)
                {
                    model->setDefaultColorIndex(group_mat_index);
                }
			}
    ...
    bool PrintMachine::isSyncedExtruder()
    {
        return isSyncedExtruderInfo;
    }

    QVector<PrintMachine::SyncExtruderInfo>& PrintMachine::syncedExtrudersInfo()
    {
        return m_SyncExtruderInfos;
    }

    void PrintMachine::resetState()
    {
        isSyncedExtruderInfo = false;
        m_SyncExtruderInfos.clear();
    }
