...
namespace creative_kernel {
  ...
  namespace {
    ...
    const auto ADVANCE_SETTING_DEFAULT_KEY{
        QStringLiteral("profile_setting/advance_setting_default") };
    ...
    auto JsonToTreeModel(
      ...
      load_properties = [](const json_node_t& json_node, const model_node_t& model_node) -> void {
      ...
      load_children = [&load_node](const json_node_t&  json_node,
                                   const model_node_t& model_node) -> void {
  ...
  UiParameterManager::UiParameterManager(QPointer<ParameterManager> parameter_manager,
      ...
      , initialized_{ false }
      , message_context_{ std::make_unique<ParameterMessageContext>() }
      ...
      , advance_setting_enabled_{ isAdvanceSettingDefault() }  {
  ...
  auto UiParameterManager::isAdvanceSettingDefault() -> bool {
    return qtuser_core::VersionSettings{}.value(ADVANCE_SETTING_DEFAULT_KEY, false).toBool();
  }

  auto UiParameterManager::setAdvanceSettingDefault(bool is_default) -> void {
    qtuser_core::VersionSettings settings{};
    if (settings.value(ADVANCE_SETTING_DEFAULT_KEY, false).toBool() == is_default) {
      return;
    }

    settings.setValue(ADVANCE_SETTING_DEFAULT_KEY, is_default);
    advanceSettingDefaultChanged();
  }

  auto UiParameterManager::isAdvanceSettingEnabled() -> bool {
    return advance_setting_enabled_;
  }

  auto UiParameterManager::setAdvanceSettingEnabled(bool enabled) -> void {
    if (advance_setting_enabled_ != enabled) {
      advance_setting_enabled_ = enabled;
      advanceSettingEnabledChanged();
    }
  }
  ...
  auto UiParameterManager::saveProcessCustomizedKeys() -> void {
    ...
    const std::function<void(const node_t&)> node_handler =
      [&keys, &node_handler](const node_t& node) mutable {
  ...
  auto UiParameterManager::clearOverrideParameter() -> bool {
    ...
    switch (engine_type_) {
      ...
      case SliceFlow::EngineType::OCRA: {
        override_tree_model_->forEach([focused_material](const auto& fork) {
          fork->forEach([focused_material](const auto& leaf) {
            if (leaf->isOverrode()) {
  ...
  auto UiParameterManager::generateSearchContext(
    ...
    const std::function<QString(const node_t&)> translate_node = [&translate](const node_t& node) {
      std::stack<QString> translation_stack;
      node_t current_node = node;
      while (!current_node->isRootNode()) {
        const auto label = current_node->getLabel();
        const auto translation = current_node->getTranslation();
        translation_stack.push(!translation.isEmpty() ? translation : translate(label));
        current_node = current_node->parentNode();
      }

      QString translation;
      while (!translation_stack.empty()) {
        if (!translation.isEmpty()) {
          translation.push_back(QStringLiteral(" : "));
        }
        translation.push_back(std::move(translation_stack.top()));
    ...
    const std::function<void(const node_t&)> node_handler =
      [&datas, &node_handler, &translate_node](const node_t& node) mutable {
  ...
  auto UiParameterManager::getMessageContext() const -> QPointer<ParameterMessageContext> {
    return message_context_.get();
  }
  ...
  auto UiParameterManager::loadUiImageMap() -> bool {
    ...
    const auto file_path =  QStringLiteral("%1/image/map.json")
        .arg(getKernel()->globalConst()->getUiParameterDirPath());
