...
namespace creative_kernel
{
    ...
    void ParameterManager::initialize()
    {
        ...
        m_MaterialLinkageModel = new MaterialLinkageModel();
    ...
    void ParameterManager::addUserMachine(QObject* machineTemplate, const QStringList& materials, const QStringList& processes)
    {
        ...
        for (const auto& entry : material_template_dir.entryInfoList(QDir::Files | QDir::NoDotAndDotDot)) {
            ...
            {
                auto material = qobject_cast<PrintMaterial*>(machine_src->materialObject(entry.completeBaseName()));
                if (!material)
                {
                    continue;
                }
                ...
                data.name = material_name + "-copy";
                data.diameter = material->diameter();
                data.isUserDef = true;
                QString dstFile = QString("%1%2/%3-%4.json").arg(uniqueName).arg("Materials").arg(data.name).arg(material_diameter);
                template_file.copy(dstFile);
                saveMateriaMeta(data, machineName);
            }
        }
        ...
        for (const auto& item : machine_src->settings()->hashSettings()){
            auto key = item->key().toStdString();
            if (engine_data_object.HasMember(key.c_str())){
                continue;
            }
            auto value = item->str().toStdString();
            engine_data_object.AddMember(rapidjson::Value(key.c_str(), allocator), rapidjson::Value(value.c_str(), allocator), allocator);
        }
        ...
        if (machine_src->extruderCount()){
            auto extruder = machine_src->extruders().at(0);
            for (const auto& item : extruder->settings()->hashSettings()) {
                auto key = item->key().toStdString();
                if (extruder_engine_data_object.HasMember(key.c_str())) {
                    continue;
                }
                auto value = item->str().toStdString();
                extruder_engine_data_object.AddMember(rapidjson::Value(key.c_str(), allocator), rapidjson::Value(value.c_str(), allocator), allocator);
            }
        }
    ...
    QString ParameterManager::currentMachineBaseName()
    {
        return m_currentMachine ? m_currentMachine->baseName() : QString("INVALID MACHINE");
    }
    ...
    QStringList ParameterManager::printDiffKeys(int printerIndex1, int printerIndex2)
    {
        ...
        if (printerIndex1 < 0 || printerIndex2 < 0 || printerIndex1 > printList.count() -1 || printerIndex2 > printList.count() - 1)
            return QStringList();
    ...
    void ParameterManager::saveCurrentProjectSettings(const QString& filename)
    {
        ...
        for (int i = 0; i < curExtruderCount(); i++)
        {
            ...
            if (material && material->userSettings())
            {
                for (auto setting : material->userSettings()->hashSettings())
                {
                    QString v = setting->str();
                    QString key = setting->key();
        ...
        document.AddMember("printer_model", rapidjson::Value(machine->inheritBaseName().toStdString().c_str(), allocator), allocator);
        ...
        document.AddMember("printer_settings_id", rapidjson::Value(machine->baseName().toStdString().c_str(), allocator), allocator);
        ...
        auto funcAddString = [&needEscape, &document, &allocator](ParameterBase* machine, QStringList machine_parameter_keys) {
            for (const auto& key : machine_parameter_keys)
            {
                ...
                if(key=="wipe_tower_x")
                {
                    auto tower_x_array = rapidjson::Value{ rapidjson::Type::kArrayType };
                    v = getAllWipeTowerPositionX(",");
                    QStringList vs = v.split(",");
                    for(auto t: vs)
                    {
                        tower_x_array.PushBack(rapidjson::Value(t.toStdString().c_str(), allocator), allocator);
                    }
                    
                    document.AddMember(rapidjson::Value(key.toStdString().c_str(), allocator), rapidjson::Value(tower_x_array, allocator), allocator);
                    continue;    
                }
                if(key=="wipe_tower_y")
                {
                    auto tower_y_array = rapidjson::Value{ rapidjson::Type::kArrayType };
                    v = getAllWipeTowerPositionY(",");
                    QStringList vs = v.split(",");
                    for(auto t: vs)
                    {
                        tower_y_array.PushBack(rapidjson::Value(t.toStdString().c_str(), allocator), allocator);
                    }
                    
                    document.AddMember(rapidjson::Value(key.toStdString().c_str(), allocator), rapidjson::Value(tower_y_array, allocator), allocator);
                    continue;    
                }
                if(!machine->userSettings())
                {
                    continue;
                }
    ...
    void ParameterManager::loadProjectSettings(const QString& filename,std::vector<float>& wipe_tower_x,std::vector<float>& wipe_tower_y)
    {
        MachineData data;
        ...
        if (document.HasMember("printer_variant") && document["printer_variant"].IsString())
        {
         ...
         if (document.HasMember("wipe_tower_x") && document["wipe_tower_x"].IsArray())
        {
            auto wipe_tower_x_array = document["wipe_tower_x"].GetArray();
            for (size_t i = 0; i < wipe_tower_x_array.Size(); i++)
            {
                if(wipe_tower_x_array[i].IsString())
                {
                    wipe_tower_x.push_back(std::stof(wipe_tower_x_array[i].GetString()));
                }
                if(wipe_tower_x_array[i].IsNumber())
                {
                    wipe_tower_x.push_back(wipe_tower_x_array[i].GetFloat());
                }
                
            }
        }
        if (document.HasMember("wipe_tower_y") && document["wipe_tower_y"].IsArray())
        {
            auto wipe_tower_y_array = document["wipe_tower_y"].GetArray();
            for (size_t i = 0; i < wipe_tower_y_array.Size(); i++)
            {
                if(wipe_tower_y_array[i].IsString())
                {
                    wipe_tower_y.push_back(std::stof(wipe_tower_y_array[i].GetString()));
                }
                if(wipe_tower_y_array[i].IsNumber())
                {
                    wipe_tower_y.push_back(wipe_tower_y_array[i].GetFloat());
                }
            }
        }
        
        data.inherits_from = printer_model;
        data.baseName = printer_settings_id;
        QString baseName;
        for (const auto& machine_data : m_machineDatas)
        {
            if (machine_data.codeName == printer_model)
            {
                baseName = machine_data.baseName;
                break;
            }
        }
        if (baseName != printer_settings_id)
        {
            data.codeName = printer_settings_id;
        }
        else
        {
        ...
        clearPath(QString("%1/%2/Materials").arg(_pfpt_user_parampack).arg(machine_unique_name));
        clearPath(QString("%1/%2/Processes").arg(_pfpt_user_parampack).arg(machine_unique_name));
        data.isUser = true;
        data.is_import = true;
        rapidjson::Document docJsonFile;
        docJsonFile.SetObject();
        ...
        for (const auto& key : profile_parameter_keys)
        {
			...
			if (document.HasMember(ckey) && document[ckey].IsArray())
			{
				auto replaceAll = [](std::string& str, const std::string& oldSubstr, const std::string& newSubstr)
				{
					size_t pos = 0;
					while ((pos = str.find(oldSubstr, pos)) != std::string::npos) {
						str.replace(pos, oldSubstr.length(), newSubstr);
						pos += newSubstr.length(); // 移动查找的位置到新替换后的子串之后
					}
					return str;
				};
				...
				for (size_t i = 0; i < arrayObj.Size(); i++)
				{
                    std::string ckey_str = arrayObj[i].GetString();
					if (!value.empty())
					{
						if (ckey_str.find("\n") != std::string::npos)
						{
        ...
        unsigned extruder_size = 0;
        if (document.HasMember("extruder") && document["extruder"].IsArray())
        {
            extruder_size = document["extruder"].Size();
        }
        ...
        if (document.HasMember("filament_settings_id") && document["filament_settings_id"].IsArray())
        {
            ...
            int size = std::max(extruder_size, array.Size());
            for (size_t i = 0; i < size; i++)
            {
                QString material_name;
                if (i < array.Size())
                {
                    material_name = array[i].GetString();
                }
                else
                {
                    material_name = "Generic PLA";
                }
                print_materials << QString("%1-1.75").arg(material_name);
            }
        }
    ...
    bool ParameterManager::exportAllCurrentSettings(const QString& file_path)
    {
        ...
        for (int i = 0; i < m_currentMachine->materialCount(); ++i)
        {
            auto material = m_currentMachine->materialAt(i);
            if (!material)
            {
                continue;
            }
            if (material->isUserDef() || !material->userSettings()->isEmpty())
            {
                auto material_file = QString("%1/%2.json").arg(material_path).arg(material->uniqueName());
        ...
        for (int i = 0; i < m_currentMachine->profilesCount(); ++i)
        {
            auto profile = m_currentMachine->profile(i);
            if (!profile)
            {
                continue;
            }
            if (!profile->isDefault() || !profile->userSettings()->isEmpty())
            {
    ...
    void creative_kernel::ParameterManager::loadParamFromProject(const QString& zip_file_name)
    {
        ...
        if (!machine_exist)
        {
            ...
            QFile jsonFile(machine_file_name);
            if (!jsonFile.exists() || !jsonFile.open(QIODevice::ReadOnly))
            {
                return;
            }
            ...
            {
                const auto& metadObj = document["metadata"].GetObj();
                if (metadObj.HasMember("inherits_from") && metadObj["inherits_from"].IsString())
                {
                    data.inherits_from = metadObj["inherits_from"].GetString();
                }
            }
            if (!data.inherits_from.isEmpty())
            {
                QStringList paramdirs = { "Materials","Overrides","Processes" };
                for (const auto& dir : paramdirs)
                {
                    QString srcPath = QString("%1/%2/%3").arg(_pfpt_user_parampack).arg(data.uniqueName()).arg(dir);
                    QString dstPath = QString("%1/%2/%3").arg(_pfpt_user_parampack).arg(data.inherits_from).arg(dir);
                    mkMutiDir(dstPath);
                    qtuser_core::copyDir(srcPath, dstPath, true);
        ...
        if (machine)
        {
            ...
            inherits_from = machine->inheritsFrom();
        }
        else
        {
            MachineData data;
            loader.loadDefault(defaultMachineFile(machine_name), machineSettingsOrg, extruderSettingsOrg, &data);
            settings_org = machineSettingsOrg;
            if (data.inherits_from.isEmpty())
            {
                inherits_from = machine_name;
            }
            else
            {
        ...
        saveDiff(settings_org, settings_prj, machineCoverFile(inherits_from));
        ...
        for (const auto& entry : material_dir.entryInfoList(QDir::Files | QDir::NoDotAndDotDot)) {
            ...
            auto material_names = fetchFileNames((QString("%1/%2/Materials").arg(_pfpt_default_parampack).arg(inherits_from)), true);
            if (!material_names.contains(material_name))
            {
                QFile(material_path).copy(defaultMaterialFile(inherits_from, material_name, true));
            }
            else
            {
                ...
                loader.loadDefault(defaultMaterialFile(inherits_from, material_name), materialSettingsOrg);
                materialSettingsOrg->appendDefault(getMetaMaterialKeys());

                saveDiff(materialSettingsOrg, materialSettingsPrj, materialCoverFile(inherits_from, material_name));
            }
        }
        ...
        auto profile_names = fetchOfficialProfileNames(inherits_from);
        for (const auto& entry : process_dir.entryInfoList(QDir::Files | QDir::NoDotAndDotDot)) {
            ...
            {
                QFile(process_path).copy(userProfileFile(inherits_from, process_name));
            }
            else
            {
                ...
                processSettingsOrg->appendDefault(getMetaProfileKeys());
        ...
        QMetaObject::invokeMethod(this, [this, machine_name = std::move(machine_name), machine = std::move(machine)]() {
            if (machine)
            {
                machine->reload();
            ...
            {
                _addImportMachine(_createMachine(machine_name));
            }
            }, Qt::ConnectionType::QueuedConnection);
    ...
    {
        sensorAnlyticsTrace("Add & Manage 3D Machine",
                            QString("Add Machine [%1]").arg(uniqueName));
    ...
    void ParameterManager::_setCurrentMachine(PrintMachine *machine, bool needEmit)
    {
        ...
        if (m_currentMachine)
        {
            ...
            disconnect(m_currentMachine, &PrintMachine::curProfileIndexChanged, this, &ParameterManager::onCurrentProfileChanged);
            disconnect(m_currentMachine, &PrintMachine::extruderAdded, this, &ParameterManager::onExtruderAdded);
            disconnect(m_currentMachine, &PrintMachine::extruderRemoved, this, &ParameterManager::onExtruderRemoved);
            ...
            const auto extruders = m_currentMachine->extruders();
            for (auto index = 0; index < extruders.size(); ++index)
            {
                auto* extruder = extruders.at(index);
                disconnect(extruder, &PrintExtruder::colorChanged, this, &ParameterManager::onExtruderColorChanged);
            }
        ...
        connect(m_currentMachine, &PrintMachine::curProfileIndexChanged, this, &ParameterManager::onCurrentProfileChanged);
        connect(m_currentMachine, &PrintMachine::extruderAdded, this, &ParameterManager::onExtruderAdded);
        connect(m_currentMachine, &PrintMachine::extruderRemoved, this, &ParameterManager::onExtruderRemoved);

        QVariantList hexColor;
        const auto extruders = m_currentMachine->extruders();
        for (auto index = 0; index < extruders.size(); ++index)
        {
            auto* extruder = extruders.at(index);
            connect(extruder, &PrintExtruder::colorChanged, this, &ParameterManager::onExtruderColorChanged);
            hexColor.push_back(extruder->color());
        }
        m_materialCenter->setFlushingParams(m_materialCenter->getFlushingVolumes(hexColor));
    ...
    QObject* ParameterManager::materialLinkageModelObject()
    {
        return materialLinkageModel();
    }

    MaterialLinkageModel* ParameterManager::materialLinkageModel()
    {
    ...
    void ParameterManager::onParameterEdited() {
      ...
      for (auto* parameter_base : parameter_bases) {
        ...
        const auto slot = [this, parameter_base](const QString& key) {
          ...
          triggerAutoSave(QList<ModelGroup*>(), AutoSaveJob::PRINTSETTINGS);
        };
    ...
    void ParameterManager::onCurrentProfileChanged()
    {
        auto profile = qobject_cast<PrintProfile*>(m_currentMachine->currentProfileObject());
        if (profile && m_currentProfile != profile)
        {
            if (m_currentProfile)
            {
                disconnect(m_currentProfile, &PrintProfile::enablePrimeTowerChanged, this, &ParameterManager::enablePrimeTowerChanged);
                disconnect(m_currentProfile, &PrintProfile::currentProcessPrimeVolumeChanged, this, &ParameterManager::currentProcessPrimeVolumeChanged);
                disconnect(m_currentProfile, &PrintProfile::currentProcessPrimeTowerWidthChanged, this, &ParameterManager::currentProcessPrimeTowerWidthChanged);
                disconnect(m_currentProfile, &PrintProfile::currentProcessLayerHeightChanged, this, &ParameterManager::currentProcessLayerHeightChanged);
                disconnect(m_currentProfile, &PrintProfile::enableSupportChanged, this, &ParameterManager::enableSupportChanged);
                disconnect(m_currentProfile, &PrintProfile::currentProcessSupportFilamentChanged, this, &ParameterManager::currentProcessSupportFilamentChanged);
                disconnect(m_currentProfile, &PrintProfile::currentProcessSupportInterfaceFilamentChanged, this, &ParameterManager::currentProcessSupportInterfaceFilamentChanged);
                disconnect(m_currentProfile, &PrintProfile::printSequenceChanged, this, &ParameterManager::printSequenceChanged);
            }
    ...
    void ParameterManager::onExtruderAdded(QObject* extruder)
    {
        if (!m_materialCenter || !m_currentMachine)
        {
            return;
        }
        QList<QColor> hexColor;
        const auto extruders = m_currentMachine->extruders();
        for (auto index = 0; index < extruders.size(); ++index)
        {
            auto* extruder = extruders.at(index);
        ...
        m_materialCenter->addExtruder(hexColor);
    }

    void ParameterManager::onExtruderRemoved(QObject* extruder)
    {
        if (!m_materialCenter || !m_currentMachine)
        {
            return;
        }
        m_materialCenter->removeExtruder();
    }

    void ParameterManager::onExtruderColorChanged()
    {
        if (!m_materialCenter || !m_currentMachine)
        {
            return;
        }
        PrintExtruder* extruder = qobject_cast<PrintExtruder*>(sender());
        const auto extruders = m_currentMachine->extruders();
        QList<QColor> hexColor;
        for (auto index = 0; index < extruders.size(); ++index)
        {
        ...
        m_materialCenter->setExtruderColor(hexColor, extruder->index());
    }
