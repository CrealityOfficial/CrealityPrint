...
namespace creative_kernel
{
	...
	{
#if SHOW_ZSEAM_POINTS
		m_retractionPoints->setEnabled(m_showRetraction && m_gcodeVisulaType == gcode::GCodeVisualType::gvt_structure);
#endif
	}
	...
	{
#if SHOW_ZSEAM_POINTS
		m_zSeamsPoints->setEnabled(m_showZseam && m_gcodeVisulaType == gcode::GCodeVisualType::gvt_structure);
#endif
	}

	void SlicePreviewScene::setUnretractVisible()
	{
		m_unretractPoints->setEnabled(m_showUnretract && m_gcodeVisulaType == gcode::GCodeVisualType::gvt_structure);
	}
...
#if SHOW_ZSEAM_POINTS
		m_zSeamsPoints->setModelMatrix(matrix);
		m_retractionPoints->setModelMatrix(matrix);
		m_unretractPoints->setModelMatrix(matrix);
#endif
	}
