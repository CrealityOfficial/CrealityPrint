...
namespace creative_kernel
{
	...
	RemotePrinterManager::RemotePrinterManager()
	{
		...
		t = std::thread(&RemotePrinterManager::uploadThread, this);
		t.detach();
	...
	void RemotePrinterManager::pushFile(const RemotePrinter& printer, const std::string& fileName, const std::string& filePath, std::function<void(std::string, std::string, float)> callback, std::function<void(std::string, std::string, int)> errorCallback)
	{
		switch (printer.type)
		{
			case RemotePrinerType::REMOTE_PRINTER_TYPE_LAN:
			{
				...
				std::future<void> future = m_pLanPrinterInterface->sendFileToDevice(ba.data(), fileName, filePath,
					[=](float progress) {
						if (callback)
						{
			...
			case RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER:
			{
				auto future = m_pKlipperInterface->sendFileToDevice(printer.ipAddress.toStdString(), printer.moonrakerPort, fileName, filePath, 
					[=](float progress) {
						if (callback)
						{
							callback(printer.macAddress.toStdString(), std::string(), progress);
			...
			case RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408:
			{
				auto future = m_pKlipper4408Interface->sendFileToDevice(printer.ipAddress.toStdString(), 80, fileName, filePath,
					[=](float progress) {
						if (callback)
						{
	...
	void RemotePrinterManager::feedInOrOutMaterial(const std::string& strIp, const std::string& boxId, const std::string& materialId, const std::string& isFeed)
	{
		m_pKlipper4408Interface->feedInOrOutMaterial(strIp, boxId, materialId, isFeed);
	}

	void RemotePrinterManager::refreshBox(const std::string& strIp, const std::string& boxId, const std::string& materialId)
	{
		m_pKlipper4408Interface->refreshBox(strIp, boxId, materialId);
	}

	void RemotePrinterManager::modifyMaterial(const std::string& strIp, const QVariantMap& materialParams)
	{
		m_pKlipper4408Interface->modifyMaterial(strIp, materialParams);
	}

	void RemotePrinterManager::getColorMap(const std::string& strIp, const std::string& filePath)
	{
		m_pKlipper4408Interface->getColorMap(strIp, filePath);
	}

	void RemotePrinterManager::boxConfig(const std::string& strIp, const std::string& autoRefill, const std::string& cAutoFeed, const std::string& cSelfTest)
	{
		m_pKlipper4408Interface->boxConfig(strIp, autoRefill, cAutoFeed, cSelfTest);
	}
	...
	void RemotePrinterManager::uploadFile(const RemotePrinter& printer, const std::string& fileName, const std::string& filePath, std::function<void(std::string, std::string, float)> callback, std::function<void(std::string, std::string, int)> errorCallback)
	{
		m_mtxUpload.lock();
		m_listUploads.push_back(std::make_tuple(printer,fileName,filePath,callback,errorCallback));
		m_mtxUpload.unlock();
	}
	...
	void creative_kernel::RemotePrinterManager::uploadThread()
	{
		while (!m_bExit)
		{
			m_mtxUpload.lock();
			if (m_listUploads.empty())
			{
				m_mtxUpload.unlock();
				std::this_thread::sleep_for(std::chrono::milliseconds(100));
				continue;
			}
			auto task = m_listUploads.front();
			m_listUploads.pop_front();
			m_mtxUpload.unlock();
			auto printer = std::get<0>(task);
			auto filename = std::get<1>(task);
			auto filePath = std::get<2>(task);
			auto call_back = std::get<3>(task);
			auto error_call_back = std::get<4>(task);
			printer.uploadCanceled = false;
			pushFile(printer,filename,filePath,call_back,error_call_back);
			printer.uploadCanceled = false;
		}
	}
