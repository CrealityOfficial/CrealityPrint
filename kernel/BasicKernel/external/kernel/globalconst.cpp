...
namespace creative_kernel {
  ...
  QStringList GlobalConst::languages() {
      return m_lanNames;
  }
...
#  endif
  ...
  /*
  //document : https://gcmomk3i2c.feishu.cn/wiki/HFLyw9jU7iIYx9kaHDEc5TDVnTc
  */
  ...
  QString GlobalConst::userFeedbackWebsite() {
    ...
    QString strLanguageType = setting.value("language_type", "en").toString();
    setting.endGroup();
    if (strLanguageType.toLower().contains("zh_cn")) {
      website += "&language=zh-cn";
    }
    else if (strLanguageType.toLower().contains("zh_tw"))
    {
        website += "&language=zh-tw";
    }
    else {
        website += "&language=en";
    }
    website += "&software=CrealityPrint";
    QString versionNum = QStringLiteral("%1.%2.%3").arg(QString::number(PROJECT_VERSION_MAJOR))
        .arg(QString::number(PROJECT_VERSION_MINOR))
        .arg(QString::number(PROJECT_VERSION_PATCH));
    website += QStringLiteral("&version=%1").arg(versionNum);

    auto device_id = QCryptographicHash::hash(QHostInfo::localHostName().toUtf8(),
        QCryptographicHash::Algorithm::Md5).toHex();
    QString cloudId = CloudAccountService().lock()->getUserId();
  ...
  QString GlobalConst::calibrationTutorialWebsite() {
    ...
    QString strLanguageType = setting.value("language_type", "en").toString();
    setting.endGroup();
    if (strLanguageType == "en") {
      return "https://wiki.creality.com/en/software/update-released/Basic-introduction/calibration-tutorial";
    } else {
      return "https://wiki.creality.com/zh/software/update-released/Basic-introduction/calibration-tutorial";
    }
  }

   QString GlobalConst::useTutorialWebsite() {
    qtuser_core::VersionSettings setting;
    setting.beginGroup("language_perfer_config");
    QString strLanguageType = setting.value("language_type", "en").toString();
    setting.endGroup();
    if (strLanguageType == "en") {
      return "https://wiki.creality.com/en/software/update-released";
    } else {
      return "https://wiki.creality.com/zh/software/update-released";
    }
  }
  ...
  QString GlobalConst::officialWebsite() {
    ...
    QString strLanguageType = setting.value("language_type", "en").toString();
    setting.endGroup();
    if (strLanguageType == "zh_CN") {
      return "https://www.creality.cn/about";
    } else if (strLanguageType == "zh_TW") {
      return "https://www.creality.cn/about";
    } else{
  ...
  QString GlobalConst::multiColorGuide() {
    qtuser_core::VersionSettings setting;
    setting.beginGroup("language_perfer_config");
    QString strLanguageType = setting.value("language_type", "en").toString();
    setting.endGroup();
    if (strLanguageType == "zh_CN") {
      return "https://wiki.creality.com/zh/software/update-released/Basic-introduction/CFS-tutorial";
    } else if (strLanguageType == "zh_TW") {
      return "https://wiki.creality.com/zh/software/update-released/Basic-introduction/CFS-tutorial";
    } else{
        return "https://wiki.creality.com/en/software/update-released/Basic-introduction/CFS-tutorial";
    }
    return "";
  }
