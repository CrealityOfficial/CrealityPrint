...
bool fix_model_by_win10_sdk_gui(Slic3r::ModelObject &model_object, int volume_idx, /*GUI::ProgressDialog& progress_dialog,const wxString& msg_header,*/  std::string& fix_result, ccglobal::Tracer* tracer)
{
    ...
    while (! finished) {
		...
		if (tracer)
		{
			tracer->progress(progress.percent/100.0f);
		}
