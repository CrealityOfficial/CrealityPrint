...
//static const float DEFAULT_FLUSH_TIME = 86.f;
            ...
            if (line == reserved_tag(ETags::First_Line_M73_Placeholder) || line == reserved_tag(ETags::Last_Line_M73_Placeholder)) {
                ...
                for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
                    ...
                    if (mode == PrintEstimatedStatistics::ETimeMode::Normal || machine.enabled) {
                        ...
                        if (!s_IsBBLPrinter)
                        {
                            //float _time = machine.time + DEFAULT_FLUSH_TIME * total_filamentchanges;
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    { EProducer::OrcaSlicer, "generated by Creality_Print" },
...
#endif // ENABLE_GCODE_VIEWER_DATA_CHECKING
    if (post_process){
        float flush_times = flush_time * m_result.print_statistics.total_filamentchanges;
