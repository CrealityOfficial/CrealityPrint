...
#define NOT_GENERATE_TIMELAPSE                                      "not_generate_timelapse"
    ...
    private:
        ...
        struct TimeProcessor
        {
            ...
            void post_process(const std::string& filename, std::vector<GCodeProcessorResult::MoveVertex>& moves, std::vector<size_t>& lines_ends, size_t total_layer_num,float filament_used = 0.0f);
        };
    ...
    public:
        ...
        void finalize(bool post_process,float filament_used = 0.0f);
        float layer_time();
        float layer_flow();
