...
bool use_default_real_value = true;
double minimal_jerk_value = 0.1;
...
// Only add a newline in case the current G-code does not end with a newline.
                                                  ...
                                                  double                             z) const
    {
        ...
        if (should_travel_to_tower) {
            ...
            gcode += gcodegen.retract();
            if (!is_creality_ams() || !hasToolChange)
            {
                gcodegen.m_avoid_crossing_perimeters.use_external_mp_once();
...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    const ConfigOption* _option = print.full_print_config().option("printer_model");
    if (_option != nullptr)
    {
        std::string printer_model = _option->serialize();
    ...
    std::string cersion = SLIC3R_APP_NAME;
    cersion += " "; 
    const ConfigOption* version = print.full_print_config().option("software_version");
    if (version != nullptr)
    {
        cersion += version->serialize();
    }

    file.write_format("; generated by %s on %s\n", cersion.c_str(), Slic3r::Utils::local_timestamp().c_str());
...
//        print_object_instances_ordering = sort_object_instances_by_max_z(print);
    ...
    this->placeholder_parser().set("layer_num", 0);
...
*/
    ...
    bench_debug_seamplacer(&print, &m_seam_placer);
    ...
    GCodeOutputStream                                                   &output_stream)
{
    ...
    const auto cooling = tbb::make_filter<LayerResult, std::string>(slic3r_tbb_filtermode::serial_in_order,
        [&cooling_buffer = *this->m_cooling_buffer.get(), &print](LayerResult in) -> std::string {

        bench_debug_generate((Slic3r::Print*)&print, in.layer_id, in.gcode);
    ...
    const auto output = tbb::make_filter<std::string, void>(slic3r_tbb_filtermode::serial_in_order,
        [this, &first_layer,&print,&output_stream,&processor = this->m_processor, &output_index](std::string s) {

        int real_index = output_index++;
        bench_debug_fanmove((Slic3r::Print*)&print, real_index, s);
        ...
        bench_debug_output((Slic3r::Print*)&print, real_index, s);
        }
    );

    int fan_mover_index = 0;
    const auto fan_mover = tbb::make_filter<std::string, std::string>(slic3r_tbb_filtermode::serial_in_order,
            [&fan_mover = this->m_fan_mover, &config = this->config(), &writer = this->m_writer, &print, &fan_mover_index](std::string in)->std::string {

        bench_debug_cooling((Slic3r::Print*)&print, fan_mover_index++, in);
    ...
    const bool                               prime_extruder)
{
    ...
    if (m_wipe_tower)
        m_wipe_tower->set_creality_ams(print.getCrealityAMS());
    ...
    this->placeholder_parser().set("layer_num", new ConfigOptionInt(m_layer_index + 1));
    ...
    if (first_layer) {
        ...
        }else if(use_default_real_value){
            gcode += m_writer.set_jerk_xy(minimal_jerk_value);
        }
        	...
        	if (m_config.slowdown_for_curled_perimeters){
        	 	ConfigOptionFloatsOrPercents dynamic_overhang_speeds(
            		{(m_config.get_abs_value("overhang_1_4_speed", ref_speed) < 0.5) ?
                 		...
                 		FloatOrPercent{m_config.get_abs_value("overhang_1_4_speed", ref_speed) , false},
            	 	(m_config.get_abs_value("overhang_2_4_speed", ref_speed) < 0.5) ?
                 		FloatOrPercent{100, true} :
                 		FloatOrPercent{m_config.get_abs_value("overhang_2_4_speed", ref_speed) , false},
             		(m_config.get_abs_value("overhang_3_4_speed", ref_speed) < 0.5) ?
                 		FloatOrPercent{100, true} :
                 		FloatOrPercent{m_config.get_abs_value("overhang_3_4_speed", ref_speed), false},
             		(m_config.get_abs_value("overhang_4_4_speed", ref_speed) < 0.5) ?
                 		FloatOrPercent{100, true} :
                 		FloatOrPercent{m_config.get_abs_value("overhang_4_4_speed", ref_speed), false},
                	(m_config.get_abs_value("overhang_4_4_speed", ref_speed) < 0.5) ?
                 		FloatOrPercent{100, true} :
                 		FloatOrPercent{m_config.get_abs_value("overhang_4_4_speed", ref_speed), false},
                	(m_config.get_abs_value("overhang_4_4_speed", ref_speed) < 0.5) ?
...
std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z/* = DBL_MAX*/)
{
    ...
    if (this->on_first_layer()) {
        ...
        if (m_config.default_jerk.value > 0 && m_config.initial_layer_jerk.value > 0) {
            ...
            jerk_to_set = minimal_jerk_value;
        }
    } else {
