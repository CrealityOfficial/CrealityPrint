...
QImage UrlToImage(const QString& url, size_t width, size_t height) {
  ...
  auto const qrcode_width = std::max<size_t>(qrcode->width, 1);
  auto const point_width = static_cast<double>(width) / static_cast<double>(qrcode_width);
  auto const point_height = static_cast<double>(height) / static_cast<double>(qrcode_width);
...
QString QrcodeImageProvider::getUrl() const {
  return url_;
}

void QrcodeImageProvider::setUrl(const QString& url) {
  url_ = url;
}
