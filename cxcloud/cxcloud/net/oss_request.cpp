...
void StartOssRequest(QPointer<OssRequest> request) {
  if (!request) { return; }
  request->state_ = OssRequest::State::INFO;
  HttpSyncPost(request.data());
}
...
void OssRequest::callWhenResponsed() {
  ...
  if (state_ == State::INFO) {
    const auto result = getResponseJson()[QStringLiteral("result")];
...
QString UploadFileRequest::getOnlineFilePath(const QString& local_path) {
  ...
  if (iter == local_online_path_map_.cend()) {
    ...
    auto online_path = QStringLiteral("%1/%2.%3")
                         .arg(prefix_path)
                         .arg(qtuser_core::calculateFileMD5(local_path))
                         .arg(suffix_);

    iter = local_online_path_map_.insert(std::make_pair(local_path, std::move(online_path))).first;
  }
...
void UploadFileRequest::handleRealTask() {
  ...
  for (const auto& local_file_path : file_list_) {
    auto progress_callback = [this, initialized = false, last_uploaded_size = 0ll](
      ...
      auto progress =
        static_cast<double>(uploaded_size_) / static_cast<double>(total_size_) * 100.0;
    ...
    oss_wrapper::OSSInterface::uploadObject(
      getOnlineFilePath(local_file_path), local_file_path, progress_callback, result_callback);
  }
}
...
bool UploadFileRequest::initOssConfig() const {
  ...
  return oss_wrapper::OSSInterface::initOSSConfig(
    getAccessKeyId(), getSecretAccessKey(), getEndPoint(), bucket, getSessionToken());
}
...
void GcodeUploadRequest::handleRealTask() {
  ...
  initOssConfig();
  m_fileKey = getFilePrefixPath() + "/" + qtuser_core::calculateFileMD5(m_fileName) + ".gcode.gz";
  oss_wrapper::OSSInterface::uploadObject(m_fileKey, m_fileName, progress, result);
}

QString GcodeUploadRequest::getRealUrlTail() const {
  return "";
}
