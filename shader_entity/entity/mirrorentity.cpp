...
void MirrorEntity::setSpaceBox(const Box3D& space_box) {
  ...
  transform_->setTranslation(space_box_.center());
...
void MirrorEntity::attach() {
    ...
    {
        camera_controller_->screenCamera()->addCameraObserver(this);
    }
}

void MirrorEntity::detach() {
  if (camera_controller_ != nullptr) {
      camera_controller_->screenCamera()->removeCameraObserver(this);
  }
}

void MirrorEntity::onCameraChanged(ScreenCamera* camera) 
{
    onCameraChanged(camera->camera()->position(), camera->camera()->upVector());
}
...
void MirrorEntity::adaptScale()
{
    if (camera_controller_)
    {
        QVector3D center = space_box_.center();
        qtuser_3d::ScreenCamera* screenCamera = camera_controller_->screenCamera();
        QPoint p0 = screenCamera->mapToScreen(center);
        QSize screenSize = screenCamera->size();
        QSize opSize = screenSize * 0.09f;
        QPoint p1;
        ...
        QVector3D c0;
        qtuser_3d::lineCollidePlane(center, screenCamera->camera()->viewVector(), ray, c0);

        float scale = (c0 - center).length();
