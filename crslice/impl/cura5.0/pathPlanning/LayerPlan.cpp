...
#include "magic/raft.h" // getTotalExtraLayers
...
LayerPlan::LayerPlan(const SliceDataStorage& storage,
    ...
    , layer_thickness(layer_thickness)
    , has_prime_tower_planned_per_extruder(application->extruderCount(), false)
    , current_mesh("")
    , last_extruder_previous_layer(start_extruder)
    , last_planned_extruder(&application->extruders()[start_extruder])
    ...
    , fill_lineWidth_diff(0)
{
    ...
    extruder_plans.reserve(application->extruderCount());
    ...
    extruder_plans.back().slowdown_level = application->extruders()[current_extruder].settings.get<bool>("cool_infill_speed_slowdown_first") ? 1 : 0;
                            ...
                            const float overhang_distance)
{
    ...
    auto getFanSpeed = [&](const GCodePathConfig& config)
    {
		...
		const double cool_fan_speed_max = settings.get<double>("cool_fan_speed_max");
        if (!cool_fan_enabled)
        {
        ...
        if (config.isBridgePath() || overhang_distance > overhang_threshold * config.getLineWidth())
            return std::min(cool_fan_speed_max, double(overhang_fan_speed * 100.));
        return GCodePathConfig::FAN_SPEED_DEFAULT;;
    };
...
void ExtruderPlan::processFanSpeedAndMinimalLayerTime(bool force_minimal_layer_time, Point starting_position, double max_cds_fan_speed, double max_fan_speed,bool cool_fan_enabled)
{
    ...
    */
    ...
    for (GCodePath& path : paths)
    {
		if (full_fan_layer && !path.isTravelPath() && cool_fan_enabled)
			path.setFanSpeed(max_fan_speed);
...
void LayerPlan::processFanSpeedAndMinimalLayerTime(Point starting_position)
{
    for (unsigned int extr_plan_idx = 0; extr_plan_idx < extruder_plans.size(); extr_plan_idx++)
    {
		...
		double max_fan_speed = extruder_settings.get<double>("cool_fan_speed_max");
        ...
        extruder_plan.processFanSpeedAndMinimalLayerTime(force_minimal_layer_time, starting_position, max_cds_fan_speed, max_fan_speed, cool_fan_enabled);
...
void LayerPlan::writeGCode(GCodeExport& gcode)
{
    ...
    static LayerIndex layer_1{ 1 - static_cast<LayerIndex>(Raft::getTotalExtraLayers(application)) };
    if (layer_nr == layer_1 && mesh_group_settings.get<bool>("machine_heated_bed"))
    {
		...
		for (ExtruderTrain& aTrain : application->extruders())
		{
    ...
    for (size_t extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        if (extruder_nr != extruder_plan.extruder_nr)
        {
            ...
            const ExtruderTrain& prev_extruder_train = application->extruders()[prev_extruder];
        ...
        else if (extruder_plan_idx == 0)
        {
            ...
            else if (layer_nr != 0 && application->extruders()[extruder_nr].settings.get<bool>("retract_at_layer_change"))
            {
        ...
        bool isAvoidPoint = false;//ͳ�Ʊ����˵ĵ� ����G2G3���ж�
        ...
        {//speedSlowDownPath �����½���һ����������ʶ�˶�path�Ƿ����ٶ�ͻ��
            for (unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
            {
        ...
        for (unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            if (! path.retract && path.config->isTravelPath() && path.points.size() == 1 && path.points[0] == gcode.getPositionXY() && z == gcode.getPositionZ())
            {
                ...
                isAvoidPoint = false;//ͳ�Ʊ����˵ĵ� ����G2G3���ж�
                continue;
            }
            else
            {
                isAvoidPoint = true;//ͳ�Ʊ����˵ĵ� ����G2G3���ж�
            }
            ...
            if (path.mesh_id != current_mesh &&  (extruder.settings.get<bool>("special_object_cancel")))
            {
                ...
                {   
                        ...
                        ((speed / paths[path_idx - i].speedSlowDownPath) > 1.1f ))     //from   large  to  small,  Reversely  is path is Travel Path
                    {
                        ...
                        {   //outwall   force  to  slowdown else  front line all no slow  ,   small  circle   also  faster
                            ...
                            float speed_slowdown_factor = application->sceneSettings().get<Ratio>("speed_slowtofast_slowdown_revise_speed_factor");
                ...
                { // normal path to gcode algorithm
                    ...
                    if (arc_configure_enable && !slope_slice_enable)
                    {
                        if (1)
                        {
                            ...
                            case PrintFeatureType::Infill:
                            {
                                ...
                                if(isAvoidPoint && !path.points.empty())//ͳ�Ʊ����˵ĵ� ����G2G3���ж�
                                {
                                ...
                                float dis = 0;
                                float dis_threshold = application->sceneSettings().get<coord_t>("speed_slowtofast_slowdown_revise_distance"); //���������һ�ξ��룬����ԭ�����ٶȣ������ٶ�50
                                ...
                                for (size_t fitting_index = 0; fitting_index < fitting_result.size(); fitting_index++)
                                {
                                    ...
                                    case EMovePathType::Linear_move:
                                    {
                                        ...
                                        for (size_t point_index = start_index; point_index < end_index + 1; point_index++)
                                        {
                                            ...
                                            {//�������Դ󲿷ֵ�Բ�����Ǿ������case��Arc_move_ccw��Linear_move��ͬ��������ٵ����ٵľ�����㣬���������һ�ξ��룬����ԭ�����ٶȣ������ٶ�50
                    ...
                    else
                    {
                        for(unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                        {
							...
							if (is_deceleration_speed)//������ ÿ����ʼ�μ���
							{
...
{
    ExtruderPlan& extruder_plan = extruder_plans[extruder_plan_idx];
    const ExtruderTrain& extruder = application->extruders()[extruder_plan.extruder_nr];
