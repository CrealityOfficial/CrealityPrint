...
#ifndef TREESUPPORTELEMENTT_H
#define TREESUPPORTELEMENTT_H
...
namespace cura54
{
    ...
    struct TreeSupportElementT
    {
        TreeSupportElementT
        (
            coord_t distance_to_top,
            size_t target_height,
            cura52::Point target_position,
            bool to_buildplate,
        ...
        TreeSupportElementT(const TreeSupportElementT& elem, cura52::Polygons* newArea = nullptr) : // copy constructor with possibility to set a new area
         ...
         */
        TreeSupportElementT(TreeSupportElementT* element_above) :
            ...
            result_on_layer(cura52::Point(-1, -1)), // set to invalid as we are a new node on a new layer
        ...
        TreeSupportElementT
        (
            const TreeSupportElementT& first,
            const TreeSupportElementT& second,
            size_t next_height,
            cura52::Point next_position,
         ...
         */
        cura52::LayerIndex target_height;
         ...
         */
        std::vector<TreeSupportElementT*> parents;
         ...
         */
        std::vector<cura52::Point> all_tips;
         ...
         */
        cura52::Polygons influence_area_limit_area;

        /*!
         * \brief Additional locations that the tip should reach
         */
        std::vector<cura52::Point> additional_ovalization_targets;


        bool operator==(const TreeSupportElementT& other) const
        {
            return target_position == other.target_position && target_height == other.target_height;
        }

        bool operator<(const TreeSupportElementT& other) const // true if me < other
        {
        ...
        void AddParents(const std::vector<TreeSupportElementT*>& adding)
        {
            for (TreeSupportElementT* ptr : adding)
            {
        ...
        void RecreateInfluenceLimitArea()
        {
            if (influence_area_limit_active)
            {
                ...
                for (cura52::Point p : all_tips)
                {
                    cura52::Polygon circle;
                    for (cura52::Point corner : TreeSupportBaseCircleT::getBaseCircle())
                    {
                        cura52::Point tmp; 
                        tmp.X = corner.X * influence_area_limit_range;
                        tmp.Y = corner.Y * influence_area_limit_range;
                        tmp.X = corner.X / double(TreeSupportBaseCircleT::base_radius);
                        tmp.Y = corner.Y / double(TreeSupportBaseCircleT::base_radius);
                        tmp.X = corner.X + p.X;
                        tmp.Y = corner.Y + p.Y;
                        circle.add(tmp);
        ...
        void setToBuildplateForAllParents(bool new_value)
        {
            ...
            std::vector<TreeSupportElementT*> grandparents{ parents };
            while (!grandparents.empty()) {
                std::vector<TreeSupportElementT*> next_parents;
                for (TreeSupportElementT* grandparent : grandparents) {
        ...
        {
            return result_on_layer !=cura52:: Point(-1, -1);
        }
...
{
    template <>
    struct hash<cura54::TreeSupportElementT>
    {
        size_t operator()(const cura54::TreeSupportElementT& node) const
        {
