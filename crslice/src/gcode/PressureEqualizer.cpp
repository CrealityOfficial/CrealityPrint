...
#include "Slice3rBase/LocalesUtils.hpp"
//#include "../GCode.hpp"
#include "Slice3rBase/Exception.hpp"
#include "PressureEqualizer.h"
#include "Slice3rBase/fast_float/fast_float.h"
...
PressureEqualizer::PressureEqualizer(double filament_diameter, float extrusion_rate, float segment_length, bool use_relative_e_distances)
{
   ...
   // }
    ...
    for (const GCodeExtrusionRole er : {GCodeExtrusionRole::None}) {
...
LayerResult PressureEqualizer::process_layer(LayerResult &input)
{
    ...
    //if (1) {
    //    std::ofstream f;
    //    f.open("C:\\Users\\cx2404\\Desktop\\input.gcode.txt", std::ofstream::app);
    //    if (f.fail()) {
    //        std::cout << "txt open failure!" << std::endl;
    //        exit(0);
    //    }
    //    if (f.is_open()) {
    ...
    //if (1) {
    //    std::ofstream f;
    //    f.open("C:\\Users\\cx2404\\Desktop\\inputResult.gcode.txt", std::ofstream::app);
    //    if (f.fail()) {
    //        std::cout << "txt open failure!" << std::endl;
    //        exit(0);
    //    }
    ...
    //            f << std::string(output_buffer.data()) << std::endl;
    //    }
    //    f << "over " << std::endl;
    //    f.close();
    //}
...
bool PressureEqualizer::process_line(const char *line, const char *line_end, GCodeLine &buf)
{
    ...
    case 'G': {
        ...
        case 1:
        {
            ...
            buf.adjustable_flow = true;//yi  this->opened_extrude_set_speed_block;
...
void PressureEqualizer::adjust_volumetric_rate(const size_t fist_line_idx, const size_t last_line_idx)
{
    ...
    while (line_idx != fist_line_idx) {
        ...
        if (m_gcode_lines[line_idx].extrusion_role == GCodeExtrusionRole::None) {            line_idx = idx_prev;
            continue;
        }
    ...
    while (line_idx != last_line_idx) {
        ...
        if (m_gcode_lines[line_idx].extrusion_role == GCodeExtrusionRole::None) {
            line_idx = idx_next;
            continue;
        }
