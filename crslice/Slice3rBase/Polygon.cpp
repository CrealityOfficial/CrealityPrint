...
Polygons polygons_simplify(const Polygons &source_polygons, double tolerance)
{
    ...
    for (const Polygon &source_polygon : source_polygons) {
        ...
        bool ccw = Clipper3r::Area(simplified) > 0.;
        for (Points &path : Clipper3r::SimplifyPolygons(ClipperUtils::SinglePathProvider(simplified), Clipper3r::pftNonZero)) {
            if (! ccw)
...
{
    if (const int poly_count_inside = Clipper3r::PointInPolygon(p, polygon.points); 
        poly_count_inside == -1)
...
bool contains(const Polygons &polygons, const Point &p, bool border_result)
{
    ...
    for (const Polygon &poly : polygons) {
        const int is_inside_this_poly = Clipper3r::PointInPolygon(p, poly.points);
