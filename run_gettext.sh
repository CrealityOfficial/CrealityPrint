...
if $FULL_MODE; then
    xgettext --keyword=L --keyword=_L --keyword=_u8L --keyword=L_CONTEXT:1,2c --keyword=_L_PLURAL:1,2 --add-comments=TRN --from-code=UTF-8 --no-location --debug --boost -f ./localization/i18n/list.txt -o ./localization/i18n/CrealityPrint.pot
...
do
    ...
    if [ -f "$dir/CrealityPrint_${lang}.po" ]; then
        if $FULL_MODE; then
            msgmerge -N -o "$dir/CrealityPrint_${lang}.po" "$dir/CrealityPrint_${lang}.po" "$pot_file"
        fi
