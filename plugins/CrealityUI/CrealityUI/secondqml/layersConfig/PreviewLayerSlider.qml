...
Slider {
    ...
    property var sliceAttain: kernel_slice_flow.sliceAttain
    ...
    function reloadConfigTag() {
        ...
        // kernel_slice_flow.updatePicture(kernel_slice_flow.sliceIndex);
        ...
        for (let height of tempList) {
            ...
            if (tagY < control.topPadding)
                continue;
            console.log(":"+layer+":height"+height+":tagY"+tagY)    
            let image = idConfigTag.createObject(control)
            image.type = "color"
            image.y = tagY
            updateImageMap(layer, image);
