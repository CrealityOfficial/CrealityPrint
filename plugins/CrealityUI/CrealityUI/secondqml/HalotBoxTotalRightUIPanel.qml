...
Rectangle {
    ...
    Connections {
        ...
        onCurMachineIndexChanged: {
            ...
            idProfileListView.currentIndex = Qt.binding(function() {
                return currentMachine.curProfileIndex;
            });
            //extruder
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            //双喷的适配
            Item {
                Layout.preferredWidth: 300 * screenScaleFactor - 18 * screenScaleFactor
                Layout.preferredHeight: extruderStack.currentIndex == 0 ? 30 * screenScaleFactor : 60 * screenScaleFactor
    ...
    Component {
        ...
        Rectangle {
            ...
            property string normalColor: isHovered ? Constants.right_panel_item_hovered_color : Constants.right_panel_item_default_color
            ...
            BasicTooltip {
                ...
                timeout: -1
                text: qsTr("The main parameter configuration of this mode: Layer Height(") + profileLayerHeight.toFixed(2) + qsTr("mm),Line Width(") + profileLineWidth.toFixed(2) + qsTr("mm),Wall Line Count(") + profileWallLineCount.toFixed(2) + qsTr("mm),Infill Density(") + profileInfillSparseDensity.toFixed(2) + qsTr("%),Build Plate Adhesion Type(") + qsTr(profileAdhesionType) + qsTr("),Printing Temperature(") + profileMaterialPrintTemperature.toFixed(2) + qsTr("℃),Build Plate Temperature(") + profileMaterialBedTemperature.toFixed(2) + "℃), " + qsTranslate("ParameterGeneralCom", "Inner Wall Speed") + "(" + profileSpeedInner.toFixed(2) + "mm/s), " + qsTranslate("ParameterGeneralCom", "Outer Wall Speed") + "(" + profileSpeedOuter.toFixed(2) + "mm/s)"
            }
