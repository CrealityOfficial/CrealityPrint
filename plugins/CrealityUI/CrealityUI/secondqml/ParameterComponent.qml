...
QtObject {
  ...
  readonly property Component parameter: Component {
    // required two object: `dataModel` and `uiModel`
    Control {
  ...
  readonly property Component lineParameter: Component {
    RowLayout {
      ...
      Text {
        ...
        color: reset_button.actived ? Constants.parameter_text_modifyed_color
    ...
    Item {
      ...
      RowLayout {
        ...
        Button {
          ...
          BasicTooltip {
            visible: text_edit_button.hovered
            timeout: -1
            position: root.toolTipPosition
            font.pointSize: Constants.labelFontPointSize_10
            textWidth: root.toolTipWidth
  ...
  readonly property Component comboBox: Component {
    CustomComboBox {
      ...
      displayText: {
        ...
        const focused_index = filterModelActived ? model.mapFromSource(focusedIndex) : focusedIndex
        return focused_index < 0 ? componentDataModel.value : translater(model.get(focused_index))
      }

      onActivated: {
        const current_index = filterModelActived ? model.mapToSource(currentIndex) : currentIndex
