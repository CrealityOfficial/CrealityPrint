...
ComboBox {
  ...
  popup: Popup {
    ...
    width: root.width
    height: {
      const maximum_count = root.maximumVisibleCount
      if (maximum_count < 0) {
        return implicitContentHeight + padding * 2
      }
      ...
      const visible_count = Math.min(maximum_count, model_count)
      return root.implicitDelegateHeight * visible_count + padding * 2
    }
  ...
  delegate: ItemDelegate {
    ...
    enabled: !list_view.ScrollBar.vertical.active
