...
QVariant VideoListModel::data(const QModelIndex& index, int role) const
{
	...
	switch (role)
	{
		...
		case VideoModelItem::E_VideoFileShowName:
			...
			break;
		case VideoModelItem::E_VideoSelected:
...
QHash<int, QByteArray> VideoListModel::roleNames() const
{
	...
	roles[VideoModelItem::E_VideoSelected] = "mItem";
	return roles;
}
...
void VideoListModel::deleteSelectedVideoFile(const QString& ipAddress)
{
	for(auto item:m_videoItemList)
	{	
		QString path = item->videoFilePath();   //item->gCodePrefixPath() + "/" + item->gCodeFileName();
		RemotePrinerType type = RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408;
		if(item->videoSelected())
			deleteVideoFile(ipAddress,path, static_cast<int>(type));
	}
}
...
void VideoListModel::switchDataSource(const QString& macAddr, RemotePrinerType remoteType, bool init)//@true: init @false: update
{
	...
	if (m_videoItemMap.contains(macAddr))
	{
		m_printerType = int(remoteType);
		emit printerTypeChanged();

		beginResetModel();
		m_videoItemList = m_videoItemMap[macAddr];
		endResetModel();
		if(&VideoListModel::videoFileSelectedCountChanged) videoFileSelectedCountChanged();
	}
...
void VideoListModel::downloadSelectedVideoFile(const QString& macAddr,const QString& ipAddress,const QString& localPath)
{
	for(auto item:m_videoItemList)
	{	
		if(item->videoSelected())
		{
			RemotePrinerType type = RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408;
			QString fileName = item->videoFileName();
			QString path = localPath + "/" + fileName;
			downloadVideoFile(macAddr,ipAddress,fileName, path, static_cast<int>(type));
		}
			
	}
}

int VideoListModel::videoFileSelectedCount()
{
	int index = 0;
	for (auto item : m_videoItemList)
	{
...
void VideoListModel::setVideoFileSelected(bool select)
{
	for(auto item: m_videoItemList)
	{	
		item->setVideoSelected(select);
	}
}
		...
		case RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408:
		{
			...
			foreach(QString fullInfo, splitList)
			{
				if (m_webInfoRegExp.exactMatch(fullInfo))
				{
					...
					connect(modelItem, &VideoModelItem::videoSelectedChanged, this, &VideoListModel::videoFileSelectedCountChanged);
...
bool VideoModelItem::videoSelected() const
{
	return m_videoSelected;
}

void VideoModelItem::setVideoSelected(bool selected)
{
	if (m_videoSelected == selected)
		return;

	m_videoSelected = selected;
	emit videoSelectedChanged();
}
