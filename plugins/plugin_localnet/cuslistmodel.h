...
public:
	...
	//发送3mf并一键打印
	Q_INVOKABLE void batch_command(const QString& fileName, const QString& filePath, int plate,const QList<QVariantMap>& colorMap= QList<QVariantMap>());

    //发送文件/一键打印
    Q_INVOKABLE void batch_command(const QString& fileName, const QString& filePath, bool sendOnly,const QList<QVariantMap>& colorMap= QList<QVariantMap>());
	//选取机器上gcode 打印
	Q_INVOKABLE void printDeviceGCode(const QString&strMac, const QString& filePath, const QList<QVariantMap>& colorMap= QList<QVariantMap>());
	//传输文件
	...
	Q_INVOKABLE bool findDeviceError();
	...
	Q_INVOKABLE QObject* getDeviceItemByIp(const QString& ip);
	...
	Q_INVOKABLE void setSyncPrinter(QString ip);
	Q_INVOKABLE int getSyncPrinter();
...
private:
	...
	QList<QVariantMap> m_multiColor = QList<QVariantMap>();
...
public:
	...
	Q_PROPERTY(int webrtcSupport READ webrtcSupport WRITE setWebrtcSupport NOTIFY webrtcSupportChanged)
	Q_PROPERTY(int feedState READ feedState WRITE setFeedState NOTIFY feedStateChanged)
	Q_PROPERTY(int cfsConnect  READ cfsConnect  WRITE setCfsConnect  NOTIFY cfsConnectChanged)
	Q_PROPERTY(bool sync READ sync WRITE setSync NOTIFY syncChanged)
	Q_PROPERTY(QObject*materialLinkageModel READ materialLinkageModel WRITE setMaterialLinkageModel)
	Q_PROPERTY(QObject* filelistModel READ filelistModel WRITE setFilelistModel)
	Q_PROPERTY(QObject* gcodeFileListModel READ gcodeFileListModel NOTIFY gcodeFileListChanged);
	...
	QObject* materialLinkageModel();
	void setMaterialLinkageModel(QObject* model);
	QObject* filelistModel();
	void setFilelistModel(QObject* model);
	
	QObject*  gcodeFileListModel();
	...
	int webrtcSupport() const;
	void setWebrtcSupport(int isSupport);

	int feedState() const;
	void setFeedState(int feedState);

	int cfsConnect() const;
	void setCfsConnect(int cfsConnect);
...
signals:
	...
	void printObjectsChanged();
	void excludedObjectsChanged();
	void currentObjectChanged();
	void maxNozzleTempChanged();
	void maxBedTempChanged();
	void machineChamberFanExistChanged();
	...
	void webrtcSupportChanged();
	void feedStateChanged();
	void cfsConnectChanged();
	void syncChanged();

	void gcodeColorsObjectChanged();
...
private:
	...
	QObject* m_materialBoxList=nullptr;
	int m_webrtcSupport;
	int m_feedState;
	int m_cfsConnect;
	bool m_sync = false;
	QObject* m_materialLinkageModel=nullptr;
	QObject* m_filelistModel=nullptr;
	QObject* m_gcodeFileListModel=nullptr;
