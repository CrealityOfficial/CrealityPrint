...
Cx3dWriter::Cx3dWriter(QString path,qtuser_core::Progressor* progressor,QObject *parent, bool isBackup) : m_progress(progressor),QObject(parent)
{
    ...
    QList<ModelN*> allModelns = modelns();
    for (size_t i = 0; i < allModelns.size(); i++)
    {
		...
		mMatrixTemp.supName = strName.section(".", 0, 0) + ".sup";
        m_contentxml.meshMatrix.push_back(mMatrixTemp);

        //QString strName = allModelns.at(i)->objectName();
        QString croped_fileName = strName.section(".", 0, 0)+ ".stl";
        ...
        {
            strPathName = fileInfo.path() + "/" + mMatrixTemp.supName;
            saveFDMSupport(mMatrixTemp.supName, strPathName, allModelns.at(i));
        }
        //else
        //{
        //    strPathName = fileInfo.path() + "/" + mMatrixTemp.supName;
        //    saveDLPSupport(mMatrixTemp.supName, strPathName, allModelns.at(i));
        //}
    }
...
void Cx3dWriter::saveFDMSupport(QString strName, QString strPathName, creative_kernel::ModelN* model)
{
    if (model->hasFDMSupport())
    {
        if (m_pModelFile->open(QIODevice::WriteOnly | QIODevice::Truncate, QuaZipNewInfo(strName, strPathName)))
        {
            QDataStream stream(m_pModelFile);
            model->fdmSupport()->save(stream);
        }
        m_pModelFile->close();
    }
}
