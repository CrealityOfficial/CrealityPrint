...
#include "qtuser3d/module/textserializer.h"
...
void Cx3dRenderEx::meshs()
{
    if (m_zip->open(QuaZip::mdUnzip))
    {
        for (QVector<QString>::iterator it = m_contentxml->meshPathName.begin();
            it != m_contentxml->meshPathName.end(); it++)
        {
            QString str = *it;
            trimesh::TriMesh* mesh = new trimesh::TriMesh();
            m_zip->setCurrentFile(*it);
            m_pModelFile->setZip(m_zip);
            if (m_pModelFile->open(QIODevice::ReadOnly))
            {
                char head[80];
...
void Cx3dRenderEx::FDMSup()
{
    if (m_zip->open(QuaZip::mdUnzip))
    {
        for (QVector<modelMatrix>::iterator it = m_contentxml->meshMatrix.begin();
            it != m_contentxml->meshMatrix.end(); it++)
        {
            QString strName = it->supName;
            m_zip->setCurrentFile(strName);
            m_pModelFile->setZip(m_zip);
            creative_kernel::FDMSupportGroup* fmdsup = new creative_kernel::FDMSupportGroup();
            if (m_pModelFile->open(QIODevice::ReadOnly))
            {
                QDataStream stream(m_pModelFile);
                fmdsup->load(stream);
            }
            m_vctFDMSup.push_back(fmdsup);
            m_pModelFile->close();
        }
        m_zip->close();
    }
...
std::vector<creative_kernel::FDMSupportGroup*> Cx3dRenderEx::getFDMSup()
{
    return m_vctFDMSup;
}
