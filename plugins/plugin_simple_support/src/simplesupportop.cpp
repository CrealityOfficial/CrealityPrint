	...
	, m_supportSize(2.0f)
{
	...
	{
		qtuser_3d::XRenderPass* pass = new qtuser_3d::XRenderPass("phong", m_cylinderEntity);
...
void SimpleSupportOp::onHoverMove(QHoverEvent* event)
{
	...
	if (pickable && m_supportMode == SupportMode::esm_add)
	{
		...
		if (tcollides.size() == 1 && tree)
		{
			...
			{
				m_cylinderMesh.reset(mmesh::ShapeCreator::createCylinderMesh(seg.t, seg.b, cylinderRadius, 4/*30*/, 45));
				Qt3DRender::QGeometry* geometry = qcxutil::createGeometryFromMesh(m_cylinderMesh.get(), nullptr);
