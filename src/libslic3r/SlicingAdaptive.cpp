...
static inline float outer_wall_width_from_slop(const SlicingAdaptive::FaceZ& face, float layer_height)
{
...
void SlicingAdaptive::prepare(const ModelObject &object)
{
	...
	if (its.indices.size() > 0) {
		...
		overhang_ratio = get_mesh_overhang(mesh.its, z_values, slicing_params_ex);
	}
        ...
        for (; ordered_id < m_faces.size(); ++ordered_id) {
            ...
            if (zspan.second > print_z) {
                ...
                if (zspan.second < print_z + EPSILON)
                    continue;
                width = std::max(outer_wall_width_from_slop(m_faces[ordered_id], layer_height), width);
            }
        }
    }
    return width;
