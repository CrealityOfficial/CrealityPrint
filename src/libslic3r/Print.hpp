...
private:
   ...
   void _transform_hole_to_polyholes();
    ...
    SupportNecessaryType    m_cache_support_necessary = SupportNecessaryType::NoNeedSupp;
    // SoftFever
    // 
    // object id
    size_t               m_id;
...
public:
    ...
    const bool is_BBL_printer() const;

    void set_is_CX_printer(bool isCXPrinter){ m_isCXPrinter = isCXPrinter; }
    const bool is_CX_printer() const{ return m_isCXPrinter; }
    ...
    void setCrealityCFS(bool isCrealityCFS) { m_isCrealityCFS = isCrealityCFS; }
    bool getCrealityCFS() const { return m_isCrealityCFS; }
...
public:
    ...
    std::unique_ptr<creality::MachineVender> m_machine_vender;
