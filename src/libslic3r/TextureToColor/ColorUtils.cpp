...
static bool smooth_region_geom_boundary(CGALMesh& mesh, std::vector<std::size_t>& face_labels, const SmoothParameters& smooth_parameters) {
    ...
    // Trace without recursive calls. A malformed or non-manifold boundary can
    // otherwise keep the lambda cycling until the worker thread overflows its
    // stack before the visited-edge check gets a chance to stop it.
    auto trace_polyline = [&](std::vector<CGAL::SM_Vertex_index>& polyline) {
        if (polyline.empty())
            return;

        const std::size_t max_steps = segment_boundary_edges.size() + 1;
        std::size_t steps = 0;
        while (steps++ < max_steps) {
            CGAL::SM_Vertex_index curr_vtx = polyline.back();
            auto degree_it = map_vtx_to_degree.find(curr_vtx);
            if (degree_it == map_vtx_to_degree.end() || degree_it->second != 2)
                return;
        ...
        BOOST_LOG_TRIVIAL(warning)
            << "Geometric smoothing boundary trace exceeded the edge limit; stopping this polyline";
    };
