                                                        ...
                                                        const Vec3d&                     object_shrinkage_compensation)
{
    ...
    params.xy_distance_overhang = support_material_extruder_dmr / 2.0;
    // BBS should be smaller than 90
    coordf_t threshold_rad          = object_config.support_threshold_angle.value > 0 ? object_config.support_threshold_angle.value + 1 : 0;
    params.threshold_rad            = Geometry::deg2rad(std::min(threshold_rad, 89.));
    params.use_xy_distance_overhang = object_config.support_xy_overrides_z == SupportDistPriority::Z_OVERRIDES_XY;
...
void smooth_layer_width_profile(std::vector<double>& layer_width_profile, size_t window_size) {
    ...
    for (size_t i = half_window; i < layer_width_profile.size() - half_window; ++i) {
        double max_value = *std::max_element(layer_width_profile.begin() + i - half_window,
                                            layer_width_profile.begin() + i + half_window + 1);
        smoothed_profile[i] = max_value;
    }
