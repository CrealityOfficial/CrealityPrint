...
uint32_t group_id(std::vector<Bridge_> &bridges, uint32_t src_id) {
...
void detect_bridge_directions(
	...
	for (uint32_t bridge_id = 0; bridge_id < uint32_t(bridges.size()); ++bridge_id) {
		Bridge_& bridge = bridges[bridge_id];
...
)
{
	...
	std::vector<Bridge_> bridges{ get_grouped_bridges(
