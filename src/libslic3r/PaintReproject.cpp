...
using AnnotationData = TriangleSelector::TriangleSplittingData;
        ...
        int preferred_root, const std::array<Vec3f, 3> &target_triangle_in_source)
    {
        ...
        while (!frontier.empty() && int(visited.size()) < MaxCoplanarSearch) {
            ...
            if (root < int(m_face_neighbors.size())) {
                const Vec3i32 &neighbors = m_face_neighbors[root];
                for (int e = 0; e < 3; ++e) {
...
private:
    ...
    {
        const Vec3i32 &indices = m_leaf_indices[leaf];
        return {
    ...
    std::vector<Vec3i32>             m_leaf_indices;
...
public:
    ...
    std::array<Vec3f, 3> triangle(const Vec3i32 &face) const
    {
        ...
        , m_masked(paintable_facets != nullptr)
    {
        ...
        for (size_t face_idx = 0; face_idx < face_count; ++face_idx) {
            if (m_masked && (*paintable_facets)[face_idx] == 0)
                continue;
            const Vec3i32 &face = m_all_indices[face_idx];
            Box3f        box;
            for (int i = 0; i < 3; ++i)
...
private:
    ...
    const std::vector<Vec3i32> &m_all_indices;
    bool                      m_masked { false };
    ...
    std::vector<Vec3i32>      m_masked_indices;
    AABBTreeIndirect::Tree3f  m_tree;
                                                ...
                                                float                           tolerance)
{
    ...
    for (size_t dst_face = 0; dst_face < dst_face_count; ++dst_face) {
        ...
        const Vec3i32 &src_face = src_mesh.its.indices[source_root];
        bool congruent = true;
                                                              ...
                                                              int                     root_count)
{
    ...
    const int             total_bits = int(data.bitstream.size());
    for (size_t i = 1; i < data.triangles_to_split.size(); ++i)
        if (data.triangles_to_split[i].triangle_idx <= data.triangles_to_split[i - 1].triangle_idx ||
            data.triangles_to_split[i].bitstream_start_idx <= data.triangles_to_split[i - 1].bitstream_start_idx)
            return {};

    std::vector<std::pair<int, int>> ranges(size_t(root_count), std::make_pair(0, 0));
    for (size_t i = 0; i < data.triangles_to_split.size(); ++i) {
        const int root = data.triangles_to_split[i].triangle_idx;
        if (root < 0 || root >= root_count)
            return {};
        const int first = data.triangles_to_split[i].bitstream_start_idx;
        const int last = i + 1 < data.triangles_to_split.size() ?
                             data.triangles_to_split[i + 1].bitstream_start_idx : total_bits;
                                           ...
                                           const std::vector<std::pair<int, int>> &src_ranges)
{
    ...
    const std::vector<bool> &src_bits = src.get_data().bitstream;
    for (size_t root = 0; root < roots.size(); ++root) {
        ...
        if (first >= last)
            continue;
        extracted.triangles_to_split.emplace_back(int(root), int(extracted.bitstream.size()));
        extracted.bitstream.insert(extracted.bitstream.end(), src_bits.begin() + first,
                                   src_bits.begin() + last);
    }
    extracted.reset_used_states();
    if (!extracted.bitstream.empty())
        extracted.update_used_states(0);
    return extracted;
}
                                             ...
                                             const std::vector<std::pair<int, int>> &src_ranges)
{
    ...
    for (size_t dst_face = 0; dst_face < origin.size(); ++dst_face) {
        ...
        spliced.bitstream.insert(spliced.bitstream.end(), src_bits.begin() + first,
                                 src_bits.begin() + last);
    }
    spliced.reset_used_states();
    if (!spliced.bitstream.empty())
        spliced.update_used_states(0);
    return spliced;
}
                               ...
                               const Transform3d      *dst_world_matrix)
{
    ...
    for (const Vec3i32 &face : dst_mesh.its.indices) {
