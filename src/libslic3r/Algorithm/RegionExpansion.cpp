    ...
    bool                   sorted)
{
    ...
    for (const Clipper2Lib_Z::Path64 &path : segments) {
        assert(path.size() >= 2);
        Clipper2Lib_Z::Point64 front = path.front();
        Clipper2Lib_Z::Point64 back  = path.back();

        // A vertex touching an existing boundary vertex may not invoke the Z callback.
        // Keep C3's guard for that rare case instead of treating an unclassified
        // open segment as a closed source contour.
        if (front != back && front.z >= 0 && back.z >= 0)
            continue;

        if (front == back && front.z < idx_boundary_end) {
            // An apparently closed segment may start on a boundary vertex. Pick a
            // point carrying the source id so the segment can still be classified.
            for (const Clipper2Lib_Z::Point64 &point : path) {
