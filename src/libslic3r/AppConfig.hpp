...
struct ProfileMachine
{
	std::string vendor;
	std::vector<ProfileModel> models;
};
...
public:
    ...
    bool get_userPresets(const std::string& model) const;
    void set_userPresets(const std::string& model,bool isClear = false);
    void set_userPresets(const AppConfig &from); 

	// CrealityPrint printer settings
	...
	// Get the CrealityPrint profile update url.
...
private:
	...
	std::map<std::string, ProfileMachine>						m_profile_machines;
