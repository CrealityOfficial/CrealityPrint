...
// When parsing a configuration value, if the old_value is not understood by this CrealityPrint version,
// it is being substituted with some default value that this CrealityPrint could work with.
    ...
    ConfigHelpers::DeserializationResult deserialize_with_substitutions(const std::string &str, bool append, ConfigHelpers::DeserializationSubstitution substitution)
    {
        ...
        while (std::getline(is, item_str, ',')) {
        	...
        	if (item_str == "nil") {
        		...
        		new_value = true;
        	} else if (item_str == "0" || item_str.empty()) {
        		new_value = false;
