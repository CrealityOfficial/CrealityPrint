// [FORMATTED BY CLANG-FORMAT 2026-05-09 13:36:17]
#pragma once
...
struct TreeSupportMeshGroupSettings
{
    ...
    explicit TreeSupportMeshGroupSettings(const PrintObject& print_object)
    {
        ...
        // this->support_xy_distance_1st_layer = this->support_xy_distance;
        ...
        this->support_tree_branch_diameter_angle = std::clamp<double>(config.tree_support_branch_diameter_angle * M_PI / 180., 0.,
                                                                      0.5 * M_PI - EPSILON);
        this->support_tree_top_rate              = 15;        //config.tree_support_top_rate.value;
    ...
    // Tree Support Max Single Core Area
    // Maximum influence area for a single core. Larger areas will be split into multiple cores using K-Means clustering.
    // This prevents over-merging which causes centroid shift and disconnected supports.
    coord_t support_tree_max_single_core_area{scaled<coord_t>(50.0) * scaled<coord_t>(1.0)};
    // Tree Support Max Branches Per Core
    // Maximum number of branches that can be supported by a single core before splitting.
    uint32_t support_tree_max_branches_per_core{10};
    // Tree Support Min Core Separation
    // Minimum distance between cores in multi-core mode to prevent overlap.
    coord_t support_tree_min_core_separation{scaled<coord_t>(2.0)};
    // Tree Support Max Branch Angle
    // Maximum allowed angle for branch connections. Branches exceeding this angle will be flagged as invalid.
    double support_tree_max_branch_angle{60.0}; // degrees
    // Tree Support Min Overlap Ratio
    // Minimum overlap ratio with lower layer to be considered grounded (for island filtering).
    double support_tree_min_overlap_ratio{0.3}; // 30%
...
struct TreeSupportSettings
{
    ...
    explicit TreeSupportSettings(const TreeSupportMeshGroupSettings& mesh_group_settings, const SlicingParameters& slicing_params)
        ...
        , maximum_move_distance_slow((angle_slow < M_PI / 2.) ? (coord_t) (tan(angle_slow) * layer_height) :
        ...
        , max_single_core_area(mesh_group_settings.support_tree_max_single_core_area)
        , max_branches_per_core(mesh_group_settings.support_tree_max_branches_per_core)
        , min_core_separation(mesh_group_settings.support_tree_min_core_separation)
        , max_branch_angle(mesh_group_settings.support_tree_max_branch_angle)
        , min_overlap_ratio(mesh_group_settings.support_tree_min_overlap_ratio)
    {
...
public:
    ...
    /*!
     * \brief Factor by which to increase the branch radius to reach the required bp_radius at layer 0. Note that this radius increase will
     * not happen in the tip, to ensure the tip is structurally sound.
     */
    ...
    /*
     * \brief Maximum influence area for a single core before splitting into multiple cores.
     */
    coord_t max_single_core_area;
    /*
     * \brief Maximum number of branches that can be supported by a single core.
     */
    uint32_t max_branches_per_core;
    /*
     * \brief Minimum distance between cores in multi-core mode.
     */
    coord_t min_core_separation;
    /*
     * \brief Maximum allowed branch connection angle (degrees).
     */
    double max_branch_angle;
    /*
     * \brief Minimum overlap ratio with lower layer to be considered grounded.
     */
...
public:
    bool operator==(const TreeSupportSettings& other) const
    {
        return branch_radius == other.branch_radius && tip_layers == other.tip_layers &&
               ...
               && max_single_core_area == other.max_single_core_area && max_branches_per_core == other.max_branches_per_core &&
               min_core_separation == other.min_core_separation && max_branch_angle == other.max_branch_angle &&
               min_overlap_ratio == other.min_overlap_ratio
    ...
    [[nodiscard]] inline coord_t getActualZ(LayerIndex layer_idx)
    {
        return layer_idx < coord_t(known_z.size())                          ? known_z[layer_idx] :
               (layer_idx - known_z.size()) * layer_height + known_z.size() ? known_z.back() :
                                                                              0;
    }

    /*!
     * \brief Set the z every Layer is printed at. Required for getActualZ to work
     * \param z[in] The z every LayerIndex is printed. Vector is used as a map<LayerIndex,coord_t> with the index of each element being the
     * corresponding LayerIndex
     * \return The radius every element should aim to achieve.
     */
    void setActualZ(std::vector<coord_t>& z) { known_z = z; }
};
