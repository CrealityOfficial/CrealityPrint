// [FORMATTED BY CLANG-FORMAT 2026-05-11 18:37:05]
...
{
    // Create a collision region covering the area outside the bed boundary so that
    // support influence areas are constrained to stay within the print bed.
    // Use a 100mm-expanded bounding box instead of a 1000mm offset to avoid
    // integer coordinate overflow (SCALING_FACTOR = 1e-6, 100mm = 1e8 units << INT32_MAX).
    BoundingBox   bbox   = get_extents(machine_border);
    const coord_t margin = coord_t(scaled<float>(100.f));
    bbox.min -= Point{margin, margin};
    bbox.max += Point{margin, margin};
    Polygons out;
    out.emplace_back(bbox.polygon()); // Large outer CCW rectangle
    machine_border.reverse();         // Bed boundary as CW hole, creating an annular region outside the bed
    out.emplace_back(std::move(machine_border));
    return out;
}

TreeModelVolumes::TreeModelVolumes(const PrintObject& print_object,
    ...
    m_max_move{std::max<coord_t>(max_move - 2, 0)}
    , m_max_move_slow{std::max<coord_t>(max_move_slow - 2, 0)}
    ,
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    m_progress_multiplier{progress_multiplier}
    ...
    m_machine_border{calculateMachineBorderCollision(build_volume.polygon())}
{
...
#else
    {
        ...
        m_support_layer_z.resize(num_layers);
        for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
            m_support_layer_z[layer_idx] = layer_z(print_object.slicing_parameters(), config, layer_idx);

        m_model_surface_mesh = std::make_unique<TriangleMesh>(print_object.model_object()->raw_mesh());
        m_model_surface_mesh->transform(print_object.trafo_centered());

        outlines.assign(num_layers, Polygons{});
        tbb::parallel_for(tbb::blocked_range<size_t>(num_raft_layers, num_layers,
                                                     std::min<size_t>(1,
                                                                      std::max<size_t>(16,
                                                                                       num_layers /
                                                                                           (8 * tbb::this_task_arena::max_concurrency())))),
                          [&](const tbb::blocked_range<size_t>& range) {
                              for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                                  outlines[layer_idx] = polygons_simplify(to_polygons(
                                                                              print_object.get_layer(layer_idx - num_raft_layers)->lslices),
...
std::optional<TreeModelVolumes::ModelSurfaceInfo> TreeModelVolumes::getModelSurfaceInfo(const Point&      target_position,
                                                                                        const LayerIndex target_layer) const
{
    if (m_model_surface_mesh == nullptr || m_model_surface_aabb_once == nullptr || target_layer < 0 ||
        size_t(target_layer) >= m_support_layer_z.size())
        return std::nullopt;

    std::call_once(*m_model_surface_aabb_once, [this]() { m_model_surface_aabb = std::make_unique<AABBMesh>(*m_model_surface_mesh); });

    const Vec2d target_xy_mm = unscaled<double>(target_position);
    const Vec3d query_point{target_xy_mm.x(), target_xy_mm.y(), m_support_layer_z[size_t(target_layer)]};
    int         face_id = -1;
    Vec3d       closest_point;
    const double distance_sq = m_model_surface_aabb->squared_distance(query_point, face_id, closest_point);
    if (face_id < 0 || !std::isfinite(distance_sq))
        return std::nullopt;

    return ModelSurfaceInfo{closest_point, m_model_surface_aabb->normal_by_face_id(face_id), std::sqrt(distance_sq)};
}
...
void TreeModelVolumes::precalculate(const PrintObject& print_object, const coord_t max_layer, std::function<void()> throw_on_cancel)
{
    ...
    const auto report_progress = [&print_object, progress_report_interval_ms, &next_progress_report_ms, &now_ms](
        ...
        const std::string message = !phase.empty() && phase.back() == ')' ?
                                        ...
                                        phase + ": " + percent;
        print_object.print()->set_status(55, message);
        BOOST_LOG_TRIVIAL(info) << message;
    };
    const auto make_status_progress = [&report_progress, &precalculating_text](size_t step) -> StatusProgress {
        const std::string phase = precalculating_text + " (" + std::to_string(step) + "/5)";
        return [&report_progress, phase](size_t completed, size_t total, bool force) {
            report_progress(phase, completed, total, force);
        };
    };
    ...
    // it may seem that the required avoidance can be of a smaller radius when going to model (no initial layer diameter for to model
    // branches) but as for every branch going towards the bp, the to model avoidance is required to check for possible merges with to model
    // branches, this assumption is in-fact wrong.
    std::unordered_map<coord_t, LayerIndex> radius_until_layer;
    // while it is possible to calculate, up to which layer the avoidance should be calculated, this simulation is easier to understand, and
    // does not need to be adjusted if something of the radius calculation is changed. Overhead with an assumed worst case of 6600 layers
    // was about 2ms
    ...
    calculateCollision(relevant_collision_radiis, throw_on_cancel, make_status_progress(1));
    ...
    calculateCollisionHolefree(relevant_hole_collision_radiis, throw_on_cancel, make_status_progress(2));
    // Let placables be calculated from calculateAvoidance() for better parallelization.
    if (m_support_rests_on_model)
        calculatePlaceables(relevant_avoidance_radiis, throw_on_cancel, make_status_progress(3));
    ...
    // Calculate the relevant avoidances in parallel as far as possible
    {
        ...
        task_group.run([this, relevant_avoidance_radiis, throw_on_cancel, status_progress = make_status_progress(4)] {
            calculateAvoidance(relevant_avoidance_radiis, true, m_support_rests_on_model, throw_on_cancel, status_progress);
        });
        task_group.run([this, relevant_avoidance_radiis, throw_on_cancel, status_progress = make_status_progress(5)] {
            calculateWallRestrictions(relevant_avoidance_radiis, throw_on_cancel, status_progress);
        });
        task_group.wait();
    }
...
const Polygons& TreeModelVolumes::getCollision(const coord_t orig_radius, LayerIndex layer_idx, bool min_xy_dist) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result = m_collision_cache.getArea({radius, layer_idx}); result)
        return (*result).get();
    if (m_precalculated) {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache) << "Had to calculate collision at radius " << radius << " and layer " << layer_idx
                                                    << ", but precalculate was called. Performance may suffer!";
...
std::optional<std::pair<coord_t, std::reference_wrapper<const Polygons>>> TreeModelVolumes::get_collision_lower_bound_area(
    LayerIndex layer_id, coord_t max_radius) const
{
...
const Polygons& TreeModelVolumes::getCollisionHolefree(coord_t radius, LayerIndex layer_idx) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result = m_collision_cache_holefree.getArea({radius, layer_idx}); result)
        return (*result).get();
    if (m_precalculated) {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache) << "Had to calculate collision holefree at radius " << radius << " and layer "
                                                    << layer_idx << ", but precalculate was called. Performance may suffer!";
...
const Polygons& TreeModelVolumes::getAvoidance(
    const coord_t orig_radius, LayerIndex layer_idx, AvoidanceType type, bool to_model, bool min_xy_dist) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result = this->avoidance_cache(type, to_model).getArea({radius, layer_idx});
        result)
        return (*result).get();

    if (m_precalculated) {
        if (to_model) {
            BOOST_LOG_TRIVIAL(error_level_not_in_cache) << "Had to calculate Avoidance to model at radius " << radius << " and layer "
                                                        << layer_idx << ", but precalculate was called. Performance may suffer!";
            tree_supports_show_error("Not precalculated Avoidance(to model) requested."sv, false);
        } else {
            BOOST_LOG_TRIVIAL(error_level_not_in_cache) << "Had to calculate Avoidance at radius " << radius << " and layer " << layer_idx
                                                        << ", but precalculate was called. Performance may suffer!";
...
const Polygons& TreeModelVolumes::getPlaceableAreas(const coord_t         orig_radius,
                                                    LayerIndex            layer_idx,
                                                    std::function<void()> throw_on_cancel) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result = m_placeable_areas_cache.getArea({radius, layer_idx}); result)
        return (*result).get();
    if (m_precalculated) {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache) << "Had to calculate Placeable Areas at radius " << radius << " and layer " << layer_idx
                                                    << ", but precalculate was called. Performance may suffer!";
...
const Polygons& TreeModelVolumes::getWallRestriction(const coord_t orig_radius, LayerIndex layer_idx, bool min_xy_dist) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result =
        ...
        return (*result).get();
    if (m_precalculated) {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache) << "Had to calculate Wall restricions at radius " << radius << " and layer "
                                                    << layer_idx << ", but precalculate was called. Performance may suffer!";
        tree_supports_show_error(min_xy_dist ? "Not precalculated Wall restriction of minimum xy distance requested )." :
    ...
    return getWallRestriction(orig_radius, layer_idx,
                              min_xy_dist); // Retrieve failed and correct result was calculated. Now it has to be retrieved.
}
                                          ...
                                          StatusProgress status_progress)
{
    ...
    if (status_progress)
        status_progress(0, keys.size(), true);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()), [&](const tbb::blocked_range<size_t>& range) {
        for (size_t ikey = range.begin(); ikey != range.end(); ++ikey) {
            ...
            if (status_progress)
                status_progress(done, keys.size(), false);
        }
    });
    if (status_progress)
        status_progress(completed.load(std::memory_order_relaxed), keys.size(), true);
}
...
void TreeModelVolumes::calculateCollision(const coord_t radius, const LayerIndex max_layer_idx, std::function<void()> throw_on_cancel)
{
    ...
    for (size_t outline_idx : layer_outline_indices)
        if (const std::vector<Polygons>& outlines = m_layer_outlines[outline_idx].second; !outlines.empty()) {
            ...
            const coord_t xy_distance =
                outline_idx == m_current_outline_idx ?
                    ...
                    // and later for each avoidance... But avoidance calculation has to be for the whole scene and can NOT be done for each
                    // outline_idx separately and combined later. so avoiding this inaccuracy seems infeasible as it would require 2x the
                    // avoidance calculations => 0.5x the performance.
            ...
            tbb::parallel_for(tbb::blocked_range<LayerIndex>(data.begin(), data.end()),
                              [&collision_areas_offsetted, &outlines, &layer_outlines_sharp = m_layer_outlines_sharp,
                               &machine_border = m_machine_border, &anti_overhang = m_anti_overhang, radius, xy_distance,
                               z_distance_bottom_layers, z_distance_top_layers, min_resolution = m_min_resolution, &data,
                               processing_last_mesh, &throw_on_cancel](const tbb::blocked_range<LayerIndex>& range) {
                                  for (LayerIndex layer_idx = range.begin(); layer_idx != range.end(); ++layer_idx) {
                                      Polygons collisions;

                                      auto iter = layer_outlines_sharp.find(layer_idx);
                                      if (iter != layer_outlines_sharp.end())
                                          append(collisions, iter->second);
                                      ...
                                      for (int i = 1; i <= z_distance_top_layers; ++i)
                                          if (int j = layer_idx + i; j < int(outlines.size())) {
                                              ...
                                              // If just the collision (including the xy distance) of the layers above is accumulated, it
                                              // leads to the following issue: Example: assuming the z distance is 2 layer
                                              // + = xy_distance
                                              // - = model
                                              ...
                                              //    +-----+
                                              //   o +-----+
                                              // If just the collision above is accumulated the overhang will get overwritten by the
                                              // xy_distance of the layer below the overhang...
                                              //
                                              // This only causes issues if the overhang area is thinner than xy_distance
                                              // Just accumulating areas of the model above without the xy distance is also problematic, as
                                              // then support may get closer to the model (on the diagonal downwards) than the user
                                              // intended. Example (s = support):
                                              //  +-----+
                                              //   +-----+
                                              //   +-----+
                                              //   s+-----+

                                              // technically the calculation below is off by one layer, as the actual distance between
                                              // plastic one layer down is 0 not layer height, as this layer is filled with said plastic.
                                              // But otherwise a part of the overhang that is expected to be supported is overwritten by the
                                              // remaining part of the xy distance of the layer below the to be supported area.
                                              coord_t required_range_x = (xy_distance - ((i - (z_distance_top_layers == 1 ? 0.5 : 0)) *
                                                                                         xy_distance / z_distance_top_layers));
                                              ...
                                              append(collisions, offset(union_ex(collision_areas_original), radius + required_range_x,
                                                                        ClipperLib::jtMiter, 1.2));
                                          }
                                      collisions = processing_last_mesh && layer_idx < int(anti_overhang.size()) ?
                                                       union_(collisions, offset(union_ex(anti_overhang[layer_idx]), radius,
                                                                                 ClipperLib::jtMiter, 1.2)) :
                                                       union_(collisions);
            ...
            if (calculate_placable) {
                ...
                tbb::parallel_for(tbb::blocked_range<LayerIndex>(std::max(z_distance_bottom_layers + 1, data.begin()), data.end()),
                                  [&collision_areas_offsetted, &outlines, &anti_overhang = m_anti_overhang, processing_last_mesh,
                                   min_resolution = m_min_resolution, z_distance_bottom_layers, xy_distance, &data_placeable,
                                   &throw_on_cancel](const tbb::blocked_range<LayerIndex>& range) {
                                      for (LayerIndex layer_idx = range.begin(); layer_idx != range.end(); ++layer_idx) {
                                          ...
                                          Polygons        placable = diff(
                                              // Inflate the surface to sit on by the separation distance to increase chance of a support
                                              // being placed on a sloped surface.
                                              offset(below, xy_distance), layer_idx_below < int(anti_overhang.size()) ?
                                                                                     union_(current, anti_overhang[layer_idx_below]) :
                                                                                     current);
...
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    {
        ...
        if (m_precalculated && m_precalculation_progress < TREE_PROGRESS_PRECALC_COLL) {
            ...
            Progress::messageProgress(Progress::Stage::SUPPORT, m_precalculation_progress * m_progress_multiplier + m_progress_offset,
                                      TREE_PROGRESS_TOTAL);
        }
    }
#endif
                                                  ...
                                                  StatusProgress status_progress)
{
    ...
    const size_t total_layers = keys.empty() ? 0 : size_t(max_layer) + 1;
    std::atomic<size_t> completed{0};
    if (status_progress)
        status_progress(0, total_layers, true);
    tbb::parallel_for(tbb::blocked_range<LayerIndex>(0, max_layer + 1, keys.size()), [&](const tbb::blocked_range<LayerIndex>& range) {
        ...
        for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
            for (RadiusLayerPair key : keys)
                if (layer_idx <= key.second) {
                    ...
                    // this union is important as otherwise holes(in form of lines that will increase to holes in a later step) can get
                    // unioned onto the area.
                    data.emplace_back(RadiusLayerPair(radius, layer_idx),
                                      polygons_simplify(offset(union_ex(this->getCollision(m_increase_until_radius, layer_idx, false)),
            ...
            const size_t done = completed.fetch_add(1, std::memory_order_relaxed) + 1;
            if (status_progress)
                status_progress(done, total_layers, false);
        }
        m_collision_cache_holefree.insert(std::move(data));
    });
    if (status_progress)
        status_progress(completed.load(std::memory_order_relaxed), total_layers, true);
}

void TreeModelVolumes::calculateAvoidance(const std::vector<RadiusLayerPair>& keys,
                                          bool                                to_build_plate,
                                          bool                                to_model,
                                          ...
                                          StatusProgress                      status_progress)
{
    ...
    std::atomic<size_t> completed{0};
    if (status_progress)
        status_progress(0, avoidance_tasks.size(), true);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, avoidance_tasks.size(), 1), [this, &avoidance_tasks, &throw_on_cancel,
        ...
        for (size_t task_idx = range.begin(); task_idx < range.end(); ++task_idx) {
            ...
            const bool collision_holefree = (task.slow() || task.holefree()) &&
                                            task.radius < m_increase_until_radius + m_current_min_xy_dist_delta;
            ...
            for (LayerIndex layer_idx = task.start_layer; layer_idx <= task.max_required_layer; ++layer_idx) {
                ...
                const Polygons& current_layer_collisions = collision_holefree ? getCollisionHolefree(task.radius, layer_idx) :
                                                                                getCollision(task.radius, layer_idx, true);
                ...
                for (int istep = 0; istep < move_steps; ++istep)
                    latest_avoidance = union_(current_layer_collisions,
                                              offset(latest_avoidance, istep + 1 == move_steps ? -last_move_step : -move_step,
...
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
            {
                ...
                if (m_precalculated && m_precalculation_progress < TREE_PROGRESS_PRECALC_COLL + TREE_PROGRESS_PRECALC_AVO) {
                    m_precalculation_progress += to_model                 ? 0.4 * TREE_PROGRESS_PRECALC_AVO / (keys.size() * 3) :
                                                 m_support_rests_on_model ? 0.4 :
                                                                            1 * TREE_PROGRESS_PRECALC_AVO / (keys.size() * 3);
                    Progress::messageProgress(Progress::Stage::SUPPORT,
                                              m_precalculation_progress * m_progress_multiplier + m_progress_offset, TREE_PROGRESS_TOTAL);
                }
            }
#endif
            ...
            const size_t done = completed.fetch_add(1, std::memory_order_relaxed) + 1;
            if (status_progress)
                status_progress(done, avoidance_tasks.size(), false);
        }
    });
    if (status_progress)
        status_progress(completed.load(std::memory_order_relaxed), avoidance_tasks.size(), true);
}
                                           ...
                                           StatusProgress status_progress)
{
    ...
    if (status_progress)
        status_progress(0, keys.size(), true);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()), [&, keys](const tbb::blocked_range<size_t>& range) {
...
void TreeModelVolumes::calculatePlaceables(const coord_t radius, const LayerIndex max_required_layer, std::function<void()> throw_on_cancel)
{
    ...
    if (start_layer == 0)
        // Placeable area at layer 0 = available bed surface minus model collision.
        // Use m_bed_area (the actual bed polygon), not m_machine_border (the outside-bed collision).
    ...
    tbb::parallel_for(tbb::blocked_range<LayerIndex>(std::max(1, start_layer), max_required_layer + 1),
                      [this, &data, radius, start_layer, &throw_on_cancel](const tbb::blocked_range<LayerIndex>& range) {
                          for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                              data[layer_idx - start_layer] =
                                  offset(union_ex(getPlaceableAreas(0, layer_idx, throw_on_cancel)),
                                         // As a placeable area is calculated by (collision of the layer below) - (collision of the current
                                         // layer) and the collision is offset by xy_distance, it can happen that a small line is considered
                                         // a flat area to place something onto, even though it is mostly xy_distance that cant support it.
                                         // Making the area smaller by xy_distance fixes this.
                                         -(radius + m_current_min_xy_dist + m_current_min_xy_dist_delta), jtMiter, 1.2);
                              if (throw_on_cancel)
                                  throw_on_cancel();
                          }
                      });
...
void TreeModelVolumes::calculateWallRestrictions(const std::vector<RadiusLayerPair>& keys, std::function<void()> throw_on_cancel,
                                                 StatusProgress status_progress)
{
    // Wall restrictions are mainly important when they represent actual walls that are printed, and not "just" the configured z_distance,
    // because technically valid placement is no excuse for moving through a wall. As they exist to prevent accidentially moving though a
    // wall at high speed between layers like thie (x = wall,i = influence area,o= empty space,d = blocked area because of z distance)
    ...
    // The radius for the upper collission has to be 0 as otherwise one may not enter areas that may be forbidden on layer_idx but not one
    // below (c = not an influence area even though it should ):
    /*
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()), [&, keys](const tbb::blocked_range<size_t>& range) {
        for (size_t key_idx = range.begin(); key_idx < range.end(); ++key_idx) {
            ...
            tbb::parallel_for(tbb::blocked_range<LayerIndex>(min_layer_bottom, max_required_layer + 1),
                              [this, &data, &data_min, radius, min_layer_bottom,
                               &throw_on_cancel](const tbb::blocked_range<LayerIndex>& range) {
                                  for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                                      ...
                                      if (!data_min.empty())
                                          data_min[layer_idx - min_layer_bottom] =
                                              polygons_simplify(intersection(getCollision(0, layer_idx, true),
                                                                             getCollision(radius, layer_idx - 1, true)),
                                                                m_min_resolution);
                                      if (throw_on_cancel)
                                          throw_on_cancel();
                                  }
...
std::vector<std::pair<TreeModelVolumes::RadiusLayerPair, std::reference_wrapper<const Polygons>>>
TreeModelVolumes::RadiusLayerPolygonCache::sorted() const
{
    ...
    assert(std::is_sorted(out.begin(), out.end(), [](auto& l, auto& r) {
        return l.first.second < r.first.second || (l.first.second == r.first.second) && l.first.first < r.first.first;
    }));
    return out;
}

void TreeModelVolumes::addSharpTail(const std::vector<std::pair<std::pair<int, int>, const ExPolygons>>& overhang_sharps)
{
    // std::vector<Polygons>& outlines        = m_layer_outlines.front().second;
    // size_t                 num_raft_layers = m_raft_layers.size();

    // for (const auto& overhang_sharp : overhang_sharps) {
    //     ExPolygons poly  = overhang_sharp.second;
    //     int  layer_idx = overhang_sharp.first.first - num_raft_layers;
    //     int   last_layer = std::max(layer_idx - 15, 10);
    //     if (last_layer < overhang_sharp.first.second)
    //         last_layer = overhang_sharp.first.second;
    //
    //     for (int idx = layer_idx - 1; idx > last_layer; idx--) {
    //         auto iter = m_layer_outlines_sharp.find(idx);
    //         if (iter != m_layer_outlines_sharp.end()) {
    //             append(iter->second, to_polygons(poly));
    //             iter->second = union_(iter->second);
    //         } else {
    //             m_layer_outlines_sharp.insert(std::pair(idx, to_polygons(poly)));
    //         }
    //     }
    // }
}
