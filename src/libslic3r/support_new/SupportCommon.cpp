    ...
    const Flow& flow)
{
    for (ExPolygon& expolygon : expolygons) {
        ...
        if (!polylines.empty()) {
            ...
            eec->no_sort            = true;
            extrusion_entities_append_paths(eec->entities, std::move(polylines), erIroning, flow_mm3_per_mm, extrusion_width,float(extrusion_height));
        }
    }
}
            ...
            auto extrude_interface = [&](SupportGeneratorLayerExtruded &layer_ex, InterfaceLayerType interface_layer_type) {
                if (! layer_ex.empty() && ! layer_ex.polygons_to_extrude().empty()) {
                    ...
                    fill_expolygons_generate_paths(
                        ...
                        // Extrusion parameters
                        ExtrusionRole::erSupportMaterialInterface, interface_flow/*, config.support_interface_pattern*/);

                    if (!is_tree(config.support_type) && interface_layer_type == InterfaceLayerType::TopContact) {
                        if (config.ironing_support_layer && slicing_params.gap_support_object > support_params.support_material_ironing_flow.height())
                        {
                            ...
                            Polygons ironing        = offset(layer_ex.polygons_to_extrude(),-(float) scale_(support_params.support_material_ironing_flow.width()), SUPPORT_SURFACES_OFFSET_PARAMETERS);
                            BoundingBox bbox           = get_extents(ironing);
                            if (bbox.size().x() >= area_ironing_interface_supported && bbox.size().y() >= area_ironing_interface_supported) {
                                       ...
                                       size_t          min_amount_offset)
{
    ...
    auto     collision_trimmed = [&collision_trimmed_buffer, &collision, &ret, distance]() -> const Polygons& {
        if (collision_trimmed_buffer.empty() && !collision.empty())
        {
            ...
            collision_trimmed_buffer = ClipperUtils::clip_clipper_polygons_with_subject_bbox(collision, get_extents(ret).inflated(_distance + SCALED_EPSILON));
        }
