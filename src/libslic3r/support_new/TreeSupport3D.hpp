// [FORMATTED BY CLANG-FORMAT 2026-05-09 13:36:16]
...
// These weights are obtained experimentally using a small sample size. Sensible weights can differ drastically based on the assumed default
// settings and model.
...
struct AreaIncreaseSettings
{
    ...
    {
        return type == other.type && increase_speed == other.increase_speed && increase_radius == other.increase_radius &&
...
struct SupportElementStateBits
{
    ...
    /*!
     * \brief An influence area is considered safe when it can use the holefree avoidance <=> It will not have to encounter holes on its way
     * downward.
     */
...
struct SupportElementState : public SupportElementStateBits
{
    ...
    /*!
     * \brief The next position this support elements wants to reach. NOTE: This is mainly a suggestion regarding direction inside the
     * influence area.
     */
    Point next_position;
...
/*!
 * \brief Get the Distance to top regarding the real radius this part will have. This is different from distance_to_top, which is can be
 * used to calculate the top most layer of the branch.
...
/*!
 * \brief Get the collision Radius of this Element. This can be smaller then the actual radius, as the drawAreas will cut off areas that may
...
struct SupportElement
{
    ...
    // Multiple center points for a merged influence area that is too large for a single core.
    // Used by the "one trunk, multiple cores" strategy to prevent over-merging.
    ...
    /*!
     * \brief Multi-core information for the "one trunk, multiple cores" strategy.
