// [FORMATTED BY CLANG-FORMAT 2026-05-12 19:33:41]
...
enum TreeNodeType { eCircle, eSquare, ePolygon };
...
struct SupportNode
{
    ...
    SupportNode(const Point  position,
    ...
    bool operator==(const SupportNode& other) const { return position == other.position; }
};
...
public:
    ...
    SupportNode* create_node(const Point  position,
                             ...
                             coordf_t     radius_         = 0)
    {
        return m_ts_data->create_node(position, distance_to_top, obj_layer_nr, support_roof_layers_below, to_buildplate, parent, print_z_,
                                      height_, dist_mm_to_top_, radius_);
    }
    ...
    std::vector<ExPolygons>                  long_bridge_roofs_by_layer;
...
private:
    ...
    // Regions where the center of a support circle with the given radius may rest on the model.
    ExPolygons get_placeable_areas(coordf_t radius, size_t layer_nr);
    ...
    Polygons get_trim_support_regions(const PrintObject& object,
