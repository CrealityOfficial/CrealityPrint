// [FORMATTED BY CLANG-FORMAT 2026-05-19 19:45:15]
#include <atomic>
#include <chrono>
#include <filesystem>
#include <math.h>
...
static int64_t floor_div_coord(coord_t value, coord_t divisor)
{
    assert(divisor > 0);
    const int64_t v = int64_t(value);
    const int64_t d = int64_t(divisor);
    return v >= 0 ? v / d : -((-v + d - 1) / d);
}

static uint64_t vertical_enforcer_grid_cell_key(const Point& point, coord_t grid_size)
{
    const int64_t cell_x = floor_div_coord(point.x(), grid_size);
    const int64_t cell_y = floor_div_coord(point.y(), grid_size);
    return (uint64_t(uint32_t(cell_x)) << 32) ^ uint64_t(uint32_t(cell_y));
}
...
#ifdef SUPPORT_TREE_DEBUG_TO_SVG
static void draw_contours_and_nodes_to_svg(std::string                      fname,
                                           ...
                                           std::vector<std::string>         colors  = {"blue", "red", "yellow"})
{
    ...
    // draw legend
    svg.draw_text(bbox.min + Point(scale_(0), scale_(0)), format("nPoints: %1%->%2%", layer_nodes.size(), lower_layer_nodes.size()).c_str(),
                  "green", 2);
    ...
    for (SupportNode* node : layer_nodes) {
        svg.draw({node->overhang}, "green", 0.5);
    }
...
static void draw_layer_mst(const std::string& fname, const std::vector<MinimumSpanningTree>& spanning_trees, const ExPolygons& outline)
{
...
static bool move_inside_expoly(const ExPolygon& polygon,
                               Point&           from,
                               double           distance          = 0,
                               double           max_move_distance = std::numeric_limits<double>::max())
{
    ...
    */
    ...
    // because we compare with vsize2_with_unscale here (no division by zero), we also need to compare by vsize2_with_unscale inside the
    // loop to avoid integer rounding edge cases
    ...
    for (const Point& p2 : polygon.contour.points) {
        ...
        {
            if (projected_p_beyond_prev_segment) { //  case which looks like:   > .
                ...
                if (dist2 < bestDist2) {
                    ...
                    if (distance == 0) {
                        ...
                        Point inward_dir = turn90_ccw(normal(ab, 10.0) +
                                                      normal(p1 - p0, 10.0)); // inward direction irrespective of sign of [distance]
        ...
        } else if (dot_prod >= ab_length2) // x is projected to beyond ab
        {
            ...
            p1                              = p2;
            continue;
        } else { // x is projected to a point properly on the line segment (not onto a vertex). The case which looks like | .
            projected_p_beyond_prev_segment = false;
    ...
    if (is_already_on_correct_side_of_boundary) // when the best point is already inside and we're moving inside, or when the best point is
                                                // already outside and we're moving outside
    {
                ...
                if (projected_p_beyond_prev_segment) { //  case which looks like:   > .
                    ...
                    if (dist2 < bestDist2) {
                        ...
                        if (distance == 0) {
                            ...
                            inward_dir = turn90_ccw(normal(ab, 10.0) +
                                                    normal(p1 - p0, 10.0)); // inward direction irrespective of sign of [distance]
                            // MM2INT(10.0) to retain precision for the eventual normalization
                            ret                                    = x + normal(inward_dir, scale_(distance));
                            is_already_on_correct_side_of_boundary = dot_with_unscale(inward_dir, p - x) * distance >= 0;
    ...
    : m_object(&object), m_slicing_params(slicing_params), m_support_params(object), m_object_config(&object.config())
{
    ...
    diameter_angle_scale_factor = std::clamp<double>(m_object_config->tree_support_branch_diameter_angle * M_PI / 180., 0.,
                                                     0.5 * M_PI - EPSILON);
    ...
    if (svg.is_opened())
        svg.draw(m_machine_border, "yellow");
#endif
...
static bool is_long_bridge_roof_candidate(const ExPolygon& expoly, coordf_t extrusion_width_scaled)
{
    BoundingBox bbox       = get_extents(expoly);
    Point       bbox_size  = bbox.size();
    coord_t     short_side = std::min(bbox_size.x(), bbox_size.y());
    coord_t     long_side  = std::max(bbox_size.x(), bbox_size.y());
    if (short_side <= 0)
        return false;

    const coord_t max_width  = std::max<coord_t>(scale_(3.0), coord_t(3.0 * extrusion_width_scaled));
    const coord_t min_length = std::max<coord_t>(scale_(6.0), coord_t(5.0 * extrusion_width_scaled));
    return short_side <= max_width && long_side >= min_length && double(long_side) / double(short_side) >= 4.0;
}

static bool is_long_internal_solid_roof_candidate(const ExPolygon& expoly)
{
    BoundingBox bbox       = get_extents(expoly);
    Point       bbox_size  = bbox.size();
    coord_t     short_side = std::min(bbox_size.x(), bbox_size.y());
    ...
    const double area_mm2 = std::abs(area(expoly)) * sqr(SCALING_FACTOR);
    return short_side <= scale_(1.2) && long_side >= scale_(34.0) && long_side <= scale_(37.5) &&
           double(long_side) / double(short_side) >= 20.0 && area_mm2 >= 18.0 && area_mm2 <= 38.0;
}
...
void TreeSupport::detect_overhangs(bool check_support_necessity /* = false*/)
{
    ...
    const int progress_percentage = check_support_necessity ? 50 : 55;
    const size_t progress_phase_count = check_support_necessity ? 2 : 6;
    ...
    const auto report_progress = [this, progress_percentage, progress_report_interval_ms, &next_progress_report_ms, &now_ms](
        ...
        const std::string message = !phase.empty() && phase.back() == ')' ?
                                        ...
                                        phase + ": " + percent;
        m_object->print()->set_status(progress_percentage, message);
        BOOST_LOG_TRIVIAL(info) << message;
    };
    const auto progress_phase = [&detecting_overhangs_text, progress_phase_count](size_t step) {
        return detecting_overhangs_text + " (" + std::to_string(step) + "/" + std::to_string(progress_phase_count) + ")";
    };
    const size_t total_layers = m_object->layer_count();
    ...
    const coordf_t extrusion_width = support_material_flow(m_object).width();
    ...
    const double        length_thresh_max_overhang    = scale_(30);
    ...
    struct OverhangCluster
    {
        ...
        OverhangCluster(const ExPolygon* expoly, int layer_nr) { push_back(expoly, layer_nr); }
        ...
        int  height() { return max_layer - min_layer + 1; }
        ...
        void check_small_overhang(coordf_t extrusion_width_scaled,
                                  double   length_thresh_small_overhang,
                                  double   radius_thresh_small_overhang,
                                  double   length_thresh_max_overhang)
        {
            ...
            for (auto it = layer_overhangs.begin(); it != layer_overhangs.end(); it++) {
                ...
                if (narrow_width > extrusion_width_scaled)
                    all_layers_are_small = false;
            ...
            avg_area /= num_layers;
            is_small_overhang = all_layers_are_small && avg_width < extrusion_width_scaled / 2 &&
                                avg_area < SQ(length_thresh_small_overhang) && this->height() < 3;
            ...
            if (erodel.empty()) {
                Point  _bbox_sz = get_extents(merged_poly).size();
                double len      = std::max(_bbox_sz.x(), _bbox_sz.y());
                if (_bbox_sz.x() > length_thresh_max_overhang || _bbox_sz.y() > length_thresh_max_overhang) {
                    // Retain the thinner overhangs with a length exceeding 30mm.
                } else {
                    is_small_overhang = true;
                    set_type(Small, true);
                }
            } else if ((bbox_sz.x() < length_thresh_small_overhang && bbox_sz.y() < length_thresh_small_overhang &&
        ...
        void check_polygon_node(float thresh_big_overhang, const ExPolygons& m_layer_outlines_below)
        {
            if (area(merged_poly) > SQ(thresh_big_overhang))
                ...
                // Only thin-plate candidates need the model outline. On painted supports,
                // eroding it for every cluster can dominate overhang classification.
                ExPolygons outlines_below_eroded = offset_ex(m_layer_outlines_below, -this->offset);
                if (!overlaps(merged_poly, outlines_below_eroded))
                    set_type(ThinPlate, true);
            }
        }
    };
    ...
    long_bridge_roofs_by_layer.clear();
    long_bridge_roofs_by_layer.resize(m_object->layer_count());
    long_internal_solid_roofs_by_layer.clear();
    long_internal_solid_roofs_by_layer.resize(m_object->layer_count());

    // calc the extrudable expolygons of each layer
    std::atomic<size_t> prepared_layers{0};
    report_progress(progress_phase(1), 0, total_layers, true);
    tbb::parallel_for(tbb::blocked_range<size_t>(0, m_object->layer_count()), [&](const tbb::blocked_range<size_t>& range) {
        for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
            ...
            layer->lslices_extrudable = intersection_ex(layer->lslices,
                                                        offset2_ex(layer->lslices, -extrusion_width_scaled / 2, extrusion_width_scaled));
            ...
            const size_t completed = prepared_layers.fetch_add(1, std::memory_order_relaxed) + 1;
            report_progress(progress_phase(1), completed, total_layers, false);
        }
    });
    report_progress(progress_phase(1), prepared_layers.load(std::memory_order_relaxed), total_layers, true);
    ...
    report_progress(progress_phase(2), 0, total_layers, true);

    const bool support_enabled = m_object_config->enable_support.value;
    auto enforcers = support_enabled ? m_object->slice_support_enforcers() : std::vector<Polygons>{};
    auto blockers  = support_enabled ? m_object->slice_support_blockers() : std::vector<Polygons>{};
    m_vertical_enforcer_points.clear();
    if (support_enabled && m_object->model_object()->is_fdm_support_painted()) {
        // BLOCKER and ENFORCER projections are independent (disjoint triangle subsets, separate output
        // vectors), so run them concurrently to halve the painted-support projection wall time.
        tbb::parallel_invoke(
        ...
        // slice_mesh_slabs emits one polygon per painted triangle per layer, so each layer can hold
        // hundreds of overlapping pieces. Every downstream clipper op (intersection_ex / diff_ex /
        // offset_ex / safe_offset_inc) scales with that polygon count and runs both in detect_overhangs
        // here and per-layer inside generate_initial_areas for organic. Collapse the painted polygons
        // into a clean shape once per layer in parallel so the rest of the pipeline pays the cheaper
        // bill.
        constexpr size_t painted_union_threshold = 8;
        const size_t     max_painted_layers      = std::max(enforcers.size(), blockers.size());
        tbb::parallel_for(tbb::blocked_range<size_t>(0, max_painted_layers), [&](const tbb::blocked_range<size_t>& range) {
            for (size_t i = range.begin(); i < range.end(); ++i) {
                if (i < enforcers.size() && enforcers[i].size() > painted_union_threshold)
                    enforcers[i] = union_(enforcers[i]);
                if (i < blockers.size() && blockers[i].size() > painted_union_threshold)
                    blockers[i] = union_(blockers[i]);
            }
        });
    }
    ...
    std::atomic<size_t> detected_layers{0};
    tbb::parallel_for(tbb::blocked_range<size_t>(0, m_object->layer_count()), [&](const tbb::blocked_range<size_t>& range) {
        for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
            if (m_object->print()->canceled())
                break;
            const size_t completed = detected_layers.fetch_add(1, std::memory_order_relaxed) + 1;
            report_progress(progress_phase(2), completed, total_layers, false);
            // FIXME the param enforce_support_layers is not set yet
            if (!(is_auto(stype) || (enforce_support_layers > 0 && layer_nr >= enforce_support_layers) ||
            ...
            coordf_t lower_layer_offset       = layer_nr < enforce_support_layers ? -0.15 * extrusion_width :
                                                                                    (float) lower_layer->height / tan(threshold_rad);
            ...
            ExPolygons blocker;
            const bool has_enforcer_layer = layer_nr < enforcers.size() && !enforcers[layer_nr].empty();
            if (has_enforcer_layer)
                enforced_overhangs = intersection_ex(diff_ex(layer->lslices_extrudable, lower_layer->lslices_extrudable),
                                                     enforcers[layer_nr]);
            if (is_auto(stype)) {
                // normal overhang
                ...
                if (layer_nr < blockers.size() && !blockers[layer_nr].empty()) {
                    ...
                    if (!blocker.empty())
                        overhangs_all_layers[layer_nr] = diff_ex(overhangs_all_layers[layer_nr], blocker);
                }
            ...
            } else if (has_enforcer_layer && lower_layer) {
                if (!enforced_overhangs.empty()) {
            ...
            if (duration > 30 || overhangs_all_layers[layer_nr].size() > 100) {
                BOOST_LOG_TRIVIAL(info) << "detect_overhangs takes more than 30 secs, skip cantilever and sharp tails detection: layer_nr="
                                        << layer_nr << " duration=" << duration;
            ...
            if (is_auto(stype) && config_detect_sharp_tails) {
                ...
                if (!blocker.empty())
                    curr = diff_ex(curr, blocker);
                if (!enforced_overhangs.empty())
                    curr = union_ex(curr, enforced_overhangs);
                ...
                for (size_t i = 0; i < poly.num_contours(); i++) {
                    ...
                    for (const auto& pt : contour.points) {
                        if (is_inside_ex(lower_layer_support_thresh, pt)) {
                            ...
                            tmp_dist_max = std::max(tmp_dist_max,
                                                    (projection_onto(lower_layer_support_thresh, pt) - pt).cast<double>().norm());
                    }
                    if (is_floating) {
                        is_cantilever = true;
    ...
    report_progress(progress_phase(2), detected_layers.load(std::memory_order_relaxed), total_layers, true);
    ...
    const coordf_t minimum_support_area = scale_(scale_(config.minimum_support_area.value));

    // check if the sharp tails should be extended higher
    report_progress(progress_phase(3), 0, total_layers, true);
    if (is_auto(stype) && config_detect_sharp_tails) {
        for (size_t layer_nr = 0; layer_nr < m_object->layer_count(); layer_nr++) {
            if (m_object->print()->canceled())
                break;
            report_progress(progress_phase(3), layer_nr + 1, total_layers, false);
            ...
            if (!enforcers.empty() && layer_nr < enforcers.size() && !enforcers[layer_nr].empty())
                curr_polys = union_ex(curr_polys, intersection_ex(diff_ex(layer->lslices_extrudable, lower_layer->lslices_extrudable),
                                                                  enforcers[layer_nr]));
            for (ExPolygon& expoly : curr_polys) {
                ...
                do {
                    ...
                    if ((get_extents(new_overhang_expolys).size() - get_extents(lower_layer_sharptails).size())
                            .both_comp(Point(scale_(5), scale_(5)), ">") ||
                        !offset_ex(new_overhang_expolys, -5.0 * extrusion_width_scaled).empty()) {
                        is_sharp_tail = false;
                        break;
                    }
    ...
    report_progress(progress_phase(3), total_layers, total_layers, true);

    // group overhang clusters
    report_progress(progress_phase(4), 0, total_layers, true);
    for (size_t layer_nr = 0; layer_nr < m_object->layer_count(); layer_nr++) {
        if (m_object->print()->canceled())
            break;
        report_progress(progress_phase(4), layer_nr + 1, total_layers, false);
        Layer* layer = m_object->get_layer(layer_nr);
    ...
    report_progress(progress_phase(4), total_layers, total_layers, true);

    size_t processed_clusters = 0;
    report_progress(progress_phase(5), 0, overhangClusters.size(), true);
    for (auto& cluster : overhangClusters) {
        ...
        if (is_auto(stype) && config_remove_small_overhangs) {
            ...
            cluster.check_small_overhang(extrusion_width_scaled, length_thresh_small_overhang, radius_thresh_small_overhang,
                                         length_thresh_max_overhang);

#ifdef SUPPORT_TREE_DEBUG_TO_SVG
            const Layer* layer1 = m_object->get_layer(cluster.min_layer);
            std::string  fname  = debug_out_path("overhangCluster_%d-%d_%.2f-%.2f_tail=%d_cantilever=%d_small=%d.svg", cluster.min_layer,
                                                 cluster.max_layer, layer1->print_z, m_object->get_layer(cluster.max_layer)->print_z,
            ...
            SVG::export_expolygons(fname, {{layer1->lslices, {"min_layer_lslices", "red", 0.5}},
                                           ...
                                           {cluster.is_cantilever() ? layer1->cantilevers :
        ...
        if (!cluster.is_type(Small) || enforce_add) {
            const int overhang_type = (!is_auto(stype) ? (cluster.type | OverhangType::Enforced) : cluster.type);
            ...
            for (auto it = cluster.layer_overhangs.begin(); it != cluster.layer_overhangs.end(); it++) {
                ...
                for (const auto& overhang : overhangs)
                    add_overhang(m_object->get_layer(layer_nr), overhang, overhang_type);
            }
        }
        ++processed_clusters;
        report_progress(progress_phase(5), processed_clusters, overhangClusters.size(), false);
    }
    report_progress(progress_phase(5), processed_clusters, overhangClusters.size(), true);

    int layers_with_overhangs = 0;
    int layers_with_enforcers = 0;
    report_progress(progress_phase(6), 0, total_layers, true);
    for (int layer_nr = 0; layer_nr < m_object->layer_count(); layer_nr++) {
        if (m_object->print()->canceled())
            break;
        report_progress(progress_phase(6), size_t(layer_nr) + 1, total_layers, false);
        ...
        if (lower_layer && !layer->sharp_tails_height.empty()) {
            ...
            for (size_t i = 0; i < layer->sharp_tails_height.size(); i++) {
                ...
                if (!areas.empty() && int(accum_height * 10) % 5 == 0) {
                    has_sharp_tails = true;
                    for (auto& area : areas) {
                        add_overhang(layer, area,
                                     accum_height < EPSILON ? (OverhangType::SharpTail | OverhangType::SharpTailLowesst) :
                                                              OverhangType::SharpTail);
                    }
#ifdef SUPPORT_TREE_DEBUG_TO_SVG
        ...
        if (max_bridge_length > 0 && layer->loverhangs.size() > 0 && lower_layer) {
            ...
            m_object->remove_bridges_from_contacts(lower_layer, layer, extrusion_width_scaled, &layer->loverhangs, max_bridge_length,
                                                   break_bridge);
            // Organic / 2D tree plant from loverhangs_with_type. Keep it in sync with the filtered loverhangs.
            std::vector<std::pair<ExPolygon, int>> typed_kept;
            typed_kept.reserve(layer->loverhangs_with_type.size());
            for (const auto& part : layer->loverhangs_with_type) {
                ExPolygons kept = intersection_ex({part.first}, layer->loverhangs);
                for (ExPolygon& poly : kept)
                    if (!poly.empty())
                        typed_kept.emplace_back(std::move(poly), part.second);
            }
            layer->loverhangs_with_type = std::move(typed_kept);
        }

        // Collect roofs after dropping short bridges, otherwise organic add_roof / 2D roof_areas
        // still plant support under max_bridge_length spans (#11937).
        if (config.support_interface_top_layers.value > 0) {
            ExPolygons long_bridge_roofs;
            ExPolygons long_internal_solid_roofs;
            for (LayerRegion* region : layer->regions()) {
                for (const Surface& surface : region->fill_surfaces.surfaces) {
                    if (surface.surface_type == stInternalSolid && is_long_internal_solid_roof_candidate(surface.expolygon))
                        long_internal_solid_roofs.emplace_back(surface.expolygon);
                    if (surface.surface_type != stBottomBridge)
                        continue;
                    if (!is_long_bridge_roof_candidate(surface.expolygon, extrusion_width_scaled))
                        continue;

                    if (!layer->loverhangs.empty()) {
                        ExPolygons bridge_overhangs = intersection_ex({surface.expolygon}, layer->loverhangs);
                        if (!bridge_overhangs.empty()) {
                            append(long_bridge_roofs, bridge_overhangs);
                            continue;
                        }
                    }
                    // Keep a roof only for spans longer than max_bridge_length. The old fallback
                    // (always add the whole surface) put short bridges back after they were filtered.
                    const Point bbox_size = get_extents(surface.expolygon).size();
                    if (max_bridge_length <= 0 ||
                        (bbox_size.x() >= max_bridge_length || bbox_size.y() >= max_bridge_length))
                        long_bridge_roofs.emplace_back(surface.expolygon);
                }
            }
            if (!long_bridge_roofs.empty())
                long_bridge_roofs_by_layer[layer_nr] = union_ex(long_bridge_roofs);
            if (!long_internal_solid_roofs.empty())
                long_internal_solid_roofs_by_layer[layer_nr] = union_ex(long_internal_solid_roofs);
        }
        ...
        if (layer_nr < long_bridge_roofs_by_layer.size() && !long_bridge_roofs_by_layer[layer_nr].empty())
            m_highest_overhang_layer = std::max(m_highest_overhang_layer, size_t(layer_nr));
        // if (nEnforced > 0) layers_with_enforcers++;
        if (!layer->cantilevers.empty())
            has_cantilever = true;
    }
    report_progress(progress_phase(6), total_layers, total_layers, true);

    BOOST_LOG_TRIVIAL(info) << "Tree support overhang detection done. " << layers_with_overhangs
                            << " layers with overhangs. nEnforced=" << layers_with_enforcers;
...
static inline BoundingBox fill_expolygon_generate_paths(
    ExtrusionEntitiesPtr& dst, ExPolygon& expolygon, Fill* filler, const FillParams& fill_params, ExtrusionRole role, const Flow& flow)
{
...
static bool is_tiny_tree_support_fragment(const ExPolygon& expoly, const Flow& flow)
{
    const double line_width = double(flow.scaled_width());
    if (line_width <= 0.)
        return false;

    // Filter only pieces around a single extrusion line; larger patches are left for the existing path logic.
    const double fragment_area_limit = sqr(1.25 * line_width);
    if (std::abs(area(expoly)) < fragment_area_limit)
        return true;

    const BoundingBox bbox       = get_extents(expoly);
    const Point       bbox_size  = bbox.size();
    const double      short_side = double(std::min(bbox_size.x(), bbox_size.y()));
    const double      long_side  = double(std::max(bbox_size.x(), bbox_size.y()));
    return short_side < 0.9 * line_width && long_side < 1.5 * line_width;
}

static void filter_tiny_tree_support_fragments(ExPolygons& expolys, const Flow& flow)
{
    expolys.erase(std::remove_if(expolys.begin(), expolys.end(),
                                 [&flow](const ExPolygon& expoly) { return is_tiny_tree_support_fragment(expoly, flow); }),
                  expolys.end());
}

static void _make_loops(
    ExtrusionEntitiesPtr& loops_entities, ExPolygons& support_area, ExtrusionRole role, size_t wall_count, const Flow& flow)
{
    Polygons                  loops;
    ...
    while (!expoly_list.empty()) {
        ...
        if (depth_iter->second + 1 < wall_count) {
            ...
            ExPolygons expolys_new = offset2_ex({expoly_list.front()}, -1.4 * float(flow.scaled_spacing()),
                                                .4 * float(flow.scaled_spacing()));
    ...
    extrusion_entities_append_loops(loops_entities, std::move(loops), role, float(flow.mm3_per_mm()), float(flow.width()),
                                    float(flow.height()));
}

static void make_perimeter_and_inner_brim(
    ExtrusionEntitiesPtr& dst, const ExPolygon& support_area, size_t wall_count, const Flow& flow, ExtrusionRole role)
{
    if (is_tiny_tree_support_fragment(support_area, flow))
        return;

    Polygons   loops;
    ExPolygons support_area_new = offset_ex(support_area, -0.5f * float(flow.scaled_spacing()), jtSquare);
    filter_tiny_tree_support_fragments(support_area_new, flow);
    _make_loops(dst, support_area_new, role, wall_count, flow);
}

static void make_perimeter_and_infill(ExtrusionEntitiesPtr& dst,
                                      const ExPolygon&      support_area,
                                      size_t                wall_count,
                                      const Flow&           flow,
                                      ExtrusionRole         role,
                                      Fill*                 filler_support,
                                      ...
                                      bool                  infill_first = true)
{
    ...
    filter_tiny_tree_support_fragments(to_infill, flow);
...
static Polylines make_transition_infill_paths(const ExPolygon& support_area,
                                              const Flow&      flow,
                                              Fill*            filler_support,
                                              double           support_density)
{
    ...
    Polylines  out;
    ExPolygons to_infill = offset_ex(support_area, -float(flow.scaled_spacing()), jtSquare);
    filter_tiny_tree_support_fragments(to_infill, flow);
    for (ExPolygon& expoly : to_infill) {
        Surface   surface(stInternal, std::move(expoly));
        Polylines polylines;
        try {
            polylines = filler_support->fill_surface(&surface, fill_params);
        } catch (InfillFailedException&) {}
        append(out, std::move(polylines));
    }
    return out;
}

static bool should_check_roof_transition_area(const ExPolygon& poly, coord_t line_width)
{
    if (line_width <= 0 || !poly.holes.empty())
        return false;

    const BoundingBox poly_bbox = get_extents(poly);
    const Point       poly_size = poly_bbox.size();
    const coord_t     long_span = std::max(poly_size.x(), poly_size.y());
    const coord_t     short_span = std::min(poly_size.x(), poly_size.y());
    if (short_span <= 0)
        return false;

    const coord_t max_small_transition_span = 40 * line_width;
    if (long_span > max_small_transition_span)
        return false;

    const double max_slender_span   = 12.0 * double(line_width);
    const double min_slender_aspect = 4.0;
    if (double(long_span) > max_slender_span && double(long_span) / double(short_span) >= min_slender_aspect)
        return false;

    const double poly_area = std::abs(area(poly));
    const double bbox_area = double(poly_size.x()) * double(poly_size.y());
    if (bbox_area <= 0. || poly_area / bbox_area < 0.35)
        return false;

    const double perimeter = poly.contour.length();
    if (perimeter <= 0.)
        return false;

    const double compactness = 4. * M_PI * poly_area / sqr(perimeter);
    return compactness >= 0.12;
}

static void filter_short_support_contours(Polylines& polylines, double min_length)
{
...
static void remove_overlapping_support_contours(Polylines& polylines, coord_t overlap_distance)
{
    ...
    for (Polyline& polyline : polylines) {
        ...
        if (!kept.empty())
            remaining = diff_pl(remaining, offset(kept, overlap_distance, ClipperLib::jtRound, scaled<float>(0.01)));
        append(kept, std::move(remaining));
    }
    polylines = std::move(kept);
}

static void filter_short_free_support_contours(Polylines& polylines, double max_free_length, double attach_distance)
{
    ...
    auto endpoint_is_connected = [&endpoints, attach_distance2 = sqr(attach_distance)](
                                     size_t polyline_idx, bool front, const Point& point) {
        for (const EndpointRef& endpoint : endpoints) {
            if (endpoint.polyline_idx == polyline_idx && endpoint.front == front)
                continue;
            if ((endpoint.point - point).cast<double>().squaredNorm() <= attach_distance2)
                return true;
        }
        return false;
    };
    ...
    polylines.erase(std::remove_if(polylines.begin(), polylines.end(),
                                   [&](const Polyline& pl) {
                                       ...
                                       const bool front_connected = endpoint_is_connected(current_idx, true, pl.points.front());
                                       const bool back_connected  = endpoint_is_connected(current_idx, false, pl.points.back());
                                       return front_connected != back_connected;
                                   }),
                    polylines.end());
}

static void filter_bridgeable_unsupported_support_contours(Polylines& polylines, const Polylines& source_contours,
                                                           double max_bridge_length, double merge_supported_gap)
{
    ...
    auto project_point = [](const Point& point, const Polyline& contour) {
        Projection best;
        if (contour.points.size() < 2)
            return best;
        ...
        for (size_t i = 1; i < contour.points.size(); ++i) {
            ...
            const Vec2d ap = point.cast<double>() - a;
            const double t = std::clamp(ap.dot(ab) / len2, 0., 1.);
            const Vec2d projected = a + t * ab;
            const double dist2 = (point.cast<double>() - projected).squaredNorm();
            if (dist2 < best.dist2) {
                best.distance = prefix + std::sqrt(len2) * t;
                best.dist2    = dist2;
                best.valid    = true;
            }
            prefix += std::sqrt(len2);
        }
        return best;
    };

    auto project_to_source_contours = [&source_contours, &project_point](const Point& point) {
        Projection best;
        for (size_t contour_idx = 0; contour_idx < source_contours.size(); ++contour_idx) {
            Projection projected = project_point(point, source_contours[contour_idx]);
            if (projected.valid && projected.dist2 < best.dist2) {
                projected.contour_idx = contour_idx;
                best = projected;
            }
        }
        return best;
    ...
    auto point_at_distance = [](const Polyline& contour, double distance) {
        ...
        for (size_t i = 1; i < contour.points.size(); ++i) {
            ...
            if (distance <= prefix + len) {
                const double t = std::clamp((distance - prefix) / len, 0., 1.);
                const Vec2d p = a + t * ab;
    ...
    auto copy_contour_interval = [&point_at_distance](const Polyline& contour, double begin, double end) {
        Polyline copied;
        const double contour_length = contour.length();
        ...
        auto append_range = [&](double range_begin, double range_end) {
            ...
            add_point(point_at_distance(contour, range_begin));

            double prefix = 0.;
            for (size_t i = 1; i < contour.points.size(); ++i) {
                const double segment_length = (contour.points[i] - contour.points[i - 1]).cast<double>().norm();
                if (segment_length <= 1e-6)
                    continue;

                const double segment_begin = prefix;
                const double segment_end   = prefix + segment_length;
                if (segment_end <= range_begin) {
                    prefix = segment_end;
                    continue;
                }
                if (segment_begin >= range_end)
                    break;

                if (segment_begin > range_begin && segment_begin < range_end)
                    add_point(contour.points[i - 1]);
                prefix = segment_end;
            }

            add_point(point_at_distance(contour, range_end));
        };
    ...
    for (size_t patch_idx = 0; patch_idx < polylines.size(); ++patch_idx) {
        ...
        for (size_t point_idx = 1; point_idx < patch.points.size(); ++point_idx) {
            ...
            const double begin = std::min(first.distance, last.distance);
            const double end   = std::max(first.distance, last.distance);
            if (end - begin <= 1e-6)
                continue;
    ...
    for (size_t contour_idx = 0; contour_idx < intervals_by_contour.size(); ++contour_idx) {
        ...
        const Polyline& source_contour = source_contours[contour_idx];
        const double source_length = source_contour.length();
        const bool closed_contour = source_contour.points.size() > 2 && source_contour.points.front() == source_contour.points.back();
        ...
        for (size_t merged_idx = 0; merged_idx < merged_intervals.size(); ++merged_idx) {
            ...
            if (!has_reliable_anchor || unsupported_length > max_bridge_length) {
                Polyline copied = copy_contour_interval(source_contour, merged.begin, merged.end);
...
static Polylines simplify_roof_1st_support_paths(Polylines patch_paths, const Polylines& upper_contours,
                                                 double min_patch_length, double max_bridge_length,
                                                 double merge_supported_gap, coord_t overlap_merge_distance)
{
    filter_bridgeable_unsupported_support_contours(patch_paths, upper_contours, max_bridge_length, merge_supported_gap);
    remove_overlapping_support_contours(patch_paths, overlap_merge_distance);

    filter_short_support_contours(patch_paths, min_patch_length);
...
static Polylines extract_roof_1st_support_demand_paths(Polylines printed_patch_paths,
                                                       double max_nonpropagating_free_length,
                                                       coord_t line_width)
{
    filter_short_free_support_contours(printed_patch_paths, max_nonpropagating_free_length, double(line_width));
    return printed_patch_paths;
}

struct Roof1stSupportPatchPaths {
    Polylines printed_paths;
    Polylines paths_to_support;
};

static Roof1stSupportPatchPaths roof_1st_support_patch_paths(const Polygons& layer_supported,
                                                             const Polygons& legal_region,
                                                             const Polylines& upper_contours,
                                                             coord_t line_width,
                                                             double max_bridge_length)
{
    if (upper_contours.empty() || line_width <= 0)
        return {};
    ...
    Polygons unsupported_region = layer_supported.empty() ? upper_demand_region : diff(upper_demand_region, layer_supported);
    if (!legal_region.empty())
        unsupported_region = intersection(unsupported_region, legal_region);
    if (unsupported_region.empty())
        return {};

    Polylines patch_paths = intersection_pl(upper_contours, unsupported_region);
    patch_paths = simplify_roof_1st_support_paths(std::move(patch_paths), upper_contours, min_patch_length,
                                                  max_bridge_length, merge_supported_gap, overlap_merge_distance);
    if (patch_paths.empty())
        return {};

    Roof1stSupportPatchPaths out;
    out.paths_to_support = extract_roof_1st_support_demand_paths(patch_paths, max_nonpropagating_free_length, line_width);
    out.printed_paths = std::move(patch_paths);
    return out;
}

static void collect_existing_support_paths_for_transition(const ExtrusionEntity& entity, Polylines& out)
{
    if (const ExtrusionEntityCollection* collection = dynamic_cast<const ExtrusionEntityCollection*>(&entity)) {
        for (const ExtrusionEntity* child : collection->entities)
            if (child != nullptr)
                collect_existing_support_paths_for_transition(*child, out);
        return;
    }

    const ExtrusionRole role = entity.role();
    if (role != erSupportMaterial && role != erSupportMaterialInterface && role != erSupportTransition)
        return;
...
void TreeSupport::generate_toolpaths()
{
    ...
    coordf_t                 nozzle_diameter         = get_physical_nozzle_diameter(*m_print_config, object_config.support_filament - 1);
    coordf_t                 layer_height            = object_config.layer_height.value;
    const int                wall_count              = object_config.tree_support_wall_count.value;
    const size_t             support_layer_count     = m_object->support_layer_count();
    const bool               repair_strong_tree_contours =
        m_support_params.support_style == smsTreeStrong && object_config.tree_support_organic_validate_repair.value;
    ...
    struct DeferredTreeContourRegion {
        Polygons           polygons;
        SupportParameters  support_params;
    };
    std::vector<Polygons>                         tree_contour_repair_regions(support_layer_count);
    std::vector<std::vector<DeferredTreeContourRegion>> deferred_tree_contour_regions(support_layer_count);
    ...
    if (support_layer_count > m_raft_layers) {
    ...
    for (; layer_nr < m_slicing_params.base_raft_layers; layer_nr++) {
        ...
        FillParams fill_params;
        coordf_t   raft_density = std::min(1., support_flow.spacing() /
                                                   (object_config.support_base_pattern_spacing.value + support_flow.spacing()));
        ...
        fill_expolygons_generate_paths(ts_layer->support_fills.entities, raft_areas1, filler_raft, fill_params, erSupportMaterial,
                                       support_flow);
    }
    ...
    for (layer_nr = m_slicing_params.base_raft_layers;
         layer_nr < m_slicing_params.base_raft_layers + m_slicing_params.interface_raft_layers; layer_nr++) {
        SupportLayer* ts_layer = m_object->get_support_layer(layer_nr);
        ...
        fill_expolygons_generate_paths(ts_layer->support_fills.entities, raft_interface_areas, filler_interface, fill_params,
                                       erSupportMaterialInterface, support_flow);

        fill_params.density = object_config.raft_first_layer_density * 0.01;
        fill_expolygons_generate_paths(ts_layer->support_fills.entities, raft_base_areas, filler_interface, fill_params, erSupportMaterial,
                                       support_flow);
    }
    ...
    for (; layer_nr < m_raft_layers; layer_nr++) {
        ...
        for (auto& poly : first_non_raft_base)
            make_perimeter_and_infill(ts_layer->support_fills.entities, poly, std::min(1, wall_count), support_flow, erSupportMaterial,
                                      filler_raft, interface_density, false);
    }
    ...
    // Generate regular tree support tool paths in parallel. Roof1stLayer is
    // handled in a small bottom-up pass afterwards because it may need to add
    // a local support patch to the already generated layer below.
    tbb::parallel_for(
        tbb::blocked_range<size_t>(m_raft_layers, support_layer_count), [&](const tbb::blocked_range<size_t>& range)

        {
            for (size_t layer_id = range.begin(); layer_id < range.end(); layer_id++) {
                ...
                std::unique_ptr<Fill> filler_interface(Fill::new_from_type(m_support_params.contact_fill_pattern));
                ...
                Flow          support_flow(support_extrusion_width, ts_layer->height, nozzle_diameter);
                Flow          interface_flow    = support_material_interface_flow(m_object,
                                                                                  ts_layer->height); // update flow using real support layer height
                ...
                size_t _wall_count = wall_count;
                for (auto& area_group : ts_layer->area_groups) {
                    ExPolygon& poly = *area_group.area;
                    ExPolygons polys;
                    FillParams fill_params;
                    if (area_group.node_type != ePolygon) {
                        _wall_count = object_config.tree_support_wall_count_tree.value;
                    } else {
                        _wall_count = object_config.tree_support_wall_count.value;
                    }
                    if (area_group.type == SupportLayer::Roof1stLayer)
                        continue;

                    if (is_tiny_tree_support_fragment(poly, area_group.type == SupportLayer::BaseType ? support_flow : interface_flow))
                        continue;
                    ...
                    if (area_group.type == SupportLayer::FloorType) {
                        ...
                        fill_expolygons_generate_paths(ts_layer->support_fills.entities, polys, filler_interface.get(), fill_params,
                        ...
                        if (m_support_params.contact_fill_pattern == ipRectilinear) {
                            ...
                            for (size_t i = 0; i < m_support_params.num_top_interface_layers; i++) {
                                if ((layer_id + i) >= m_object->support_layer_count()) {
                                    break;
                                }

                                auto cur_ts_layer = m_object->get_support_layer(layer_id + i);
                                if (cur_ts_layer == nullptr)
                                    break;
                                Layer* obj_layer = m_object->get_layer_at_bottomz(cur_ts_layer->print_z +
                                                                                      object_config.support_top_z_distance.value,
                                                                                  layer_height / 10.);
                        ...
                        for (auto& single_poly : union_ex(polys))
                            make_perimeter_and_infill(temp_support_fills->entities, single_poly, 1, interface_flow,
                                                      erSupportMaterialInterface, filler_interface.get(), interface_density, false);
                        ...
                        SupportMaterialPattern support_pattern = (area_group.node_type != ePolygon) ?
                                                                     m_object_config->support_base_pattern_tree :
                                                                     m_object_config->support_base_pattern;
                        ...
                        InfillPattern base_fill_pattern      = area_group.node_type != ePolygon ? m_support_params.base_fill_pattern_tree :
                                                                                                  m_support_params.base_fill_pattern;
                        std::shared_ptr<Fill> filler_support = std::shared_ptr<Fill>(
                            Fill::new_from_type(layer_id == 0 ? ipConcentric : base_fill_pattern));
                        filler_support->set_bounding_box(bbox_object);
                        filler_support->spacing = support_flow.spacing();
                        double angle            = (area_group.node_type != ePolygon) ? angles_tree[layer_id % angles_tree.size()] :
                                                                                       angles[layer_id % angles.size()];
                        ...
                        if (layer_id == 0) {
                            ...
                            fill_expolygons_with_sheath_generate_paths(ts_layer->support_fills.entities, loops, filler_support.get(),
                                                                       density, erSupportMaterial, flow, m_support_params, true, false);
                        } else {
                            if (need_infill && m_support_params.base_fill_pattern != ipLightning) {
                                ...
                                make_perimeter_and_infill(ts_layer->support_fills.entities, poly, std::max(min_wall_count, _wall_count),
                                                          flow, erSupportMaterial, filler_support.get(), support_density);
                            } else {
                                SupportParameters support_params = m_support_params;

                                if (area_group.node_type != ePolygon)
                                    support_params.tree_branch_diameter_double_wall_area_scaled = _wall_count > 1 ?
                                                                                                      0.1 :
                                                                                                  _wall_count == 0 ?
                                                                                                      0.25 * sqr(scaled<double>(5.0)) *
                                                                                                          M_PI :
                                                                                                      std::numeric_limits<double>::max();
                                if (area_group.need_extra_wall && _wall_count == 0)
                                    support_params.tree_branch_diameter_double_wall_area_scaled = 0.1;
                                if (repair_strong_tree_contours) {
                                    append(tree_contour_repair_regions[layer_id], loops);
                                    deferred_tree_contour_regions[layer_id].push_back({loops, support_params});
                                } else {
                ...
                if (m_support_params.base_fill_pattern == ipLightning) {
                    ...
                    if (printZ_to_lightninglayer.find(print_z) == printZ_to_lightninglayer.end())
                        continue;
                    // TODO:
                    // 1.the second parameter of convertToLines seems to decide how long the lightning should be trimmed from its root, so
                    // that the root wont overlap/detach the support contour.
                    ...
                    // 3.function connect_infill() and variable 'params' helps create connection pattern along contours between two
                    // lightning roots,
                    //  strengthen lightnings while it may make support harder. decide to enable it or not. if yes, proper values for params
                    //  are remained to be tested
    ...
    const coordf_t max_support_bridge_length = scale_(object_config.max_bridge_length.value);
    std::vector<ContourSupportLayerInput> roof_support_layers(support_layer_count);
    auto model_region_for_support_z = [this](coordf_t support_print_z) -> const ExPolygons* {
        if (m_object->layer_count() == 0)
            return nullptr;

        size_t object_layer_idx = 0;
        while (object_layer_idx + 1 < m_object->layer_count() &&
               m_object->get_layer(int(object_layer_idx + 1))->print_z <= support_print_z + EPSILON)
            ++object_layer_idx;

        const Layer* object_layer = m_object->get_layer(int(object_layer_idx));
        return object_layer != nullptr && object_layer->print_z <= support_print_z + EPSILON ?
                   &object_layer->lslices :
                   nullptr;
    };

    if (repair_strong_tree_contours) {
        std::vector<ContourSupportLayerInput> tree_contour_repair_layers(support_layer_count);
        tbb::parallel_for(
            tbb::blocked_range<size_t>(m_raft_layers, support_layer_count),
            [&](const tbb::blocked_range<size_t>& range) {
                for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
                    SupportLayer* ts_layer = m_object->get_support_layer(layer_id);
                    if (ts_layer == nullptr || ts_layer->height < EPSILON)
                        continue;

                    Flow support_flow(support_extrusion_width, ts_layer->height, nozzle_diameter);
                    ContourSupportLayerInput& input = tree_contour_repair_layers[layer_id];
                    if (!tree_contour_repair_regions[layer_id].empty())
                        input.legal_region = union_safety_offset(tree_contour_repair_regions[layer_id]);
                    input.print_z      = ts_layer->print_z;
                    input.line_width   = support_flow.scaled_width();
                    input.model_region = model_region_for_support_z(ts_layer->print_z);
                    collect_existing_support_paths_for_transition(ts_layer->support_fills, input.printed_paths);
                    input.valid        = true;
                }
            });

        ContourSupportPlan tree_contour_repair_plan =
            plan_tree_contour_repair_paths(std::move(tree_contour_repair_layers), m_raft_layers,
                                           max_support_bridge_length, false);
        ...
        tbb::parallel_for(
            ...
            [&](const tbb::blocked_range<size_t>& range) {
                for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
                    ...
                    Flow support_flow(support_extrusion_width, ts_layer->height, nozzle_diameter);
                    for (const DeferredTreeContourRegion& region : deferred_tree_contour_regions[layer_id])
                        tree_supports_generate_paths(ts_layer->support_fills.entities, region.polygons,
                                                     support_flow, region.support_params);

                    if (!tree_contour_repair_plan.patch_paths_by_layer[layer_id].empty())
                        extrusion_entities_append_paths(
                            ts_layer->support_fills.entities,
                            std::move(tree_contour_repair_plan.patch_paths_by_layer[layer_id]),
                            erSupportMaterial, support_flow.mm3_per_mm(), support_flow.width(), support_flow.height());
    ...
    auto configure_roof_1st_layer_filler_angle = [&](Fill& filler, size_t layer_id, const ExPolygons& polys) {
        filler.angle = Geometry::deg2rad(object_config.support_angle.value + 90.);

        auto interface_layer_above = m_object->get_support_layer(layer_id + m_support_params.num_top_interface_layers);
        if (interface_layer_above == nullptr)
            return;

        Layer* bridge_layer = m_object->get_layer_at_bottomz(interface_layer_above->print_z +
                                                                 object_config.support_top_z_distance.value,
                                                             layer_height / 10.);
        if (bridge_layer == nullptr)
            return;
        ...
        for (size_t region_id = 0; region_id < bridge_layer->regions().size(); ++region_id) {
            ...
            for (const auto surface : layerm->fill_surfaces.surfaces) {
                if (surface.surface_type == stBottomBridge && overlaps(polys, surface.expolygon)) {
                    filler.angle = surface.bridge_angle + (m_support_params.num_top_interface_layers - 1) * M_PI_2;
                    bridge_found = true;
                    break;
                }
            }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, support_layer_count),
        [&](const tbb::blocked_range<size_t>& range) {
            for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
                SupportLayer* support_layer = m_object->get_support_layer(support_layer_id);
                if (support_layer == nullptr || support_layer->height < EPSILON)
                    continue;

                Flow patch_flow(support_extrusion_width, support_layer->height, nozzle_diameter);
                roof_support_layers[support_layer_id].print_z = support_layer->print_z;
                roof_support_layers[support_layer_id].line_width = patch_flow.scaled_width();
                roof_support_layers[support_layer_id].model_region = model_region_for_support_z(support_layer->print_z);
                collect_existing_support_paths_for_transition(support_layer->support_fills,
                                                              roof_support_layers[support_layer_id].printed_paths);
                roof_support_layers[support_layer_id].valid = true;
            }
        });
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(m_raft_layers, support_layer_count), [&](const tbb::blocked_range<size_t>& range) {
            for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
                ...
                Flow roof_1st_layer_flow = support_material_interface_flow(m_object, ts_layer->height);
                const coord_t line_width = roof_1st_layer_flow.scaled_width();
                const double  min_patch_length = 2.0 * double(line_width);
                const coord_t overlap_merge_distance = line_width / 4;
                Flow support_flow(support_extrusion_width, ts_layer->height, nozzle_diameter);
                std::unique_ptr<Fill> local_filler_Roof1stLayer(Fill::new_from_type(ipRectilinear));
                local_filler_Roof1stLayer->set_bounding_box(bbox_object);
                local_filler_Roof1stLayer->spacing = roof_1st_layer_flow.spacing();

                ts_layer->area_groups.erase(
                    std::remove_if(ts_layer->area_groups.begin(), ts_layer->area_groups.end(),
                                   [&roof_1st_layer_flow, min_patch_length](const SupportLayer::AreaGroup& area_group) {
                                       if (area_group.type != SupportLayer::Roof1stLayer)
                                           return false;

                                       if (is_tiny_tree_support_fragment(*area_group.area, roof_1st_layer_flow))
                                           return true;

                                       Polylines initial_paths = to_polylines(*area_group.area);
                                       filter_short_support_contours(initial_paths, min_patch_length);
                                       return initial_paths.empty();
                                   }),
                    ts_layer->area_groups.end());
                ...
                for (auto& area_group : ts_layer->area_groups) {
                    ...
                    ExPolygon& poly = *area_group.area;
                    if (is_tiny_tree_support_fragment(poly, roof_1st_layer_flow))
                        continue;

                    ExPolygons polys = layer_id == 0 ? ExPolygons{poly} : offset_ex(poly, 0.5 * support_flow.scaled_width());
                    configure_roof_1st_layer_filler_angle(*local_filler_Roof1stLayer, layer_id, polys);

                    Polylines transition_paths =
                        make_transition_infill_paths(poly, roof_1st_layer_flow, local_filler_Roof1stLayer.get(), interface_density);
                    remove_overlapping_support_contours(transition_paths, overlap_merge_distance);
                    filter_short_support_contours(transition_paths, min_patch_length);
                    if (transition_paths.empty())
                        continue;

                    const bool needs_transition_check = should_check_roof_transition_area(poly, line_width);
                    Polylines initial_paths;
                    if (needs_transition_check) {
                        initial_paths = to_polylines(offset_ex(poly, -0.5 * double(line_width), jtSquare));
                        remove_overlapping_support_contours(initial_paths, overlap_merge_distance);
                        filter_short_support_contours(initial_paths, min_patch_length);
                        append(roof_support_layers[layer_id].input_paths, std::move(initial_paths));
                    }
                    append(roof_support_layers[layer_id].supported_anchor_paths, std::move(transition_paths));
                }
                remove_overlapping_support_contours(roof_support_layers[layer_id].input_paths, overlap_merge_distance);
                filter_short_support_contours(roof_support_layers[layer_id].input_paths, min_patch_length);
            }
        });

    ContourSupportPlan roof_support_plan =
        plan_contour_support_paths(std::move(roof_support_layers), m_raft_layers, max_support_bridge_length, false);
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(m_raft_layers, support_layer_count), [&](const tbb::blocked_range<size_t>& range) {
            for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
                ...
                if (!roof_support_plan.patch_paths_by_layer[layer_id].empty()) {
                    Flow support_flow(support_extrusion_width, ts_layer->height, nozzle_diameter);
                    extrusion_entities_append_paths(ts_layer->support_fills.entities, std::move(roof_support_plan.patch_paths_by_layer[layer_id]),
                                                    erSupportMaterial, support_flow.mm3_per_mm(), support_flow.width(), support_flow.height());
                }
                ...
                for (auto& area_group : ts_layer->area_groups) {
                    ...
                    Polylines transition_paths =
                        make_transition_infill_paths(poly, roof_1st_layer_flow, local_filler_Roof1stLayer.get(), interface_density);
                    remove_overlapping_support_contours(transition_paths, line_width / 4);
                    filter_short_support_contours(transition_paths, min_patch_length);
                    if (transition_paths.empty())
                        continue;

                    ExtrusionEntityCollection* temp_support_fills = new ExtrusionEntityCollection();
                    extrusion_entities_append_paths(temp_support_fills->entities, std::move(transition_paths), erSupportTransition,
                                                    roof_1st_layer_flow.mm3_per_mm(), roof_1st_layer_flow.width(),
                                                    roof_1st_layer_flow.height());
                    temp_support_fills->no_sort = true;
                    if (!temp_support_fills->entities.empty())
                        ts_layer->support_fills.entities.push_back(temp_support_fills);
                    else
                        delete temp_support_fills;
...
void TreeSupport::generate()
{
    ...
    detect_overhangs();
    Diagnostics::fingerprint("path.support.tree.overhangs", *m_object, static_cast<const PrintObject &>(*m_object).layers());
    ...
    Diagnostics::fingerprint("path.support.tree.contact_nodes", *m_object, contact_nodes);

    // Drop nodes to lower layers.
    m_object->print()->set_status(60, _u8L("Generating support"));
    drop_nodes();
    Diagnostics::fingerprint("path.support.tree.dropped_nodes", *m_object, contact_nodes);

    smooth_nodes();
    Diagnostics::fingerprint("path.support.tree.smoothed_nodes", *m_object, contact_nodes);

    // Generate support areas.
    m_object->print()->set_status(65, _u8L("Generating support"));
    draw_circles();
    Diagnostics::fingerprint("path.support.tree.areas", *m_object, static_cast<const PrintObject &>(*m_object).support_layers());

    m_object->print()->set_status(70, _u8L("Generating support"));
    generate_toolpaths();
    Diagnostics::fingerprint("path.support.tree.toolpaths", *m_object, static_cast<const PrintObject &>(*m_object).support_layers());
...
coordf_t TreeSupport::calc_radius(coordf_t mm_to_top) { return calc_branch_radius(base_radius, mm_to_top, diameter_angle_scale_factor); }
...
ExPolygons TreeSupport::get_placeable_areas(coordf_t radius, size_t layer_nr)
{
    return shrink_ex(get_collision(0, layer_nr), scale_(radius));
}
...
Polygons TreeSupport::get_trim_support_regions(const PrintObject& object,
                                               ...
                                               const coordf_t     gap_xy)
{
    ...
    idx_object_layer_overlapping = Layer::idx_higher_or_equal(object.layers().begin(), object.layers().end(), idx_object_layer_overlapping,
    ...
    for (; i < object.layers().size(); ++i) {
        ...
        for (const ExPolygon& expoly : object_layer.lslices) {
            ...
            coordf_t trimming_offset = is_sharptail ? scale_(sharp_tail_xy_gap) : is_overlap ? gap_xy_scaled : scale_(no_overlap_xy_gap);
    ...
    if (!m_slicing_params.soluble_interface && m_object_config->thick_bridges) {
        ...
        for (; i < object.layers().size(); ++i) {
            ...
            for (LayerRegion* region : object_layer.regions()) {
                ...
                polygons_append(polygons_trimming, offset(region->fill_surfaces.filter_by_type(stBottomBridge), trimming_offset,
...
void TreeSupport::draw_circles()
{
    ...
    const int CIRCLE_RESOLUTION = 25; // The number of vertices in each circle.
    ...
    for (const PrintObject* object : print->objects()) {
        size_t skirt_layers = print->has_infinite_skirt() ? object->layer_count() :
                                                            std::min(size_t(print_config.skirt_height.value), object->layer_count());
    ...
    std::atomic<int64_t> next_progress_report_ms{now_ms()};
    const auto report_progress = [this, progress_report_interval_ms, &next_progress_report_ms, &now_ms](
                                     const std::string& phase, size_t completed, size_t total, bool force) {
        const int64_t current_ms = now_ms();
        if (force) {
            next_progress_report_ms.store(current_ms + progress_report_interval_ms, std::memory_order_relaxed);
        ...
        const std::string message = !phase.empty() && phase.back() == ')' ?
                                        phase.substr(0, phase.size() - 1) + ", " + percent + ")" :
                                        phase + ": " + percent;
        m_object->print()->set_status(65, message);
        BOOST_LOG_TRIVIAL(info) << message;
    };
    const std::string generating_layers_text = generating_tree_areas_text + " (1/2)";
    const size_t total_area_layers = m_ts_data->layer_heights.size();
    std::atomic<size_t> completed_area_layers{0};
    const auto complete_area_layer = [&]() {
        const size_t completed = completed_area_layers.fetch_add(1, std::memory_order_relaxed) + 1;
        report_progress(generating_layers_text, completed, total_area_layers, false);
    };
    report_progress(generating_layers_text, 0, total_area_layers, true);

    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, m_ts_data->layer_heights.size()), [&](const tbb::blocked_range<size_t>& range)

        {
            for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
                ...
                const size_t long_bridge_roof_layer_nr = obj_layer_nr + 1;
                const bool   has_long_bridge_roof      = top_interface_layers > 0 &&
                                                  long_bridge_roof_layer_nr < long_bridge_roofs_by_layer.size() &&
                                                  !long_bridge_roofs_by_layer[long_bridge_roof_layer_nr].empty();
                if (ts_layer->height < EPSILON) {
                    complete_area_layer();
                    continue;
                }

                std::vector<TreeNodeType> base_areas_types;
                ...
                ExPolygons                model_landing_base_areas;
                    ...
                    if (collision.empty()) {
                        collision = offset_ex(m_ts_data->m_layer_outlines[obj_layer_nr],
                                              sharp_tail ? scale_(top_z_distance) :
                                                                                scale_((obj_layer_nr == 0) ? config.support_object_first_layer_gap :
                        ...
                        if (top_z_distance > EPSILON) {
                            ...
                            for (size_t layer_id = obj_layer_nr + 1;
                                 layer_id < m_ts_data->m_layer_outlines.size() && (accum_height += m_object->get_layer(layer_id)->height) &&
                ...
                if (curr_layer_nodes.empty() && !has_long_bridge_roof) {
                ...
                for (const SupportNode* p_node : curr_layer_nodes) {
                    ...
                    if (node.type == ePolygon || (node.distance_to_top < 0 && !node.is_sharp_tail)) {
                        ...
                        const double node_radius = get_radius(p_node);
                        double       scale       = node_radius / branch_radius;
                        ...
                        if (std::abs(moveX) > 0.001 &&
                            std::abs(moveY) > 0.001) { // draw ellipse along movement direction
                            ...
                            for (auto vertex : branch_circle.points) {
                                vertex             = Point(matrix[0] * vertex.x() + matrix[1] * vertex.y(),
                        ...
                        area = avoid_object_remove_extra_small_parts(ExPolygon(circle),
                                                                     get_collision(node.is_sharp_tail && node.distance_to_top <= 0));
                        ...
                        if (node.need_extra_wall)
                            append(extra_wall_area, area);
                        if (node.overhang_degree >= 2)
                            append(cooldown_area, area);

                        // Merge overhang to get a smoother, continuous interface surface.
                        // In buildplate-only mode some lower nodes may be removed, so only
                        // expand the top contact/interface layers and keep ordinary branch
                        // bases driven by the generated tree geometry.
                        const bool node_emits_roof =
                            m_support_params.num_top_interface_layers > 0 && obj_layer_nr > 0 &&
                            (((node.support_roof_layers_below == 0 || node.support_roof_layers_below == 1) &&
                        ...
                        if (top_interface_layers > 0 &&
                            ...
                            (!on_buildplate_only || node_emits_roof)) {
                    ...
                    if (obj_layer_nr > 0 && node.distance_to_top < 0)
                    ...
                    else if (m_support_params.num_top_interface_layers > 0 && obj_layer_nr > 0 &&
                             (node.support_roof_layers_below == 0 || node.support_roof_layers_below == 1) &&
                             node.distance_to_top >= m_support_params.num_top_interface_layers) {
                        ...
                        if (!node.to_buildplate)
                            append(model_landing_base_areas, area);
                        base_areas_types.push_back(node.type);
                        max_layers_above_base = std::max(max_layers_above_base, node.dist_mm_to_top);
                    }
                }

                ExPolygons _base_areas = base_areas;
                ...
                if (has_long_bridge_roof) {
                    ExPolygons long_bridge_roofs = diff_clipped(long_bridge_roofs_by_layer[long_bridge_roof_layer_nr], get_collision(false));
                    append(roof_areas, intersection_ex(long_bridge_roofs, m_machine_border));
                    max_layers_above_roof = std::max(max_layers_above_roof, m_ts_data->layer_heights[layer_nr].height);
                    interface_id          = int(long_bridge_roof_layer_nr % std::max<size_t>(top_interface_layers, 1));
                }

                // roof_1st_layer and roof_areas may intersect, so need to subtract roof_areas from roof_1st_layer
                roof_1st_layer = diff_ex(roof_1st_layer,
                                         ClipperUtils::clip_clipper_polygons_with_subject_bbox(roof_areas, get_extents(roof_1st_layer)));
                roof_1st_layer = intersection_ex(roof_1st_layer, m_machine_border);

                ExPolygons roofs;
                append(roofs, roof_1st_layer);
                append(roofs, roof_areas);
                append(roofs, roof_gap_areas);

                // slightly slow down the slicing speed
                base_areas = diff_ex_2(base_areas, collision_base.empty() ? m_ts_data->get_collision(0, obj_layer_nr) : collision_base);
                ...
                std::vector<TreeNodeType> base_areas_types_new;
                if (_base_areas.size() == base_areas_types.size()) {
                    base_areas_types_new.resize(ts_layer->base_areas.size(), TreeNodeType::eCircle);
                    for (int i = 0; i < ts_layer->base_areas.size(); i++) {
                        for (int j = 0; j < _base_areas.size(); j++) {
                            if (ts_layer->base_areas[i].overlaps(_base_areas[j])) {
                                if (base_areas_types_new[i] != TreeNodeType::ePolygon && base_areas_types[j] == TreeNodeType::ePolygon)
                                    base_areas_types_new[i] = base_areas_types[j];
                            }
                        }
                    }
                }
                ...
                int   cnt         = 0;
                bool  _bdiff      = ts_layer->base_areas.size() == base_areas_types_new.size();
                ...
                for (auto& expoly : ts_layer->base_areas) {
                    ...
                    if (_bdiff)
                        area_groups.emplace_back(&expoly, SupportLayer::BaseType, max_layers_above_base, base_areas_types_new[cnt]);
                    else
                        area_groups.emplace_back(&expoly, SupportLayer::BaseType, max_layers_above_base);
                    cnt++;
                ...
                ts_layer->lslices         = ts_layer->support_islands;
    ...
    report_progress(generating_layers_text, total_area_layers, total_area_layers, true);

    if (with_lightning_infill) {
        const std::string optimizing_layers_text = generating_tree_areas_text + " (2/2)";
        const size_t total_optimization_layers = contact_nodes.size() > 1 ? contact_nodes.size() - 1 : 0;
        report_progress(optimizing_layers_text, 0, total_optimization_layers, true);
        std::vector<Polygons> contours;
        std::vector<Polygons> overhangs;
        for (int layer_nr = 1; layer_nr < contact_nodes.size(); layer_nr++) {
            report_progress(optimizing_layers_text, size_t(layer_nr - 1), total_optimization_layers, false);
            if (print->canceled())
                break;
            const std::vector<SupportNode*>& curr_layer_nodes = contact_nodes[layer_nr];
            SupportLayer*                    ts_layer         = m_object->get_support_layer(layer_nr + m_raft_layers);
            ...
            for (layer_nr_lower; layer_nr_lower >= 0; layer_nr_lower--) {
                if (!m_object->get_support_layer(layer_nr_lower + m_raft_layers)->area_groups.empty())
                    break;
            }
            if (layer_nr_lower <= 0)
                continue;
        ...
        coordf_t support_density         = std::min(1., m_support_material_flow.spacing() / support_spacing *
                                                            2); // for lightning infill the density is defined differently, so need to double it
        generator = std::make_unique<FillLightning::Generator>(m_object, contours, overhangs, []() {}, support_density);
        report_progress(optimizing_layers_text, total_optimization_layers, total_optimization_layers, true);
    }
        ...
        auto intersects_contour = [](Polygon poly, ExPolygon expoly, Point& pt_on_poly, Point& pt_on_expoly, Point& pt_far_on_poly,
        ...
        for (int layer_nr = contact_nodes.size() - 1; layer_nr > 0; layer_nr--) {
            report_progress(optimizing_layers_text, total_optimization_layers - size_t(layer_nr),
                            total_optimization_layers, false);
            if (print->canceled())
                break;
                ...
                for (const auto& hole : area->holes) {
                    ...
                    for (auto& area_group_lower : area_groups_lower) {
                        ...
                        if (base_area_lower.contour.contains(hole.points.front()) &&
                            ...
                            break;
                        } else if (holePropagationInfos.find(&hole) != holePropagationInfos.end() &&
                                   std::get<0>(holePropagationInfos[&hole]) > 0 &&
                                   base_area_lower.contour.contains(std::get<2>(holePropagationInfos[&hole]))) {
                            // after the hole connects to contour, shrink it gradually until it vanishes while moving it outwards. The
                            // moving direction is defined in the previous step
                            Polygon hole_lower = hole;
                            ...
                            if (!hole_expanded.empty()) {
                                base_area_lower.holes.push_back(std::move(hole_expanded[0]));
                                holePropagationInfos.insert(
                                    {&base_area_lower.holes.back(), {std::get<0>(holePropagationInfos[&hole]) - 1, direction, farPoint}});
                            }
                            break;
                        }
                    }
                    {
                        ...
                        for (auto& roof1 : ts_layer->roof_1st_layer) {
                            ...
                            bool is_inside_hole = std::all_of(roof1.contour.points.begin(), roof1.contour.points.end(),
                                                              [&hole](Point& pt) { return hole.contains(pt); });
                            if (is_inside_hole) {
    ...
    for (int layer_nr = m_object->support_layer_count() - 1; layer_nr >= 0; layer_nr--) {
        ...
        append(roofs, roof_1st_layer);
        if (base_areas.empty() && roof_areas.empty() && roof_1st_layer.empty())
            continue;
        draw_contours_and_nodes_to_svg(debug_out_path("circles_%d_%.2f.svg", layer_nr, ts_layer->print_z), base_areas, roofs,
                                       ts_layer->lslices_extrudable, {}, {}, {"base", "roof", "lslices"}, {"blue", "red", "black"});
    }
...
static void smooth_filter(const Polygon& poly, int filter_size, std::unordered_map<Point, Point, PointHash>& movements, double max_move)
{
    ...
    for (int i = 0; i < n; i++) {
        Point filter_pt(0, 0);
        for (int j = (i + n - filter_size) % n; j != (i + filter_size + 1) % n; j = (j + 1) % n)
            filter_pt += pts[j];
        newpoly[i] = filter_pt / (2 * filter_size + 1);
        newpoly[i] -= center;
    }
    if (newpoly.area() < SQ(scale_(10.)))
        newpoly.scale(SQ(scale_(10.)) / newpoly.area());

    for (int i = 0; i < n; i++) {
        auto& pt   = pts[i];
        Point move = newpoly[i] - (pt - center);
        if (vsize2_with_unscale(move) > SQ(max_move))
            move = normal(move, scale_(max_move));
        movements[pt] = move;
    }
}
...
void TreeSupport::drop_nodes()
{
    ...
    const int      wall_count              = std::max(1, config.tree_support_wall_count_tree.value);
    double         tan_angle               = tan(angle); // when nodes are thick, they can move further. this is the max angle
    const coordf_t max_move_distance       = (angle < M_PI / 2) ? (coordf_t) (tan_angle * layer_height) * wall_count :
                                                                  std::numeric_limits<coordf_t>::max();
    const double   max_move_distance2      = max_move_distance * max_move_distance;
    const size_t   tip_layers =
        base_radius / layer_height; // The number of layers to be shrinking the circle to create a tip. This produces a 45 degree angle.
    ...
    const std::string precalculating_text = _u8L("Precalculating tree support");
    ...
    const auto report_progress = [this, progress_report_interval_ms, &next_progress_report_ms, &now_ms](
        ...
        const std::string message = !phase.empty() && phase.back() == ')' ?
                                        phase.substr(0, phase.size() - 1) + ", " + percent + ")" :
                                        phase + ": " + percent;
        m_object->print()->set_status(60, message);
        BOOST_LOG_TRIVIAL(info) << message;
    };
    ...
    if (1) {
        ...
        const std::string collecting_radii_text = precalculating_text + " (1/2)";
        const size_t total_precalculation_layers = contact_nodes.size() > 1 ? contact_nodes.size() - 1 : 0;
        report_progress(collecting_radii_text, 0, total_precalculation_layers, true);
        for (size_t layer_nr = contact_nodes.size() - 1; layer_nr > 0; layer_nr--) {
            ...
            report_progress(collecting_radii_text, contact_nodes.size() - layer_nr, total_precalculation_layers, false);
        }
        report_progress(collecting_radii_text, total_precalculation_layers, total_precalculation_layers, true);
        // parallel pre-compute avoidance
        size_t total_radius_tasks = 0;
        for (const std::set<coordf_t>& layer_radii : all_layer_radius)
            total_radius_tasks += layer_radii.size();
        std::atomic<size_t> completed_radius_tasks{0};
        const std::string calculating_avoidance_text = precalculating_text + " (2/2)";
        report_progress(calculating_avoidance_text, 0, total_radius_tasks, true);
        tbb::parallel_for(tbb::blocked_range<size_t>(0, contact_nodes.size() - 1), [&](const tbb::blocked_range<size_t>& range) {
            for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
                for (auto node_radius : all_layer_radius[layer_nr]) {
                    ...
                    const size_t completed = completed_radius_tasks.fetch_add(1, std::memory_order_relaxed) + 1;
                    report_progress(calculating_avoidance_text, completed, total_radius_tasks, false);
                }
            }
        });
        report_progress(calculating_avoidance_text, completed_radius_tasks.load(std::memory_order_relaxed), total_radius_tasks, true);

        double duration{std::chrono::duration_cast<second_>(clock_::now() - t0).count()};
        BOOST_LOG_TRIVIAL(debug) << "finish pre calculate_avoidance. before m_avoidance_cache.size()="
                                 << m_ts_data->m_avoidance_cache.size() << ", takes " << duration << " secs.";
    }

    m_spanning_trees.resize(contact_nodes.size());
    // m_mst_line_x_layer_contour_caches.resize(contact_nodes.size());

    const size_t total_propagation_layers = contact_nodes.size() > 1 ? contact_nodes.size() - 1 : 0;
    report_progress(propagating_text, 0, total_propagation_layers, true);
    for (size_t layer_nr = contact_nodes.size() - 1; layer_nr > 0; layer_nr--) // Skip layer 0, since we can't drop down the vertices there.
    {
        if (m_object->print()->canceled())
            break;

        const size_t propagated_layers = contact_nodes.size() - layer_nr;
        report_progress(propagating_text, propagated_layers, total_propagation_layers, false);
        ...
        std::deque<std::pair<size_t, SupportNode*>>
            unsupported_branch_leaves; // All nodes that are leaves on this layer that would result in unsupported ('mid-air') branches.
        ...
        auto support_model_landing_should_stop = [&](const ExPolygons& support_remainder) {
            if (support_remainder.empty() || obj_layer_nr_next >= m_object->layer_count())
                return false;

            const double candidate_area       = area(support_remainder);
            const ExPolygons next_model_placeable = get_placeable_areas(0, obj_layer_nr_next);
            if (candidate_area <= 0. || next_model_placeable.empty())
                return false;

            const ExPolygons next_model_contact = intersection_ex(support_remainder, next_model_placeable);
            const double next_model_contact_area = area(next_model_contact);
            const bool has_strong_model_landing = next_model_contact_area >= 0.45 * candidate_area;
            if (!has_strong_model_landing)
                return false;

            ExPolygons rest_after_model_landing = diff_ex(support_remainder, next_model_contact);
            if (rest_after_model_landing.empty())
                return true;

            ExPolygons propagated_rest = diff_ex(rest_after_model_landing, get_collision(0, obj_layer_nr_next));
            if (propagated_rest.empty())
                return true;

            const double propagated_rest_area = area(propagated_rest);
            const coord_t line_width = scale_(support_extrusion_width);
            if (line_width <= 0)
                return propagated_rest_area < 0.5 * candidate_area;

            const Point bbox_size          = get_extents(propagated_rest).size();
            const coord_t propagated_width = std::min(bbox_size.x(), bbox_size.y());
            const bool rest_eroded_empty   = offset_ex(propagated_rest, -line_width / 2, ClipperLib::jtMiter, 1.2).empty();
            const bool direct_narrow_stop  = propagated_width <= coord_t(3.0 * double(line_width)) || rest_eroded_empty;
            const bool small_wide_stop     = propagated_rest_area < 0.5 * candidate_area &&
                                         propagated_width <= coord_t(5.0 * double(line_width));

            return direct_narrow_stop || small_wide_stop;
        };

        auto should_stop_current_node_on_model_landing = [&](const SupportNode& node) {
            if (support_on_buildplate_only || obj_layer_nr_next >= m_object->layer_count())
                return false;

            Polygon circle = make_circle(scale_(node.radius), 0.00789 * scale_(node.radius));
            circle.translate(node.position);
            const ExPolygon circle_area(circle);
            const ExPolygons collision = get_collision(0, obj_layer_nr);
            ExPolygons support_remainder = avoid_object_remove_extra_small_parts(circle_area, collision);

            return support_model_landing_should_stop(support_remainder);
        };

        // Group together all nodes for each part.
        const ExPolygons&                                               parts = m_ts_data->m_layer_outlines_below[obj_layer_nr];
        std::vector<std::unordered_map<Point, SupportNode*, PointHash>> nodes_per_part(
            1 + parts.size()); // All nodes that aren't inside a part get grouped together in the 0th part.
        for (SupportNode* p_node : layer_contact_nodes) {
            ...
            if (!node.to_buildplate) {
                ...
                if (support_on_buildplate_only) {
                    // Hand-painted support is an explicit user request. A node may be temporarily inside the model outline while
                    // routing outward, so don't prune it immediately unless there is no lower layer left to escape to.
                    if (node.is_manual_enforcer && layer_nr > 1) {
                        // Keep routing downward.
                    } else if (node.is_manual_enforcer && layer_nr == 1) {
                        const coordf_t branch_radius = get_radius(p_node);
                        Polygon circle = make_circle(scale_(branch_radius), 0.00789 * scale_(branch_radius));
                        circle.translate(node.position);
                        const ExPolygons collision = offset_ex(
                            m_ts_data->m_layer_outlines[obj_layer_nr],
                            scale_((obj_layer_nr == 0) ? config.support_object_first_layer_gap.value : m_ts_data->m_xy_distance));
                        const ExPolygons support_remainder =
                            avoid_object_remove_extra_small_parts(ExPolygon(circle), collision);
                        if (!support_remainder.empty() && area(support_remainder) >= SQ(scale_(0.25))) {
                ...
                if (!support_on_buildplate_only) {
                    ...
                    if (support_model_landing_should_stop(support_remainder))
                        continue;

                    if (!support_remainder.empty()) {
                        p_node->overhang = support_remainder[0];
                        if (support_remainder[0].area() < SQ(scale_(0.5)) ||
                            (!node.parent->to_buildplate && !overlaps({node.overhang}, {node.parent->overhang}))) {
                            p_node->valid        = false;
            ...
            for (size_t part_index = 0; part_index < parts.size(); part_index++) {
                ...
                if (is_inside_ex(parts[part_index],
                                 node.position)) // If it's inside, the distance is 0 and this part is considered the best.
                {
        ...
        for (size_t group_index = 0; group_index < nodes_per_part.size(); group_index++) {
            ...
            std::vector<SupportNode*> merge_nodes;
            merge_nodes.reserve(nodes_vec.size());
            for (const auto& entry : nodes_vec)
                merge_nodes.emplace_back(entry.second);
            std::sort(merge_nodes.begin(), merge_nodes.end(), [](const SupportNode* lhs, const SupportNode* rhs) {
                return lhs->position.x() != rhs->position.x() ? lhs->position.x() < rhs->position.x() :
                                                                lhs->position.y() < rhs->position.y();
            });

            // In the first pass, calculate merge candidates in parallel, then apply them in a stable order.
            {
                enum class MergeCandidateType {
                    polygon_absorb, polygon_fade, circle_absorb, circle_pair, polygon_pair
                };
                struct MergeCandidate
                {
                    MergeCandidateType type;
                    SupportNode*       target;
                    ExPolygon          merged_overhang;
                };

                // This is the value returned by get_max_move_dist(node, 2), calculated without touching its lazy cache.
                const auto max_move_distance_squared = [&](const SupportNode* node) {
                    const double move_distance = node->max_move_dist == 0 ?
                        std::min(tan_angle * node->height, support_extrusion_width) : node->max_move_dist;
                    return SQ(move_distance);
                };

                const auto& nodes_by_position = nodes_this_part;
                std::vector<std::vector<MergeCandidate>> candidates_by_node(merge_nodes.size());
                tbb::parallel_for(tbb::blocked_range<size_t>(0, merge_nodes.size()), [&](const tbb::blocked_range<size_t>& range) {
                    for (size_t node_index = range.begin(); node_index < range.end(); ++node_index)
                    {
                        SupportNode* node = merge_nodes[node_index];
                        if (!node->valid || node->fading)
                            continue;

                        const std::vector<Point>& neighbours = mst.adjacent_nodes(node->position);
                        std::vector<MergeCandidate>& candidates = candidates_by_node[node_index];
                        candidates.reserve(neighbours.size());

                        if (node->type == ePolygon)
                        {
                            // Remove circle neighbours inside this polygon, or combine overlapping polygon neighbours.
                            for (const Point& neighbour_position : neighbours)
                            {
                                SupportNode* neighbour = nodes_by_position.at(neighbour_position);
                                if (!neighbour->valid || neighbour->fading)
                                    continue;

                                if (neighbour->type == ePolygon)
                                {
                                    const bool compatible_distance =
                                        (node->distance_to_top < 0 && neighbour->distance_to_top < 0) ||
                                        (node->distance_to_top > m_support_params.num_top_interface_layers + 1 &&
                                            neighbour->distance_to_top > m_support_params.num_top_interface_layers + 1);
                                    if (!compatible_distance)
                                        continue;

                                    ExPolygons overhang_shrunk = shrink_ex({ node->overhang }, scale_(support_extrusion_width));
                                    if (overhang_shrunk.empty() || !overlaps(overhang_shrunk, { neighbour->overhang }))
                                        continue;

                                    ExPolygons merged = union_ex({ node->overhang }, { neighbour->overhang });
                                    if (merged.size() == 1)
                                        candidates.push_back(
                                            { MergeCandidateType::polygon_pair, neighbour, std::move(merged.front()) });
                                }
                                else
                                {
                                    const coord_t neighbour_radius = scale_(neighbour->radius);
                                    const Point pt_north = neighbour_position + Point(0, neighbour_radius);
                                    const Point pt_south = neighbour_position - Point(0, neighbour_radius);
                                    const Point pt_west = neighbour_position - Point(neighbour_radius, 0);
                                    const Point pt_east = neighbour_position + Point(neighbour_radius, 0);
                                    const bool contained =
                                        is_inside_ex(node->overhang, neighbour_position) && is_inside_ex(node->overhang, pt_north) &&
                                        is_inside_ex(node->overhang, pt_south) && is_inside_ex(node->overhang, pt_west) &&
                                        is_inside_ex(node->overhang, pt_east);
                                    if (contained)
                                        candidates.push_back({ MergeCandidateType::polygon_absorb, neighbour, {} });
                                    else if (is_inside_ex(node->overhang, neighbour_position))
                                        candidates.push_back({ MergeCandidateType::polygon_fade, neighbour, {} });
                                }
                            }
                        }
                        else if (neighbours.size() == 1)
                        {
                            // We have just two non-polygon leaf nodes, and they are close enough to combine.
                            SupportNode* neighbour = nodes_by_position.at(neighbours.front());
                            if (vsize2_with_unscale(neighbours.front() - node->position) < max_move_distance_squared(node) &&
                                mst.adjacent_nodes(neighbours.front()).size() == 1 && neighbour->type != ePolygon)
                                candidates.push_back({ MergeCandidateType::circle_pair, neighbour, {} });
                        }
                        else if (neighbours.size() > 1)
                        {
                            // Merge close non-polygon neighbours into this node, subject to the original size condition.
                            for (const Point& neighbour_position : neighbours)
                            {
                                if (vsize2_with_unscale(neighbour_position - node->position) >= max_move_distance_squared(node))
                                    continue;
                                SupportNode* neighbour = nodes_by_position.at(neighbour_position);
                                if (neighbour->type == ePolygon)
                                    continue;
                                if (node->dist_mm_to_top < neighbour->dist_mm_to_top)
                                    continue;
                                candidates.push_back({ MergeCandidateType::circle_absorb, neighbour, {} });
                            }
                        }
                    }
                });

                // Applying candidates in coordinate order replaces the undefined TBB callback order with one
                // deterministic callback order. Candidate eligibility and MST neighbour order stay unchanged.
                for (size_t node_index = 0; node_index < merge_nodes.size(); ++node_index)
                {
                    SupportNode* node = merge_nodes[node_index];
                    if (!node->valid || node->fading)
                        continue;
                    ...
                    const std::vector<Point>& neighbours = mst.adjacent_nodes(node->position);
                    if (node->type != ePolygon && !neighbours.empty())
                        get_max_move_dist(node, 2);

                    for (MergeCandidate& candidate : candidates_by_node[node_index])
                    {
                        SupportNode* neighbour = candidate.target;

                        // The original parallel loop let multiple callbacks consume the same target. Accepting the
                        // first candidate in the deterministic callback order makes that ownership conflict stable.
                        if (!neighbour->valid)
                            continue;

                        if (candidate.type == MergeCandidateType::polygon_fade)
                        {
                            if (neighbour->fading)
                                continue;
                            neighbour->fading = true;
                            continue;
                        }

                        if (candidate.type == MergeCandidateType::polygon_absorb || candidate.type == MergeCandidateType::circle_absorb)
                        {
                            if (candidate.type == MergeCandidateType::polygon_absorb && neighbour->fading)
                                continue;
                            node->merged_neighbours.push_front(neighbour);
                            node->merged_neighbours.insert(node->merged_neighbours.end(), neighbour->merged_neighbours.begin(), neighbour->merged_neighbours.end());
                            neighbour->valid = false;
                            continue;
                        }

                        if (candidate.type == MergeCandidateType::polygon_pair)
                        {
                            if (neighbour->fading)
                                continue;
                            Point next_position = candidate.merged_overhang.contour.centroid();
                            SupportNode* next_node = m_ts_data->create_node(
                                next_position, std::max(node->distance_to_top, neighbour->distance_to_top) + 1,
                                obj_layer_nr_next,
                                std::max(node->support_roof_layers_below, neighbour->support_roof_layers_below) - 1, true, node,
                                print_z_next, height_next);
                            next_node->max_move_dist = 0;
                            next_node->overhang = std::move(candidate.merged_overhang);
                        ...
                        assert(candidate.type == MergeCandidateType::circle_pair);
                        Point    next_position = (node->position + neighbour->position) / 2;
                        ...
                        if (node->parent && neighbour->parent)
                            node_parent = node->dist_mm_to_top >= neighbour->dist_mm_to_top ? node : neighbour;
                        else
                            node_parent = node->parent ? node : neighbour;
                        node_parent->merged_neighbours.push_front(node_parent == node ? neighbour : node);

                        const bool to_buildplate = !is_inside_ex(get_collision(0, obj_layer_nr_next), next_position);
                        SupportNode* next_node = m_ts_data->create_node(
                            next_position, node_parent->distance_to_top + 1, obj_layer_nr_next,
                            node_parent->support_roof_layers_below - 1, to_buildplate, node_parent, print_z_next, height_next);
            ...
            tbb::parallel_for_each(nodes_vec.begin(), nodes_vec.end(), [&](const std::pair<const Point, SupportNode*>& entry) {
                ...
                if (node.fading) {
                    ...
                    if (next_radius < EPSILON)
                        return;
                    SupportNode* next_node   = m_ts_data->create_node(node.position, p_node->distance_to_top + 1, obj_layer_nr_next,
                                                                      p_node->support_roof_layers_below - 1, node.to_buildplate, p_node,
                                                                      print_z_next, height_next);
                    next_node->max_move_dist = 0;
                ...
                if (node.type == ePolygon) {
                    ...
                    if (m_support_params.num_top_interface_layers > 0 && obj_layer_nr_next > 0 && node.support_roof_layers_below == 1 &&
                        node.distance_to_top >= m_support_params.num_top_interface_layers)
                        overhangs_next = safe_offset_inc(overhangs_next, scale_(max_move_distance), get_collision(0, obj_layer_nr_next),
                    ...
                    for (auto& overhang : overhangs_next) {
                        ...
                        if (overhang.area() > node.origin_area / 2. && overhang.area() > SQ(scale_(10.))) {
                            ...
                            for (auto& pt : overhang.contour.points) {
                                ...
                                if (!is_inside_ex(to_expolygons({contour}), tmp) &&
                                    !is_inside_ex(m_ts_data->m_layer_outlines_below[obj_layer_nr], tmp))
                                    pt = tmp;
                            }
                        }
                        ...
                        if (node.support_roof_layers_below < 0 && overhang.area() > node.origin_area / 2. &&
                            ...
                            ExPolygons shrink_overhangs = union_ex(
                                shrink_ex(safe_union({overhang}), double(scale_(max_move_distance / 2.))));
                            ...
                            SupportNode* next_node   = m_ts_data->create_node(next_pt, p_node->distance_to_top + 1, obj_layer_nr_next,
                                                                              p_node->support_roof_layers_below - 1, to_buildplate, p_node,
                                                                              print_z_next, height_next);
                            next_node->max_move_dist = 0;
                ...
                if (group_index > 0 && node.to_buildplate && is_inside_ex(get_collision(0, obj_layer_nr), node.position)) {
                    ...
                    {
                        if (support_on_buildplate_only && node.is_manual_enforcer && layer_nr > 1) {
                            // Keep routing downward.
                        } else if (support_on_buildplate_only) {
                            unsupported_branch_leaves.push_front({layer_nr, p_node});
                            return;
                        } else {
                            p_node->valid = false;
                     ...
                     (neighbours.size() == 1 && nodes_this_part[neighbours[0]]->type == ePolygon)*/)) // Only nodes that aren't about to collapse.
                {
                    ...
                    for (const Point& neighbour : neighbours) {
                        ...
                        coordf_t neighbour_bottom_radius =
                            calc_radius(neighbour_node->dist_mm_to_top + neighbour_node->print_z);
                        double   max_converge_distance   = tan_angle * (p_node->print_z - DO_NOT_MOVER_UNDER_MM) +
                ...
                if (is_line_cut_by_contour(node.position, to_outside) || dist2_to_outer > max_move_distance2 * SQ(obj_layer_nr) ||
                    ...
                    const coordf_t max_move_between_samples = max_move_distance + radius_sample_resolution +
                                                              EPSILON; // 100 micron extra for rounding errors.
                    // use get_collision instead of get_avoidance here (See STUDIO-4252)
                    bool is_outside = move_out_expolys(get_collision(next_radius, obj_layer_nr_next), candidate_vertex,
                ...
                Point movement;
                if (support_on_buildplate_only && node.is_manual_enforcer && !node.to_buildplate && dist2_to_outer > EPSILON)
                    movement = direction_to_outer;
                else if (support_on_buildplate_only)
                    movement = move_to_neighbor_center + direction_to_outer * 2;
                else if (!is_strong)
                    movement = move_to_neighbor_center * 2 +
                               (dist2_to_outer > EPSILON ? direction_to_outer * (1 / dist2_to_outer) : Point(0, 0));
                else {
                ...
                if (group_index == 0 && 0) {
                    // Avoid collisions.
                    const coordf_t max_move_between_samples = get_max_move_dist(&node, 1) + radius_sample_resolution +
                                                              EPSILON; // 100 micron extra for rounding errors.
                    bool is_outside = move_out_expolys(avoidance_next, next_layer_vertex, radius_sample_resolution + EPSILON,
                                                       max_move_between_samples);
                    if (!is_outside) {
                        Point candidate_vertex = node.position;
                        is_outside             = move_out_expolys(avoidance_next, candidate_vertex, radius_sample_resolution + EPSILON,
                                                                  max_move_between_samples);
                        if (is_outside) {
                ...
                // A branch may route to the build plate only after its center has left the full XY
                // collision band. Leaving the raw model outline is not enough, otherwise a branch
                // in the clearance band is treated as a build-plate branch and can print too close
                // to the model side wall.
                const bool   to_buildplate  = !is_inside_ex(next_collision, next_layer_vertex);
                if (!to_buildplate && should_stop_current_node_on_model_landing(node)) {
                ...
                SupportNode* next_node      = m_ts_data->create_node(next_layer_vertex, node.distance_to_top + 1, obj_layer_nr_next,
                                                                     node.support_roof_layers_below - 1, to_buildplate, p_node, print_z_next,
                                                                     height_next);
        ...
        if (contact_nodes[layer_nr].empty() == false) {
            draw_contours_and_nodes_to_svg(debug_out_path("contact_points_%.2f.svg", contact_nodes[layer_nr][0]->print_z),
                                           get_collision(0, obj_layer_nr_next), get_avoidance(branch_radius_temp, obj_layer_nr),
                                           m_ts_data->m_layer_outlines[obj_layer_nr], contact_nodes[layer_nr], contact_nodes[layer_nr_next],
                                           {"overhang", "avoid", "outline"}, {"blue", "red", "yellow"});
        ...
        for (auto& layer_contact_nodes : contact_nodes) {
            if (!layer_contact_nodes.empty())
                layer_contact_nodes.erase(std::remove_if(layer_contact_nodes.begin(), layer_contact_nodes.end(),
                                                         [](SupportNode* node) { return node->is_processed; }),
                                          layer_contact_nodes.end());
        }
    }
    report_progress(propagating_text, total_propagation_layers, total_propagation_layers, true);
...
void TreeSupport::smooth_nodes()
{
    ...
    const double support_line_width = support_material_flow(m_object).width();
    float max_move = scale_(support_line_width / 2);
                ...
                for (size_t k = 0; k < iterations; k++) {
                    for (size_t i = 1; i < pts.size() - 1; i++) {
                        ...
                        if (k == iterations - 1) {
                            ...
                            branch[i]->overhang_degree = std::min(10.0, unscale_((branch[i]->movement).cast<double>().norm()) /
                                                                            support_line_width * 10);
                            if (branch[i]->parents.size() > 1 ||
                                (branch[i]->movement.x() > max_move || branch[i]->movement.y() > max_move) ||
                                (total_height > thresh_tall_branch && branch[i]->dist_mm_to_top < thresh_dist_to_top))
                                branch[i]->need_extra_wall = true;
                            BOOST_LOG_TRIVIAL(trace) << "smooth_nodes: layer_nr=" << layer_nr << ", i=" << i << ", pt=" << pt
                                                     << ", movement=" << branch[i]->movement << ", radius=" << branch[i]->radius;
                        }
                    }
                    if (k < iterations - 1) {
                        std::swap(pts, pts1);
...
std::vector<LayerHeightData> TreeSupport::plan_layer_heights()
{
    ...
    if (!m_support_params.independent_layer_height) {
        ...
        for (const Layer* l : m_object->layers())
            obj_layer_zs.emplace_back((float) l->print_z);
        ...
        for (; it2 != bounds.end(); it2++) {
            ...
            coordf_t dist = z2 - z1;
            if (dist < min_layer_height - EPSILON)
                continue;
        ...
        for (auto it = z_heights.begin(); it != z_heights.end(); it++, i++) {
            ...
            while (obj_layer_nr < obj_layer_zs.size() && obj_layer_zs[obj_layer_nr] < print_z - height / 2)
                obj_layer_nr++;
    ...
    for (size_t i = 0; i < layer_heights.size(); i++, support_layer_nr++) {
        SupportLayer* ts_layer = m_object->add_tree_support_layer(support_layer_nr, layer_heights[i].print_z, layer_heights[i].height,
                                                                  layer_heights[i].print_z);
    ...
    for (int layer_nr = 0; layer_nr < contact_nodes.size(); layer_nr++) {
        ...
        auto         it    = std::min_element(layer_heights.begin(), layer_heights.end(),
                                              [node1](const LayerHeightData& l1, const LayerHeightData& l2) {
    ...
    for (int layer_nr = 0; layer_nr < contact_nodes.size(); layer_nr++) {
        if (contact_nodes[layer_nr].empty())
            continue;
        SupportNode* node1 = contact_nodes[layer_nr].front();
        BOOST_LOG_TRIVIAL(debug) << format("plan_layer_heights node1->layer_nr,printz,height,distance_to_top: %d, %.2f,%.2f, %d", layer_nr,
                                           node1->print_z, node1->height, node1->distance_to_top)
                                 << ", object_layer_zs[" << layer_heights[layer_nr].obj_layer_nr
                                 << "]=" << m_object->get_layer(layer_heights[layer_nr].obj_layer_nr)->print_z;
        coordf_t new_height = layer_heights[layer_nr].height;
        if (std::abs(node1->height - new_height) < EPSILON)
            continue;
        if (top_z_distance < EPSILON && node1->height < EPSILON)
            continue; // top_z_distance==0, this is soluable interface
        ...
        BOOST_LOG_TRIVIAL(debug) << format("plan_layer_heights adjust node's height print_z[%d]=%.2f: (%.3f,%d)->(%.3f,%.3f,%d)", layer_nr,
                                           node1->print_z, node1->height, node1->distance_to_top, new_height, accum_height, -num_layers);
    ...
    for (size_t i = 0; i < layer_heights.size(); i++) {
        // if (layer_heights[i].height > EPSILON)
        BOOST_LOG_TRIVIAL(trace) << format("plan_layer_heights[%d] print_z, height: %.2f, %.2f", i, layer_heights[i].print_z,
                                           layer_heights[i].height);
    }
...
void TreeSupport::generate_contact_points()
{
    ...
    const Point rotated_dims = Point(bounding_box_size(0) * cos_angle + bounding_box_size(1) * sin_angle,
                                     bounding_box_size(0) * sin_angle + bounding_box_size(1) * cos_angle) /
                               2;
    ...
    //}
    //  }
    const int z_distance_top_layers = round_up_divide(scale_(z_distance_top), scale_(layer_height)) +
                                      1; // Support must always be 1 layer below overhang.
    ...
    if (m_support_params.soluble_interface && xy_expansion < EPSILON)
        xy_expansion = scale_(2);
    ...
    constexpr double                                  vertical_enforcer_cleanup_grid_size_mm = 0.5;
    constexpr size_t                                  vertical_enforcer_cleanup_trigger_points_per_layer = 500;
    const coord_t                                     vertical_enforcer_grid_size =
        scaled<coord_t>(vertical_enforcer_cleanup_grid_size_mm);
    for (auto& pt_and_normal : m_vertical_enforcer_points) {
        ...
        if (iter != zs.end()) {
            ...
            if (layer_nr > 0 && layer_nr < contact_nodes.size()) {
                ...
                vertical_enforcer_points_by_layers[layer_nr].push_back({point, scaled(to_2d(normal))});
            }
        }
    }
    for (size_t layer_nr = 0; layer_nr < vertical_enforcer_points_by_layers.size(); ++layer_nr) {
        auto& layer_points = vertical_enforcer_points_by_layers[layer_nr];
        if (layer_points.size() <= vertical_enforcer_cleanup_trigger_points_per_layer)
            continue;

        std::unordered_set<uint64_t>         seen_cells;
        std::vector<std::pair<Point, Point>> deduplicated;
        deduplicated.reserve(layer_points.size());
        for (const auto& point_and_normal : layer_points) {
            if (seen_cells.insert(vertical_enforcer_grid_cell_key(point_and_normal.first, vertical_enforcer_grid_size)).second)
                deduplicated.emplace_back(point_and_normal);
        }
        layer_points = std::move(deduplicated);
    }
        ...
        for (size_t layer_nr = range.begin(); layer_nr < range.end(); layer_nr++) {
            ...
            auto insert_point = [&](Point pt, const ExPolygon& overhang, double radius, bool force_add = false, bool add_interface = true) {
                ...
                if (force_add || !already_inserted.count(hash_pos)) {
                    ...
                    if (bottom_z - z_distance_top < m_object->get_layer(0)->print_z)
                        return contact_node; // dont add it if overlapping with the initial layer
                    contact_node           = m_ts_data->create_node(pt, -gap_layers, layer_nr - 1, roof_layers + (gap_layers > 0 ? 1 : 0),
                                                                    to_buildplate, SupportNode::NO_PARENT, bottom_z, z_distance_top, 0, radius);
                    contact_node->overhang    = overhang;
                    contact_node->origin_area = overhang.area();
            ...
            for (const auto& overhang_with_type : layer->loverhangs_with_type) {
                ...
                const bool is_manual_enforcer  = overhang_type & OverhangType::Enforced;
                ...
                if (!is_sharp_tail && (config.support_expansion.value > EPSILON))
                    overhangs = safe_offset_inc({overhang_part}, scale_(config.support_expansion.value), relevant_forbidden,
                                                scale_(MIN_BRANCH_RADIUS * 1.75), 0, 1);
                const bool hybrid_normal_overhang =
                    m_support_params.support_style == smsTreeHybrid &&
                    !is_sharp_tail &&
                    ((overhang_type & (BigFlat | ThinPlate)) || area(overhang_part) > SQ(m_support_params.thresh_big_overhang));
                if (hybrid_normal_overhang) {
                    overhangs_regular           = intersection_ex(overhangs, offset_ex(m_ts_data->m_layer_outlines_below[layer_nr - 1],
                                                                                       scale_(config.support_object_xy_distance.value)));
                    ExPolygons overhangs_normal = offset2_ex(diff_ex({overhang_part}, overhangs_regular), scale_(extrusion_width),
                                                             -scale_(extrusion_width));
                    if (add_interface && xy_expansion > EPSILON && !is_sharp_tail)
                        overhangs_normal = safe_offset_inc(overhangs_normal, xy_expansion,
                                                           offset_ex(m_ts_data->m_layer_outlines_below[layer_nr - 1],
                                                                     scale_(config.support_object_xy_distance.value)),
                                                           scale_(MIN_BRANCH_RADIUS * 1.75), 0, 1);
                    ...
                    for (auto& overhang : overhangs_normal_split) {
                        ...
                        if (!contact_node)
                            continue;
                        if (is_manual_enforcer) {
                            contact_node->is_manual_enforcer = true;
                        }
                ...
                for (auto& overhang : overhangs_regular) {
                    if (is_sharp_tail && !m_support_params.soluble_interface && overhang.area() < SQ(scale_(2.)))
                        add_interface = false;
                    BoundingBox overhang_bounds = get_extents(overhang);
                    ...
                    for (size_t i = 0; i < edge_cache.holeCount() + 1; i++) {
                        ...
                        while (distance < 1) {
                            ...
                            SupportNode* contact_node = insert_point(pt, overhang, radius, false, add_interface);
                            if (contact_node && is_manual_enforcer)
                                contact_node->is_manual_enforcer = true;
                            distance += step;
                        }
                    }
                    ...
                    for (Point candidate : grid_points) {
                        if (overhang_bounds.contains(candidate)) {
                            // BBS: move_inside_expoly shouldn't be used if candidate is already inside, as it moves point to boundary and
                            // the inside is not well supported!
                            bool is_inside = is_inside_ex(overhang_inner, candidate);
                            if (is_inside) {
                                SupportNode* contact_node = insert_point(candidate, overhang, radius, false, add_interface);
                                if (contact_node && is_manual_enforcer)
                                    contact_node->is_manual_enforcer = true;
                            }
                        }
            ...
            for (auto& pt_and_normal : vertical_enforcer_points_by_layers[layer_nr]) {
                ...
                auto node = insert_point(vertical_enforcer_point, ExPolygon(), calc_radius(0.), false, true);
                if (node)
            ...
            for (auto node : curr_nodes) {
                all_nodes.emplace_back(node->position(0), node->position(1), scale_(node->print_z));
            }
#ifdef SUPPORT_TREE_DEBUG_TO_SVG
            if (!curr_nodes.empty())
                draw_contours_and_nodes_to_svg(debug_out_path("init_contact_points_%.2f.svg", bottom_z), layer->loverhangs,
                                               layer->lslices_extrudable, m_ts_data->m_layer_outlines_below[layer_nr],
                                               contact_nodes[layer_nr], contact_nodes[layer_nr - 1],
                                               {"overhang", "lslices", "outlines_below"});
#endif
        }
    }); // end tbb::parallel_for
    ...
    : m_xy_distance(xy_distance), m_radius_sample_resolution(radius_sample_resolution)
{
    ...
    for (std::size_t layer_nr = 0; layer_nr < object.layers().size(); ++layer_nr) {
        ...
        for (const ExPolygon& poly : layer->lslices) {
            append(outline, union_ex_2(poly.simplify_p(scale_(m_radius_sample_resolution))));
        }
        ...
        if (layer_nr == 0)
            ...
            if (layer_nr % 10 == 0)
                m_layer_outlines_below[layer_nr] = union_ex_2(m_layer_outlines_below[layer_nr]);
        }
    }
}
...
SupportNode* TreeSupportData::create_node(const Point  position,
                                          ...
                                          coordf_t     print_z_,
                                          coordf_t     height_,
                                          coordf_t     dist_mm_to_top_,
                                          coordf_t     radius_)
{
    // this function may be called from multiple threads, need to lock
    m_mutex.lock();
    std::unique_ptr<SupportNode> node    = std::make_unique<SupportNode>(position, distance_to_top, obj_layer_nr, support_roof_layers_below,
                                                                         to_buildplate, parent, print_z_, height_, dist_mm_to_top_, radius_);
...
const ExPolygons& TreeSupportData::calculate_avoidance(const RadiusLayerPair& key) const
{
    ...
    // assert(ret.second);
    //  BOOST_LOG_TRIVIAL(debug) << format("calculate_avoidance: radius=%.2f, layer_nr=%d, recursions=%d, avoidance_areas=%d", radius,
    //  layer_nr, key.recursions,
