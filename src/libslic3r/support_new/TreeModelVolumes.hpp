...
inline constexpr ScaledCoordOnly<Tout> scaled_copy(const Tin &v) noexcept
{
    //return static_cast<Tout>(std::round(v / SCALING_FACTOR));
    return Tout(v / Tin(SCALING_FACTOR_INTERNAL));
}

static constexpr const double  SUPPORT_TREE_EXPONENTIAL_FACTOR = 1.5;
static constexpr coord_t       SUPPORT_TREE_EXPONENTIAL_THRESHOLD = scaled_copy<coord_t>(1. * SUPPORT_TREE_EXPONENTIAL_FACTOR);
static constexpr coord_t       SUPPORT_TREE_COLLISION_RESOLUTION  = scaled_copy<coord_t>(0.5);
...
public:
    ...
    void addSharpTail(const std::vector<std::pair<std::pair<int, int>, const ExPolygons>>& overhang_sharps);
...
private:
    ...
    std::map<int, Polygons>                                                     m_layer_outlines_sharp;
    /*!
