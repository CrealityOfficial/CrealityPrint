...
struct SupportParameters {
    ...
    SupportParameters(const PrintObject& object)
    {
        ...
        if (object.layer_count() > 0) {
            for (LayerRegion* layerm : object.get_layer(0)->regions())
                if (!layerm->slices.empty()) {
                    ...
                    ironing_layer_height                           = _ironing_layer_height * ironing_spacing /
                                           this->support_material_interface_flow.nozzle_diameter();
                    ironing_width =Flow::rounded_rectangle_extrusion_width_from_spacing(float(this->support_material_interface_flow.nozzle_diameter()),
                                                                             float(ironing_layer_height));
                    break;
                }
        }
        ...
        //this->gap_xy_first_layer = this->gap_xy;
        ...
        this->base_fill_pattern_tree =
            object_config.support_base_pattern_tree == smpHoneycomb ? ipHoneycomb :
            this->support_density > 0.95 || this->with_sheath ? ipRectilinear : ipSupportBase;
        ...
        //int tree_support_wall_count = is_tree(object_config.support_type) ? object_config.tree_support_wall_count_tree.value : object_config.tree_support_wall_count.value;
        tree_branch_diameter_double_wall_area_scaled = tree_support_wall_count > 1 ? 0.1 :
                                                       tree_support_wall_count == 0 ? 0.25 * sqr(scaled<double>(5.0)) * M_PI :
    ...
    Flow        support_material_ironing_flow;
    ...
    InfillPattern           base_fill_pattern_tree;
