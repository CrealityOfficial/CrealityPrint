...
public:
    ...
    static std::string  get_cloud_type_string(Preset::Type type);
    static Preset::Type get_type_from_string(std::string type_str);

    //Creality
    std::vector<std::string> compatibles() const;
...
//        assert(opt != nullptr);
    ...
    void                set_user_presets_from_appconfig(const AppConfig &app_config);
    ...
    static double convert_pellet_flow_to_filament_diameter(double pellet_flow_coefficient){ return sqrt(4 / (PI * pellet_flow_coefficient)); }
    ...
    static const std::vector<std::string>&  machine_options();
...
public:
    ...
    void save_system_preset(const std::string& dir_path, const std::string& type, Preset* preset);
    ...
    //Creality
    std::string findInheritsVendor(const std::string& name);
