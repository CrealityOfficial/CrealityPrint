...
void InterlockingGenerator::generate_embedding_wall(PrintObject* print_object){
    ...
    coord_t DEFAULT_BEAM_WIDTH = scaled(0.2); // ÀýÈçÄ¬ÈÏ2mm
    ...
    tbb::parallel_for(
        ...
        [print_object, beam_width, boundary_avoidance, cell_size, interface_dilation, air_dilation, air_filtering](const tbb::blocked_range<size_t>& range) {
            ...
            for(size_t region_a_index = 0; region_a_index < layer->region_count(); region_a_index++){
                const PrintRegion& print_region = layer->get_region(region_a_index)->region();
                const PrintRegionConfig& config = print_region.config();

                for(size_t region_b_index = region_a_index + 1; region_b_index < layer->region_count(); region_b_index++){
                    const PrintRegion& print_region_other = layer->get_region(region_b_index)->region();
                    const PrintRegionConfig& config_other = print_region_other.config();
                    // wall to infill
                    if (!(config_other.embedding_wall_into_infill || config.embedding_wall_into_infill)|| layer->is_perimeter_compatible(print_region, print_region_other) ||
