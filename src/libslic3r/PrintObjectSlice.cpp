...
void PrintObject::slice_volumes()
{
    ...
    if(print->config().ignore_inner_color && !is_mm_painted)
    {
	    tbb::parallel_for(
	        tbb::blocked_range<size_t>(0, m_layers.size()),
			[this, &region_slices](const tbb::blocked_range<size_t>& range) {
	            for (size_t layer_id = range.begin(); layer_id < range.end(); ++ layer_id) {
	                m_print->throw_if_canceled();
	                Layer *layer = m_layers[layer_id];

                    size_t region_count = layer->region_count();
                    std::vector<ExPolygons*> layer_regions;
                    for(int i = 0; i < region_count; i++)
                    {
                        layer_regions.push_back(&region_slices[i][layer_id]);
                    }
                    std::vector<int> region_flags;
                    creality::contains(layer_regions, region_flags);

                    for(int i : region_flags)
                    {
                        layer->get_region(i)->all_inner = true;
                    }
	            }
	        });
    }
    ...
    bench_debug_lslices(m_print, this);
