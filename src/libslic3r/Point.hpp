...
inline bool larger_than_ex(const Point &v, const coord_t sqrt_len)
{
    if (std::abs(v.x()) > sqrt_len || std::abs(v.y()) > sqrt_len)
        return true;
    return v.cast<int64_t>().squaredNorm() > sqrt_len * sqrt_len;
}
