...
#define PLATE_THUMBNAIL_SMALL_WIDTH     96
#define PLATE_THUMBNAIL_SMALL_HEIGHT    96

#define UPLOAD_PLATE_THUMBNAIL_WIDTH     256
#define UPLOAD_PLATE_THUMBNAIL_HEIGHT    256
...
#define COMATIBLE_ORCA_3MF_VERSION        "2.1.0"
#define COMATIBLE_BAMBU_3MF_VERSION        "1.8.2"
...
enum class SaveStrategy
{
    ...
    UploadToPrinter = 0x100000 | WithGcode | Silence  | SplitModel,
};
...
struct CxCloudGcodeStoreParameter
{
	...
	std::string model_user;	
	std::string model_copyright;

    std::string face_image_file;
    std::string config_image_file;

    std::vector<std::pair<int, std::string>> gcode_files;
    std::vector<std::pair<int, std::string>> gcode_md5_files;
    std::vector<std::pair<int, std::string>> thumbnail_files; 

    Model* model = nullptr;
    DynamicPrintConfig* config = nullptr;
};

extern bool CxCloud_store_gcode(const CxCloudGcodeStoreParameter& parameter);
