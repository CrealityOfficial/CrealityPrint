...
const std::string CREALITY_CONFIG_FILE = "Metadata/creality.config";
...
static const std::string appName3mf = "Creality_Print";
static const std::string fileVersion3mf = "1.0";
static const std::string fileType3mf = "Undefined";
...
constexpr bool kDetailed3mfImportTimingLogs = false;
...
static size_t nozzle_count_from_project_config(const DynamicPrintConfig &config)
{
    const auto *opt = config.option<ConfigOptionFloats>("nozzle_diameter");
    return opt != nullptr ? opt->values.size() : 0;
}

static std::string validate_project_filament_mapping(const DynamicPrintConfig &config)
{
    if (Preset::printer_technology(config) != ptFFF)
        return {};
    if (!config.has("filament_map_mode") || !config.has("filament_map"))
        return "Project filament nozzle mapping fields are incomplete.";

    const auto *filament_map = config.option<ConfigOptionInts>("filament_map");
    if (filament_map == nullptr)
        return "Project filament nozzle mapping has an invalid value type.";

    return validate_complete_filament_map(filament_map->values,
                                          physical_filament_count_from_project_config(config),
                                          nozzle_count_from_project_config(config));
}

static bool prepare_project_filament_mapping_after_load(DynamicPrintConfig &config,
                                                        bool &migrated,
                                                        std::string &error)
{
    migrated = false;
    ...
    const bool has_mode = config.has("filament_map_mode");
    const bool has_map  = config.has("filament_map");
    const size_t filament_count = physical_filament_count_from_project_config(config);
    const size_t nozzle_count   = nozzle_count_from_project_config(config);

    // Missing assignments are resolved by the normal pre-slice flow, when
    // plate usage is known and a manual choice can be requested from the user.
    if (!has_mode) {
        config.set_key_value("filament_map_mode", new ConfigOptionEnum<FilamentMapMode>(fmmAutoForSaving));
        migrated = true;
    }
    if (!has_map) {
    ...
    const auto *saved_map = config.option<ConfigOptionInts>("filament_map");
    if (saved_map == nullptr) {
        error = "Project filament nozzle mapping has an invalid value type.";
        return false;
    }
    if (saved_map->values.size() > filament_count ||
        std::any_of(saved_map->values.begin(), saved_map->values.end(), [nozzle_count](int nozzle) {
            return nozzle < 0 || size_t(nozzle) > nozzle_count;
        })) {
        error = validate_complete_filament_map(saved_map->values, filament_count, nozzle_count);
        return false;
    }

    const std::vector<int> filament_map = config.option<ConfigOptionInts>("filament_map")->values;
    config.set_key_value("filament_map_2", new ConfigOptionInts(build_filament_map_2(filament_map)));

    std::vector<int> volume_map(filament_count, 0);
    if (const auto *saved_volume_map = config.option<ConfigOptionInts>("filament_volume_map"); saved_volume_map != nullptr) {
        const size_t copy_count = std::min(saved_volume_map->values.size(), volume_map.size());
        std::copy_n(saved_volume_map->values.begin(), copy_count, volume_map.begin());
    }
    config.set_key_value("filament_volume_map", new ConfigOptionInts(std::move(volume_map)));
    return true;
}
...
static ProjectFilamentIdInfo filament_id_info_from_project_config(const DynamicPrintConfig &config)
{
    ...
    // Current virtual IDs enumerate all allocated entries, including incomplete
    // rows that retain their slots. Preserve IDs already valid in the current
    // scheme and map trailing legacy entries by allocated-slot order.
    const size_t legacy_auto_count = physical_count * (physical_count - 1) / 2;
    const size_t legacy_total      = physical_count + legacy_auto_count + current_mixed.virtual_count();
    ...
    for (const MixedFilament &mixed : current_mixed.mixed_filaments()) {
        if (!mixed.occupies_virtual_slot())
            continue;
        const size_t old_id = physical_count + legacy_auto_count + slot_idx + 1;
        const size_t new_id = physical_count + slot_idx + 1;
...
#define _(s) Slic3r::I18N::translate(s)
    ...
    class _BBS_3MF_Importer : public _BBS_3MF_Base
    {
        ...
        struct Geometry
        {
            ...
            TriangleAttributes custom_supports;
            TriangleAttributes custom_seam;
            TriangleAttributes mmu_segmentation;
            TriangleAttributes fuzzy_skin;
            // BBS
            TriangleAttributes face_properties;

            bool empty() { return vertices.empty() || triangles.empty(); }

            void append_triangle_attributes(size_t triangle_id, const char** attributes, unsigned int num_attributes)
            {
                if (attributes == nullptr || num_attributes % 2 != 0)
                    return;
                for (unsigned int attribute = 0; attribute < num_attributes; attribute += 2) {
                    const char* key = attributes[attribute];
                    const char* value = attributes[attribute + 1];
                    if (key == nullptr || value == nullptr || *value == '\0')
                        continue;

                    TriangleAttributes* destination = nullptr;
                    if (::strcmp(key, CUSTOM_SUPPORTS_ATTR) == 0)
                        destination = &custom_supports;
                    else if (::strcmp(key, CUSTOM_SEAM_ATTR) == 0)
                        destination = &custom_seam;
                    else if (::strcmp(key, MMU_SEGMENTATION_ATTR) == 0)
                        destination = &mmu_segmentation;
                    else if (::strcmp(key, CUSTOM_FUZZY_SKIN_ATTR) == 0)
                        destination = &fuzzy_skin;
                    else if (::strcmp(key, FACE_PROPERTY_ATTR) == 0)
                        destination = &face_properties;

                    if (destination != nullptr)
                        destination->push_back({triangle_id, value});
                }
            }
        ...
        // Semantic version of Creality Print, that generated this 3MF.
    ...
    private:
        ...
        bool _extract_project_config_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat, DynamicPrintConfig& config, ConfigSubstitutionContext& subs_context, Model& model);
        ...
        for (mz_uint i = 0; i < num_entries; ++i) {
            if (mz_zip_reader_file_stat(&archive, i, &stat)) {
                ...
                if (boost::algorithm::iequals(name, BBS_PROJECT_CONFIG_FILE)) {
                    ...
                    if (!_extract_project_config_from_archive(archive, stat, config, config_substitutions, model))
                        return false;
                }
        ...
        int plate_id)
    {
        ...
        if constexpr (kDetailed3mfImportTimingLogs)
        ...
        // BBS: load relationships
        const auto relationships_parse_start = kDetailed3mfImportTimingLogs ? ModelImportClock::now() : ModelImportClock::time_point{};
        if constexpr (kDetailed3mfImportTimingLogs)
        ...
        if (sub_index == -1) {
            ...
            const auto sub_relationships_parse_start = kDetailed3mfImportTimingLogs ? ModelImportClock::now() : ModelImportClock::time_point{};
            if constexpr (kDetailed3mfImportTimingLogs)
            ...
            boost::mutex mutex;
            const auto submodel_parse_start = kDetailed3mfImportTimingLogs ? ModelImportClock::now() : ModelImportClock::time_point{};
            if constexpr (kDetailed3mfImportTimingLogs)
            ...
            const auto submodel_merge_start = kDetailed3mfImportTimingLogs ? ModelImportClock::now() : ModelImportClock::time_point{};
            if constexpr (kDetailed3mfImportTimingLogs)
            ...
            for (auto obj_importer : m_object_importers) {
                m_current_objects.merge(obj_importer->object_list);
                m_group_id_to_color.merge(obj_importer->object_group_id_to_color);
                delete obj_importer;
            }
        ...
        //extract model files
        const auto root_model_parse_start = kDetailed3mfImportTimingLogs ? ModelImportClock::now() : ModelImportClock::time_point{};
        if constexpr (kDetailed3mfImportTimingLogs)
        ...
        for (const IdToModelObjectMap::value_type& object : m_objects) {
            ...
            int object_id = object.second+1;
            if(m_is_oldcreality_3mf)
                object_id = object.first.second;
            IdToLayerHeightsProfileMap::iterator obj_layer_heights_profile = m_layer_heights_profiles.find(object_id);
...
//        model.adjust_min_z();
    ...
    bool _BBS_3MF_Importer::_extract_xml_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat, XML_StartElementHandler start_handler, XML_EndElementHandler end_handler)
    {
        ...
        bool bNeedCheckModel = std::string(stat.m_filename) == "3D/_rels/3dmodel.model.rels";
        bool bNeedCheckModelSetting = std::string(stat.m_filename) == "Metadata/model_settings.config";
        bool bNeedCheckSliceInfo = std::string(stat.m_filename) == "Metadata/slice_info.config";
        int datalen = (int)stat.m_uncomp_size;
        if (!XML_ParseBuffer(m_xml_parser, datalen, 1)) {
            if(bNeedCheckModel || bNeedCheckModelSetting || bNeedCheckSliceInfo)
            {
                std::string rels_content = std::string((const char*)parser_buffer, stat.m_uncomp_size);
                std::regex targetRegex;
                if(bNeedCheckModel)
                {
                    targetRegex = std::regex(R"(Target=\"([^\"]*)\")");
                }
                else if(bNeedCheckModelSetting)
                {
                    targetRegex = std::regex(R"(key=\"name\".*value=\"([^\"]*)\")");
                    m_objects_metadata.clear();
                }else if(bNeedCheckSliceInfo)
                {
                    //(value\s=\s.*(/\s)>)
                    targetRegex = std::regex(R"(value\s=(\s.*/\s)>)");
                }
                std::string          new_rels_content = rels_content;
                std::sregex_iterator iter(rels_content.begin(), rels_content.end(), targetRegex);
                auto end = std::sregex_iterator();
                size_t step = 0;
                while (iter != end) {
                    std::string content = (*iter)[1].str();
                    std::string newTarget = xml_escape(content);
                    if(bNeedCheckSliceInfo)
                    {
                        newTarget = content.substr(0,content.size()-1);
                    }
                    
                    auto pos = rels_content.find(content) + step;
                    if(pos != std::string::npos)
                    {
                        new_rels_content.replace(pos, content.size(), newTarget);
                    }
                    step += newTarget.size() - content.size();
                    ++iter;
                }
                XML_ParserFree(m_xml_parser);
                m_xml_parser = XML_ParserCreate(nullptr);
                ...
                XML_SetElementHandler(m_xml_parser, start_handler, end_handler);
                XML_SetCharacterDataHandler(m_xml_parser, _BBS_3MF_Importer::_handle_xml_characters);
                void* new_parser_buffer = XML_GetBuffer(m_xml_parser, (int)new_rels_content.size());
                memcpy(new_parser_buffer, new_rels_content.c_str(), new_rels_content.size());
                datalen = new_rels_content.size();
                if (XML_ParseBuffer(m_xml_parser, datalen, 1))
                {
                    return true;
                }
            ...
            char error_buf[1024];
            ::snprintf(error_buf, 1024, "Error %s(%s) while parsing xml file at line %d",stat.m_filename, XML_ErrorString(XML_GetErrorCode(m_xml_parser)), (int)XML_GetCurrentLineNumber(m_xml_parser));
            add_error(error_buf);
            return false;
    ...
    bool _BBS_3MF_Importer::_extract_project_config_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat, DynamicPrintConfig& config, ConfigSubstitutionContext& config_substitutions, Model& model)
    {
        if (stat.m_uncomp_size > 0) {
            ...
            bool mapping_migrated = false;
            std::string mapping_error;
            if (!prepare_project_filament_mapping_after_load(config, mapping_migrated, mapping_error)) {
                add_error("Invalid project filament nozzle mapping: " + mapping_error);
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": " << mapping_error;
                return false;
            }
            if (mapping_migrated)
                BOOST_LOG_TRIVIAL(info) << "3mf filament nozzle mapping migration: initialized missing mapping fields";
    ...
    void _BBS_3MF_Importer::_extract_brim_ear_points_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)
    {
        ...
        int version = 0;
        const std::string key("brim_points_format_version=");
        if (!objects.empty() && boost::algorithm::starts_with(objects.front(), key)) {
            version = std::atoi(objects.front().c_str() + key.size());
            objects.erase(objects.begin());
        }

        const size_t values_per_point = version == 0 ? 4 : version == 1 ? 5 : 0;
        if (values_per_point == 0) {
            add_error("Unsupported brim ear points format version");
            return;
        }
        ...
        for (const std::string& object : objects) {
            ...
            boost::split(object_data, object, boost::is_any_of("|"), boost::token_compress_off);
            if (object_data.size() != 2) {
                add_error("Error while reading brim ear object data");
                continue;
            }
            ...
            boost::split(object_data_id, object_data[0], boost::is_any_of("="), boost::token_compress_off);
            if (object_data_id.size() != 2) {
                add_error("Error while reading brim ear object id");
                continue;
            }

            const int object_id = std::atoi(object_data_id[1].c_str());
            if (object_id <= 0) {
                add_error("Found invalid brim ear object id");
                continue;
            }

            if (m_brim_ear_points.find(object_id) != m_brim_ear_points.end()) {
                add_error("Found duplicated brim ear points");
                continue;
            }

            std::vector<std::string> object_data_points;
            boost::split(object_data_points, object_data[1], boost::is_any_of(" "), boost::token_compress_on);
            if (object_data_points.size() % values_per_point != 0) {
                add_error("Invalid brim ear points data");
                continue;
            }

            BrimPoints brim_ear_points;
            brim_ear_points.reserve(object_data_points.size() / values_per_point);
            for (size_t i = 0; i < object_data_points.size(); i += values_per_point) {
                brim_ear_points.emplace_back(float(std::atof(object_data_points[i].c_str())),
            ...
            if (!brim_ear_points.empty())
                m_brim_ear_points.emplace(object_id, std::move(brim_ear_points));
        }
    }
    ...
    bool _BBS_3MF_Importer::_handle_end_model()
    {
        ...
        if (!m_is_bbl_3mf) {
            // if the 3mf was not produced by CrealityPrint and there is only one object,
    ...
    bool _BBS_3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        if (m_curr_object) {
            ...
            m_curr_object->geometry.append_triangle_attributes(
                m_curr_object->geometry.triangles.size() - 1, attributes, num_attributes);
        }
        return true;
    }
    ...
    bool _BBS_3MF_Importer::_handle_end_metadata()
    {
        if ((m_curr_metadata_name == BBS_3MF_VERSION)||(m_curr_metadata_name == BBS_3MF_VERSION1)) {
            ...
            // SLIC3R_APP_KEY - CREALITYPRINT_VERSION
            ...
            else if (boost::starts_with(m_curr_characters, "Creality_Print V")) {
                m_is_bbl_3mf = true;
                // Creality_Print V6.0.0.75 Alpha
                std::regex versionRegex(R"(V(\d+\.\d+\.\d+))");
                std::smatch matches;
                if (std::regex_search(m_curr_characters, matches, versionRegex)) {
                     m_bambuslicer_generator_version = Semver::parse( matches[1]);
                     Semver file_version = *m_bambuslicer_generator_version;
                     if(file_version.maj()<=5)
                    {
                            m_is_oldcreality_3mf = true;
                        }
                } else {
            ...
            else if (boost::starts_with(m_curr_characters, "Creality_Tools V"))
            {
        ...
        /*} else if (m_curr_metadata_name == BBS_FDM_SUPPORTS_PAINTING_VERSION) {
            ...
            check_painting_version(m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
                _(L("The selected 3MF contains FDM supports painted object using a newer version of CrealityPrint and is not compatible.")));
        } else if (m_curr_metadata_name == BBS_SEAM_PAINTING_VERSION) {
            m_seam_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_seam_painting_version, SEAM_PAINTING_VERSION,
                _(L("The selected 3MF contains seam painted object using a newer version of CrealityPrint and is not compatible.")));
        } else if (m_curr_metadata_name == BBS_MM_PAINTING_VERSION) {
            m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_mm_painting_version, MM_PAINTING_VERSION,
                _(L("The selected 3MF contains multi-material painted object using a newer version of CrealityPrint and is not compatible.")));*/
    ...
    bool _BBS_3MF_Importer::_handle_end_config_plater()
    {
        ...
        // Creality 3mf files have a bug
        if(m_curr_plater->plate_index<0)
        {
            m_curr_plater->plate_index = 0;
        }
        while(m_curr_plater->plate_index==0||m_plater_data.count(m_curr_plater->plate_index)>0)
        {
            m_curr_plater->plate_index++;
        }
    ...
    bool _BBS_3MF_Importer::_generate_volumes_new(ModelObject& object, const std::vector<Component> &sub_objects, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (unsigned int index = 0; index < sub_objects.size(); index++)
        {
            ...
            if (!shared_volume){
                ...
                // BBS: Allocate dense mesh properties only when the file actually contains one.
                if (!sub_object->geometry.face_properties.empty()) {
                    its.properties.resize(triangles_count);
                    for (const Geometry::TriangleAttribute& attribute : sub_object->geometry.face_properties) {
                        assert(attribute.triangle_id < triangles_count);
                        its.properties[attribute.triangle_id].from_string(attribute.value);
                    }
                }
                ...
                volume = object.add_volume(std::move(triangle_mesh), ModelVolumeType::MODEL_PART, true, false);
                ...
                volume = object.add_volume_with_shared_mesh(*shared_volume, ModelVolumeType::MODEL_PART, false);
            ...
            // Reuse annotations together with a shared mesh. They are indexed by the same triangles.
            {
                if (shared_volume != nullptr) {
                    // A new annotation may initially have the same timestamp as the source.
                    // Touching it prevents assign() from treating the copy as a no-op.
                    volume->supported_facets.touch();
                    volume->seam_facets.touch();
                    volume->mmu_segmentation_facets.touch();
                    volume->fuzzy_skin_facets.touch();
                    volume->supported_facets.assign(shared_volume->supported_facets);
                    volume->seam_facets.assign(shared_volume->seam_facets);
                    volume->mmu_segmentation_facets.assign(shared_volume->mmu_segmentation_facets);
                    volume->fuzzy_skin_facets.assign(shared_volume->fuzzy_skin_facets);
                } else {
                    volume->supported_facets.reserve(sub_object->geometry.custom_supports.size());
                    volume->seam_facets.reserve(sub_object->geometry.custom_seam.size());
                    volume->mmu_segmentation_facets.reserve(sub_object->geometry.mmu_segmentation.size());
                    volume->fuzzy_skin_facets.reserve(sub_object->geometry.fuzzy_skin.size());
                    for (const Geometry::TriangleAttribute& attribute : sub_object->geometry.custom_supports) {
                        volume->supported_facets.set_triangle_from_string(attribute.triangle_id, attribute.value);
                    }
                    for (const Geometry::TriangleAttribute& attribute : sub_object->geometry.custom_seam) {
                        volume->seam_facets.set_triangle_from_string(attribute.triangle_id, attribute.value);
                    }
                    for (const Geometry::TriangleAttribute& attribute : sub_object->geometry.mmu_segmentation) {
                        volume->mmu_segmentation_facets.set_triangle_from_string(attribute.triangle_id, attribute.value);
                    }
                    for (const Geometry::TriangleAttribute& attribute : sub_object->geometry.fuzzy_skin) {
                        volume->fuzzy_skin_facets.set_triangle_from_string(attribute.triangle_id, attribute.value);
                    }
        ...
        // Adding each volume separately would serialize an ever-growing object repeatedly.
    ...
    bool _BBS_3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            if (!m_is_bbl_3mf) {
                // if the 3mf was not produced by CrealityPrint and there is only one instance,
                // bake the transformation into the geometry to allow the reload from disk command
                // to work properly
                if (object.instances.size() == 1) {
                    triangle_mesh.transform(object.instances.front()->get_transformation().get_matrix(), false);
    ...
    */
    ...
    bool _BBS_3MF_Importer::ObjectImporter::_handle_object_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        if (current_object) {
            ...
            current_object->geometry.append_triangle_attributes(
                current_object->geometry.triangles.size() - 1, attributes, num_attributes);
        }
        return true;
    }
    ...
    bool _BBS_3MF_Importer::ObjectImporter::_extract_object_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)
    {
        ...
        struct CallbackData
        {
            ...
            CallbackData(XML_Parser& parser, _BBS_3MF_Importer::ObjectImporter& importer, const mz_zip_archive_file_stat& stat)
                : parser(parser), importer(importer), stat(stat)
            {}
        };
        ...
        try
        {
            mz_file_write_func callback = [](void* pOpaque, mz_uint64 file_ofs, const void* pBuf, size_t n)->size_t {
                CallbackData* data = (CallbackData*)pOpaque;
                const bool parse_ok = XML_Parse(data->parser, (const char*)pBuf, (int)n,
                    (file_ofs + n == data->stat.m_uncomp_size) ? 1 : 0) != XML_STATUS_ERROR;
                if (!parse_ok || data->importer.object_parse_error()) {
                    char error_buf[1024];
                    ::snprintf(error_buf, 1024, "Error (%s) while parsing '%s' at line %d",
                        data->importer.object_parse_error_message(), data->stat.m_filename,
                        (int)XML_GetCurrentLineNumber(data->parser));
                    throw Slic3r::FileIOError(error_buf);
                }
                return n;
            };
            res = mz_zip_reader_extract_to_callback(&archive, stat.m_file_index, callback, &data, 0);
        }
        catch (const version_error& e)
        {
    ...
    class _BBS_3MF_Exporter : public _BBS_3MF_Base
    {
        ...
        bool m_upload_to_printer { false };  // whether to upload to printer
    ...
    public:
        ...
        bool CxCloud_save_gcode_3mf(const CxCloudGcodeStoreParameter& paramter);
    ...
    private:
        ...
        bool _save_model_to_file(const std::string& filename,
            ...
            En3mfType type_3mf = En3mfType::From_Creality);
        ...
        bool _add_upload_thumbnail_file_to_archive(mz_zip_archive& archive, const ThumbnailData& thumbnail_data, const char* local_path, int index, bool generate_small_thumbnail = false);
        ...
        bool _add_model_file_to_archive(const std::string& filename, mz_zip_archive& archive, const Model& model, ObjectToObjectDataMap& objects_data, Export3mfProgressFn proFn = nullptr, BBLProject* project = nullptr, En3mfType type_3mf = En3mfType::From_Creality) const;
        ...
        bool _add_project_config_file_to_archive(mz_zip_archive& archive, const DynamicPrintConfig &config, Model& model, En3mfType type_3mf = En3mfType::From_Creality);
        ...
        bool _add_slice_info_config_file_to_archive(mz_zip_archive &archive, const Model &model, PlateDataPtrs &plate_data_list, const ObjectToObjectDataMap &objects_data, const DynamicPrintConfig& config, En3mfType type_3mf = En3mfType::From_Creality);
        bool _add_creality_config_file_to_archive(mz_zip_archive& archive, const DynamicPrintConfig& config);
    ...
    bool _BBS_3MF_Exporter::CxCloud_save_gcode_3mf(const CxCloudGcodeStoreParameter& parameter)
    {
		mz_zip_archive archive;
		mz_zip_zero_struct(&archive);

		if (!open_zip_writer(&archive, parameter.gcode_3mf))
		{
			return false;
		}
		...
		} lock{ archive, &parameter.gcode_3mf };
        ...
        //face image
        if (boost::filesystem::exists(parameter.face_image_file))
        {
			std::string dst_in_3mf = "Metadata/face_image.png";
			if (!_add_file_to_archive(archive, dst_in_3mf, parameter.face_image_file))
			{
				return false;
			}
        }
        ...
        if (boost::filesystem::exists(parameter.face_image_file))
        {
			std::string dst_in_3mf = "Metadata/config_image.png";
			if (!_add_file_to_archive(archive, dst_in_3mf, parameter.face_image_file))
			{
				return false;
			}
        ...
        //gcode file
        for (auto &gcode_file : parameter.gcode_files)
        {
            if (!boost::filesystem::exists(gcode_file.second))
            {
                continue;
            }

            std::string dst_in_3mf = (boost::format("Metadata/plate_%1%.gcode") % (gcode_file.first)).str();
			if (!_add_file_to_archive(archive, dst_in_3mf, gcode_file.second))
			{
				return false;
			}
        }
        ...
        //thumbnail file
		for (auto& thumbnail_file : parameter.thumbnail_files)
		{
			if (!boost::filesystem::exists(thumbnail_file.second))
			{
				continue;
			}

			std::string dst_in_3mf = (boost::format("Metadata/plate_%1%.png") % (thumbnail_file.first)).str();
			if (!_add_file_to_archive(archive, dst_in_3mf, thumbnail_file.second))
			{
				return false;
			}
		}
		...
		//thumbnail file
		for (auto& md5_file : parameter.gcode_md5_files)
		{
			if (!boost::filesystem::exists(md5_file.second))
			{
				continue;
			}

			std::string dst_in_3mf = (boost::format("Metadata/plate_%1%.gcode.md5") % (md5_file.first)).str();
			if (!_add_file_to_archive(archive, dst_in_3mf, md5_file.second))
			{
				return false;
			}
		}

		//add the project_setting file
		if (parameter.config)
		{
            const std::string mapping_error = validate_project_filament_mapping(*parameter.config);
            if (!mapping_error.empty()) {
                add_error("Invalid project filament nozzle mapping: " + mapping_error);
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": " << mapping_error;
                return false;
            }
			const std::string& temp_path = parameter.work_directory;
			std::string temp_file = temp_path + std::string("/") + "_temp_xxxx.config";
			std::string        version{ CREALITYPRINT_VERSION };
			parameter.config->save_to_json(temp_file, std::string("project_settings"), std::string("project"), version);
			if (!_add_file_to_archive(archive, BBS_PROJECT_CONFIG_FILE, temp_file))
			{
				return false;
			}
		}
		...
		if (!_add_relationships_file_to_archive(archive, {}, {}, {}, temp_data, -1))
		{
        ...
        //add "Metadata/slice_info.config"
        {
			...
			stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-CX-Client-Type" << "\" " << VALUE_ATTR << "=\"" << "creality_print" << "\"/>\n";
			stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-CX-Client-Version" << "\" " << VALUE_ATTR << "=\"" << convert_to_full_version(CREALITYPRINT_VERSION) << "\"/>\n";
			stream << "  </" << SLICE_HEADER_TAG << ">\n";
			stream << "</" << CONFIG_TAG << ">\n";
        ...
        // Adds model file ("3D/3dmodel.model").
        {
            ...
            metadata_item_map["cloud_model_id"]        = parameter.model_id;
            metadata_item_map["cloud_model_name"]      = parameter.model_name;
            metadata_item_map["cloud_model_user"]      = parameter.model_user;
            metadata_item_map["cloud_model_copyright"] = parameter.model_copyright;

			metadata_item_map[BBL_APPLICATION_TAG] = (boost::format("%1% V%2% %3%") % "Creality_Print" % CREALITYPRINT_VERSION % PROJECT_VERSION_EXTRA).str();

			for (auto item : metadata_item_map) 
            {
				stream << " <" << METADATA_TAG << " name=\"" << item.first << "\">" << xml_escape(item.second) << "</" << METADATA_TAG << ">\n";
			}

            //the empty resources tag
        ...
        //do not remove the file if success
    ...
    bool _BBS_3MF_Exporter::save_model_to_file(StoreParams& store_params)
    {
        ...
        m_upload_to_printer = store_params.strategy & SaveStrategy::UploadToPrinter;
        ...
        bool result = _save_model_to_file(filename + ".tmp", *store_params.model, store_params.plate_data_list, store_params.project_presets, store_params.config,
                                          ...
                                          store_params.proFn, store_params.calibration_thumbnail_data,
                                          store_params.id_bboxes, store_params.project, store_params.export_plate_idx, store_params.type_3mf);
        if (result) {
        ...
        En3mfType type_3mf)
    {
        ...
        // Adds content types file ("[Content_Types].xml";).
        // The content of this file is the same for each CrealityPrint 3mf.
        if (!_add_content_types_file_to_archive(archive)) {
            return false;
        }
        ...
        if (!m_skip_static) {
            ...
            for (unsigned int index = 0; index < thumbnail_data.size(); index++)
            {
                ...
                {
                    if(m_upload_to_printer)  //BBS: when upload multi plate gcode(3mf) to printer
                    {
                        if (!_add_upload_thumbnail_file_to_archive(archive, *thumbnail_data[index], "Metadata/plate", index, true)) {
                            return false;
                        }
                    }
                    else {
        ...
        //if (!m_skip_model)
        {
            if (!_add_model_file_to_archive(filename, archive, model, objects_data, proFn, project, type_3mf)) { return false; }
        ...
        if (!_add_creality_config_file_to_archive(archive, *config)) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__ << boost::format(", _add_creality_config_file_to_archive failed\n");
            return false;
        }
...
/**
 * @brief Adds a upload thumbnail file to the archive for the given object. (the original ThumbnailData size is(512,512), the upload thumbnail size is(256,256))
 *
 * @param[in] archive The zip archive to add the thumbnail to.
 * @param[in] thumbnail_data The thumbnail data to add.
 * @param[in] local_path The local path of the thumbnail file.
 * @param[in] index The index of the thumbnail file (will be part of the filename).
 * @param[in] generate_small_thumbnail If true, a small thumbnail file will be generated and added to the archive.
 *
 * @return true if the thumbnail was added successfully, false otherwise.
 */
    bool _BBS_3MF_Exporter::_add_upload_thumbnail_file_to_archive(mz_zip_archive& archive, const ThumbnailData& thumbnail_data, const char* local_path, int index, bool generate_small_thumbnail)
    {
        bool res = false;
        ...
        {
            //generate small size of thumbnail
            std::vector<unsigned char> upload_pixels;
            upload_pixels.resize(UPLOAD_PLATE_THUMBNAIL_WIDTH * UPLOAD_PLATE_THUMBNAIL_HEIGHT * 4);
            /* step width and step height */
            int sw = thumbnail_data.width / UPLOAD_PLATE_THUMBNAIL_WIDTH;
            int sh = thumbnail_data.height / UPLOAD_PLATE_THUMBNAIL_HEIGHT;
            int clampped_width = sw * UPLOAD_PLATE_THUMBNAIL_WIDTH;
            int clampped_height = sh * UPLOAD_PLATE_THUMBNAIL_HEIGHT;
            ...
            for (int i = 0; i < clampped_height; i += sh) {
                for (int j = 0; j < clampped_width; j += sw) {
                    ...
                    b = std::clamp(0, b / sw / sh, 255);
                    a = std::clamp(0, a / sw / sh, 255);
                    upload_pixels[4 * (i / sw * UPLOAD_PLATE_THUMBNAIL_WIDTH + j / sh) + 0] = (unsigned char)r;
                    upload_pixels[4 * (i / sw * UPLOAD_PLATE_THUMBNAIL_WIDTH + j / sh) + 1] = (unsigned char)g;
                    upload_pixels[4 * (i / sw * UPLOAD_PLATE_THUMBNAIL_WIDTH + j / sh) + 2] = (unsigned char)b;
                    upload_pixels[4 * (i / sw * UPLOAD_PLATE_THUMBNAIL_WIDTH + j / sh) + 3] = (unsigned char)a;
                }
            }
            size_t upload_png_size = 0;
            void* upload_png_data = tdefl_write_image_to_png_file_in_memory_ex((const void*)upload_pixels.data(), UPLOAD_PLATE_THUMBNAIL_WIDTH, UPLOAD_PLATE_THUMBNAIL_HEIGHT, 4, &upload_png_size, MZ_DEFAULT_COMPRESSION, 1);
            if (upload_png_data != nullptr) {
                std::string thumbnail_name = (boost::format("%1%_%2%.png") % local_path % (index + 1)).str();
                res = mz_zip_writer_add_mem(&archive, thumbnail_name.c_str(), (const void*)upload_png_data, upload_png_size, MZ_NO_COMPRESSION);
                mz_free(upload_png_data);
            }

            if (!res) {
                add_error("Unable to add upload thumbnail file to archive");
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__ << boost::format(", Unable to add upload thumbnail file to archive\n");
            }
        }
        ...
        if (generate_small_thumbnail && thumbnail_data.is_valid()) {
            ...
            if (!res) {
                add_error("Unable to add small upload thumbnail file to archive");
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__ << boost::format(", Unable to add small upload thumbnail file to archive\n");
            }
        }
    ...
    */
    bool _BBS_3MF_Exporter::_add_model_file_to_archive(const std::string& filename, mz_zip_archive& archive, const Model& model, ObjectToObjectDataMap& objects_data, Export3mfProgressFn proFn, BBLProject* project, En3mfType type_3mf) const
    {
...

        {
            ...
            if (!sub_model) {
                ...
                //Only BambuStudio will check the major version of the 3MF file
                std::string application_tag;
                switch (type_3mf) {
                case Slic3r::En3mfType::From_Creality: {
                    application_tag = (boost::format("%1% V%2% %3%") % "Creality_Print" % CREALITYPRINT_VERSION % PROJECT_VERSION_EXTRA).str();
                    break;
                }
                case Slic3r::En3mfType::From_Orca: {
                    application_tag = (boost::format("%1%-%2%") % "OrcaSlicer" % COMATIBLE_ORCA_3MF_VERSION).str();
                    break;
                }
                case Slic3r::En3mfType::From_BBS: {
                    application_tag = (boost::format("%1%-%2%") % "BambuStudio" % COMATIBLE_BAMBU_3MF_VERSION).str();
                    break;
                }
                default: break;
                }
                metadata_item_map[BBL_APPLICATION_TAG] = application_tag;
            }
    ...
    bool _BBS_3MF_Exporter::_add_brim_ear_points_file_to_archive(mz_zip_archive& archive, Model& model)
    {
        ...
        for (const ModelObject* object : model.objects) {
            ...
            std::snprintf(buffer, sizeof(buffer), "object_id=%u|", object_id);
            out += buffer;

            for (size_t i = 0; i < brim_points.size(); ++i) {
                const BrimPoint& point = brim_points[i];
                std::snprintf(buffer, sizeof(buffer), i == 0 ? "%f %f %f %f" : " %f %f %f %f",
                              point.pos(0), point.pos(1), point.pos(2), point.head_front_radius);
                out += buffer;
            }
            out += "\n";
        }
        ...
        if (!out.empty()) {
            out.insert(0, std::string("brim_points_format_version=") + std::to_string(brim_points_format_version) + "\n");
    ...
    bool _BBS_3MF_Exporter::_add_project_config_file_to_archive(mz_zip_archive& archive, const DynamicPrintConfig &config, Model& model, En3mfType type_3mf)
    {
        const std::string mapping_error = validate_project_filament_mapping(config);
        if (!mapping_error.empty()) {
            add_error("Invalid project filament nozzle mapping: " + mapping_error);
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": " << mapping_error;
            return false;
        ...
        switch (type_3mf) {
        case Slic3r::En3mfType::From_BBS: {
            version = COMATIBLE_BAMBU_3MF_VERSION;
            break;
        }
        case Slic3r::En3mfType::From_Orca: {
            version = COMATIBLE_ORCA_3MF_VERSION;
            break;
        }
        case Slic3r::En3mfType::From_Creality: {
            break;
        ...
        config.save_to_json(temp_file, std::string("project_settings"), std::string("project"), version);
    ...
    bool _BBS_3MF_Exporter::_add_slice_info_config_file_to_archive(mz_zip_archive& archive, const Model& model, PlateDataPtrs& plate_data_list, const ObjectToObjectDataMap &objects_data, const DynamicPrintConfig& config,En3mfType type_3mf)
    {
        std::stringstream stream;
        // Store mesh transformation in full precision, as the volumes are stored transformed and they need to be transformed back
        // when loaded as accurately as possible.
        ...
        // save slice header for debug
        stream << "  <" << SLICE_HEADER_TAG << ">\n";
        if (type_3mf == En3mfType::From_Creality) {
            stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-CX-Client-Type"    << "\" " << VALUE_ATTR << "=\"" << "creality_print" << "\"/>\n";
            stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-CX-Client-Version" << "\" " << VALUE_ATTR << "=\"" << convert_to_full_version(CREALITYPRINT_VERSION) << "\"/>\n";
        }
		else if (type_3mf == En3mfType::From_BBS || type_3mf == En3mfType::From_Orca) {
        stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-BBL-Client-Type"    << "\" " << VALUE_ATTR << "=\"" << "slicer" << "\"/>\n";
			stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "X-BBL-Client-Version" << "\" " << VALUE_ATTR << "=\"" << convert_to_full_version(COMATIBLE_BAMBU_3MF_VERSION) << "\"/>\n";
		}
        stream << "  </" << SLICE_HEADER_TAG << ">\n";
    ...
    bool _BBS_3MF_Exporter::_add_creality_config_file_to_archive(mz_zip_archive& archive, const DynamicPrintConfig& config)
    {
        /*
        nlohmann::json j;
        j["version"] = std::string(CREALITYPRINT_VERSION);
        j["stage_version"] = std::string(PROJECT_VERSION_EXTRA);
        std::string out    = j.dump();
        if (!mz_zip_writer_add_mem(&archive, CREALITY_CONFIG_FILE.c_str(), (const void*) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION)) {
            add_error("Unable to add creality config file to archive");
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", store  creality config to 3mf,  length %1%, failed\n") % out.length();
            return false;
        }*/
        ...
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "Company"
               << "\" " << VALUE_ATTR << "=\"" << std::string(SLIC3R_APP_KEY) << "\"/>\n";
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "Application"
               << "\" " << VALUE_ATTR << "=\"" << appName3mf << "\"/>\n";
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "AppVersion"
               << "\" " << VALUE_ATTR << "=\"" << std::string(CREALITYPRINT_VERSION) << "\"/>\n";
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "AppStage"
               << "\" " << VALUE_ATTR << "=\"" << std::string(PROJECT_VERSION_EXTRA) << "\"/>\n";
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "FileVersion"
               << "\" " << VALUE_ATTR << "=\"" << fileVersion3mf << "\"/>\n";

        std::string file_type_3mf = fileType3mf;
        if(config.has("Application"))
        {
            const Slic3r::ConfigOptionString* application = config.opt<Slic3r::ConfigOptionString>("Application");
            if(application)
                file_type_3mf = application->value;
        }

        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "FileType"
               << "\" " << VALUE_ATTR << "=\"" << file_type_3mf.c_str() << "\"/>\n";
        std::string profile_version_3mf;
        if (config.has("printer_profile_version")) {
            const Slic3r::ConfigOptionString* profile_version = config.opt<Slic3r::ConfigOptionString>("printer_profile_version");
            if (profile_version)
                profile_version_3mf = profile_version->value;
        }
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
               << "ProfileVersion"
               << "\" " << VALUE_ATTR << "=\"" << profile_version_3mf.c_str() << "\"/>\n";
        std::string date = Slic3r::Utils::utc_timestamp(Slic3r::Utils::get_current_time_utc());
        // keep only the date part of the string
        date = date.substr(0, 10);
        stream << "    <" << METADATA_TAG << " " << KEY_ATTR << "=\""
        ...
        if (!mz_zip_writer_add_mem(&archive, CREALITY_CONFIG_FILE.c_str(), (const void*) out.data(), out.length(),
            ...
            add_error("Unable to add model config file to archive");
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":" << __LINE__
                                     << boost::format(", store  slice-info to 3mf,  length %1%, failed\n") % out.length();
            return false;
        }
...
bool bbs_is_creality_3mf(const char *path)
{
    if (path == nullptr)
        return false;
    ...
    struct ZipReaderCloser {
        mz_zip_archive *archive;
        ~ZipReaderCloser() { close_zip_reader(archive); }
    } closer { &archive };

    const Creality3MFConfigInfo info = read_creality_3mf_config_from_archive(archive);
    return info.has_config && info.is_creality;
}
...
bool CxCloud_store_gcode(const CxCloudGcodeStoreParameter& paramter)
{
	// All export should use "C" locales for number formatting.
	CNumericLocalesSetter locales_setter;
	return _BBS_3MF_Exporter().CxCloud_save_gcode_3mf(paramter);
}
