...
namespace client
{
    ...
    struct MyContext : public ConfigOptionResolver {
        ...
        size_t physical_nozzle_index() const {
            const DynamicConfig* source = external_config != nullptr ? external_config : config;
            return source != nullptr ?
                get_physical_nozzle_index(*source, static_cast<unsigned int>(current_extruder_id)) :
                current_extruder_id;
        }
            ...
            if (opt.opt->is_vector()) {
                ...
                case coFloats: {
                    ...
                    if (opt_floats->size() == 1) {  // old version
                        ...
                        output.set_d(static_cast<const ConfigOptionFloatsNullable *>(opt.opt)->get_at(ctx->physical_nozzle_index()));
                    }
                    break;
                }
                case coFloatsOrPercents: {
                    ...
                    if (boost::ends_with(opt_key, "line_width")) {
                        output.set_d(Flow::extrusion_width(opt_key, *ctx, static_cast<unsigned int>(ctx->physical_nozzle_index())));
                        break;
                    }
                    size_t index = 0;  // old version only one value
                    if (opt_floats->size() > 1) {
                        index = ctx->physical_nozzle_index();
                    }
                    if (!opt_floats->get_at(index).percent) {
                        ...
                        if (opt_value.percent) {
                            ...
                            while (true) {
                                ...
                                if (opt_parent->type() == coFloats) {
                                    v *= static_cast<const ConfigOptionFloats *>(opt_parent)->get_at(ctx->physical_nozzle_index());
                                    break;
                                }
                    ...
                    if (boost::ends_with(opt_key, "line_width")) {
                        ...
                        for (;;) {
                            ...
                            if (boost::ends_with(opt_def->ratio_over, "line_width")) {
                                // Extrusion width supports defaults and a complex graph of dependencies.
                                assert(opt_parent->type() == coFloatsOrPercents);
                                v *= Flow::extrusion_width(opt_def->ratio_over, static_cast<const ConfigOptionFloatsOrPercentsNullable*>(opt_parent),
                                    *ctx, static_cast<unsigned int>(ctx->physical_nozzle_index()));
                                break;
                            }
                            if (opt_parent->type() == coFloats) {
                                v *= static_cast<const ConfigOptionFloats *>(opt_parent)->get_at(ctx->physical_nozzle_index());
