    ...
    for (LayerRegionPtrs::iterator layerm = m_regions.begin(); layerm != m_regions.end(); ++ layerm) 
    	if ((*layerm)->slices.empty()) {
	        ...
	        if (layerms.size() == 1) {  // optimization
	            ...
	            (*layerm)->make_perimeters((*layerm)->slices, {*layerm}, &(*layerm)->fill_surfaces, &(*layerm)->fill_no_overlap_expolygons, this->loop_nodes);
                ...
                layerm_config->make_perimeters(new_slices, layerms, &fill_surfaces, &fill_no_overlap, this->loop_nodes);
