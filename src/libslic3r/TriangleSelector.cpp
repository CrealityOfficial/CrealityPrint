    ...
    const FacetSubdivisionCancelCallback &cancel)
{
    ...
    while (!candidates.empty() && result.error_area > target_error) {
        ...
        for (int child_idx = 0; child_idx < 4; ++child_idx) {
            ...
            const Vec3i32 neighbors = this->child_neighbors(parent, candidate.neighbors, child_idx);
            FacetSubdivisionMeasurement measurement =
...
void TriangleSelector::split_triangle(int facet_idx, const Vec3i32 &neighbors, const Transform3f *trafo)
{
    if (m_triangles[facet_idx].is_split()) {
