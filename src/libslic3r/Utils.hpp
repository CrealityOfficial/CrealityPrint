...
*   that means: The response is Step 0 of the "entry" function of the "wifi" module, and empty extra info
*/
extern std::function<void(const std::string& modul, const std::string& function, int code, const std::string& message)> ADD_TEST_RESPONE;
...
inline bool is_creality_open(const std::string& url) { return boost::starts_with(url, "crealityprintlink://open"); }
...
inline bool is_supported_open_protocol(const std::string& url) { return is_orca_open(url) || is_prusaslicer_open(url) || is_bambustudio_open(url) || is_cura_open(url) || is_creality_open(url); }
...
std::string header_slic3r_generated_cloud();
