...
std::vector<AABBMesh::hit_result> AABBMesh::query_ray_hits(const Vec3d& s, const Vec3d& dir, std::vector<int> idxOverhangs) const
{
    std::vector<AABBMesh::hit_result> outs;
    std::vector<igl::Hit>             hits;
    m_aabb->intersect_ray(*m_tm, s, dir, hits);
    ...
    hits.erase(std::remove_if(hits.begin(), hits.end(),
        [&idxOverhangs](const igl::Hit& hit) {
            return std::find(idxOverhangs.begin(), idxOverhangs.end(), hit.id) != idxOverhangs.end();
        }),
        hits.end());
