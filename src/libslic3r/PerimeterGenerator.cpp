...
public:
    ...
    PerimeterGeneratorLoop(const Polygon &polygon, unsigned short depth, bool is_contour, bool fuzzify, bool is_small_width_perimeter = false) :
        polygon(polygon), is_contour(is_contour), is_smaller_width_perimeter(is_small_width_perimeter), depth(depth), fuzzify(false) {}
...
static void detect_bridge_wall(const PerimeterGenerator& perimeter_generator,
                               ExtrusionPaths&           paths,
                               const Polylines&          remain_polines,
                               ExtrusionRole             role,
                               double                    mm3_per_mm,
                               float                     width,
                               float                     height)
{
    for (Polyline poly : remain_polines) {
        ...
        if (line.length() < poly.length()) {
            extrusion_paths_append(paths, std::move(poly), overhang_sampling_number - 1, int(0), role, mm3_per_mm, width, height);
            continue;
        }
        // bridge wall
        extrusion_paths_append(paths, std::move(poly), overhang_sampling_number, int(0), role, mm3_per_mm, width, height);
    }
}
...
// TODO: Do more experiments to analyze the value of overhang_degree
static bool has_contain_bridge(const ExtrusionEntityCollection entities)
{
    for (auto ee : entities) {
        ...
        const ExtrusionPaths& paths = loop->paths;
        for (auto path : paths) {
            if (!is_bridge(path.role()) && path.get_overhang_degree() < 4)
                continue;
            return true;
        }
    }
...
static ExtrusionEntityCollection traverse_loops_classic(const PerimeterGenerator &perimeter_generator, const PerimeterGeneratorLoops &loops, ThickPolylines &thin_walls,
    bool &steep_overhang_contour, bool &steep_overhang_hole)
{
    ...
    const int object_layer_index = perimeter_generator.layer_id - perimeter_generator.object_config->raft_layers;
    bool overhangs_reverse = perimeter_generator.config->overhang_reverse &&
                             object_layer_index >= 0 && object_layer_index % 2 == 0; // Reverse on odd visible layers (1-based), excluding raft offset.
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            if (!detect_overhang_speed) {
                ...
                if (!middle_overhang_polyines.empty()) {
                    detect_overhang_degree(lower_polygons_series->front(), role, extrusion_mm3_per_mm, extrusion_width,
                                           (float) perimeter_generator.layer_height, middle_overhang_polyines,
                                           overhang_dist_boundary->first, overhang_dist_boundary->second, paths, perimeter_index);
                }
    ...
    if (! thin_walls.empty()) {
        // CP: update zero_point, this code comes from bambu, can save a few seconds 
        BoundingBox bbox;
        for (auto& entity : coll.entities) {
            bbox.merge(entity->as_polyline().bounding_box());
        }
        ...
        // Prusa VFA, The external perimeter has value 0, the first internal perimeter has 1, and so on. 
        variable_width(thin_walls, erExternalPerimeter, perimeter_generator.ext_perimeter_flow, 0, coll.entities); 
        thin_walls.clear();
    }
    ...
    for (const std::pair<size_t, bool> &idx : chain) {
        ...
        if (idx.first >= loops.size()) {
            ...
            ExtrusionEntityCollection children = traverse_loops_classic(perimeter_generator, loop.children, thin_walls, steep_overhang_contour, steep_overhang_hole);
...
static void detect_brigde_wall_arachne(const PerimeterGenerator&  perimeter_generator,
                                       ExtrusionPaths&            paths,
                                       const ClipperLib_Z::Paths& path_overhang,
                                       const ExtrusionRole        role,
                                       const Flow&                flow,
                                       const uint32_t             perimeter_index)
{
    for (ClipperLib_Z::Path path : path_overhang) {
        ...
        if (line.length() < thick_polyline.length()) {
            extrusion_path_append(paths, std::move(thick_polyline), role, flow, perimeter_index, overhang_sampling_number - 1);
            continue;
        }

        extrusion_path_append(paths, std::move(thick_polyline), role, flow, perimeter_index, overhang_sampling_number);
    }
}
    ...
    for (PerimeterGeneratorArachneExtrusion& pg_extrusion : pg_extrusions) {
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            extrusion_paths_append(temp_paths, clip_extrusion(extrusion_path, lower_slices_paths, ClipperLib_Z::ctIntersection), role,
                                   is_external ? perimeter_generator.ext_perimeter_flow : perimeter_generator.perimeter_flow, extrusion->inset_idx);
            ...
            bool zero_z_support = (perimeter_generator.object_config->enable_support ||
                                   perimeter_generator.object_config->enforce_support_layers > 0) &&
                                  perimeter_generator.object_config->support_top_z_distance.value == 0;

            if (zero_z_support)
                detect_brigde_wall_arachne(perimeter_generator, paths, path_overhang, role,
                                           is_external ? perimeter_generator.ext_perimeter_flow : perimeter_generator.perimeter_flow,
                                           extrusion->inset_idx);
            else
                detect_brigde_wall_arachne(perimeter_generator, paths, path_overhang, erOverhangPerimeter,
                                           perimeter_generator.overhang_flow, extrusion->inset_idx);
            ...
            extrusion_paths_append(paths, *extrusion, role, is_external ? perimeter_generator.ext_perimeter_flow : perimeter_generator.perimeter_flow, extrusion->inset_idx);
        }
...
void PerimeterGenerator::process_classic()
{
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        const int config_wall_loops = this->config->wall_loops.value;
        int       base_wall_count   = config_wall_loops + surface.extra_perimeters;
        if (base_wall_count < 0)
            base_wall_count = 0;

        int wall_count            = base_wall_count;
        int sparse_infill_density = this->config->sparse_infill_density.value;

        // When user sets wall_loops to 0 (so base_wall_count == 0), treat it as "no walls" and do not
        // let auxiliary features (alternate extra wall, only-one-wall options) force any walls.
        if (base_wall_count > 0 && this->config->alternate_extra_wall && this->layer_id % 2 == 1 && !m_spiral_vase && sparse_infill_density > 0) // add alternating extra wall
        {
            int  other_wall_loops = -1;
            bool is_wall_loops_eq = false;
            if (this->compatible_regions != nullptr && this->compatible_regions->size() > 1) {
                for (const LayerRegion* other_region : *this->compatible_regions) {
                    other_wall_loops                      = 0;
                    const PrintRegionConfig& other_config = other_region->region().config();
                    other_wall_loops                      = other_config.wall_loops.value;
                    is_wall_loops_eq                      = (this->config->wall_loops.value == other_config.wall_loops.value);
                }
            }
            if (this->config->embedding_wall_into_infill && other_wall_loops == 0 && is_wall_loops_eq)
                wall_count += 2;
            else
                wall_count++;
        }
        ...
        if (wall_count > 0 && this->layer_id == 0 && this->config->only_one_wall_first_layer)
            wall_count = 1;

        // Set the topmost layer to be one wall
        if (wall_count > 0 && config->only_one_wall_top && this->upper_slices == nullptr)
            wall_count = 1;

        // Legacy code below uses 0-based loop index.
        int loop_number = wall_count - 1;  // -1 means "no walls"
...
//                     //}
            ...
            ExtrusionEntityCollection entities = traverse_loops_classic(*this, contours.front(), thin_walls, steep_overhang_contour, steep_overhang_hole);
            ...
            // CP: Print the inner walls first when encountering overhanging sections. (When using AdaptiveOuterInner)
            bool adaptive_order      = this->config->wall_sequence == WallSequence::AdaptiveOuterInner;
            bool has_overlaps        = has_contain_bridge(entities);
            bool is_outer_wall_first = this->config->wall_sequence == WallSequence::OuterInner ||
                                       this->config->wall_sequence == WallSequence::AdaptiveOuterInner;
            if (is_outer_wall_first ||
                //BBS: always print outer wall first when there indeed has brim.
                (this->layer_id == 0 &&
                    this->object_config->brim_type == BrimType::btOuterOnly &&
                    this->object_config->brim_width.value > 0)) {
                if (!adaptive_order || !has_overlaps)
                    entities.reverse();
            }
            // Orca: sandwich mode.
            else if (this->config->wall_sequence == WallSequence::InnerOuterInner && this->layer_id > 0) {
        ...
        if (! gaps.empty()) {
            ...
            if (! polylines.empty()) {
                ...
                variable_width(polylines, erGapFill, this->solid_infill_flow, std::nullopt, gap_fill.entities);
 ...
 * @param reference_indices A set of indices representing the reference points.
 ...
 */
static std::vector<int> find_all_touching_perimeters(const std::vector<PerimeterGeneratorArachneExtrusion> &entities,
                                                     const std::unordered_set<int>                         &reference_indices,
                                                     size_t                                                  threshold_external,
                                                     size_t                                                  threshold_internal,
                                                     size_t                                                  considered_inset_idx)
{
    std::unordered_set<int> touching_indices;

    for (const int ref_idx : reference_indices) {
        const auto &reference_entity = entities[ref_idx];
        Points      reference_points = Arachne::to_points(*reference_entity.extrusion);
        for (size_t i = 0; i < entities.size(); ++i) {
            // Skip already considered references and the reference entity.
            if (reference_indices.count(int(i)) > 0)
                continue;
            const auto &entity = entities[i];
            // Ignore inset index 0 (external) perimeters from the re-ordering even if they are touching.
            if (entity.extrusion->inset_idx == 0)
                continue;
            // Find inset index perimeters that match the requested inset index.
            if (entity.extrusion->inset_idx != considered_inset_idx)
                continue;

            Points points   = Arachne::to_points(*entity.extrusion);
            double distance = MultiPoint::minimumDistanceBetweenLinesDefinedByPoints(reference_points, points);
            size_t threshold = (reference_entity.extrusion->inset_idx == 0) ? threshold_external : threshold_internal;
            if (distance <= threshold)
                touching_indices.insert(int(i));
        }
    }
    return std::vector<int>(touching_indices.begin(), touching_indices.end());
}
 ...
 */
static std::vector<PerimeterGeneratorArachneExtrusion> reorder_perimeters_by_proximity(
    std::vector<PerimeterGeneratorArachneExtrusion> entities,
    size_t                                          threshold_external,
    size_t                                          threshold_internal)
{
    std::vector<PerimeterGeneratorArachneExtrusion> reordered;
    std::unordered_set<int>                         included_indices;

    // Reorder perimeters starting from a given reference index.
    auto reorder_from_reference = [&](int reference_index) {
        std::unordered_set<int> first_level_indices;
        first_level_indices.insert(reference_index);

        std::vector<int> first_level_touching_indices = find_all_touching_perimeters(
            entities, first_level_indices, threshold_external, threshold_internal, 1);
        // The longest first neighbour is most likely the dominant proximate perimeter.
        if (!first_level_touching_indices.empty()) {
            auto max_it = std::max_element(first_level_touching_indices.begin(), first_level_touching_indices.end(),
                [&entities](int a, int b) {
                    return entities[size_t(a)].extrusion->getLength() < entities[size_t(b)].extrusion->getLength();
                });
            std::iter_swap(max_it, first_level_touching_indices.end() - 1);
        }

        reordered.push_back(entities[size_t(reference_index)]);
        included_indices.insert(reference_index);

        for (int idx : first_level_touching_indices) {
            if (included_indices.count(idx) == 0) {
                reordered.push_back(entities[size_t(idx)]);
                included_indices.insert(idx);
            }
        }

        size_t current_inset_idx = 2;
        while (true) {
            std::unordered_set<int> current_level_indices(first_level_touching_indices.begin(), first_level_touching_indices.end());
            std::vector<int> current_level_touching_indices = find_all_touching_perimeters(
                entities, current_level_indices, threshold_external, threshold_internal, current_inset_idx);

            if (current_level_touching_indices.empty())
                break;

            current_level_touching_indices.erase(
                std::remove_if(current_level_touching_indices.begin(),
                               current_level_touching_indices.end(),
                               [&](int idx) { return included_indices.count(idx) > 0; }),
                current_level_touching_indices.end());

            if (!current_level_touching_indices.empty()) {
                auto max_it = std::max_element(current_level_touching_indices.begin(), current_level_touching_indices.end(),
                    [&entities](int a, int b) {
                        return entities[size_t(a)].extrusion->getLength() < entities[size_t(b)].extrusion->getLength();
                    });
                std::iter_swap(max_it, current_level_touching_indices.begin());
            }

            for (int idx : current_level_touching_indices) {
                if (included_indices.count(idx) == 0) {
                    reordered.push_back(entities[size_t(idx)]);
                    included_indices.insert(idx);
                }
            ...
            first_level_touching_indices = current_level_touching_indices;
            ++current_inset_idx;
        }
    };

    for (size_t ref_idx = 0; ref_idx < entities.size(); ++ref_idx) {
        if (entities[ref_idx].extrusion->inset_idx == 0 && included_indices.count(int(ref_idx)) == 0)
            reorder_from_reference(int(ref_idx));
    }

    for (size_t i = 0; i < entities.size(); ++i) {
        if (included_indices.count(int(i)) == 0)
...
/**
 * @brief Reorders the vector to bring external perimeter contours (inset index 0) to the front.
 *
 * @param ordered_extrusions The vector of PerimeterGeneratorArachneExtrusion to reorder.
 */
static void bring_contours_to_front(std::vector<PerimeterGeneratorArachneExtrusion> &ordered_extrusions)
{
...
void PerimeterGenerator::process_arachne()
{
    ...
    for (const Surface& surface : all_surfaces) {
        ...
        // Base user-configured wall count.
        const int config_wall_loops = this->config->wall_loops.value;
        // detect how many perimeters must be generated for this island
        int loop_number = config_wall_loops + surface.extra_perimeters - 1; // 0-indexed loops
        int sparse_infill_density = this->config->sparse_infill_density.value;
        // When user sets wall_loops to 0, treat it as "no walls" and do not
        // let auxiliary features (alternate extra wall, only-one-wall options)
        // force any walls to be printed.
        if (config_wall_loops > 0 && this->config->alternate_extra_wall && this->layer_id % 2 == 1 && !m_spiral_vase && sparse_infill_density > 0) // add alternating extra wall
        {
            int  other_wall_loops = -1;
            bool is_wall_loops_eq = false;
            if (this->compatible_regions != nullptr && this->compatible_regions->size() > 1) {
        ...
        if (config_wall_loops > 0 && is_bottom_layer && this->config->only_one_wall_first_layer)
            loop_number = 0;
        ...
        if (config_wall_loops > 0 && is_topmost_layer && loop_number > 0 && config->only_one_wall_top)
            loop_number = 0;
        ...
        const int inner_loop_number = (config->only_one_wall_top && upper_slices != nullptr && config_wall_loops > 0) ? loop_number - 1 : -1;

        // Set one perimeter when TopSurfaces is selected.
        if (config->only_one_wall_top && config_wall_loops > 0)
            loop_number = 0;
        ...
        int end_perimeter = -1;
        int direction = -1;
        const bool has_outer_brim_on_first_layer =
            this->layer_id == 0 &&
            this->object_config->brim_type == BrimType::btOuterOnly &&
            this->object_config->brim_width.value > 0;
        ...
        bool is_outer_wall_first =
            ...
            this->config->wall_sequence == WallSequence::AdaptiveOuterInner ||
            (this->config->wall_sequence == WallSequence::InnerOuterInner && this->layer_id > 0) ||
            has_outer_brim_on_first_layer;
        if (is_outer_wall_first) {
            start_perimeter = 0;
       ...
       // printf("New Layer: Layer ID %d\n",layer_id); //debug - new layer
        if (this->config->wall_sequence == WallSequence::InnerOuterInner && this->layer_id > 0) {
            if (ordered_extrusions.size() > 2) { // 3 walls minimum needed to do inner outer inner ordering
                int position = 0; // index to run the re-ordering for multiple external perimeters in a single island.
                int arr_i, arr_j = 0;    // indexes to run through the walls in the for loops
                int outer, first_internal, second_internal, max_internal, current_perimeter; // allocate index values

                // To address scenarios where Arachne clustering puts non-contour extrusions first.
                bring_contours_to_front(ordered_extrusions);

                // Expand thresholds slightly to cover rounding issues.
                ...
                coord_t threshold_external = apply_precise_outer_wall
                    ...
                    : (this->ext_perimeter_flow.scaled_spacing() / 2.0 + this->perimeter_flow.scaled_spacing() / 2.0);
                threshold_external = coord_t(threshold_external * expand_factor);
                coord_t threshold_internal = coord_t(this->perimeter_flow.scaled_spacing() * expand_factor);

                ordered_extrusions = reorder_perimeters_by_proximity(ordered_extrusions, size_t(threshold_external), size_t(threshold_internal));
                std::vector<PerimeterGeneratorArachneExtrusion> reordered_extrusions = ordered_extrusions;
                ...
                while (position < reordered_extrusions.size()) {
                    ...
                    if (outer > -1 && first_internal > -1 && second_internal > -1) { // found all three perimeters to re-order? If not the perimeters will be processed outside in.
                        ...
                        inner_outer_extrusions.resize(max_internal - position + 1); // allocate number of perimeters before a new island.
        ...
        if (ExtrusionEntityCollection extrusion_coll = traverse_extrusions(*this, ordered_extrusions, steep_overhang_contour, steep_overhang_hole); !extrusion_coll.empty()) {
            // CP: Print the inner walls first when encountering overhanging sections. (When using AdaptiveOuterInner)
            bool adaptive_order = this->config->wall_sequence == WallSequence::AdaptiveOuterInner;
            bool has_overlaps   = has_contain_bridge(extrusion_coll);
            if (adaptive_order && has_overlaps) {
