...
int ConfigBase::load_from_json(const std::string &file, ConfigSubstitutionContext& substitution_context, bool load_inherits_to_config, std::map<std::string, std::string>& key_values, std::string& reason)
{
    ...
    try {
        ...
        std::list<std::string> different_machine_settings_to_factory;
        std::list<std::string> different_process_settings_to_factory;
        std::list<std::list<std::string>> different_filaments_settings_to_factory;
        std::string creality_flush_volumes_matrix;
        std::string creality_flush_multiplier;
        bool is_creality = false;
        auto get_diff_settings = [&](json& obj,std::list<std::string>& different_settings_to_factory) {
                        //BBS: parse the different_settings_to_factory
                       json machine = obj;
                        for (auto mit = machine.begin(); mit != machine.end(); mit++) {
                            if (mit.value().is_object()) {
                                if(mit.key() == "different_settings_to_factory") {
                                   json different_settings = mit.value();
                                   
                                      for (auto dit = different_settings.begin(); dit != different_settings.end(); dit++) {
                                        if (dit.value().is_string()) {
                                            if(dit.key() == "flush_volumes_matrix"){
                                                creality_flush_volumes_matrix = dit.value();
                                            }else if(dit.key() == "flush_multiplier") {
                                                creality_flush_multiplier = dit.value();
                                            }else{
                                                different_settings_to_factory.push_back(dit.key());
                                            }
        ...
        for (auto it = j.begin(); it != j.end(); it++) {
            if (boost::iequals(it.key(),BBL_JSON_KEY_VERSION)) {
                ...
                if (it.value().is_string()) {
                    std::string opt_value = it.value();
                    if(is_creality && opt_value.empty())
                    {
                        const ConfigOptionDef* optdef = config_def->get(opt_key);
                        if(optdef!=nullptr && !(optdef->type == coString || optdef->type == coStrings))
                        {
                            continue;
                        }
                    }
                    if (opt_key == "gcode_flavor_test") {

                        opt_key = "prime_tower_position_type";
                        if (opt_value == "left_upper")
                            opt_value = "Left Upper";
                        else if (opt_value == "middle_upper")
                            opt_value = "Middle Upper";
                        else if (opt_value == "right_upper")
                            opt_value = "Right Upper";
                        else if (opt_value == "left_center")
                            opt_value = "Left Center";
                        else if (opt_value == "middle_center")
                            opt_value = "Middle Center";
                        else if (opt_value == "right_center")
                            opt_value = "Right Center";
                        else if (opt_value == "left_below")
                            opt_value = "Left Below";
                        else if (opt_value == "middle_below")
                            opt_value = "Middle Below";
                    ...
                    if(opt_key == "extruder")
                    {
                        for (auto iter = it.value().begin(); iter != it.value().end(); iter++) {
                            if (iter.value().is_object()) {
                                    ...
                                    json diff_json = iter.value();
                                    for (auto mit = diff_json.begin(); mit != diff_json.end(); mit++) {
                                    {
                                        if(mit.key() == "material")
                                        {
                                            //for (auto iter2 = mit.value().begin(); iter2 != mit.value().end(); iter2++) {
                                             //   if(iter2.key()=="different_settings_to_factory")
                                           //     {
                                                    std::list<std::string> different_settings_to_factory;
                                                    get_diff_settings(mit.value(), different_settings_to_factory);
                                                    different_filaments_settings_to_factory.push_back(different_settings_to_factory);
                                           //     }
                                            //}
                                        }
                ...
                }else if (it.value().is_object()) 
                {
                    ...
                    if (opt_key == "machine") 
                    {
                        get_diff_settings(it.value(),different_machine_settings_to_factory);
                    }
                    if (opt_key == "process") 
                    {
                        get_diff_settings(it.value(),different_process_settings_to_factory);
                    }
                }
                else {
                    //should not happen
        ...
        if(!different_process_settings_to_factory.empty() || !different_filaments_settings_to_factory.empty() || !different_machine_settings_to_factory.empty()) {
            std::string value_str;
            bool first = true;
            //first process
            std::string process_keys = "";
            for (auto iter = different_process_settings_to_factory.begin(); iter != different_process_settings_to_factory.end(); iter++) {
                 process_keys += *iter + ";";
                 
            }
            value_str += "\"";
            value_str += escape_string_cstyle(process_keys);
            value_str += "\";";
            //then filaments
            for(auto filaments : different_filaments_settings_to_factory) {
                std::string filament_keys = "";
                for (auto iter = filaments.begin(); iter != filaments.end(); iter++) {
                    filament_keys += *iter + ";";
                }
                value_str += "\"";
                value_str += escape_string_cstyle(filament_keys);
                value_str += "\";";
            }
            
            //then machine
            std::string machine_keys = "";
            for (auto iter = different_machine_settings_to_factory.begin(); iter != different_machine_settings_to_factory.end(); iter++) {
                  machine_keys += *iter + ";";
            }
            value_str += "\"";
            value_str += escape_string_cstyle(machine_keys);
            value_str += "\"";
            this->set_deserialize("different_settings_to_system", value_str, substitution_context);
            if(!creality_flush_multiplier.empty())
            {
                this->set_deserialize("flush_multiplier", creality_flush_multiplier, substitution_context);
            }
            if(!creality_flush_volumes_matrix.empty())
            {
                this->set_deserialize("flush_volumes_matrix", creality_flush_volumes_matrix, substitution_context);
            }
        }
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or CrealityPrint header.
    ...
    {
        //BBS
        std::string bambuslicer_gcode_header = "; CrealityPrint";

        std::string crealityprint_gcode_header = std::string("; generated by ");
        crealityprint_gcode_header += SLIC3R_APP_NAME;

        // for version 5.1
        const std::string SLIC3R_APP_NAME_1            = "Creality_Print";
        std::string crealityprint_gcode_header_1 = std::string("; generated by ");
        crealityprint_gcode_header_1 += SLIC3R_APP_NAME_1;

        // for Creality Cloud Slicer
        const std::string CLOUD_SLIC3R_APP_NAME            = "Creality_Cloud_Slicer";
        std::string crealityprint_cloud_gcode_header = std::string("; generated by ");
        crealityprint_cloud_gcode_header += CLOUD_SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(crealityprint_gcode_header_1.c_str(), line_c, strlen(crealityprint_gcode_header_1.c_str())) == 0 ||
                strncmp(crealityprint_gcode_header.c_str(), line_c, strlen(crealityprint_gcode_header.c_str())) == 0 ||
                strncmp(crealityprint_cloud_gcode_header.c_str(), line_c, strlen(crealityprint_cloud_gcode_header.c_str())) == 0
                ) {
                header_found = true;
                break;
            }
...
//BBS: add json support
void ConfigBase::save_to_json(const std::string& file,
                              ...
                              const std::string  base_id) const
{
    ...
    if (!base_id.empty()) {
        j[BBL_JSON_KEY_BASE_ID] = base_id;
    }
