                           ...
                           &support_fill_octree = support_fill_octree](const tbb::blocked_range<size_t>& range) {
                              for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                                  ...
                                  m_layers[layer_idx]->make_fills(adaptive_fill_octree.get(), support_fill_octree.get(),
                                                                  this->center_offset(), this->m_lightning_generator.get());
                              }
                          });
        m_print->throw_if_canceled();
...
void PrintObject::rebuild_lockedzag_infill_after_skin_replan()
{
    const auto& adaptive_fill_octree = this->m_adaptive_fill_octrees.first;
    const auto& support_fill_octree  = this->m_adaptive_fill_octrees.second;
                                                     ...
                                                     const std::vector<t_config_option_key>& opt_keys)
{
    ...
    auto has_interlocking_beam = [](const ConfigOptionResolver& config) {
        const auto* interlocking_beam = config.option<ConfigOptionBool>("interlocking_beam");
        return interlocking_beam != nullptr && interlocking_beam->value;
    };
    const bool interlocking_beam_enabled = m_config.interlocking_beam.value ||
                                            has_interlocking_beam(old_config) || has_interlocking_beam(new_config);
        ...
        if (role_filament_changed && interlocking_beam_enabled) {
            ...
            if (opt_key == "brim_type") {
                ...
                if (old_brim_type->value == btOuterOnly || new_brim_type->value == btOuterOnly)
                   ...
                   opt_key == "top_bottom_infill_wall_overlap" || opt_key == "external_infill_margin" || opt_key == "seam_gap" ||
                   opt_key == "role_based_wipe_speed" || opt_key == "wipe_on_loops" || opt_key == "wipe_speed") {
            steps.emplace_back(posPerimeters);
        } else if (opt_key == "small_area_infill_flow_compensation" || opt_key == "small_area_infill_flow_compensation_model") {
            steps.emplace_back(posSlice);
            ...
            if (m_config.support_top_z_distance == 0.) {
                ...
                steps.emplace_back(posSlice);
                   ...
                   opt_key == "support_interface_pattern" || opt_key == "support_interface_min_area" ||
                   opt_key == "support_interface_loop_pattern" || opt_key == "support_interface_filament" ||
                   opt_key == "support_interface_not_for_body" || opt_key == "support_interface_spacing" ||
                   opt_key == "support_bottom_interface_spacing" // BBS
                   || opt_key == "support_base_pattern" || opt_key == "support_base_pattern_tree" || opt_key == "support_style" ||
                   opt_key == "support_object_xy_distance" || opt_key == "support_object_first_layer_gap" ||
                   opt_key == "support_base_pattern_spacing" || opt_key == "support_expansion" || opt_key == "ironing_support_layer" ||
                   opt_key == "tree_hybrid_cross_height"
                   //|| opt_key == "independent_support_layer_height" // BBS
                   || opt_key == "support_threshold_angle" || opt_key == "raft_expansion" || opt_key == "raft_first_layer_density" ||
                   opt_key == "raft_first_layer_expansion" || opt_key == "bridge_no_support" || opt_key == "max_bridge_length" ||
                   opt_key == "initial_layer_line_width" || opt_key == "tree_support_adaptive_layer_height" ||
                   ...
                   opt_key == "tree_support_branch_diameter_double_wall" || opt_key == "tree_support_branch_angle" ||
                   opt_key == "tree_support_branch_angle_organic" || opt_key == "tree_support_angle_slow" ||
                   opt_key == "tree_support_organic_validate_repair" ||
                   opt_key == "tree_support_wall_count" || opt_key == "tree_support_wall_count_tree") {
            steps.emplace_back(posSupportMaterial);
...
#endif
        } else if (opt_key == "interface_shells" || opt_key == "infill_combination" || opt_key == "bottom_shell_thickness" ||
                   opt_key == "top_shell_thickness" || opt_key == "minimum_sparse_infill_area" || opt_key == "sparse_infill_filament" ||
                   opt_key == "solid_infill_filament" || opt_key == "sparse_infill_line_width" || opt_key == "infill_direction" ||
                   opt_key == "solid_infill_direction" || opt_key == "rotate_solid_infill_direction" ||
...
bool PrintObject::invalidate_step(PrintObjectStep step)
{
    ...
    // It also decides about what the flush_into_infill / wipe_into_object / flush_into_support / flush_into_skeleton features will do,
                ...
                for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer) {
                    ...
                    if (detect_top) {
                        if (upper_layer) {
                            ExPolygons upper_slices = interface_shells ?
                                                          diff_ex(layerm_slices_surfaces,
                    ...
                    if (detect_bottom) {
                        if (lower_layer) {
                            ...
                            surfaces_append(bottom,
                                            opening_ex(diff_ex(layerm_slices_surfaces, lower_layer->lslices, ApplySafetyOffset::Yes),
                                                       offset),
                                            surface_type_bottom_other);
                    ...
                    if (!top.empty() && !bottom.empty()) {
                        ...
                        if (!cracks.empty()) {
                            ...
                            if (run_small_crack_refine) { // Only detect small cracks for non-first layer, because first layer should always
                                                          // be bottom
...
void PrintObject::bridge_over_infill()
{
    ...
    auto determine_bridging_angle = [](const Polygons& bridged_area, const Lines& anchors, InfillPattern dominant_pattern,
        ...
        case ip3DHoneycomb:
        case ipCrossHatch: return (infill_direction + 45.0) * 2.0 * M_PI / 360.;
        default: break;
        }
...
{
    bool sparse_infill { false };
    bool solid_infill { false };
    bool wall { false };
};

enum class ExtruderRoleApplyMode
{
    FillDefaultOnly,
    OverrideInherited,
};
...
{
    return {is_explicit_role_filament_value(config.sparse_infill_filament.value),
            is_explicit_role_filament_value(config.solid_infill_filament.value),
            is_explicit_role_filament_value(config.wall_filament.value)};
}
                                         ...
                                         ExtruderRoleApplyMode      extruder_mode = ExtruderRoleApplyMode::FillDefaultOnly)
{
    ...
    if (opt_extruder)
        if (int extruder = opt_extruder->value; extruder != 0) {
            const bool override_inherited = extruder_mode == ExtruderRoleApplyMode::OverrideInherited;
            if (!role_state.sparse_infill && (override_inherited || is_default_role_filament(out.sparse_infill_filament.value)))
                out.sparse_infill_filament.value = extruder;
            if (!role_state.solid_infill && (override_inherited || is_default_role_filament(out.solid_infill_filament.value)))
                out.solid_infill_filament.value = extruder;
            if (!role_state.wall && (override_inherited || is_default_role_filament(out.wall_filament.value)))
                ...
                if (one_of(it->first, keys_extruders)) {
                    ...
                    if (!is_default_role_filament(extruder)) {
                        ...
                        if (it->first == "sparse_infill_filament")
                            role_state.sparse_infill = true;
                        else if (it->first == "solid_infill_filament")
                            role_state.solid_infill = true;
...
{
    auto* opt_extruder = in.opt<ConfigOptionInt>(key_extruder);
    if (opt_extruder == nullptr)
        return;
    const int extruder = opt_extruder->value;
    if (extruder == 0)
        return;

    // Treat an explicit "extruder" key as a role-wide material selection.
    // Role-specific filament keys may still override it below.
    out.sparse_infill_filament.value = extruder;
    out.solid_infill_filament.value  = extruder;
    out.wall_filament.value          = extruder;
    role_state.sparse_infill         = true;
                                                  ...
                                                  bool                      single_extruder_multi_material)
{
    ...
    (void) single_extruder_multi_material;
    PrintRegionConfig         config     = default_or_parent_region_config;
    RoleFilamentOverrideState role_state = role_filament_override_state(config);

    // Modifier volume extruder is a local override of the parent region material.
    if (volume.is_modifier())
        apply_explicit_role_extruder(config, volume.config.get(), role_state);
                                              ...
                                              std::vector<coordf_t>&   layer_height_profile)
{
    ...
    if (!layer_height_profile.empty() &&
        ...
        ((layer_height_profile.size() & 1) != 0 ||
         // Last entry must be at the top of the object. The profile is stored in object-local Z,
         // so raft/contact offsets in object_print_z_min must not be included here.
         std::abs(layer_height_profile[layer_height_profile.size() - 2] - slicing_parameters.object_print_z_height()) > 1e-3))
        layer_height_profile.clear();

    if (layer_height_profile.empty() || (slicing_parameters.first_object_layer_height_fixed() &&
                                         !is_approx(layer_height_profile[1], slicing_parameters.first_object_layer_height))) {
...
// Anonymous namespace for mixed filament helper functions
namespace {

// Mixed state cadence for height-based dithering
...
// Collect Z-ranges where mixed filaments are painted
static std::vector<t_layer_height_range> collect_mixed_painted_z_ranges(const PrintObject& print_object, coordf_t object_height)
{
    ...
    // Simplified: return the full object height range for mixed filaments.
    // In production, painted facets should be analyzed to get precise ranges.
    mixed_ranges.emplace_back(0.0f, object_height);

    return mixed_ranges;
}

// Calculate height A and B for a mixed filament state
...
// Get the height for a mixed state at a specific Z position
