...
int getLineStart(int width, int height, const unsigned char* bits)
{
    int LineStart = -1;
...
int getLineEnd(int width, int height, const unsigned char* bits)
{
    int LineEnd = -1;
...
std::unique_ptr<CompressedImageBuffer> compress_thumbnail_crpng(const ThumbnailData &data)
{
    ...
    out->pos_s = getLineStart(data.width, data.height, data.pixels.data());
    out->pos_e = getLineEnd(data.width, data.height, data.pixels.data());
    out->data = tdefl_write_image_to_png_file_in_memory_ex((const void*)data.pixels.data(), data.width, data.height, 4, &out->size, MZ_DEFAULT_LEVEL, 1);
    return out;
}
