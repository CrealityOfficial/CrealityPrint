...
{                     
    ...
    auto& ps                   = m_result.print_statistics;
    ...
    int toolCount = m_result.tool_change_path.size();
    ...
    for (int i = 0; i < toolCount; ++i) {
        ...
        for (int newTool : m_result.tool_change_path[i]) {
            double volume = m_result.tool_change_volumes_map[i][newTool] * m_result.flush_multiplier;
            //flushLengths[newTool] += volume / lengthFactor;
            //flushWeights[newTool] += volume * weightFactor;
            m_result.print_statistics.flush_per_filament[newTool] += volume;
        }
    }
};
            ...
            if (a == Z) {
                ...
                if (curr.abs_axis_feedrate[a] != 0.0f) {
                    float axis_max_feedrate = get_axis_max_feedrate(static_cast<PrintEstimatedStatistics::ETimeMode>(i),
                                                                    static_cast<Axis>(a));
                    if (axis_max_feedrate != 0.0f)//������ٶ�
                        min_feedrate_factor = std::min<float>(min_feedrate_factor, axis_max_feedrate / curr.abs_axis_feedrate[a]);
                }
            }
        }
        //BBS: update curr.feedrate
        curr.feedrate *= min_feedrate_factor;//���¼����ٶȣ�Ѳ�����յ㣩---���Ѳ���ٶ�
        curr.max_cruise_v2 = sqr(curr.feedrate);//��ֵ���Ѳ���ٶ�
        block.feedrate_profile.cruise = curr.feedrate;
        // calculates block acceleration
...
{
    // ��ʱ�ü��ٶȵ����ֵ��Ϊ���ٶȵ����ֵ m_time_processor.machines[id].max_acceleration�����Ժ�͹̼�ȷ���ڻ�
    size_t id = static_cast<size_t>(mode);
    if (id < m_time_processor.machines.size()) {
