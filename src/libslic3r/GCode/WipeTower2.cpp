                                ...
                                unsigned int new_tool, float wipe_volume)
{
	...
	float length_to_extrude = volume_to_length(0.25f * std::accumulate(m_filpar[old_tool].ramming_speed.begin(), m_filpar[old_tool].ramming_speed.end(), 0.f),
										...
										layer_height_par);
    float ramming_depth     = (int(length_to_extrude / width) + 1) *(m_perimeter_width * m_filpar[old_tool].ramming_line_width_multiplicator *
                                m_filpar[old_tool].ramming_step_multiplicator) *m_extra_spacing_ramming;
        ...
        for (int i=0; i<int(m_layer_info->tool_changes.size()); ++i) {
            ...
            if (i == idx) {
                ...
                float volume_we_need_depth_for = std::max(0.f, volume_left_to_wipe - length_to_volume(toolchange.first_wipe_line,
                                                 m_perimeter_width * m_extra_flow,m_layer_info->height));
