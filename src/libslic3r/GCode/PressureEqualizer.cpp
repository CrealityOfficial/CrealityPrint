...
bool PressureEqualizer::process_line(const char *line, const char *line_end, GCodeLine &buf)
{
    ...
    case 'G': {
        ...
        case 1:
        {
            ...
            if (m_current_extrusion_role == ExtrusionRole::erExternalPerimeter) {
                m_current_perimeter_index = 0;
            } else if (m_current_extrusion_role == ExtrusionRole::erPerimeter) {
...
void PressureEqualizer::push_line_to_output(const size_t line_idx, float new_feedrate, const char *comment)
{
    ...
    else if (line.extrusion_role == ExtrusionRole::erPerimeter && line.perimeter_index.has_value()) {
