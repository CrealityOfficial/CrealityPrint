 ...
 */
        ...
        , max_depth(max_depth)
    {
        ...
        for (SierpinskiTriangle* node : sequence)
        {
            if (node->getValueError() < -allowed_length_error)
            {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("Node is subdivided without the appropriate value! value_error: {} from base {} el: {} er: {}, while the realized_length = {}");
                BOOST_LOG_TRIVIAL(info) << node->getValueError() << node->requested_length << node->error_left << node->error_right << node->realized_length;
                assert(false);
            }
        }
        debugCheck(true);
    ...
    void SierpinskiFill::createLowerBoundSequence()
    {
        ...
        for (int iteration = 0; iteration < 999; iteration++)
        {
            ...
            if (!change)
            {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("Finished after {} iterations, with a max depth of {}.") << iteration + 1<< max_depth;
                break;
            }
        }
    }
    ...
    void SierpinskiFill::redistributeLeftoverErrors(std::list<SierpinskiTriangle*>::iterator begin, std::list<SierpinskiTriangle*>::iterator end, bool distribute_subdivision_errors)
    {
        ...
        for (auto it = begin; it != end && std::next(it) != end; ++it)
        {
            ...
            if (std::abs(node->error_right + next->error_left) > allowed_length_error)
            {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("Nodes aren't balanced! er: {} next el: {}") << node->error_right<< next->error_left;
                assert(false);
            }
        ...
        { // there is no significant left-over error
            if (distribute_subdivision_errors && total_superfluous_error < -allowed_length_error)
            {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("RedistributeLeftoverErrors shouldn't be called if the node isn't to be subdivided. Total error: {}") << total_superfluous_error;
                assert(false);
            }
            return;
        }
    ...
    void SierpinskiFill::balanceErrors(std::list<SierpinskiFill::SierpinskiTriangle*>::iterator begin, std::list<SierpinskiFill::SierpinskiTriangle*>::iterator end)
    {
        ...
        if (total_remaining_value_error < added - allowed_length_error)
        {
            ...
            BOOST_LOG_TRIVIAL(error) << boost::format("total_remaining: {} should be > {}") << total_remaining_value_error << added;
            assert(false);
        }
        ...
        if (std::abs(subtracted - added) > allowed_length_error)
        {
            ...
            BOOST_LOG_TRIVIAL(error) << boost::format("Redistribution didn't distribute well!! added {} subtracted {}") << added<< subtracted;
            assert(false);
        }
    ...
    void SierpinskiFill::diffuseError()
    {
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("pair_constrained_nodes: {}, constrained_nodes: {}, unconstrained_nodes: {}, subdivided_nodes: {}")
    ...
    Polygon SierpinskiFill::generateCross() const
    {
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format("realized_length: {}, requested_length: {}  :: {}% error");
        BOOST_LOG_TRIVIAL(info) << realized_length << requested_length << .01 * static_cast<int>(10000 * error);
        return ret;
    }
    ...
    Polygon SierpinskiFill::generateCross(coord_t z, coord_t min_dist_to_side, coord_t pocket_size) const
    {
        ...
        std::function<Point(int, Edge)> get_edge_crossing_location = [z, min_dist_to_side](const coord_t period, const Edge e)->Point
        {
    ...
    void SierpinskiFill::debugCheck(bool check_subdivision)
    {
        if (std::abs(sequence.front()->error_left) > allowed_length_error)
        {
            ...
            BOOST_LOG_TRIVIAL(error) << boost::format("First node has error left!");
            assert(false);
        }
        if (std::abs(sequence.back()->error_right) > allowed_length_error)
        {
            //LOGW("Last node has error right!");
            BOOST_LOG_TRIVIAL(error) << boost::format("Last node has error right!");
            assert(false);
        }
        ...
        for (auto it = sequence.begin(); it != sequence.end(); ++it)
        {
            ...
            if (std::abs(node->error_right + next->error_left) > allowed_length_error)
            {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("Consecutive nodes in fractal don't have the same error! er: {} , nel: {}")
            ...
            if (check_subdivision && node->getValueError() < -allowed_length_error)
            {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("Fractal node shouldn't have been subdivided!");
                assert(false);
            }
        }
    }
