...
void MixedFilamentDialog::build_ui()
{
    ...
    // Dialog background: match system title bar color
    const wxColour dlg_bg = wxGetApp().dark_mode() ? wxColour(42, 42, 46) : wxColour(230, 230, 230);
    // Panel background: lighter gray
    const wxColour panel_bg = wxGetApp().dark_mode() ? wxColour(58, 58, 62) : wxColour(255, 255, 255);

    SetBackgroundColour(dlg_bg);
    SetSize(FromDIP(480), FromDIP(620));

    auto* main_sizer = new wxBoxSizer(wxVERTICAL);

    // ---- Upper panel (lighter background, rounded corners via paint) ----
    auto* upper_panel = new wxPanel(this, wxID_ANY);
    upper_panel->SetBackgroundStyle(wxBG_STYLE_PAINT);
    upper_panel->Bind(wxEVT_PAINT, [upper_panel, panel_bg, dlg_bg](wxPaintEvent&) {
        wxPaintDC dc(upper_panel);
        wxSize sz = upper_panel->GetClientSize();
    ...
    // Left side: material selection + gradient
    m_right_sizer = new wxBoxSizer(wxVERTICAL);
    m_right_sizer->Add(create_material_selection(upper_panel), 0, wxEXPAND);
    m_right_sizer->Add(create_gradient_section(upper_panel), 0, wxEXPAND | wxTOP, FromDIP(7));

    m_triangle_sizer = create_triangle_picker(upper_panel);
    m_right_sizer->Add(m_triangle_sizer, 0, wxEXPAND | wxTOP, FromDIP(4));

    top_content_sizer->Add(m_right_sizer, 1, wxTOP | wxLEFT | wxRIGHT, FromDIP(10));

    // Right side: preview panel (top-right, fixed width)
    top_content_sizer->Add(create_preview_panel(upper_panel), 0, wxTOP | wxRIGHT, FromDIP(15));

    upper_sizer->Add(top_content_sizer, 0, wxEXPAND);

    // Ratio slider: 90% of panel width, left-aligned
    m_ratio_sizer = create_ratio_slider(upper_panel);
    auto* ratio_wrapper = new wxBoxSizer(wxHORIZONTAL);
    ratio_wrapper->Add(m_ratio_sizer, 9, wxEXPAND);
    ...
    lower_panel->SetBackgroundStyle(wxBG_STYLE_PAINT);
    lower_panel->Bind(wxEVT_PAINT, [lower_panel, panel_bg, dlg_bg](wxPaintEvent&) {
        wxPaintDC dc(lower_panel);
        wxSize sz = lower_panel->GetClientSize();
        dc.SetBackground(wxBrush(dlg_bg));
        dc.Clear();
        dc.SetBrush(wxBrush(panel_bg));
        dc.SetPen(*wxTRANSPARENT_PEN);
    ...
    lower_panel->SetBackgroundColour(panel_bg);
    auto* lower_sizer = new wxBoxSizer(wxVERTICAL);

    lower_sizer->Add(create_recommendation_grid(lower_panel), 1, wxEXPAND | wxALL, FromDIP(15));
    lower_panel->SetSizer(lower_sizer);

    main_sizer->Add(lower_panel, 1, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, FromDIP(12));
    ...
    main_sizer->Add(m_warning_sizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, FromDIP(12));
    m_warning_panel->Hide();

    // ---- Bottom panel (same bg as panels, full width, no bottom margin) ----
    auto* bottom_panel = new wxPanel(this, wxID_ANY);
    bottom_panel->SetBackgroundColour(panel_bg);
    bottom_panel->SetMinSize(wxSize(-1, FromDIP(52)));
    auto* bottom_sizer = new wxBoxSizer(wxHORIZONTAL);
    bottom_sizer->AddStretchSpacer();
    bottom_sizer->Add(create_button_panel(bottom_panel), 0, wxALIGN_CENTER_VERTICAL);
    bottom_sizer->AddStretchSpacer();
    bottom_panel->SetSizer(bottom_sizer);

    main_sizer->Add(bottom_panel, 0, wxEXPAND | wxTOP, FromDIP(5));
...
wxBoxSizer* MixedFilamentDialog::create_preview_panel(wxWindow* parent)
{
    auto* sizer = new wxBoxSizer(wxVERTICAL);
    sizer->SetMinSize(wxSize(FromDIP(100), -1));

    m_preview_canvas = new wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(FromDIP(100), FromDIP(100)));
    m_preview_canvas->SetMinSize(wxSize(FromDIP(100), FromDIP(100)));
    m_preview_canvas->SetMaxSize(wxSize(FromDIP(100), FromDIP(100)));
    ...
    m_preview_canvas->Bind(wxEVT_PAINT, [this](wxPaintEvent&) {
        ...
        dc.SetBrush(wxBrush(m_preview_canvas->GetParent()->GetBackgroundColour()));
        dc.SetPen(*wxTRANSPARENT_PEN);
    ...
    auto* label = new wxStaticText(parent, wxID_ANY, _L("Effect Preview"), wxDefaultPosition, wxSize(FromDIP(100), -1), wxALIGN_CENTER_HORIZONTAL);
    label->SetForegroundColour(wxColour("#909090"));
    label->SetFont(::Label::Body_13);
    sizer->Add(label, 0, wxEXPAND | wxTOP, FromDIP(5));

    return sizer;
}

wxBoxSizer* MixedFilamentDialog::create_material_selection(wxWindow* parent)
{
    auto* sizer = new wxBoxSizer(wxVERTICAL);

    // Summary panel draws N components dynamically
    m_summary_panel = new wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(FromDIP(234), FromDIP(40)));
    ...
    m_summary_panel->Bind(wxEVT_PAINT, [this](wxPaintEvent&) {
        ...
        wxColour sum_bg   = wxGetApp().dark_mode() ? wxColour(42, 42, 46) : wxColour(230, 230, 230);
    ...
    auto* sel_label = new wxStaticText(parent, wxID_ANY, _L("Select Mixed Materials"));
    ...
    for (size_t i = 0; i < m_result.components.size(); ++i) {
        ...
        auto* lbl = new wxStaticText(parent, wxID_ANY, lbl_text);
    ...
    m_btn_add_material = new Button(parent, _L("+ Add Material"));
    m_btn_add_material->SetBackgroundColor(parent->GetBackgroundColour());
    m_btn_add_material->SetBorderColor(StateColor::darkModeColorFor(wxColour("#CECECE")));
    m_btn_add_material->SetTextColor(StateColor::darkModeColorFor(wxColour("#262E30")));
    m_btn_add_material->SetCornerRadius(FromDIP(2));
    ...
    btn_sizer->Add(m_btn_add_material, 1, wxEXPAND | wxRIGHT, FromDIP(5));

    m_btn_remove_material = new Button(parent, _L("- Delete Material"));
    m_btn_remove_material->SetBackgroundColor(parent->GetBackgroundColour());
    m_btn_remove_material->SetBorderColor(StateColor::darkModeColorFor(wxColour("#CECECE")));
    m_btn_remove_material->SetTextColor(StateColor::darkModeColorFor(wxColour("#262E30")));
    m_btn_remove_material->SetCornerRadius(FromDIP(2));
    ...
    btn_sizer->Add(m_btn_remove_material, 1, wxEXPAND | wxLEFT, FromDIP(5));
...
wxBoxSizer* MixedFilamentDialog::create_ratio_slider(wxWindow* parent)
{
    auto* sizer = new wxBoxSizer(wxVERTICAL);

    auto* ratio_label = new wxStaticText(parent, wxID_ANY, _L("Ratio"));
    ...
    m_ratio_bar = new wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(-1, FromDIP(27)));
...
wxBoxSizer* MixedFilamentDialog::create_triangle_picker(wxWindow* parent)
{
    auto* sizer = new wxBoxSizer(wxVERTICAL);

    // Ratio label (left-aligned, separate from triangle)
    auto* ratio_label_tri = new wxStaticText(parent, wxID_ANY, _L("Ratio"));
    ratio_label_tri->SetForegroundColour(wxColour("#909090"));
    ratio_label_tri->SetFont(::Label::Body_12);
    sizer->Add(ratio_label_tri, 0, wxBOTTOM, FromDIP(4));
    ...
    m_triangle_panel->Bind(wxEVT_PAINT, [this, get_vertices](wxPaintEvent&) {
        wxBufferedPaintDC pdc(m_triangle_panel);
        wxGCDC dc(pdc);
        wxSize sz = m_triangle_panel->GetClientSize();
        auto [v0, v1, v2] = get_vertices();

        wxColour tri_bg = m_triangle_panel->GetParent()->GetBackgroundColour();
    ...
    sizer->Add(m_triangle_panel, 0, wxALIGN_CENTER_HORIZONTAL);

    return sizer;
}

wxBoxSizer* MixedFilamentDialog::create_gradient_section(wxWindow* parent)
{
...
wxBoxSizer* MixedFilamentDialog::create_recommendation_grid(wxWindow* parent)
{
    ...
    auto* rec_label = new wxStaticText(parent, wxID_ANY, _L("Mixing Recommendations"));
    rec_label->SetForegroundColour(StateColor::darkModeColorFor(wxColour("#262E30")));
    rec_label->SetFont(::Label::Head_14);
    outer->Add(rec_label, 0, wxBOTTOM, FromDIP(8));

    m_recommendation_scroll = new wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxSize(-1, FromDIP(116)));
    m_recommendation_scroll->SetScrollRate(0, 5);
    m_recommendation_scroll->SetBackgroundColour(parent->GetBackgroundColour());

    m_recommendation_grid = new wxWrapSizer(wxHORIZONTAL, 0);
    auto* scroll_inner_sizer = new wxBoxSizer(wxVERTICAL);
    scroll_inner_sizer->Add(m_recommendation_grid, 1, wxEXPAND);
...
wxBoxSizer* MixedFilamentDialog::create_button_panel(wxWindow* parent)
{
    auto* sizer = new wxBoxSizer(wxHORIZONTAL);

    m_btn_cancel = new Button(parent, _L("Cancel"));
...
void MixedFilamentDialog::on_add_material()
{
    ...
    auto* lbl = new wxStaticText(m_upper_panel, wxID_ANY, lbl_text);
    lbl->SetFont(::Label::Body_12);
    row->Add(lbl, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
