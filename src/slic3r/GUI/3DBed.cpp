...
bool Bed3D::set_gcode_shape(const Pointfs& printable_area,  const double printable_height, const Preset* preset, const Vec2d position, bool with_reset)
{
    if (!preset)
        return false;
    ...
    m_vendor    = Vendor::Default;
    PresetBundle* bundle = wxGetApp().preset_bundle;
    if (bundle != nullptr) {
        std::string vendor_name = bundle->printers.getVendor(*preset);
        m_vendor = vendor_name == "Creality" ? Vendor::Creality : Vendor::Default;
    }
...
void Bed3D::render_model(const Transform3d& view_matrix, const Transform3d& projection_matrix)
{
    ...
    if (m_model.get_filename() != m_model_filename) {
        if (m_model.init_from_file(m_model_filename)) {
            update_model_offset();
        }
    }

    auto color = DispConfig().getColor(DispConfig::e_ct_interSel);
    ColorRGBA rgba; rgba.r(color.x); rgba.g(color.y); rgba.b(color.z); rgba.a(color.w);
    m_model.set_color(rgba);
    GLShaderProgram* shader = wxGetApp().get_shader("gouraud_light");
