...
public:
    ...
    struct ObjList_Texture
    {
        enum IM_TEXTURE_NAME {
            texFold = 0,
            texUnfold,
            texPrintable,
            texUnprintable,
        ...
        {
            float pcs = 1.0 / texCount;
            ImVec2 uv;
            if (is_press) {
                uv = ImVec2(name * pcs, 0.0f);
            } else {
                uv = ImVec2(name * pcs, 0.5f);
            }
            return uv;
        }

        ImVec2 get_texture_uv1(IM_TEXTURE_NAME name, bool is_press) 
        { 
            float pcs = 1.0 / texCount;
            ImVec2 uv  = get_texture_uv0(name, is_press);
            ImVec2 uv1 = ImVec2(uv.x + pcs, uv.y + 0.5f);
            return uv1;
        }
...
public:
    ...
    bool can_change_part_type();
    void do_change_part_type(ModelVolumeType type);
...
//    void OnChar(wxKeyEvent& event);
    ...
    void render_volume(ObjectDataViewModelNode* volume);
