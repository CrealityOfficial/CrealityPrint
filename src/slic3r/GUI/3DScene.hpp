...
public:
    ...
    int load_real_wipe_tower_preview(int obj_idx,float pos_x,float pos_y,const TriangleMesh& wt_mesh, const TriangleMesh& brim_mesh,bool render_brim,
    ...
    void render(GUI::ERenderPipelineStage render_pipeline_stage, ERenderType type, bool disable_cullface, const Transform3d& view_matrix, const Transform3d& projection_matrix,
    ...
    void update_colors_by_config(const DynamicPrintConfig *config);

    // Returns a vector containing the sorted list of all the print_zs of the volumes contained in this collection
    std::vector<double> get_current_print_zs(bool active_only) const;
    void getVolumeZrange(double&minz,double&maxz)const;
