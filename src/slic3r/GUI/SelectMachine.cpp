...
static wxString MACHINE_BED_TYPE_STRING[BED_TYPE_COUNT] = {
    //_L("Auto"),
    _L("Cool Plate") + " / " + _L("PLA Plate"),
    _L("Engineering Plate"),
    _L("Smooth PEI Plate") + "/" + _L("High temperature Plate"),
    _L("Textured PEI Plate")};
    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    select_use_ams = create_ams_checkbox(_L("Enable CFS"), this, _L("Enable CFS"));
...
void SelectMachineDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text;
        if (params.size() > 0)
            msg_text = wxString::Format(_L("Filament %s exceeds the number of CFS slots. Please update the printer firmware to support AMS slot assignment."), params[0]);
        else
            msg_text = _L("Filament exceeds the number of CFS slots. Please update the printer firmware to support CFS slot assignment.");
        update_print_status_msg(msg_text, true, false);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
    } else if (status == PrintDialogStatus::PrintStatusAmsMappingSuccess){
        wxString msg_text = _L("Filaments to CFS slots mappings have been established. You can click a filament above to change its mapping CFS slot");
        update_print_status_msg(msg_text, false, false);
        Enable_Send_Button(true);
        Enable_Refresh_Button(true);
    } else if (status == PrintDialogStatus::PrintStatusAmsMappingInvalid) {
        wxString msg_text = _L("Please click each filament above to specify its mapping CFS slot before sending the print job");
        update_print_status_msg(msg_text, true, false);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
        ...
        wxString msg_text;
        if (params.size() > 1)
            msg_text = wxString::Format(_L("Filament %s does not match the filament in CFSslot %s. Please update the printer firmware to support CFS slot assignment."), params[0], params[1]);
        else
            msg_text = _L("Filament does not match the filament in CFS slot. Please update the printer firmware to support CFS slot assignment.");
        update_print_status_msg(msg_text, true, false);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
        ...
        wxString msg_text = _L("The printer firmware only supports sequential mapping of filament => CFS slot.");
        update_print_status_msg(msg_text, false, false);
        Enable_Send_Button(true);
        Enable_Refresh_Button(true);
...
bool SelectMachineDialog::is_same_nozzle_type(std::string& filament_type, std::string& tag_nozzle_type)
{
    ...
    auto printer_nozzle_hrc = creality::Filament::get_hrc_by_nozzle_type(nozzle_type);
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent &event)
{
    ...
    if (has_unknown_filament) {
        has_slice_warnings = true;
        confirm_text.push_back(ConfirmBeforeSendInfo(_L("There are some unknown filaments in the CFS mappings. Please check whether they are the required filaments. If they are okay, press \"Confirm\" to start printing.")));
    }
...
void SelectMachineDialog::update_show_status()
{
    ...
    // do ams mapping if No CFS result
...
void EditDevNameDialog::on_edit_name(wxCommandEvent &e)
{
     ...
     : wxPanel(parent, winid, pos, size)
 {
     ...
     m_background_bitmap = ScalableBitmap(this,"thumbnail_grid",550);
 ...
 void ThumbnailPanel::set_thumbnail(wxImage &img)
 {
    ...
    bitmap_with_background.Create(m_background_size.GetWidth(), m_background_size.GetHeight());
     dc.SelectObject(bitmap_with_background);
     dc.DrawBitmap(m_background_bitmap.bmp(), 0, 0);

    int x = (bitmap_with_background.GetWidth() - m_bitmap.GetWidth()) / 2;
    int y = (bitmap_with_background.GetHeight() - m_bitmap.GetHeight()) / 2;
 ...
 {
    bool drawBackground = m_force_draw_background || 
                          (wxGetApp().dark_mode() && m_brightness_value < SHOW_BACKGROUND_BITMAP_PIXEL_THRESHOLD);
    ...
    if (drawBackground) {
        ...
        dc.Blit(0, 0, m_background_size.GetWidth(), m_background_size.GetHeight(), &memdc, 0, 0);
    } else {
 ...
 void ThumbnailPanel::set_force_draw_background(bool force_draw_background)
 {
     m_force_draw_background = force_draw_background;
 }

 void ThumbnailPanel::set_background_size(const wxSize &size)
 {
