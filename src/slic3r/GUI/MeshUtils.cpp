...
bool MeshRaycaster::second_closest_hit(const Vec2d&         mouse_pos,
                                       ...
                                       size_t*              facet_idx) const
{
    ...
    size_t hit_id        = 0;
    size_t second_hit_id = 1;
    if (clipping_plane != nullptr) {
        while (hit_id < hits.size() && clipping_plane->is_point_clipped(trafo * hits[hit_id].position())) {
            ++hit_id;
        }
        second_hit_id = hit_id + 1;
        while (second_hit_id < hits.size() && clipping_plane->is_point_clipped(trafo * hits[second_hit_id].position())) {
            ++second_hit_id;
        }
    }

    if (second_hit_id >= hits.size())
        return false; // no intersection found

    const AABBMesh::hit_result& hit = hits[second_hit_id];
