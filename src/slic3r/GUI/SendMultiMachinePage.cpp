...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //AMS
    if (!obj_->has_ams()) {
        DrawTextWithEllipsis(dc, _L("No CFS"), FromDIP(SEND_LEFT_DEV_NAME), left);
    }
    else {
        DrawTextWithEllipsis(dc, _L("CFS"), FromDIP(SEND_LEFT_DEV_NAME), left);
    }
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    wxBoxSizer* use_ams_sizer = create_item_radiobox(_L("Use CFS"), main_page, "", 0, "use_ams");
    ...
    m_ams = new Button(m_table_head_panel, _L("CFS Status"), "toolbar_double_directional_arrow", wxNO_BORDER, ICON_SIZE, false);
