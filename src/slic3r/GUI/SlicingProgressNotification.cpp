	...
	GLCanvas3D& canvas, float& initial_y, bool move_from_overlay, float overlay_width)
{
    ...
    const float progress_panel_height = 0.0f;
	...
	float right_gap = (move_from_overlay ? overlay_width + m_line_height * 5 : 0);
	m_window_pos = ImVec2((float)cnv_size.get_width() - right_gap - m_window_width, (float)cnv_size.get_height() - m_top_y);
	imgui.set_next_window_pos(10, m_window_pos.y, ImGuiCond_Always, 0.0f, 0.0f);
...
void Slic3r::GUI::NotificationManager::SlicingProgressNotification::render_text(const ImVec2& pos)
{
	...
	if(m_sp_state == SlicingProgressState::SP_PROGRESS)	{
		//one line text
		ImVec4 iv = m_is_dark ? ImVec4(1.0f, 1.0f, 1.0f, 1.0f) : ImVec4(0.13f, 0.13f, 0.13f, 1.0f);
		push_style_color(ImGuiCol_Text, iv, m_state == EState::FadingOut, m_current_fade_opacity);
		ImGui::SetCursorScreenPos(pos);
		imgui.text(m_text1.substr(0, m_endlines[0]).c_str());
		ImGui::PopStyleColor();
	}
...
void Slic3r::GUI::NotificationManager::SlicingProgressNotification::render_bar(const ImVec2& pos, const ImVec2& size)
{
	...
	ImColor progress_color = ImColor(23, 204, 95, (int)(255 * m_current_fade_opacity));
    ImColor bg_color       = ImColor(74, 74, 77, (int) (255 * m_current_fade_opacity));
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(lineStart, lineEnd, bg_color, 4.0f, ImDrawFlags_RoundCornersAll);
    ImGui::GetWindowDrawList()->AddRectFilled(lineStart, midPoint, progress_color, 4.0f, ImDrawFlags_RoundCornersAll);
	
	// percentage text
	//ImVec2 text_pos = ImVec2(pos.x, pos.y + size.y + m_line_height * 0.2f);
    float textGap  = 8.0f;
    ImVec2 text_pos = ImVec2(pos.x + size.x + textGap, pos.y - 4.0);
	std::string text;
