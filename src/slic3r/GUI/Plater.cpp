...
#include "slic3r/Utils/FlushVolume.hpp"

#define MINIZ_GZ_IMPLEMENTATION
#include "libslic3r/miniz_gzip.h"
...
#define COMMON_HOVER_BORDER_ICON_SIZE wxSize(FromDIP(24), FromDIP(24))
...
{
    Plater*                               plater;
    RemotePrint::PrinterBoxFilamentPanel* m_cx_panel_box_filament     = nullptr;
    FilamentPanel*                        m_cx_panel_filament_content = nullptr;
    wxPanel*                              scrolled;
    ...
    HoverBorderIcon*  m_bpButton_add_filament;
    HoverBorderIcon*  m_bpButton_del_filament;
    HoverBorderIcon*  m_bpButton_set_filament;
    int                m_menu_filament_id = -1;
    wxScrolledWindow* m_scrolledWindow_filament_content;
    wxStaticLine*     m_staticline2;
    wxPanel*          m_panel_project_title;
    ScalableButton*   m_filament_icon       = nullptr;
    HoverBorderIcon*  m_flushing_volume_btn = nullptr;
    HoverBorderIcon*  m_auto_mapping_btn    = nullptr;
    // wxSearchCtrl* m_search_bar = nullptr;
    // Search::SearchObjectDialog* dia = nullptr;
    // StaticBox *pSearchBk = nullptr;//The border style of the search control
...
Sidebar::Sidebar(Plater* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        if (nullptr != p->m_panel_printer_content) {
            ...
            m_bed_type_list = new ComboBox(p->m_panel_printer_content, wxID_ANY, wxString(""), wxDefaultPosition, {-1, FromDIP(30)}, 0,
...

    {
        ...
        p->m_panel_filament_title->SetBackgroundColor(0xFFFFFF);
        p->m_panel_filament_title->SetBackgroundColor2(0xFFFFFF);

        wxString state   = "expand";
        wxString theme   = is_dark ? "dark" : "light";
        wxString imgPath = wxString::Format("%s_%s_default", state, theme);

        HoverBorderIcon* fold_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString, imgPath, wxDefaultPosition,
                                                        COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);


    p->m_panel_filament_title->Bind(wxEVT_LEFT_UP, [fold_btn, is_dark, this](wxMouseEvent& e) {
        wxString state   = p->m_cx_panel_filament_content->IsShown() ? "expand" : "fold";
        wxString theme   = is_dark ? "dark" : "light";
        wxString imgPath = wxString::Format("%s_%s_default", state, theme);
        if (fold_btn) {
            fold_btn->SetIcon(imgPath);
            fold_btn->Refresh();
        }
        ...
        p->m_filament_icon = new ScalableButton(p->m_panel_filament_title, wxID_ANY, is_dark ? "materialLogo_d" : "material_light_default");
        p->m_staticText_filament_settings = new Label(p->m_panel_filament_title, Label::Head_14, _L("Material"), LB_PROPAGATE_MOUSE_EVENT);
        ...
        const int BUTTON_DIP_GAP   = FromDIP(3);
        const int BUTTON_SPACE_GAP = FromDIP(5);

        // BBS
        // auto maaping button
        p->m_auto_mapping_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString,
                                                    is_dark ? "auto_mapping_dark" : "auto_mapping_light", wxDefaultPosition,
                                                    COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
        p->m_auto_mapping_btn->SetToolTip_(_L("Auto Mapping"));
        p->m_auto_mapping_btn->setDisableIcon("auto_mapping_disable");
        ...
        p->m_flushing_volume_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString,
                                                       is_dark ? "flush_dark_default" : "flush_light_default", wxDefaultPosition,
                                                       COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
        ...
        bSizer39->Add(p->m_flushing_volume_btn, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, BUTTON_DIP_GAP);
        bSizer39->Hide(p->m_flushing_volume_btn);
        bSizer39->Add(BUTTON_SPACE_GAP, 0, 0, 0, 0);

        HoverBorderIcon* add_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString,
                                                       is_dark ? "addMaterial_dark_default" : "addMaterial_light_default",
                                                       wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
        add_btn->SetToolTip_(_L("Add one filament"));
        // add_btn->Bind(wxEVT_BUTTON, [this, scrolled_sizer](wxCommandEvent& e){
        add_btn->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
            if (nullptr != p->m_cx_panel_filament_content && !p->m_cx_panel_filament_content->can_add())
                return;

            int         filament_count = p->m_cx_panel_filament_content->size() + 1;
        ...
        bSizer39->Add(add_btn, 0, wxALIGN_CENTER | wxALL, BUTTON_DIP_GAP);
        bSizer39->Add(BUTTON_SPACE_GAP, 0, 0, 0, 0);

        HoverBorderIcon* del_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString,
                                                       is_dark ? "delMaterial_dark_default" : "delMaterial_light_default",
                                                       wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
        del_btn->SetToolTip(_L("Remove last filament"));
        del_btn->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
            if (!p->m_cx_panel_filament_content->can_delete())
                return;
        ...
        bSizer39->Add(del_btn, 0, wxALIGN_CENTER_VERTICAL, BUTTON_DIP_GAP);
        bSizer39->Add(BUTTON_SPACE_GAP, 0, 0, 0, 0);

        HoverBorderIcon* set_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString,
                                                       is_dark ? "setting_dark_default" : "setting_light_default", wxDefaultPosition,
                                                       COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
        set_btn->SetToolTip(_L("Set filaments to use"));
        set_btn->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
            p->editing_filament = -1;
            // wxGetApp().params_dialog()->Popup();
            // wxGetApp().get_tab(Preset::TYPE_FILAMENT)->restore_last_select_item();
        ...
        fold_btn->Bind(wxEVT_LEFT_DOWN, [fold_btn, is_dark, this](auto& e) {
            wxString state   = p->m_cx_panel_filament_content->IsShown() ? "fold" : "expand";
            wxString theme   = is_dark ? "dark" : "light";
            wxString imgPath = wxString::Format("%s_%s_default", state, theme);
            if (fold_btn) {
                fold_btn->SetIcon(imgPath);
            ...
            if (e.GetPosition().x > (p->m_flushing_volume_btn->IsShown() ? p->m_flushing_volume_btn->GetPosition().x :
        ...
        bSizer39->Add(FromDIP(5), 0, 0, 0, 0);
        ...
        scrolled_sizer->Add(p->m_cx_panel_box_filament, 0, wxEXPAND, 0);
        p->m_cx_panel_box_filament->Show(false);

        scrolled_sizer->AddSpacer(FromDIP(2));

        // add filament content
        p->m_cx_panel_filament_content = new FilamentPanel(p->scrolled);
        if (nullptr != p->m_cx_panel_filament_content) {
            p->m_cx_panel_filament_content->SetBackgroundColour(wxColour(255, 255, 255));
            p->m_cx_panel_filament_content->add_filament();
        }

        scrolled_sizer->Add(p->m_cx_panel_filament_content, 0, wxEXPAND, 0);
    }

    { // a horizontal line
        auto spliter_x = new ::StaticLine(p->scrolled, false, " ");
        spliter_x->SetLineColour(wxColour("#D8D8DD"));
        scrolled_sizer->Add(spliter_x, 0, wxEXPAND, 0);
    }

    {
        ...
        p->sizer_params->SetMinSize(wxSize(0, FromDIP(200)));

        // the border style of the search control
        //   p->pSearchBk = new StaticBox(p->scrolled);
        // p->pSearchBk->SetBorderColor(StateColor(std::make_pair(0xDBDBDB, (int)StateColor::Disabled), std::make_pair(0x15BF59,
        // (int)StateColor::Hovered), 	std::make_pair(0xDBDBDB, (int)StateColor::Normal)));
        //   //p->pSearchBk->SetBackgroundColor(StateColor(std::make_pair(0xF0F0F1, (int)StateColor::Disabled), std::make_pair(*wxWHITE,
        //   (int)StateColor::Normal))); p->pSearchBk->SetCornerRadius(0); p->pSearchBk->SetBorderWidth(1);
        //   //p->sizer_params->Add(p->pSearchBk, 0, wxBOTTOM |wxRIGHT| wxEXPAND, 1);

        // wxBoxSizer* sizer_search = new wxBoxSizer(wxVERTICAL);
        // p->pSearchBk->SetSizer(sizer_search);

        //   p->m_search_bar = new wxSearchCtrl(p->pSearchBk, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER
        //   | wxBORDER_NONE); p->m_search_bar->SetBackgroundColour(wxColour("#FFFFFF")); sizer_search->Add(p->m_search_bar, 0, wxALL |
        //   wxEXPAND, 1);
        ...
        //   p->m_search_bar->SetForegroundColour("#15BF59");
    ...
    this->Bind(wxEVT_SIZE, [this](wxSizeEvent& e) {
        wxPostEvent(p->m_cx_panel_box_filament, wxTimerEvent());
        e.Skip();
    });
}

Sidebar::~Sidebar() {}
wxString Sidebar::get_filament_map_string()
{
...
void Sidebar::create_printer_preset(int iType)
{
    ...
    if (wxID_OK == res) {
        ...
        if (res == wxID_OK) {
            // p->editing_filament = -1;
            // if (nullptr!=p->combo_printer && p->combo_printer->switch_to_tab())
            //     p->editing_filament = 0;

            p->plater->sidebar_printer().edit_filament();
        }
...
{
    if (current_extruder_count >= p->m_cx_panel_filament_content->size())
        return;
    while (p->m_cx_panel_filament_content->size() > current_extruder_count) {
        const int last = p->m_cx_panel_filament_content->size() - 1;
        p->m_cx_panel_filament_content->del_filament(last);
    }
}
void Sidebar::remove_unused_filament_combos_delete(const size_t indexcmb) 
{
    if (indexcmb >= p->m_cx_panel_filament_content->size())
        return;
    p->m_cx_panel_filament_content->del_filament(indexcmb);
}
void Sidebar::update_all_preset_comboboxes()
{
    ...
    if (print_tech == ptFFF) {
        p->m_cx_panel_filament_content->update();
    }

    if (nullptr != p->combo_printer && p->combo_printer)
        p->combo_printer->update();

    p->plater->sidebar_printer().update();
...
#else
        const size_t filament_cnt = p->m_cx_panel_filament_content->size();
#endif
...
void Sidebar::msw_rescale()
{
    ...
    //                                                             } )
    //     combo->msw_rescale();
    if (nullptr != p->combo_printer)
        p->combo_printer->msw_rescale();

    if (p->m_cx_panel_filament_content)
        p->m_cx_panel_filament_content->msw_rescale();

    p->plater->sidebar_printer().rescale();
...
void Sidebar::sys_color_changed()
{
    ...
    if (nullptr != p->m_printer_icon)
        p->m_printer_icon->msw_rescale();
    if (nullptr != p->m_printer_setting)
        p->m_printer_setting->msw_rescale();
    p->m_filament_icon->msw_rescale();
    p->m_bpButton_add_filament->msw_rescale();
    p->m_bpButton_del_filament->msw_rescale();
    ...
    bool is_dark = wxGetApp().dark_mode();
    p->m_filament_icon->SetBitmap_(is_dark ? "materialLogo_d" : "material_light_default");
    p->m_auto_mapping_btn->SetBitmap_(is_dark ? "auto_mapping_dark" : "auto_mapping_light");
    p->m_flushing_volume_btn->SetBitmap_(is_dark ? "flush_dark_default" : "flush_light_default");
    p->m_bpButton_add_filament->SetBitmap_(is_dark ? "addMaterial_dark_default" : "addMaterial_light_default");
    p->m_bpButton_del_filament->SetBitmap_(is_dark ? "delMaterial_dark_default" : "delMaterial_light_default");
    p->m_bpButton_set_filament->SetBitmap_(is_dark ? "setting_dark_default" : "setting_light_default");

    p->m_auto_mapping_btn->on_sys_color_changed(is_dark);
    p->m_bpButton_add_filament->on_sys_color_changed(is_dark);
    p->m_flushing_volume_btn->on_sys_color_changed(is_dark);
    p->m_bpButton_del_filament->on_sys_color_changed(is_dark);
    p->m_bpButton_set_filament->on_sys_color_changed(is_dark);
    ...
    p->m_cx_panel_filament_content->sys_color_changed();
    p->plater->sidebar_printer().sys_color_changed();
...
void Sidebar::jump_to_option(const std::string& opt_key, Preset::Type type, const std::wstring& category)
{
    ...
    // Creality: process flush multiplier is special
    if (opt_key == "flush_multiplier") {
        wxGetApp().sidebar().show_flushDialog();
        return;
    }
...
void Sidebar::on_filaments_change(size_t num_filaments)
{
    ...
    size_t i = p->m_cx_panel_filament_content->size();
    while (i < num_filaments) {
        if (!p->m_cx_panel_filament_content->add_filament()) {
            break;
        }
        i = p->m_cx_panel_filament_content->size();
    }
...
void Sidebar::update_filament_panel() { p->m_cx_panel_filament_content->update(); }
// Creality
void Sidebar::show_flushDialog()
{
    if (p->m_flushing_volume_btn) {
        ...
        event.SetEventObject(p->m_flushing_volume_btn);
        event.SetId(p->m_flushing_volume_btn->GetId());
        wxPostEvent(p->m_flushing_volume_btn, event);
    }
}
void Sidebar::add_filament()
{
    // BBS: limit filament choices to 16
    if (!p->m_cx_panel_filament_content->can_add())
        return;
    wxColour new_col = Plater::get_next_color_for_filament();
...
{
    if (!p->m_cx_panel_filament_content->can_delete()) return;
...
{
#if 0
    if (p->m_cx_panel_filament_content->size() <= 1)
        return;
    wxBusyCursor busy;
    size_t       filament_count = p->m_cx_panel_filament_content->size() - 1;
    //if (filament_id == size_t(-2)) {
...
void Sidebar::change_filament(size_t from_id, size_t to_id) { delete_filament_bbl(from_id, int(to_id)); }
...
void Sidebar::on_bed_type_change(BedType bed_type)
{
    ...
    p->plater->sidebar_printer().on_bed_type_change(bed_type);
}
...
void Sidebar::sync_ams_list()
{
    ...
    if (list.empty()) {
        MessageDialog dlg(this, _L("No CFS filaments. Please select a printer in 'Device' page to load CFS info."),
    ...
    struct SyncAmsDialog : MessageDialog
    {
        SyncAmsDialog(wxWindow* parent, bool first)
            : MessageDialog(
                  parent,
                  first ?
                      _L("Sync filaments with CFS will drop all current selected filament presets and colors. Do you want to continue?") :
                      _L("Already did a synchronization, do you want to sync only changes or resync all?"),
                  _L("Sync filaments with CFS"),
                  0)
        {
            if (first) {
    ...
    for (int i = 0; i < p->m_cx_panel_filament_content->size(); ++i) {
    ...
    if (n == 0) {
        MessageDialog dlg(this, _L("There are no compatible filaments, and sync is not performed."), _L("Sync filaments with CFS"), wxOK);
        dlg.ShowModal();
        return;
    }
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
                          _L("There are some unknown filaments mapped to generic preset. Please update Creality Print or restart Creality "
                             "Print to check if there is an update to system presets."),
                          _L("Sync filaments with CFS"), wxOK);
        dlg.ShowModal();
    }
...
wxPanel* Sidebar::filament_panel() { return p->m_cx_panel_filament_content; }

HoverBorderIcon* Sidebar::autoMap_button() { return p->m_auto_mapping_btn; }
...
bool Sidebar::is_multifilament() { return p->m_cx_panel_filament_content->size() > 1; }
...
{
    p->searcher.init(get_search_inputs(comAdvanced/*m_mode*/));
}

void Sidebar::update_mode(const int mode)
{
    if (mode == -1) {
...
int Sidebar::filament_size() { return p->m_cx_panel_filament_content->size(); }

void Sidebar::update_filament(int index) { p->m_cx_panel_filament_content->update(index); }
...
void Sidebar::on_current_device_changed(wxCommandEvent& event)
{
    ...
    if (!device_data.isMultiColorDevice) {
        ...
        if (p->m_cx_panel_box_filament) {
            ...
            p->m_cx_panel_filament_content->update_box_filament_sync_state(true);
            p->m_cx_panel_filament_content->reset_filament_sync_state();
        }
...
void Sidebar::show_box_filament_content(bool bShow)
{
    if (p->m_cx_panel_box_filament) {
        p->m_cx_panel_box_filament->Show(bShow);
    }

    if (p->m_cx_panel_filament_content) {
        p->m_cx_panel_filament_content->update_box_filament_sync_state(bShow);
    }
...
void Sidebar::on_mapping_device_filament(wxCommandEvent& event)
{
    if (p->m_cx_panel_box_filament) {
        p->m_cx_panel_box_filament->on_auto_device_filament_mapping();
    }
...
void Sidebar::on_show_box_color_selection(wxCommandEvent& event)
{
    if (p->m_cx_panel_box_filament) {
        Slic3r::GUI::BoxColorSelectPopupData* popupData = static_cast<Slic3r::GUI::BoxColorSelectPopupData*>(event.GetClientData());
        p->m_cx_panel_box_filament->on_show_box_color_selection(popupData->popup_position, popupData->filament_item_index);
        delete popupData;
    }
}
...
void Sidebar::on_auto_sync_current_device_filament(wxCommandEvent& event)
{
    ...
    if (p->m_cx_panel_filament_content) {
        if (device_data.materialBoxes.size() == 0) {
            ...
            show_box_filament_content(true);
            p->m_cx_panel_filament_content->on_auto_mapping_filament(device_data);
        }
    }
}

void Sidebar::on_re_sync_all_filaments(const std::string& printer_ip)
{
    if (p->m_cx_panel_filament_content) {
        p->m_cx_panel_filament_content->on_re_sync_all_filaments(printer_ip);
    }
}
...
void Sidebar::auto_calc_flushing_volumes(const int modify_id)
{
    ...
    int                        nozzleVolume   = printer_config.option<ConfigOptionFloat>("nozzle_volume")->value;
    const std::vector<double>& init_matrix    = (project_config.option<ConfigOptionFloats>("flush_volumes_matrix"))->values;
    const std::vector<double>& init_extruders = (project_config.option<ConfigOptionFloats>("flush_volumes_vector"))->values;
                ...
                if (is_to_support) {
                    ...
                    for (int j = 0; j < multi_colours[from_idx].size(); ++j) {
                        ...
                        for (int k = 0; k < multi_colours[modify_id].size(); ++k) {
                            ...
                            bool is_diff_filament = isDifferentFilament(from_idx, modify_id);
                            if (is_diff_filament) {
                                flushing_volume = std::max(flushing_volume, nozzleVolume * 3);
                            }
                        }
                    }
                    if (is_from_support)
                    ...
                    for (int j = 0; j < multi_colours[modify_id].size(); ++j) {
                        ...
                        for (int k = 0; k < multi_colours[to_idx].size(); ++k) {
                            ...
                            int volume         = calculator.calc_flush_vol(from.Alpha(), from.Red(), from.Green(), from.Blue(), to.Alpha(),
                                                                           to.Red(), to.Green(), to.Blue());
                            flushing_volume    = std::max(flushing_volume, volume);
                            bool is_diff_filament = isDifferentFilament(to_idx, modify_id);
                            if (is_diff_filament) {
                                flushing_volume = std::max(flushing_volume, nozzleVolume * 3);
                            }
                        }
...
void Sidebar::set_edit_filament(int index) { p->editing_filament = index; }

void Sidebar::set_menu_filement_id(int id) { 
    p->m_menu_filament_id = id;
  }
...
class AuiArt : public wxAuiDefaultDockArt
{
public:
    AuiArt() : wxAuiDefaultDockArt()
    {
        m_sashSize   = 1;
        m_borderSize = 0;
    }
...
{
public:
    AuiMgr() : wxAuiManager() { this->SetArtProvider(new AuiArt()); }
...
struct Plater::priv
{
    ...
    Upload3mfToCloudDialog* m_upload_3mf_dlg = nullptr;

    UploadGcodeToCloudDialog* m_upload_gcode_dlg = nullptr;
    ...
    // Creality
    ThumbnailData m_gcode_thumbnail;
    ...
    CollapseBar::GLToolbar collapse_toolbar;
    ProcessBar::GLToolbar  process_toolbar;
    ...
    static const std::regex pattern_cxprj;
    ...
    void                  check_gcode_path_contain_in_bed(int plate_index);
    bool                  check_enable_in_only_gcode_mode();
    void                  enable_sidebar(bool enabled);
    void                  collapse_sidebar(bool collapse);
    void                  update_sidebar(bool from_collapse = false, bool force_update = false);
    ...
    void add_warning(const Slic3r::PrintStateBase::Warning& warning, size_t oid, int plate_index);
    ...
    bool warnings_dialog(int plate_index = PLATE_ALL_IDX);

    void on_action_printer(SimpleEvent&);
    ...
    // Creality: add gcode bed shape
    void set_gcode_bed_shape(const Pointfs& shape, const Pointfs& exclude_area, const double printable_height, const Preset&);
    ...
    bool show_warning_dialog_popup_window{true};
    ...
    void on_action_send_to_local_net_printer(bool isall = false);
    void on_action_upload_3mf(bool isall = false);
    void on_action_upload_gcode();
    ...
    void did_select_bed_type(const std::string& bed_type);
...
private:
    ...
    void on_action_send_to_local_net_printer(SimpleEvent&);
    void on_action_upload_3mf(SimpleEvent&);
    ...
    std::vector<std::tuple<Slic3r::PrintStateBase::Warning, size_t, int>> current_warnings;
...
const std::regex Plater::priv::pattern_cxprj(".*cxprj", std::regex::icase);
...
Plater::priv::priv(Plater* q, MainFrame* main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys({
		...
		"nozzle_height","skirt_type", "skirt_loops", "skirt_speed","min_skirt_length", "skirt_distance",
        ...
        "wipe_tower_filament","best_object_pos"
        }))
    ...
    , collapse_toolbar(CollapseBar::GLToolbar::Normal, "Collapse")
    , process_toolbar(ProcessBar::GLToolbar::Normal, "ProcessBar")
    ...
    , partplate_list(this->q, &model)
{
    ...
    this->q->Bind(EVT_RE_SYNC_ALL, &priv::on_re_sync_all, this);
    this->q->Bind(EVT_DEVICE_LIST_UPDATED, &priv::on_device_list_updated, this);
    this->q->Bind(EVT_NOTIFY_PLATE_THUMBNAIL_UPDATE, &priv::on_notify_update_plate_thumbnail, this);
    this->q->Bind(EVT_CURRENT_DEVICE_CHANGED, &priv::on_current_device_changed, this);
    this->q->Bind(EVT_AUTO_SYNC_CURRENT_DEVICE_FILAMENT, &priv::on_auto_sync_current_device_filament, this);
...
#ifdef __linux__
    main_frame->Bind(wxEVT_IDLE, [=](wxIdleEvent& event) {
        if (sidebar) {
            sidebar->Refresh();
            sidebar->Update();
        }
        if (main_frame->m_param_panel) {
            main_frame->m_param_panel->Refresh();
            main_frame->m_param_panel->Update();
        }
    });
#endif
    ...
    m_aui_mgr.AddPane(sidebar_printer, wxAuiPaneInfo()
                                           .Name("sidebar_printer")
                                           //   .Left()
                                           .TopDockable(false)
                                           ...
                                           .CaptionVisible(true)
                                           .CloseButton(true)
                                           .FloatingSize(wxSize(400, 150))
                                           .Float()
                                           .Show(false)
                                           .BestSize(wxSize(400, 150)));
    ...
    m_aui_mgr.AddPane(sidebar, wxAuiPaneInfo()
                                   ...
                                   .BestSize(wxSize(38 * wxGetApp().em_unit(), 90 * wxGetApp().em_unit())));

    q->Bind(wxEVT_AUI_PANE_CLOSE, [](wxAuiManagerEvent& e) {
        wxAuiPaneInfo* panel = e.GetPane();
        if (nullptr != dynamic_cast<SidebarPrinter*>(panel->window)) {
            SidebarPrinter::bShow = false;
        }
    });
...

    {
        ...
        if (!layout.empty()) {
            ...
            auto& side_print            = m_aui_mgr.GetPane(sidebar_printer); //
            if (side_print.IsFloating()) {
                side_print.Hide();
                m_aui_mgr.Update();
            }
        }
    ...
    if (wxGetApp().is_editor()) {
        ...
        // 3DScene/Toolbar:
        view3D_canvas->Bind(EVT_GLTOOLBAR_PRINTER, &priv::on_action_printer, this);
        ...
        // BBS: add clone
        view3D_canvas->Bind(EVT_GLTOOLBAR_CLONE,
                            [q](SimpleEvent&) { q->canvas3D()->triger_extra_render_event(GLCanvas3D::ERenderEvent::ObjectCloneOptions); });
    ...
    if (wxGetApp().is_editor()) {
        ...
        preview->get_wxglcanvas()->Bind(EVT_CUSTOMEVT_TICKSCHANGED, [this](wxCommandEvent& event) {
            ...
            // Creality: update thumbnails add or remove layer colors
            if (tick_event_type == Type::ToolChange) {
                this->q->update_all_plate_thumbnails(true);
    ...
    if (wxGetApp().is_editor()) {
        ...
        q->Bind(EVT_GLTOOLBAR_SEND_TO_LOCAL_NET_PRINTER, &priv::on_action_send_to_local_net_printer, this);
        //q->Bind(EVT_GLTOOLBAR_MULT_SEND_TO_LOCAL_NET_PRINTER, &priv::on_action_send_to_local_net_printer, this);
        ...
        this->q->Bind(EVT_RESTORE_PROJECT, [this, last = last_backup](wxCommandEvent& e) {
            ...
            if (Slic3r::has_restore_data(last_backup, originfile)) {
                ...
                MessageDialog dialog = MessageDialog(this->q, _L("Previous unsaved project detected, do you want to restore it?"),
                                                     wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Restore"),
...
Slic3r::GUI::SidebarPrinter& Plater::sidebar_printer() { return *p->sidebar_printer; }

void Plater::set_calib_model(bool _value) { p->m_is_calib_model = _value; }

void Plater::update_plate_thumbnail(wxCommandEvent& event) { p->on_notify_update_plate_thumbnail(event); }
...
wxColour Plater::get_next_color_for_filament()
{
    ...
    wxColour colors[FILAMENT_SYSTEM_COLORS_NUM] = {*wxYELLOW,
                                                   *wxRED,
                                                   *wxBLUE,
                                                   *wxCYAN,
                                                   *wxLIGHT_GREY,
...
void Plater::priv::check_gcode_path_contain_in_bed(int plate_index)
{
...
bool Plater::priv::check_enable_in_only_gcode_mode()
{
    if (q->m_only_gcode) {
        return q->m_last_loaded_3mf.empty();
    }
...
{
    bool tmp_enabled = enabled;
    if (check_enable_in_only_gcode_mode())
        tmp_enabled = false;

    sidebar_layout.is_enabled = tmp_enabled;
    update_sidebar();
}
...
void Plater::priv::update_sidebar(bool from_collapse, bool force_update)
{
    auto& sidebar = m_aui_mgr.GetPane(this->sidebar);
...
std::vector<size_t> Plater::priv::load_files(const std::vector<fs::path>& input_files, LoadStrategy strategy, bool ask_multi)
{
    ...
    for (size_t i = 0; i < input_files.size(); ++i) {
       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
        ...
        const bool type_3mf = std::regex_match(path.string(), pattern_3mf) || std::regex_match(path.string(), pattern_cxprj);
            ...
            if (type_3mf) {
                ...
                Semver             file_version;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa || (load_model && !load_config)) {
                        ...
                        int filament_size = sidebar->filament_size();
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Creality Print, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by CrealityPrint, load geometry data only."), _L("Load 3mf"));
                    //     }
                    ...
                    if ((project_presets.size() > 0) && load_config) {
                        ...
                        PresetBundle&              preset_bundle                      = *wxGetApp().preset_bundle;
                        bool                       has_single_extruder_multi_material = false;
                        for (unsigned int i = 0; i < project_presets.size(); i++) {
                            if (project_presets[i]->type == Preset::TYPE_PRINTER) {
                                if (project_presets[i]->config.has("single_extruder_multi_material")) {
                                    const ConfigOption* single_extruder = project_presets[i]->config.option(
                                        "single_extruder_multi_material");
                                    has_single_extruder_multi_material = single_extruder->getBool();
                                }
                            }
                        }
                        for (unsigned int i = 0; i < project_presets.size(); i++) {
                ...
                if (file_version.maj() == 5 && file_version.min() < 2) {
                    creality_3mf_old = true;
                }
                if (load_config) {
                    if (!config.empty()) {
                        Preset::normalize(config);
                        if (creality_3mf_old) {
                            auto internalName2PrinterName = [](const std::string& internalName) -> std::string {
                                if (internalName == "F001") {
                                    return "Ender-3 V3";
                                }
                                if (internalName == "F002") {
                                    return "Ender-3 V3 Plus";
                                }
                                if (internalName == "F003") {
                                    return "CR-10 SE";
                                }
                                if (internalName == "F004") {
                                    return "Ender-5 Max";
                                }
                                if (internalName == "F005") {
                                    return "Ender-3 V3 KE";
                                }
                                if (internalName == "F008") {
                                    return "K2 Plus";
                                }
                                if (internalName == "F018") {
                                    return "Creality Hi";
                                }
                                if (internalName == "CR-K1") {
                                    return "K1";
                                }
                                if (internalName == "CR-K1 Max") {
                                    return "K1 Max";
                                }
                                return internalName;
                            };
                            // process old 3mf file
                            bool        isPrinterInherit = false;
                            std::string printer_model    = internalName2PrinterName(
                                config.option<ConfigOptionString>("printer_model", true)->value);
                            std::string printer_settings_id = internalName2PrinterName(
                                config.option<ConfigOptionString>("printer_settings_id", true)->value);
                            if (printer_model != printer_settings_id) {
                                isPrinterInherit = true;
                            } else {
                                if (printer_settings_id.find("Creality") != 0) {
                                    printer_settings_id = "Creality " + printer_settings_id;
                                }
                            }
                            if (printer_model.find("Creality") != 0) {
                                printer_model = "Creality " + printer_model;
                            }
                            config.set("printer_model", printer_model);
                            std::string printer_variant   = config.option<ConfigOptionString>("printer_variant", true)->value;
                            std::string print_settings_id = config.option<ConfigOptionString>("print_settings_id", true)->value;
                            if (printer_settings_id.find(" nozzle") == std::string::npos) {
                                printer_settings_id = (boost::format("%s %s nozzle") % printer_settings_id % printer_variant).str();
                            }
                            config.set("printer_settings_id", printer_settings_id);
                            if (isPrinterInherit) {
                                printer_settings_id = (boost::format("%s %s nozzle") % printer_model % printer_variant).str();
                            }
                            std::vector<std::string> inherits_group;
                            inherits_group.push_back("");
                            // process filament
                            std::vector<std::string> filament_preset_name =
                                config.option<ConfigOptionStrings>("filament_settings_id", true)->values;
                            for (unsigned int index = 0; index < filament_preset_name.size(); index++) {
                                std::string filament_settings_id = filament_preset_name[index];
                                if (filament_settings_id.find("@") != std::string::npos) {
                                    filament_settings_id = TrimEnd(filament_settings_id.substr(0, filament_settings_id.find("@")));
                                }

                                std::string officail_filament_name = (boost::format("%s @%s") % filament_settings_id % printer_settings_id)
                                                                         .str();
                                Preset* filament_preset = wxGetApp().preset_bundle->filaments.find_preset(officail_filament_name);
                                if (filament_preset != nullptr) {
                                    filament_preset_name[index] = officail_filament_name;
                                }
                                inherits_group.push_back("");
                            }
                            config.set_key_value("filament_settings_id", new ConfigOptionStrings(filament_preset_name));

                            if (print_settings_id.find("@") != std::string::npos) {
                                print_settings_id = TrimEnd(print_settings_id.substr(0, print_settings_id.find("@")));
                            }
                            print_settings_id      = (boost::format("%s @%s") % print_settings_id % printer_settings_id).str();
                            Preset* process_preset = wxGetApp().preset_bundle->prints.find_preset(print_settings_id);
                            if (process_preset != nullptr) {
                                config.set("print_settings_id", print_settings_id);
                            }

                            std::vector<std::string> current_different_settings =
                                config.option<ConfigOptionStrings>("different_settings_to_system", true)->values;

                            // process inherits
                            if (isPrinterInherit || current_different_settings.size() >= 3 && current_different_settings[2].empty()) {
                                inherits_group.push_back(printer_settings_id);
                                Preset* preset = wxGetApp().preset_bundle->printers.find_preset(printer_settings_id);
                                if (preset != nullptr) {
                                    std::vector<std::string> dirty_options = preset->config.diff(config);

                                    std::string current_different_settings_str = "";
                                    for (auto option : dirty_options) {
                                        if (option == "default_filament_profile" || option == "default_print_profile" ||
                                            option == "inherits" || option == "printer_model" || option == "printer_settings_id" ||
                                            option == "support_multi_bed_types") {
                                            continue;
                                        }
                                        if (file_version.patch() < 5) {
                                            if (option == "extruder_colour" || option == "retract_when_changing_layer" ||
                                                option == "deretraction_speed" || option == "max_layer_height" ||
                                                option == "retract_before_wipe" || option == "retraction_speed" || option == "wipe" ||
                                                option == "wipe_distance") {
                                                continue;
                                            }
                                        }
                                        current_different_settings_str += option + ";";
                                    }
                                    if (current_different_settings.size() > 0) {
                                        current_different_settings[current_different_settings.size() - 1] =
                                            current_different_settings[current_different_settings.size() - 1] +
                                            current_different_settings_str;
                                    } else {
                                        current_different_settings.push_back("");
                                        current_different_settings.push_back("");
                                        current_different_settings.push_back(current_different_settings_str);
                                    }
                                    config.set_key_value("different_settings_to_system",
                                                         new ConfigOptionStrings(current_different_settings));
                                    config.set_key_value("inherits_group", new ConfigOptionStrings(inherits_group));
                                }
                            }
                        }
                        // single_extruder_multi_material_priming
                        if (config.has("single_extruder_multi_material")) {
                            const ConfigOption* single_extruder = config.option("single_extruder_multi_material");
                            if (single_extruder->getBool()) {
                        ...
                        if (choise.empty() || choise != "true") {
                            ...
                            if (validated == VALIDATE_PRESETS_MODIFIED_GCODES) {
                                ...
                                for (std::set<std::string>::iterator it = modified_gcodes.begin(); it != modified_gcodes.end(); ++it)
                                    warning_message += "-" + *it + "\n";
                                warning_message += "\n";
                                // show_info(q, _L("The 3mf has following modified G-codes in filament or printer presets:") +
                                // warning_message+ _L("Please confirm that these modified G-codes are safe to prevent any damage to the
                                // machine!"), _L("Modified G-codes"));

                                MessageDialog
                                    dlg(q,
                                        _L("The 3mf has following modified G-codes in filament or printer presets:") + warning_message +
                                            _L("Please confirm that these modified G-codes are safe to prevent any damage to the machine!"),
                                        _L("Modified G-codes"));
                                dlg.show_dsa_button();
                                auto res = dlg.ShowModal();
                                ...
                                // _L("Please confirm that the G-codes within these presets are safe to prevent any damage to the
                                // machine!"), _L("Customized Preset"));
                                MessageDialog dlg(q,
                                                  _L("The 3mf has following customized filament or printer presets:") +
                                                      from_u8(warning_message) +
                                                      _L("Please confirm that the G-codes within these presets are safe to prevent any "
                        ...
                        // always load config
                        {
                            ...
                            if (filament_settings_id != nullptr && default_filament_profile != nullptr && default_filament_profile->values.size() > 0) {
                                for (auto& item : filament_settings_id->values) {
                                    if (item == "Default Filament") {
                                        item = default_filament_profile->values[0];
                                    }
                                }
                            }
        ...
        if (load_model) {
            ...
            if (creality_3mf_old) {
                config->option<ConfigOptionPoints>("printable_area")->values;
                int plate_count = partplate_list.get_plate_count();
                partplate_list.get_plate_size(current_width, current_depth, current_height);

                model.convert_from_creality5(Vec2d(current_width, current_depth),
                                             Vec2d(partplate_list.plate_stride_x(), partplate_list.plate_stride_y()), plate_count);
            }
    ...
    if (load_model && load_config) {
        if (model.objects.empty()) {
            ...
            if (first_plate && first_plate->is_slice_result_valid()) {
                ...
                wxGetApp().mainframe->enable_tab_event(true);
            } else {
                // set to 3d tab
                q->select_view_3D("3D");
                // select plate 0 as default
    ...
    try
    {
        if(wxGetApp().is_privacy_checked()) {
            ...
            std::string file_formats, file_paths, file_sizes;
            for (const auto& path : input_files) {
            ...
            js["file_format"] = file_formats;
            js["file_path"] = file_paths;
            js["file_size"] = file_sizes;
            ...
            wxGetApp().track_event("import_file", js.dump());
        }
    }
    catch (...)
    {
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs& model_objects, bool allow_negative_z, bool split_object)
{
    ...
    for (ModelObject* model_object : model_objects) {
        ...
        for (size_t i = 0; i < object->instances.size(); ++i) {
            ...
            const double   ratio2    = ratio(2);
            if (max_ratio > 10000) {
                MessageDialog
                    dlg(q, _L("Your object appears to be too large, Do you want to scale it down to fit the heat bed automatically?"),
...
fs::path Plater::priv::get_export_file_path(GUI::FileType file_type)
{
    ...
    } else if (file_type == FT_CXPROJECT) {
        output_file = into_path(get_project_filename(".cxprj"));
    }
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    case FT_GCODE:
    case FT_OBJ:
    case FT_CXPROJECT: wildcard = file_wildcards(file_type); break;
    case FT_PROJECT: wildcard = GUI::format_wxstr("%s|%s", file_wildcards(FT_3MF), file_wildcards(FT_CXPROJECT)); break;
    ...
    case FT_CXPROJECT: {
        output_file.replace_extension("cxprj");
        dlg_title = _L("Save file as:");
        break;
    }
    case FT_OBJ: {
    ...
    wxFileDialog dlg(q, dlg_title,
                     ...
                     from_path(output_file.stem()), wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT | wxPD_APP_MODAL);
    ...
    std::string dlg_wildcard = dlg.GetWildcard().ToStdString();
    std::string pattern      = "(*" + extension + ")|";

    if (dlg_wildcard.find(pattern) == std::string::npos) {
    ...
    for (auto& ap : movable) {
        if (std::find(move_top_left_ids.begin(), move_top_left_ids.end(), ap.itemid) != move_top_left_ids.end()) {
            ...
            this->get_notification_manager()->push_notification(NotificationType::BBLPlateInfo,
                                                                ...
                                                                into_u8(_L(
                                                                    "Current tray is full; Unarranged models are placed above Tray 1.")));
        } else {
...
void Plater::update_mode(const int mode)
{
    ...
    if (mode == -1) {
        ...
        app_mode = static_cast<ConfigOptionMode>(mode);
    }
    ProcessBar::GLToolbarItem* item = p->process_toolbar.get_item("Speed");
    if (nullptr != item) {
        item->set_visible(app_mode == ConfigOptionMode::comAdvanced);
        p->process_toolbar.set_layout_dirty();
    }
    sidebar_printer().update();
}
...
void Plater::priv::reload_from_disk()
{
    ...
    for (size_t i = 0; i < input_paths.size(); ++i) {
        ...
        try {
            ...
            // BBS: backup
            new_model = Model::read_from_file(path, nullptr, nullptr, LoadStrategy::AddDefaultInstances | LoadStrategy::LoadModel,
                                              &plate_data, &project_presets, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                                              0, obj_color_fun);
            for (ModelObject* model_object : new_model.objects) {
                model_object->center_around_origin();
                model_object->ensure_on_bed();
        ...
        for (auto [obj_idx, vol_idx] : selected_volumes) {
            ...
            if (has_source || has_name) {
                ...
                new_volume->set_transformation(old_volume->get_transformation().get_matrix() *
                                               old_volume->source.transform.get_matrix_no_offset() * transform *
...
void Plater::priv::check_sidebar_state_in_only_gcode_mode()
{
    if (current_panel != preview)
        return;

    // Add sidebar and toolbar collapse logic;
    this->enable_sidebar(!check_enable_in_only_gcode_mode());

    // "m_last_loaded_3mf"  and  "m_exported_file" share the same meaning
    if (check_enable_in_only_gcode_mode()) {
...
void Plater::priv::on_select_bed_type(wxCommandEvent& evt)
{
    ...
    if (is_creality_vendor) {
        selection = q->get_bed_type_list(selection);
    }

    if (selection >= 0 && selection < print_config_def.get("curr_bed_type")->enum_values.size()) {
        std::string bed_type_name = print_config_def.get("curr_bed_type")->enum_values[selection];
        did_select_bed_type(bed_type_name);
    }
    wxGetApp().plater()->sidebar().update_filament_panel();
}

void Plater::priv::did_select_bed_type(const std::string& bed_type_name)
{
    PresetBundle&               preset_bundle = *wxGetApp().preset_bundle;
...
void Plater::priv::on_select_preset(wxCommandEvent& evt)
{
    ...
    if (PresetComboBox::LabelItemType::LABEL_ITEM_WIZARD_ADD_PRINTERS == marker) {
        sidebar->create_printer_preset(0);
        return;
    }

    if (PresetComboBox::LabelItemType::LABEL_ITEM_WIZARD_ADD_NOZZLE == marker) {
        sidebar->create_printer_preset(1);
        return;
    }
    ...
    // update item data.
    if (preset_type == Preset::TYPE_FILAMENT) {
        sidebar->update_filament(combo->get_filament_idx());
    }
}
    ...
    if (evt.status.flags &
        ...
        for (auto const& warning : state.warnings) {
            if (warning.current) {
                ...
                int current_plate_index = wxGetApp().plater()->get_partplate_list().get_curr_plate()->get_index();
                notification_manager->push_slicing_warning_notification(warning.message, false, model_object, object_id, warning_step,
                                                                        warning.message_id, current_plate_index, notif_level);
                add_warning(warning, object_id.id, current_plate_index);
            }
        }
    }
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format("exit.");
...
{
    if (show_warning_dialog && show_warning_dialog_popup_window)
        warnings_dialog();
}
...
void Plater::priv::add_warning(const Slic3r::PrintStateBase::Warning& warning, size_t oid, int plate_index)
{
    for (auto& it : current_warnings) {
        if (warning.message_id == std::get<0>(it).message_id && plate_index == std::get<2>(it)) {
            if (warning.message_id != 0 || (warning.message_id == 0 && warning.message == std::get<0>(it).message)) {
                if (warning.message_id != 0)
                    std::get<0>(it).message = warning.message;
                return;
            }
        }
    }
    current_warnings.emplace_back(std::tuple<Slic3r::PrintStateBase::Warning, size_t, int>(warning, oid, plate_index));
}
...
void Plater::priv::clear_warnings(int plate_index)
{
    ...
    if(plate_index < 0) {
        ...
        warnings.erase(
            std::remove_if(warnings.begin(), warnings.end(),
                [plate_index](const std::tuple<Slic3r::PrintStateBase::Warning, size_t, int>& t) {
                    return std::get<2>(t) == plate_index;
                }),
            warnings.end());
    }
    
}
bool Plater::priv::warnings_dialog(int plate_index)
{
    ...
    for (auto const& it : current_warnings) {
        int warn_plate = std::get<2>(it);
        if(plate_index < 0 || warn_plate == plate_index) {
            has_warning = true;
            const auto& warning = std::get<0>(it);
    ...
    MessageDialog msg_window(this->q, from_u8(text), _L("warnings"), wxOK | wxCANCEL);
    const auto    res = msg_window.ShowModal();
    return res == wxID_CANCEL;
}
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent& evt)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(":slicing plate %1% finished, start checking gcode path outside or not...") %
                                   m_cur_slice_plate;
    check_gcode_path_contain_in_bed(partplate_list.get_curr_plate_index());
    ...
    if(only_has_gcode_need_preview)
    {
        GCodeProcessorResult* current_result = partplate_list.get_current_slice_result();
    ...
    if (is_finished) {
        ...
        // wxGetApp().mainframe->m_topbar->SetSelection(size_t(MainFrame::tpPreview)-1);
    } else {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
    ...
    if (is_finished) {
        if (Slic3r::BLCompareTestFlow::enabled()) {
            Slic3r::BLCompareTestFlow::endBLTest();
            std::function<void(std::string)> callback = [this](std::string _3mf_file) {
                wxString wxfile(_3mf_file.c_str(), wxConvWhateverWorks);
                GUI::wxGetApp().plater_->select_view_3D("3D");
                GUI::wxGetApp().mainframe->open_recent_project(0, wxfile);
            };
            Slic3r::BLCompareTestFlow::nextBLTest(callback);
            //         if(Slic3r::BLCompareTestFlow::isBLTestEnd()) {
            //             Slic3r::BLCompareTestFlow::writeResultData();
            //             std::exit(0);
            //}
        }
#if AUTOMATION_TOOL
...
void Plater::priv::on_action_send_to_multi_machine(SimpleEvent&)
{
    ...
    CxSentToPrinterDialog dlg(this->q,CxSentToPrinterDialog::SendType::Multi);
    dlg.ShowModal();
...
void Plater::priv::on_re_sync_all(wxCommandEvent& evt)
{
    std::string printer_ip = evt.GetString().ToStdString();
    wxGetApp().sidebar().on_re_sync_all_filaments(printer_ip);
}

void Plater::priv::on_device_list_updated(wxCommandEvent& event) {}

void Plater::priv::on_current_device_changed(wxCommandEvent& event) { wxGetApp().sidebar().on_current_device_changed(event); }
...
void Plater::priv::on_auto_sync_current_device_filament(wxCommandEvent& event)
{
    ...
    if (current_panel && q->is_preview_shown()) {
        wxGetApp().plater()->get_current_canvas3D()->enable_select_plate_toolbar(false);
        wxGetApp().plater()->get_current_canvas3D()->enable_select_plate_toolbar(true);
...
{
    wxVariant* combinedVariant = static_cast<wxVariant*>(event.GetClientData());
    if (!combinedVariant) {
        return;
    }

    wxVariantList variantList = combinedVariant->GetList();

    std::vector<std::string> extruder_match_colors;
    std::vector<int>         plate_extruders;
    int                      index = 0;
    wxVariantList::iterator  it    = variantList.begin();
    for (; it != variantList.end(); it++, index++) {
        wxVariant theVariant = **it;
        if (theVariant.GetType() == "list") {
            wxVariantList theList = theVariant.GetList();
            for (wxVariantList::Node* node = theList.GetFirst(); node; node = node->GetNext()) {
                wxVariant* varPtr = node->GetData();
                if (varPtr) {
                    wxVariant& var = *varPtr;
                    if (0 == index) {
                        extruder_match_colors.push_back(var.GetString().ToStdString());
                    } else if (1 == index) {
                        plate_extruders.push_back(var.GetInteger());
                    }
                }
            }
        }
    ...
    assert(plate_extruders.size() == extruder_match_colors.size());
    ...
    std::vector<std::string> original_colors = colors->values; //
    int                      extruderId      = 0;
    for (int i = 0; i < extruder_match_colors.size(); i++) {
        extruderId = plate_extruders[i] - 1;

        if (extruderId < 0 || extruderId >= colors->values.size())
            continue;

        colors->values[extruderId] = extruder_match_colors[i];
    }
    ...
    colors->values = original_colors; // restore color
    delete combinedVariant;
}
...
void Plater::priv::on_action_upload_3mf(bool isall)
{
    ...
    if (isall) {
        m_upload_3mf_dlg->prepare(PLATE_ALL_IDX);
    } else {
        m_upload_3mf_dlg->prepare(partplate_list.get_curr_plate_index());
    }

    m_upload_3mf_dlg->Show(true);
}
...
void Plater::priv::on_action_upload_gcode()
{
    ...
    m_upload_gcode_dlg->prepare(partplate_list.get_curr_plate_index());

    // m_upload_gcode_dlg->Show(true);
    m_upload_gcode_dlg->doModel();
}
...
void Plater::priv::on_action_send_to_local_net_printer(SimpleEvent&)
{
    if (q != nullptr) {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << ":received send_to_local_net_printer event\n";
        q->send_to_local_net_printer();
    }
}

void Plater::priv::on_action_upload_3mf(SimpleEvent&)
{
    if (q != nullptr) {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << ":received send_to_local_net_printer event\n";
        q->upload_3mf();
...
void Plater::priv::on_apple_change_color_mode(wxSysColourChangedEvent& evt)
{
    ...
    DispConfig().setDarkMode(m_is_dark);
...
void Plater::priv::apply_color_mode()
{
    ...
    DispConfig().setDarkMode(is_dark);
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    // update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - CrealityPrint");
...
bool Plater::priv::init_collapse_toolbar()
{
    ...
    if (collapse_toolbar.is_valid())
        // already initialized
        return true;

    collapse_toolbar.set_layout_type(CollapseBar::GLToolbar::Layout::Vertical);
    collapse_toolbar.set_horizontal_orientation(CollapseBar::GLToolbar::Layout::HO_Right);
    collapse_toolbar.set_vertical_orientation(CollapseBar::GLToolbar::Layout::VO_Top);

    CollapseBar::GLToolbarItem::Data item;
    item.left.action_callback = []() { wxGetApp().plater()->collapse_sidebar(!wxGetApp().plater()->is_sidebar_collapsed()); };
    if (!collapse_toolbar.init_item(item))
        return false;
...
bool Plater::priv::init_process_toolbar()
{
    if (wxGetApp().is_gcode_viewer())
        return true;

    if (process_toolbar.get_items_count() > 0)
        // already initialized
        return true;

    ProcessBar::BackgroundTexture::Metadata background_data;
    float                                   sc = wxGetApp().plater()->get_current_canvas3D()->get_scale();
    background_data.filename                   = m_is_dark ? "process_toolbar_background_dark.png" : "process_toolbar_background.png";
    background_data.left                       = 5 * sc;
    background_data.top                        = 5 * sc;
    background_data.right                      = 5 * sc;
    background_data.bottom                     = 5 * sc;

    if (!process_toolbar.init(background_data))
        return false;

    process_toolbar.set_layout_type(ProcessBar::GLToolbar::Layout::Vertical);
    process_toolbar.set_horizontal_orientation(ProcessBar::GLToolbar::Layout::HO_Center);
    process_toolbar.set_vertical_orientation(ProcessBar::GLToolbar::Layout::VO_Center);
    process_toolbar.set_border(0.0f);
    process_toolbar.set_separator_size(5.0 * sc);
    process_toolbar.set_gap_size(2.0 * sc);

    process_toolbar.set_icons_size(30);

    process_toolbar.del_all_item();

    {
        process_toolbar.add_separator();
    }

    {
        ProcessBar::GLToolbarItem::Data data;
        data.name                 = "Frequent";
        data.tooltip              = _utf8(L("Frequent"));
        data.icon_filename        = ProcessBar::GLToolbar::get_process_toolbar_item_icon_file(data.name, m_is_dark, false);
        data.sprite_id            = 0;
        data.left.action_callback = []() {
            wxPanel*  atab      = wxGetApp().params_panel()->get_current_tab();
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Frequent"));
            }
        };

        process_toolbar.add_item(data, ProcessBar::GLToolbarItem::Action);
        process_toolbar.get_item(L("Frequent"), false)->set_visible(false);
    }

    {
        ProcessBar::GLToolbarItem::Data data;
        data.name                 = "Quality";
        data.tooltip              = _utf8(L("Quality"));
        data.icon_filename        = ProcessBar::GLToolbar::get_process_toolbar_item_icon_file(data.name, m_is_dark, false);
        data.sprite_id            = 1;
        data.left.action_callback = []() {
            wxPanel*  atab      = wxGetApp().params_panel()->get_current_tab();
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Quality"));
            }
        };
...

    {
        ...
        data.left.action_callback = []() {
            ...
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Strength"));
            }
        };
        process_toolbar.add_item(data, ProcessBar::GLToolbarItem::Action);
    }

    {
        ProcessBar::GLToolbarItem::Data data;
        data.name                 = "Speed";
        data.tooltip              = _utf8(L("Speed"));
        data.icon_filename        = ProcessBar::GLToolbar::get_process_toolbar_item_icon_file(data.name, m_is_dark, false);
        data.sprite_id            = 3;
        data.visible              = ConfigOptionMode::comAdvanced == Slic3r::GUI::wxGetApp().get_mode();
        data.left.action_callback = []() {
            wxPanel*  atab      = wxGetApp().params_panel()->get_current_tab();
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Speed"));
            }
        };
        process_toolbar.add_item(data, ProcessBar::GLToolbarItem::Action);
    }

    {
        ...
        data.left.action_callback = []() {
            ...
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Support"));
            }
        };
        process_toolbar.add_item(data, ProcessBar::GLToolbarItem::Action);
    }
    ...
    {
        ProcessBar::GLToolbarItem::Data data;
        data.name                 = "Multimaterial";
        data.tooltip              = _utf8(L("Multimaterial"));
        data.icon_filename        = ProcessBar::GLToolbar::get_process_toolbar_item_icon_file(data.name, m_is_dark, false);
        data.sprite_id            = 5;
        data.left.action_callback = []() {
            wxPanel*  atab      = wxGetApp().params_panel()->get_current_tab();
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Multimaterial"));
            }
        };
        process_toolbar.add_item(data, ProcessBar::GLToolbarItem::Action);
    }

    {
        ...
        data.left.action_callback = []() {
            ...
            TabPrint* tab_print = dynamic_cast<TabPrint*>(atab);
            if (tab_print != nullptr) {
                tab_print->select_item(L("Others"));
            }
        };
        process_toolbar.add_item(data, ProcessBar::GLToolbarItem::Action);
    }
    ...
    Slic3r::GUI::ProcessBar::GLToolbarItem* qItem = process_toolbar.get_item(L("Quality"), false);
    if(qItem)
        qItem->set_state(Slic3r::GUI::ProcessBar::GLToolbarItem::Pressed);
                                 ...
                                 bool               force_as_custom)
{
    ...
    if (new_shape) {
        ...
        partplate_list.determine_model_position(true);
...
void Plater::priv::set_gcode_bed_shape(const Pointfs& shape,
                                       const Pointfs& exclude_areas,
                                       const double   printable_height,
                                       const Preset&  preset)
{
    ...
    // BBS: add shape position
    Vec2d shape_position = partplate_list.get_current_shape_position();
    bool  new_shape      = bed.set_gcode_shape(shape, printable_height, &preset, shape_position);
    ...
    if (new_shape) {
        ...
        partplate_list.set_shapes(shape, exclude_areas, texture, height_to_lid, height_to_rod);

        Vec2d new_shape_position = partplate_list.get_current_shape_position();
        if (shape_position != new_shape_position)
            bed.set_gcode_shape(shape, printable_height, &preset, shape_position);
    }
}
...
std::string w2s(wxString sSrc) { return std::string(sSrc.mb_str()); }
...
void Plater::priv::check_printer_standard_status()
{
    ...
    if (-1 == sVendor.find("Creality")) { // Only handle Creality models
        return;
    }
...
void Plater::priv::on_action_printer(SimpleEvent&)
{
    ...
    auto& sidebar = m_aui_mgr.GetPane(this->sidebar_printer);
    if (sidebar.IsFloating()) {
        sidebar.FloatingPosition(pos);
    }
...
// BBS: save logic
int Plater::save_project(bool saveAs, FileType ft, En3mfType type_3mf)
{
...
void Plater::import_model_id(wxString download_info)
{
    ...
    if(download_info.StartsWith(wxT("http")) || download_info.StartsWith(wxT("https")))
    {
    ...
    boost::thread import_thread = Slic3r::create_thread([&percent, &cont, &cancel, &retry_count, max_retries, &msg, &target_path,
                                                         &download_ok, download_url, &filename] {
        ...
        try {
            ...
            if (extension.IsEmpty()) {
                auto end_pos = download_url.find_first_of("?");
                if (end_pos != wxString::npos) {
                    auto name = fs::path(download_url.substr(0, end_pos).wx_str()).filename().string();
                    extension = std::string(std::find(name.begin(), name.end(), '.'), name.end()).c_str();
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg  = _L("Importing to Creality Print failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    if (download_ok) {
        ...
        if (target_path.extension() != ".3mf") {
            if (target_path.extension() == ".gz") {
                boost::filesystem::path stemPath = boost::filesystem::path(target_path).parent_path() /
                                                   boost::filesystem::path(target_path).stem();
                this->load_files(wxArrayString(1, stemPath.wstring()));
            } else {
                this->load_files(wxArrayString(1, target_path.wstring()));
            }
        } else
...
{
    const auto calib_pa_name = wxString::Format(L"pressure_advance_test");
    ...
    if (params.bed_type == BedType::btPTE) {
        filament_config->set_key_value("textured_plate_temp", new ConfigOptionInts(1, (int) params.bed_temp));
        filament_config->set_key_value("textured_plate_temp_initial_layer", new ConfigOptionInts(1, (int) params.bed_temp));
    } else if (params.bed_type == BedType::btDEF) {
        filament_config->set_key_value("customized_plate_temp", new ConfigOptionInts(1, (int) params.bed_temp));
        filament_config->set_key_value("customized_plate_temp_initial_layer", new ConfigOptionInts(1, (int) params.bed_temp));
    } else if (params.bed_type == BedType::btER) {
        filament_config->set_key_value("epoxy_resin_plate_temp", new ConfigOptionInts(1, (int) params.bed_temp));
        filament_config->set_key_value("epoxy_resin_plate_temp_initial_layer", new ConfigOptionInts(1, (int) params.bed_temp));
    } else {
        filament_config->set_key_value("hot_plate_temp", new ConfigOptionInts(1, (int) params.bed_temp));
        filament_config->set_key_value("hot_plate_temp_initial_layer", new ConfigOptionInts(1, (int) params.bed_temp));
    }
    switch (params.mode) {
...
void Plater::_calib_pa_tower(const Calib_Params& params)
{
    ...
    p->set_project_name("tower_with_seam");
    if (!model().objects.empty()) {
    ...
    auto wall_speed = CalibPressureAdvance::find_optimal_PA_speed(
        full_config, full_config.get_abs_value("line_width", nozzle_diameter)==0?nozzle_diameter: full_config.get_abs_value("line_width", nozzle_diameter),
...
void Plater::calib_flowrate(int pass, float flowrate)
{
    if (pass != 1 && pass != 2)
        return;
    ...
    double layer_height       = std::round(nozzle_diameter / 2.0 * 100) / 100; // prefer 0.2 layer height for 0.4 nozzle
    first_layer_height        = std::max(first_layer_height, layer_height);
    first_layer_height        = std::round(first_layer_height * 100) / 100;
    ...
    for (auto _obj : model().objects) {
        ...
        _obj->config.set_key_value("print_flow_ratio", new ConfigOptionFloat(flowrate * (1.0f + modifier / 100.f)));
    }
...
void Plater::calib_Arc_lerance()
{
    const auto calib_name = wxString::Format(L"Arc fitting test");
    if (new_project(false, false, calib_name) == wxID_CANCEL)
        return;

    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "arc" / "arc.3mf").string());
    ...
    Flow   infill_flow                   = Flow(nozzle_diameter * 1.2f, layer_height, nozzle_diameter);
    double filament_max_volumetric_speed = filament_config->option<ConfigOptionFloats>("filament_max_volumetric_speed")->get_at(0);
    double max_infill_speed              = filament_max_volumetric_speed / (infill_flow.mm3_per_mm());
    double internal_solid_speed          = std::floor(std::min(print_config->opt_float("internal_solid_infill_speed"), max_infill_speed));
    double top_surface_speed             = std::floor(std::min(print_config->opt_float("top_surface_speed"), max_infill_speed));
    ...
    for (auto _obj : model().objects) {
        _obj->ensure_on_bed();
        print_config->set_key_value("enable_arc_fitting", new ConfigOptionBool{true});

        // extract flowrate from name, filename format: flowrate_xxx
        std::string obj_name = _obj->name.substr(0, _obj->name.rfind("."));
        int         modifier = stof(obj_name);
        _obj->config.set_key_value("arc_tolerance", new ConfigOptionInt(modifier));
    }
    ...
    Calib_Params _params;
    _params.mode = CalibMode::Calib_Arc2Lerance;
    p->background_process.fff_print()->set_calib_params(_params);
}

void Plater::calib_temp(const Calib_Params& params)
{
    const auto calib_temp_name = wxString::Format(L"temperature_tower");
    ...
    set_calib_model(false);
    p->background_process.fff_print()->set_calib_params(params);
}

void Plater::calib_max_vol_speed(const Calib_Params& params)
{
    const auto calib_vol_speed_name = wxString::Format(L"Max_volumetric_speed");
    ...
    set_calib_model(true);
    add_model(false, Slic3r::resources_dir() + "/calib/volumetric_speed/Max_volumetric_speed.step");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".step");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 5) {
            // 如果找到并且.step 是后缀，则删除该后缀
            model().objects[0]->name = model().objects[0]->name.substr(0, pos);
        }
    }
...
{
    const auto calib_retraction_name = wxString::Format(L"retraction_tower");
...
void Plater::calib_retraction_speed(const Calib_Params& params)
{
    const auto calib_retraction_name = wxString::Format(L"retraction_speed_tower");
    if (new_project(false, false, calib_retraction_name) == wxID_CANCEL)
        return;

    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Retraction_tower_speed)
        return;

    add_model(false, Slic3r::resources_dir() + "/calib/retraction/retraction_speed_tower.stl");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".stl");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 4) {
            // 如果找到并且.stl 是后缀，则删除该后缀
...
void Plater::calib_limit_speed(const Calib_Params& params)
{
    const auto _name = wxString::Format(L"limitSpeed");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;

    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Limit_Speed)
        return;

    set_calib_model(true);
    add_model(false, Slic3r::resources_dir() + "/calib/limit_speed/limitSpeed.stl");
    ...
    print_config->set_key_value("acceleration_limit_mess_enable", new ConfigOptionBool(false));
    print_config->set_key_value("speed_limit_to_height_enable", new ConfigOptionBool(false));
    print_config->set_key_value("top_shell_layers", new ConfigOptionInt(0));
    print_config->set_key_value("bottom_shell_layers", new ConfigOptionInt(0));
    print_config->set_key_value("default_acceleration", new ConfigOptionFloat((int) params.acc_speed));
    print_config->set_key_value("inner_wall_acceleration", new ConfigOptionFloat((int) params.acc_speed));
    print_config->set_key_value("outer_wall_acceleration", new ConfigOptionFloat((int) params.acc_speed));
    print_config->set_key_value("sparse_infill_acceleration", new ConfigOptionFloatOrPercent((int) params.acc_speed, false));
    print_config->set_key_value("sparse_infill_pattern", new ConfigOptionEnum<InfillPattern>(InfillPattern::ipGrid));
    ...
    //  cut upper
    auto obj_bb = obj->bounding_box_exact();
    auto height = ((params.end - params.start) / params.step + 1) * 5;
    if (height < obj_bb.size().z()) {
        cut_horizontal(0, 0, height, ModelObjectCutAttribute::KeepLower);
    }
...
void Plater::calib_speed_tower(const Calib_Params& params)
{
    const auto _name = wxString::Format(L"speedtower");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Speed_Tower)
        return;

    add_model(false, Slic3r::resources_dir() + "/calib/speed_tower/speedtower.stl");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".stl");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 4) {
            // 如果找到并且.stl 是后缀，则删除该后缀
...
void Plater::calib_jitter_speed(const Calib_Params& params)
{
    const auto _name = wxString::Format(L"X_Y_Jerk");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_X_Y_Jerk)
        return;

    add_model(false, Slic3r::resources_dir() + "/calib/x_y_jerk/X_Y_Jerk.stl");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".stl");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 4) {
            // 如果找到并且.stl 是后缀，则删除该后缀
...
{
    const auto _name = wxString::Format(L"fanSpeed");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Fan_Speed)
        return;

    add_model(false, Slic3r::resources_dir() + "/calib/fan_speed/fanSpeed.stl");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".stl");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 4) {
            // 如果找到并且.stl 是后缀，则删除该后缀
    ...
    filament_config->set_key_value("slow_down_layer_time", new ConfigOptionFloats{0.0});
    filament_config->set_key_value("fan_cooling_layer_time", new ConfigOptionFloats{0.0});
    ...
    filament_config->set_key_value("enable_special_area_additional_cooling_fan", new ConfigOptionBools{false});
    if (params.bed_type == BedType::btPTE) {
        filament_config->set_key_value("textured_plate_temp", new ConfigOptionInts(1, (int) params.bed_temp));
        filament_config->set_key_value("textured_plate_temp_initial_layer", new ConfigOptionInts(1, (int) params.bed_temp));
    } else if (params.bed_type == BedType::btDEF) {
    ...
    //  cut upper
    auto obj_bb = obj->bounding_box_exact();
    auto height = ((params.end - params.start) / params.step + 1) * 7.8;
    if (height < obj_bb.size().z()) {
        cut_horizontal(0, 0, height, ModelObjectCutAttribute::KeepLower);
    }
...
void Plater::calib_limit_acc(const Calib_Params& params)
{
    const auto _name = wxString::Format(L"limitSpeed");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Limit_Acceleration)
        return;
    ...
    print_config->set_key_value("acceleration_limit_mess_enable", new ConfigOptionBool(false));
    print_config->set_key_value("speed_limit_to_height_enable", new ConfigOptionBool(false));
    print_config->set_key_value("inner_wall_speed", new ConfigOptionFloat((int) params.print_speed));
    print_config->set_key_value("outer_wall_speed", new ConfigOptionFloat((int) params.print_speed));
    print_config->set_key_value("sparse_infill_speed", new ConfigOptionFloat((int) params.print_speed));
    print_config->set_key_value("gap_infill_speed", new ConfigOptionFloat((int) params.print_speed));
    print_config->set_key_value("sparse_infill_pattern", new ConfigOptionEnum<InfillPattern>(ipGrid));
...
void Plater::calib_acc_tower(const Calib_Params& params)
{
    const auto _name = wxString::Format(L"accSpeedTower");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Acceleration_Tower)
        return;

    add_model(false, Slic3r::resources_dir() + "/calib/acc_speed_tower/accSpeedTower.stl");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".stl");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 4) {
            // 如果找到并且.stl 是后缀，则删除该后缀
...
void Plater::calib_dec_acc(const Calib_Params& params)
{
    const auto _name = wxString::Format(L"accel2decel");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    if (params.mode != CalibMode::Calib_Accel2Decel)
        return;

    add_model(false, Slic3r::resources_dir() + "/calib/dec_speed/accel2decel.stl");
    if (!model().objects.empty()) {
        size_t pos = model().objects[0]->name.rfind(".stl");
        if (pos != std::string::npos && pos == model().objects[0]->name.length() - 4) {
            // 如果找到并且.stl 是后缀，则删除该后缀
    ...
    print_config->set_key_value("top_shell_layers", new ConfigOptionInt(0));
    print_config->set_key_value("bottom_shell_layers", new ConfigOptionInt(0));
    print_config->set_key_value("accel_to_decel_enable", new ConfigOptionBool{true});
    if (params.bed_type == BedType::btPTE) {
        filament_config->set_key_value("textured_plate_temp", new ConfigOptionInts(1, (int) params.bed_temp));
        filament_config->set_key_value("textured_plate_temp_initial_layer", new ConfigOptionInts(1, (int) params.bed_temp));
    } else if (params.bed_type == BedType::btDEF) {
    ...
    //  cut upper
    auto obj_bb = obj->bounding_box_exact();
    auto height = ((params.start - params.end) * params.high_step / params.step + params.high_step);
    if (height < obj_bb.size().z()) {
        cut_horizontal(0, 0, height, ModelObjectCutAttribute::KeepLower);
    }
...
{
    const auto _name = wxString::Format(L"VFA");
    if (new_project(false, false, _name) == wxID_CANCEL)
        return;
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
...
void Plater::load_gcode(const wxString& filename)
{
...
#if _DEBUG
    {
        Slic3r::DynamicPrintConfig full_config   = wxGetApp().preset_bundle->full_config();
        ConfigOptionFloats*        wipe_x_option = dynamic_cast<ConfigOptionFloats*>(full_config.option("wipe_tower_x"));
        ConfigOptionFloats*        wipe_y_option = dynamic_cast<ConfigOptionFloats*>(full_config.option("wipe_tower_y"));

        if (wipe_x_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 1 wipe_tower_x : %1%.") % wipe_x_option->get_at(0);
        if (wipe_y_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 1 wipe_tower_y : %1%.") % wipe_y_option->get_at(0);
    }
#endif
    ...
    //*current_result = std::move(processor.extract_result()); 
     current_result->take(processor.extract_result());  //optimize:replace copy assign
    // current_result->filename = filename;
    }
    // Creality: apply GCode thumbnail
    if (current_result) {
        int maxIndex = current_result->image_data.size() - 1;
        if (maxIndex >= 0) {
            p->m_gcode_thumbnail.width  = current_result->image_data[maxIndex].first[0];
            p->m_gcode_thumbnail.height = current_result->image_data[maxIndex].first[1];
            p->m_gcode_thumbnail.pixels = current_result->image_data[maxIndex].second;

            std::string   path = get_gcode_picture_path();
            fs::path      filePath(path);
            std::ofstream outFile(path, std::ios::out | std::ios::binary);
            if (outFile.is_open()) {
                outFile.write((const char*) current_result->image_data[maxIndex].second.data(),
                              current_result->image_data[maxIndex].second.size());
                outFile.close();
            }
        }
    }
    ...
    // cache wipe tower x,y
    DynamicPrintConfig& proj_cfg      = wxGetApp().preset_bundle->project_config;
    ConfigOptionFloats* wipe_x_option = dynamic_cast<ConfigOptionFloats*>(proj_cfg.option("wipe_tower_x"));
    ConfigOptionFloats* wipe_y_option = dynamic_cast<ConfigOptionFloats*>(proj_cfg.option("wipe_tower_y"));
    float               wipe_x        = wipe_x_option->get_at(0);
    float               wipe_y        = wipe_y_option->get_at(0);

    current_print.apply(this->model(), wxGetApp().preset_bundle->full_config());

    // BBS: add cost info when drag in gcode
    // Creality: add weight, filament length info
    ...
    current_print.set_print_uuid(current_result->gcode_uuid);
...
#if _DEBUG
    {
        ...
        if (wipe_x_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 2 wipe_tower_x : %1%.") % wipe_x_option->get_at(0);
        if (wipe_y_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 2 wipe_tower_y : %1%.") % wipe_y_option->get_at(0);
    }
#endif

    // when load gcode file, need to calculate the gcode path bounding box
    int current_palate_index = p->partplate_list.get_curr_plate_index();
    p->check_gcode_path_contain_in_bed(current_palate_index);
    ...
    // restore wipe tower x,y
    ConfigOptionFloat wt_x_opt(wipe_x);
    ConfigOptionFloat wt_y_opt(wipe_y);
    wipe_x_option->set_at(&wt_x_opt, 0, 0);
    wipe_y_option->set_at(&wt_y_opt, 0, 0);
#if _DEBUG
    {
        Slic3r::DynamicPrintConfig full_config   = wxGetApp().preset_bundle->full_config();
        ConfigOptionFloats*        wipe_x_option = dynamic_cast<ConfigOptionFloats*>(full_config.option("wipe_tower_x"));
        ...
        if (wipe_x_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" final wipe_tower_x : %1%.") % wipe_x_option->get_at(0);
        if (wipe_y_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" final wipe_tower_y : %1%.") % wipe_y_option->get_at(0);
    }
#endif
    ...
    try
    {
        if(wxGetApp().is_privacy_checked()) {
            ...
            js["file_path"] = filename.ToStdString();
            js["file_size"] = file_size_str;
            js["operation_date"] = Slic3r::Utils::utc_timestamp(Slic3r::Utils::get_current_time_utc());
            js["app_version"] = GUI_App::format_display_version().c_str();
...
#if _DEBUG
    {
        ...
        if (wipe_x_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 4 wipe_tower_x : %1%.") % wipe_x_option->get_at(0);
        if (wipe_y_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 4 wipe_tower_y : %1%.") % wipe_y_option->get_at(0);
    }
#endif
}
...
void Plater::update_all_plate_thumbnails(bool force_update)
{
    ...
    if (hasChange && is_preview_shown())
        get_preview_canvas3D()->update_plate_thumbnails(true);
}
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_confirm->SetBorderColor(wxColour(255, 255, 255));
...
void ProjectDropDialog::select_radio(int index)
{
    ...
    while (it) {
        RadioSelector* rs = it->GetData();
        if (rs->m_groupid == groupid && rs->m_select_id == index)
...
void ProjectDropDialog::on_select_radio(wxMouseEvent& event)
{
    ...
    while (it) {
        RadioSelector* rs = it->GetData();
        if (rs->m_radiobox->GetId() == event.GetId())
            groupid = rs->m_groupid;
        it = it->GetNext();
    }
...
void ProjectDropDialog::on_select_cancel(wxMouseEvent& event) { EndModal(wxID_CANCEL); }
...
bool Plater::load_files(const wxArrayString& filenames)
{
    ...
    // BBS: check file types
    // std::sort(normal_paths.begin(), normal_paths.end(), [](fs::path obj1, fs::path obj2) { return obj1.filename().string() <
    // obj2.filename().string(); });
...
int Plater::get_3mf_file_count(std::vector<fs::path> paths)
{
    ...
    for (const auto& path : paths) {
        if (wxString(encode_path(path.filename().string().c_str())).EndsWith("3mf"))
            count++;
        else if (wxString(encode_path(path.filename().string().c_str())).EndsWith("cxprj")) {
            count++;
        }
    }
    return count;
...
bool Plater::isSliceAll() const { return p->m_slice_all; }
...
{
    if (p->model.objects.empty() && !only_gcode_mode())
        return;
    ...
    try {
        ...
        if (!only_gcode_mode()) {
            ...
            default_output_file = get_last_loaded_gcode();
        }
    ...
    fs::path output_path;
    {
        ...
        if (dlg.ShowModal() == wxID_OK) {
            ...
            if (output_path.extension() != ".gcode")
    ...
    if (!output_path.empty()) {
        ...
        if (!only_gcode_mode()) {
            ...
            fs::copy_file(default_output_file, output_path);
        }
...
void Plater::send_to_local_net_printer(bool isall) { p->on_action_send_to_local_net_printer(isall); }

void Plater::upload_3mf(bool isall) { p->on_action_upload_3mf(isall); }
...
void Plater::export_cxprj()
{
    wxString path = p->get_export_file(FT_CXPROJECT);
    if (path.empty()) {
        return;
    }
    const std::string path_u8 = into_u8(path);
...
void Plater::save_the_popup_successfully(const string& strPath)
{
    if (strPath.empty()) {
        return;
    }
    ...
    if (fs::exists(fsPath)) {
        MessageDialog msg(this, _L("Saved successfully. Open Local Folder"), _L("Information"), wxYES_NO | wxYES_DEFAULT);
        if (msg.ShowModal() == wxID_YES) {
            wxLaunchDefaultApplication(from_u8(fsPath.parent_path().string()));
        }
    }
}
...
// BBS: backup
int Plater::export_3mf(
    const boost::filesystem::path& output_path, SaveStrategy strategy, int export_plate_idx, Export3mfProgressFn proFn, En3mfType type_3mf)
{
    int ret = 0;
    // if (p->model.objects.empty()) {
    ...
    bool     is_3mf        = path.Lower().EndsWith(".3mf");
    bool     is_cxprj      = path.Lower().EndsWith(".cxprj");
    ...
    store_params.type_3mf                   = type_3mf;
...
void Plater::reslice()
{
    ...
    if (model().calib_pa_pattern) {
        ...
        model().calib_pa_pattern->generate_custom_gcodes(wxGetApp().preset_bundle->full_config(), preset_bundle->is_bbl_vendor(), model(),
...
void Plater::set_gcode_bed_shape(const Pointfs& shape, const Pointfs& exclude_area, const double printable_height, const Preset& preset) const
{
    p->set_gcode_bed_shape(shape, exclude_area, printable_height, preset);
}
...
{
    if (result->creality_extruder_colors.empty()) {
        ...
        return result->creality_complete_extruder_colors;
    }
}
...
void Plater::clear_before_change_mesh(int obj_idx, bool simplify)
{
    ...
    if (paint_removed) {
        ...
        get_notification_manager()->push_notification(NotificationType::CustomSupportsAndSeamRemovedAfterRepair,
                                                      NotificationManager::NotificationLevel::PrintInfoNotificationLevel,
                                                      simplify ?
                                                          _u8L("Custom supports and color painting were removed before simplified.") :
...
bool Plater::init_process_toolbar() { return p->init_process_toolbar(); }
...
std::string Plater::get_gcode_picture_path() { return data_dir() + "/preview.png"; }

ThumbnailData Plater::get_gcode_thumbnail() { return p->m_gcode_thumbnail; }
            ...
            if (is_preview_shown()) {
                if (need_slice) { // from preview's thumbnail
                    if ((invalidated & PrintBase::APPLY_STATUS_INVALIDATED) || (gcode_result->moves.empty())) {
                        ...
                        for (int i = 0; i < p->partplate_list.get_plate_count(); i++) {
                            int tmp_plate_index = p->partplate_list.get_plate(i)->get_index();
                            if (plate_index != tmp_plate_index) {
                                p->notification_manager->close_plate_critical_slicing_notification(tmp_plate_index);
                            }
                        }

                        p->notification_manager->check_plate_critical_slicing_notification(plate_index);
...
int Plater::select_plate_by_hover_id(int hover_id, bool right_click, bool isModidyPlateName)
{
    ...
    if (action == PartPlate::e_at_unlock_triangle) {
        // select plate
        ret = p->partplate_list.select_plate(plate_index);
        if (!ret) {
            SimpleEvent event(EVT_GLCANVAS_PLATE_SELECT);
    ...
    } else if ((action == PartPlate::e_at_close) && (!right_click)) {
        // delete plate
        ret = delete_plate(plate_index);
    } else if ((action == PartPlate::e_at_dir) && (!right_click)) {
        // arrange the plate
    ...
    } else if ((action == PartPlate::e_at_lay) && (!right_click)) {
        // arrange the plate
    ...
    } else if ((action == PartPlate::e_at_edit) && (!right_click)) {
        // set the plate type
        ret = select_plate(plate_index);
        if (!ret) {
            static PlateNameEditDialog dlg(this, wxID_ANY, _L("Edit Plate Name"));
            PartPlate*                 curr_plate = p->partplate_list.get_curr_plate();
...
void Plater::show_object_info()
{
    ...
    if (imperial_units)
        ...
        string s0 = (boost::format(_utf8("%.2f")) % size(0)).str();
        string s1 = (boost::format(_utf8("%.2f")) % size(1)).str();
        string s2 = (boost::format(_utf8("%.2f")) % size(2)).str();
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8(
            "\n" + _L("Tips:") + "\n" +
            _L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Creality Print(windows) or CAD softwares."));
    }
#endif // APPLE & LINUX
    ...
    if (err.type == StringExceptionType::STRING_EXCEPT_FILAMENT_NOT_MATCH_BED_TYPE) {
        try {
            ...
            if (extruder_id < preset_bundle->filament_presets.size()) {
                ...
                err.string = format(_L("Plate% d: %s is not suggested to be used to print filament %s(%s). If you still want to do this "
...
const CollapseBar::GLToolbar& Plater::get_collapse_toolbar() const { return p->collapse_toolbar; }

CollapseBar::GLToolbar& Plater::get_collapse_toolbar() { return p->collapse_toolbar; }

const ProcessBar::GLToolbar& Plater::get_process_toolbar() const { return p->process_toolbar; }

ProcessBar::GLToolbar& Plater::get_process_toolbar() { return p->process_toolbar; }
...
void Plater::invalid_slice_result_need_reslice()
{
    reset_gcode_toolpaths();
    get_view3D_canvas3D()->reset_sequential_print_clearance();
...
void Plater::select_bed_type(const std::string& bed_type_name) { p->did_select_bed_type(bed_type_name); }

void Plater::set_bed_type_list(std::map<int, int>& bed_type_list) { m_bed_type_list_creality.swap(bed_type_list); }
...
//      Epoxy Resin Plate
//
int Plater::get_bed_type_list(int index)
{
    auto iter = m_bed_type_list_creality.find(index + 1);
    if (iter != m_bed_type_list_creality.end())
        return iter->second - 1;

    return (m_bed_type_list_creality.size() > 0) ? m_bed_type_list_creality.begin()->first : 2;
}

int Plater::get_bed_type_list_second(int index)
{
    for (auto iter : m_bed_type_list_creality) {
        if (iter.second == index) {
            return iter.first;
        }
    }

    return (m_bed_type_list_creality.size() > 0) ? m_bed_type_list_creality.begin()->first : 0;
}

BoundingBoxf3      Plater::get_bed_extended_bounding_box()
{ 
    return p->bed.extended_bounding_box();
}

void Plater::show_send_localnet_printer_dlg(bool show) {}

void Plater::create_send_to_printer_dlg() {}

std::vector<int> Plater::get_gcode_extruders_in_only_gcode_mode()
{
    std::vector<int>      out_extruders;
    GCodeProcessorResult* current_result = get_partplate_list().get_current_slice_result();
    if (current_result) {
        auto&  ps         = current_result->print_statistics;
        double total_cost = 0.0;
...
void Plater::check_sidebar_state_in_only_gcode_mode() { p->check_sidebar_state_in_only_gcode_mode(); }
...
wxMenu* Plater::filament_action_menu(int active_filament_menu_id) { 
    p->sidebar->set_menu_filement_id(active_filament_menu_id);
    return p->menus.filament_action_menu(active_filament_menu_id); 
}
