    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // BBS
    // auto maaping button
    p->m_auto_mapping_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString, is_dark ? "auto_mapping_dark" : "auto_mapping_light", wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
    ...
    // add wiping dialog   
    p->m_flushing_volume_btn = new  HoverBorderIcon(p->m_panel_filament_title, wxEmptyString, is_dark ? "flush_dark_default" : "flush_light_default", wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
    ...
    p->m_flushing_volume_btn->Bind(wxEVT_LEFT_DOWN, ([parent](auto &e)
        {
            auto& project_config = wxGetApp().preset_bundle->project_config;
    ...
    HoverBorderIcon* add_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString, is_dark ? "addMaterial_dark_default" : "addMaterial_light_default", wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
    ...
    HoverBorderIcon* del_btn =  new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString, is_dark ? "delMaterial_dark_default" : "delMaterial_light_default", wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
    ...
    HoverBorderIcon* set_btn = new HoverBorderIcon(p->m_panel_filament_title, wxEmptyString, is_dark ? "setting_dark_default" : "setting_light_default", wxDefaultPosition, COMMON_HOVER_BORDER_ICON_SIZE, wxTE_PROCESS_ENTER);
    ...
    p->m_cx_panel_box_filament->setExactMaterialCb([&](bool has_exact_material) { p->m_auto_mapping_btn->setEnable(has_exact_material); });
 ...
 //   p->pSearchBk = new StaticBox(p->scrolled);
	//p->pSearchBk->SetBorderColor(StateColor(std::make_pair(0xDBDBDB, (int)StateColor::Disabled), std::make_pair(0x15BF59, (int)StateColor::Hovered),
	//	std::make_pair(0xDBDBDB, (int)StateColor::Normal)));
 //   //p->pSearchBk->SetBackgroundColor(StateColor(std::make_pair(0xF0F0F1, (int)StateColor::Disabled), std::make_pair(*wxWHITE, (int)StateColor::Normal)));
 //   p->pSearchBk->SetCornerRadius(0);
 //   p->pSearchBk->SetBorderWidth(1);
 ...
 //   p->m_search_bar = new wxSearchCtrl(p->pSearchBk, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER | wxBORDER_NONE);
 //   p->m_search_bar->SetBackgroundColour(wxColour("#FFFFFF"));
 //   sizer_search->Add(p->m_search_bar, 0, wxALL | wxEXPAND, 1);
...
void Sidebar::update_all_preset_comboboxes()
{
    ...
    if (preset_bundle.use_bbl_network()) {
        ...
        if(nullptr!=connection_btn) connection_btn->Hide();
     ...
     //   ams_btn->Hide();
        ...
        if(url.empty())
            ...
            print_btn_type = preset_bundle.is_bbl_vendor() ? MainFrame::PrintSelectType::ePrintPlate : MainFrame::PrintSelectType::eSendToLocalNetPrinter;
        }
...
void Sidebar::sys_color_changed()
{
    ...
    if(nullptr != p->m_printer_icon)p->m_printer_icon->msw_rescale();
    if(nullptr != p->m_printer_setting)p->m_printer_setting->msw_rescale();
    p->m_filament_icon->msw_rescale();
    p->m_bpButton_add_filament->msw_rescale();
    p->m_bpButton_del_filament->msw_rescale();
    p->m_bpButton_set_filament->msw_rescale();
...
void Sidebar::update_filament_panel() 
{
    p->m_cx_panel_filament_content->update();
}
//Creality 
...
void Sidebar::add_filament() {
    ...
    if (!p->m_cx_panel_filament_content->can_add()) return;
    wxColour    new_col        = Plater::get_next_color_for_filament();
...
void Sidebar::sync_ams_list()
{
    ...
    if (list.empty()) {
        MessageDialog dlg(this,
            _L("No CFS filaments. Please select a printer in 'Device' page to load CFS info."),
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update Creality Print or restart Creality Print to check if there is an update to system presets."),
            _L("Sync filaments with CFS"), wxOK);
        dlg.ShowModal();
    }
...
{
    return p->m_cx_panel_filament_content;
}
...
{ 
    return p->m_cx_panel_filament_content->size() > 1;
}
...
int Sidebar::filament_size()
{
    return p->m_cx_panel_filament_content->size();
}


void Sidebar::update_filament(int index)
{ 
    p->m_cx_panel_filament_content->update(index); 
}
...
void Sidebar::set_edit_filament(int index)
{
    p->editing_filament = index;
}
...
Slic3r::GUI::SidebarPrinter& Plater::sidebar_printer()
{
    return *p->sidebar_printer;
}

void Plater::set_calib_model(bool _value) 
{ 
    p->m_is_calib_model = _value; 
}

void Plater::update_plate_thumbnail(wxCommandEvent& event)
{
    p->on_notify_update_plate_thumbnail(event);
}
       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
                    ...
                    if ((project_presets.size() > 0) && load_config) {
                        ...
                        for (unsigned int i = 0; i < project_presets.size(); i++) {
                            if (project_presets[i]->type == Preset::TYPE_PRINTER) {
                                ...
                                {
                                    const ConfigOption* single_extruder = project_presets[i]->config.option("single_extruder_multi_material");
                ...
                if (load_config) {
                    if (!config.empty()) {
                        ...
                        if(creality_3mf_old) {
                           ...
                           std::string printer_model = internalName2PrinterName(config.option<ConfigOptionString>("printer_model", true)->value);
                           std::string printer_settings_id = internalName2PrinterName(config.option<ConfigOptionString>("printer_settings_id", true)->value);
                           ...
                           for(unsigned int index = 0; index < filament_preset_name.size(); index ++)
                           {
                               std::string filament_settings_id = filament_preset_name[index];
                                ...
                                std::string officail_filament_name = (boost::format("%s @%s")%filament_settings_id%printer_settings_id).str();
                          ...
                          if(print_settings_id.find("@") != std::string::npos) {
                           ...
                           if(process_preset != nullptr)
                           {
                            ...
                            std::vector<std::string> current_different_settings = config.option<ConfigOptionStrings>("different_settings_to_system", true)->values;
                           ...
                           //process inherits
                            if(isPrinterInherit || current_different_settings.size()>=3 && current_different_settings[2].empty())
                            {
                                ...
                                if(preset != nullptr)
                                {
                                    ...
                                    {
                                        if(option == "default_filament_profile" || option == "default_print_profile" || option == "inherits" || option == "printer_model" || option == "printer_settings_id" || option == "support_multi_bed_types")
                                        {
                                            continue;
                                        }
                                        if(file_version.patch()<5)
                                        {
                                            if(option == "extruder_colour" || option == "retract_when_changing_layer" || option == "deretraction_speed" || option == "max_layer_height" || option == "retract_before_wipe"|| option == "retraction_speed" || option == "wipe"|| option == "wipe_distance")
                                            {
                                                continue;
                                            }
                                        }
                                    ...
                                    {
                                        current_different_settings[current_different_settings.size()-1] = current_different_settings[current_different_settings.size()-1] + current_different_settings_str;
                                    }else{
                                        current_different_settings.push_back("");
                                        current_different_settings.push_back("");
                                        current_different_settings.push_back(current_different_settings_str);
                                    }
                                    config.set_key_value("different_settings_to_system",new ConfigOptionStrings(current_different_settings));
        ...
        if (load_model) {
             ...
             if(creality_3mf_old)
            {
                ...
                model.convert_from_creality5(Vec2d(current_width,current_depth),Vec2d(partplate_list.plate_stride_x(),partplate_list.plate_stride_y()),plate_count);
            }
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    switch (file_type) {
        ...
        case FT_PROJECT: wildcard = GUI::format_wxstr("%s|%s", file_wildcards(FT_3MF), file_wildcards(FT_CXPROJECT));
            break;
        default:
    ...
    wxFileDialog dlg(q, dlg_title,
        is_shapes_dir(out_dir) ? from_u8(wxGetApp().app_config->get_last_dir()) : from_path(output_file.parent_path()), from_path(output_file.stem()),
...
void Plater::priv::on_mapping_device_filament(wxCommandEvent& event)
{
    wxGetApp().sidebar().on_mapping_device_filament(event);
}

void Plater::priv::on_show_box_color_selection(wxCommandEvent& event)
{
    wxGetApp().sidebar().on_show_box_color_selection(event);
}
...
bool Plater::priv::init_process_toolbar()
{
    ...
    process_toolbar.set_icons_size(30*sc);
...
void Plater::priv::set_gcode_bed_shape(const Pointfs& shape, const Pointfs& exclude_areas, const double printable_height, const Preset& preset)
{
    //Orca: reduce resolution for large bed printer
    BoundingBoxf bed_size = get_extents(shape);
    if (bed_size.size().maxCoeff() <= LARGE_BED_THRESHOLD)
...
//        if (!m_agent) return;
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_complete([&cont, &download_ok, tmp_path, &target_path, extension](std::string body, unsigned /* http_status */) {
                    ...
                    if (extension.Contains(".gz")) {
                        ...
                        in.push(infile);
                        boost::filesystem::path stem = boost::filesystem::path(target_path).parent_path() / boost::filesystem::path(target_path).stem();
    ...
    if (download_ok) {
        ...
        if (target_path.extension() != ".3mf")
        {
            ...
            {
                boost::filesystem::path stemPath = boost::filesystem::path(target_path).parent_path() / boost::filesystem::path(target_path).stem();
    ...
    *current_result = std::move(processor.extract_result());
    ...
    if (current_result) {
        ...
        if (maxIndex >= 0) {
            ...
            if (outFile.is_open()) {
                outFile.write((const char*) current_result->image_data[maxIndex].second.data(), current_result->image_data[maxIndex].second.size());
                outFile.close();
            }
        }
    }
...
void Plater::update_all_plate_thumbnails(bool force_update)
{
    ...
    for (int i = 0; i < get_partplate_list().get_plate_count(); i++) {
        ...
        if (force_update || !plate->no_light_thumbnail_data.is_valid()) {
            get_view3D_canvas3D()->render_thumbnail(plate->no_light_thumbnail_data, plate->plate_thumbnail_width, plate->plate_thumbnail_height, thumbnail_params, Camera::EType::Ortho, false, false,
                                                    true);
            hasChange = true;
        }
    }
...
void Plater::send_to_local_net_printer(bool isall)
{
    p->on_action_send_to_local_net_printer(isall);
}

void Plater::upload_3mf(bool isall)
{
    p->on_action_upload_3mf(isall);
}
...
// BBS: backup
int Plater::export_3mf(const boost::filesystem::path& output_path, SaveStrategy strategy, int export_plate_idx, Export3mfProgressFn proFn, En3mfType type_3mf)
{
    int ret = 0;
    //if (p->model.objects.empty()) {
...
bool Plater::init_process_toolbar()
{
    return p->init_process_toolbar();
}
...
std::string Plater::get_gcode_picture_path() 
{ 
    return data_dir() + "/preview.png"; 
}

ThumbnailData Plater::get_gcode_thumbnail() 
{ 
    return p->m_gcode_thumbnail; 
}
...
const CollapseBar::GLToolbar& Plater::get_collapse_toolbar() const
{
    return p->collapse_toolbar;
}

CollapseBar::GLToolbar& Plater::get_collapse_toolbar()
{
    return p->collapse_toolbar;
}

const ProcessBar::GLToolbar& Plater::get_process_toolbar() const
{
    return p->process_toolbar;
}

ProcessBar::GLToolbar& Plater::get_process_toolbar()
{
    return p->process_toolbar;
}
...
void Plater::select_bed_type(const std::string& bed_type_name) 
{ 
    p->did_select_bed_type(bed_type_name);
}

void Plater::set_bed_type_list(std::map<int, int>& bed_type_list) 
{ 
    m_bed_type_list_creality.swap(bed_type_list); 
}
