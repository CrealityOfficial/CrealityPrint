...
//        m_ProfileJson = gen_profile_json();
//        auto merger_profile_data = [&profile_json_mutex, this](json& patch) {
//            std::lock_guard<std::mutex> lk(profile_json_mutex);
//            // model 
//            if (patch.contains("model")) {
//                for (const auto& item : patch["model"]) {
//                    m_ProfileJson["model"].push_back(item);
//                }
//            }
//            // machine
//            if (patch.contains("machine")) {
//                m_ProfileJson["machine"].merge_patch(patch["machine"]);
//            }
//            // filament
//            if (patch.contains("filament")) {
//                m_ProfileJson["filament"].merge_patch(patch["filament"]);
//            }
//            // process
//            if (patch.contains("process")) {
//                for (const auto& item : patch["process"]) {
//                    m_ProfileJson["process"].push_back(item);
//                }
//            }
//        };
//
...
//                               [this, gen_profile_json, merger_profile_data](const boost::filesystem::path& path) {
...
//        });
        ...
        std::string g_vendor_name = "Creality";
    #endif
        ProfileFamilyLoader().ParallelLoadProfileJson(m_ProfileJson, m_MachineJson, bbl_bundle_rsrc, g_vendor_name);
...
//int GuideFrame::LoadProfileFamily(std::string strVendor, std::string strFilePath, json& outputJson)
//{
...
//            [this, &pmachine, vendor_dir, user_vendor_dir, bHasUserVendor, 
//            &mapInfo, &outputJson, &mapInfoMutex, &outputJsonMutex](const tbb::blocked_range<int>& range) {
...
//        tbb::parallel_for(tbb::blocked_range{0, nsize}, 
//            [this, &pmodels, bHasUserVendor, vendor_dir, 
//            user_vendor_dir, &outputJsonMutex, strVendor, 
//            &mapInfo, &outputJson](const tbb::blocked_range<int>& range) {
//            for (auto n = range.begin(); n != range.end(); ++n) {
...
//
//        tbb::parallel_for(tbb::blocked_range{0, nsize}, 
//            [this, &pFilament, &outputJson, &outputJsonMutex,
//            bHasUserVendor, vendor_dir, user_vendor_dir, &tFilaList](const tbb::blocked_range<int>& range) {
//            for (auto n = range.begin(); n != range.end(); n++) {
//                json OneFF = pFilament.at(n);
//
//                std::string s1 = OneFF["name"];
...
//        tbb::parallel_for(tbb::blocked_range{0, nsize}, 
//            [this, &pProcess, bHasUserVendor, vendor_dir, 
//            user_vendor_dir, &outputJson, &outputJsonMutex](const tbb::blocked_range<int>& range) {
//            for (auto n = range.begin(); n != range.end(); n++) {
