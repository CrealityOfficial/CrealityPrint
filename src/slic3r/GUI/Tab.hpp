...
/*#ifdef __WXOSX__
	...
	HoverBorderIcon*		m_btn_delete_preset;
	...
	std::vector<HoverBorderIcon*>	m_hover_border_icons = {};
...
public:
	...
	virtual void create_preset_tab();
	...
	void        save_preset(std::string name = std::string(), bool detach = false, bool save_to_project = false, bool from_input = false, std::string input_name = "", bool isSave = false);
	...
	std::vector<PageShp> get_pages() { return m_pages; }
	...
	void			select_item(wxString item);
...
public:
	...
	wxPanel* m_boxPanel = nullptr;
private:
	TabPresetComboBox* m_wcb_choiceVendor = nullptr;
	TabPresetComboBox* m_wcb_choicePrintType = nullptr;
};
...
class TabbarPresetCombox : public ::ComboBox
{
public:
	TabbarPresetCombox(wxWindow* parent, const wxSize& size = wxDefaultSize);
    ~TabbarPresetCombox();
...
class HTabbarPrinter : public wxPanel
{
public:
	HTabbarPrinter(ParamsPanel* parent, Preset::Type preset_type, PresetBundle* preset_bundle);

    void ShowPrinter(bool show);
    void ShowFilament(bool show);

    void SetMahineType(const wxString& type);
    void SetMahineName(const wxString& name);

	void OnUpdateVendor(const std::map<std::string, ProfileMachine>& machines, const std::string& vendor);
	void OnUpdateModel(const std::vector<const ProfileMachine*> machines, const std::string& model);

private:
	ParamsPanel* m_parent = nullptr;
	PresetBundle* m_preset_bundle{ nullptr };
	PresetCollection* m_printer_collection{ nullptr };
	PresetCollection* m_filament_collection{ nullptr };

	Label* m_label_machine_type = nullptr;
	Label* m_label_machine_name = nullptr;

	wxBoxSizer* m_machine_sizer = nullptr;
	wxBoxSizer* m_vendor_model_sizer = nullptr;

	TabbarPresetCombox* m_vendor_choice = nullptr;
	TabbarPresetCombox* m_model_choice = nullptr;

	StaticBox* m_search_box = nullptr;
	ScalableButton* m_search_button = nullptr;
	TextInput* m_search_text_input = nullptr;
	SwitchButton* m_mode_view = nullptr;
	wxStaticText* m_mode_title = nullptr;
};

struct TabbarDataViewModelNode
{
	int64_t index;
	TabbarDataViewModelNode* parent;
	bool is_container;
	std::vector<TabbarDataViewModelNode*> children;
	bool available = true;
	wxString text;
	std::string name;
};

class TabbarDataViewModel : public wxDataViewModel
{
public:
    TabbarDataViewModel() = default;

	~TabbarDataViewModel() override;

	unsigned int GetColumnCount() const override;

	wxString GetColumnType(unsigned int col) const override;

	wxString GetText(const wxDataViewItem& item) const;
	...
	wxDataViewItem AddRoot(const wxString& name);

	wxDataViewItem AddChild(const wxDataViewItem& parent, const wxString& child, const std::string& name);

	void DeleteChildren(const wxDataViewItem& parent);

	void SetAvailable(const wxDataViewItem& item);

    bool GetAttr(const wxDataViewItem& WXUNUSED(item), unsigned int WXUNUSED(col), wxDataViewItemAttr& WXUNUSED(attr)) const override;

private:
	wxDataViewItem _AddChild(int parent, const wxString& child, const std::string& name);
	std::pair<wxDataViewItem, wxDataViewItem> _CreateNode(int parent, const wxString& child, const std::string& name);

private:
	int m_next_index = 1;
	std::vector<int> m_roots;
	std::unordered_map<int64_t, TabbarDataViewModelNode> m_nodes;
};

class VTabbarPrinter : public wxPanel
{
public:
...
private:
	...
	TabbarDataViewModel m_printer_veiw_store;
	wxDataViewCtrl* m_priner_view;
	wxDataViewItem m_printer_user_item;
	wxDataViewItem m_printer_system_item;
};

class TabbarController
{
public:
	virtual ~TabbarController() = default;

	virtual void OnInit(Preset& preset) = 0;
	virtual void OnExit() = 0;

	virtual void OnSelectName(const wxString& name) = 0;
};


class TabbarCtrlPrinter : public TabbarController
{
public:
	TabbarCtrlPrinter(ParamsPanel* parent, HTabbarPrinter* h_tabbar, VTabbarPrinter* v_tabbar, PresetBundle* preset_bundle);

    ~TabbarCtrlPrinter() = default;

	void OnInit(Preset& preset) override;
	void OnExit() override;

	void OnSelectName(const wxString& name) override;

	void OnSelectWildVendorModel();
	void OnSelectWildVendor();
	void OnSelectWildModel(const wxString& vendor);
	void OnSelectVendor(const wxString& vendor);
	void OnSelectVendorModel(const wxString& vendor, const wxString& model);

private:
	void _SetPrimitivePreset(const Preset& preset);
	std::string _GetPrimitivePreset() const;

    void _SetCurrentPreset(Preset* preset);
    void _ResetCurrentPreset();

private:
	ParamsPanel* m_parent = nullptr;

	HTabbarPrinter* m_h_tabbar = nullptr;
	VTabbarPrinter* m_v_tabbar = nullptr;
	PresetCollection* m_printer_collection{ nullptr };

    std::string m_primitive_preset_name;
    bool m_current_preset_visible = false;
    Preset* m_current_preset = nullptr;

	std::unordered_set<std::string> m_presets;
	const std::map<std::string, ProfileMachine>* m_system_machines = nullptr;
};

class TabbarCtrlFilament : public TabbarController
{
public:
	TabbarCtrlFilament(ParamsPanel* parent, HTabbarPrinter* h_tabbar, VTabbarPrinter* v_tabbar, PresetBundle* preset_bundle);

    ~TabbarCtrlFilament() = default;

    void OnInit(Preset& preset) override;
	void OnExit() override;

    void OnSelectVendor(const wxString& vendor);
    void OnSelectModel(const wxString& model);
    void OnSelectName(const wxString& name) override;
