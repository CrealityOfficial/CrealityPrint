...
WebViewPanel::WebViewPanel(wxWindow *parent)
        : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
 {
    ...
    // wxString url     = wxString::Format("http://127.0.0.1:%d/#/",wxGetApp().get_server_port()); // 127.0.0.1 访问创想云 cdn 图片会 403，因此使用 localhost
    wxString url = wxString::Format("http://localhost:%d/homepage/index.html#/Community/HomePage", wxGetApp().get_server_port());


    // dev 使用局域网地址：可以找【刘明，或其他前端开发同事】，按要求启动一个 http 服务，用于调试
    // wxString url = wxString::Format("http://172.23.212.115:9090/index.html#/Community/HomePage");

    if (strlang != "")
        url = wxString::Format("%s?lang=%s", url, strlang);
    else
        url = wxString::Format("%s?lang=en_US", url, strlang);  // default language is English
    
    std::string versionExtra = std::string(PROJECT_VERSION_EXTRA);
    versionExtra.erase(std::remove(versionExtra.begin(), versionExtra.end(), ' '), versionExtra.end());
    std::string version = std::string(CREALITYPRINT_VERSION);
    version.erase(std::remove(version.begin(), version.end(), ' '), version.end());
    url = wxString::Format("%s&version=%s", url, version);
   // url = wxString::Format("%s&type=%s", url, "beta"); // alpha 还是 beta
    url = wxString::Format("%s&type=%s", url, versionExtra); // alpha 还是 beta

    std::string country_code = wxGetApp().app_config->get("region");
    std::string use_inches = wxGetApp().app_config->get("use_inches");
    url = wxString::Format("%s&region=%s", url, country_code);
    url = wxString::Format("%s&use_inches=%s", url, use_inches);
...
// tao
void WebViewPanel::update_all_device(const std::string& jsStr)
{
    RunScript(from_u8(jsStr));
}
