...
inline constexpr const char* kDecomposeHyperPlaType     = "Hyper PLA";
...
// Resolve the official (JSON-defined) hex for a base color by reading
// base_color_spectra.json via the closed-source K-M library. Falls back to
// the supplied `fallback` colour only if the resource is unavailable.
// `mode` selects the recipe family (CMYW / RYBW); pass the same mode the
// caller intends to feed into the K-M engine so the swatch and the
// recipe stay aligned.
