...
void wxMediaCtrl2::Load(wxURI url)
{
    ...
    }
    {
        ...
        if (path.empty() || !wxFile::Exists(path) || clsid != CLSID_BAMBU_SOURCE) {
            if (boost::filesystem::exists(dll_path)) {
                CallAfter(
                    [dll_path] {
                    int res = wxMessageBox(_L("has not correctly been registered for media playing! Press Yes to re-register it. You will be promoted twice"), _L("Error"), wxYES_NO);
                    if (res == wxYES) {
                ...
                CallAfter([] {
                    wxMessageBox(_L("Missing component registered for media playing! Please re-install or seek after-sales help."), _L("Error"), wxOK);
                });
            }
        ...
        if (path != dll_path) {
            ...
            if (!notified) CallAfter([dll_path] {
                int res = wxMessageBox(_L("Using a from a different install, video play may not work correctly! Press Yes to fix it."), _L("Warning"), wxYES_NO | wxICON_WARNING);
                if (res == wxYES) {
    ...
    if (!hasplugins) {
        CallAfter([] {
            wxMessageBox(_L("Your system is missing H.264 codecs for GStreamer, which are required to play video.  (Try installing the gstreamer1.0-plugins-bad or gstreamer1.0-libav packages, then restart Creality Print?)"), _L("Error"), wxOK);
        });
        m_error = 101;
        wxMediaEvent event(wxEVT_MEDIA_STATECHANGED);
        event.SetId(GetId());
