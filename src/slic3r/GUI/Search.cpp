...
bool OptionsSearcher::search(const std::string &search, bool force /* = false*/, Preset::Type type/* = Preset::TYPE_INVALID*/)
{
    ...
    auto need_add = [](const std::wstring& string, wchar_t delimiter)->bool {

        function split = [](const std::wstring& s, wchar_t delimiter)-> std::vector<std::wstring> {
            std::vector<std::wstring> tokens;
            boost::split(tokens, s, boost::is_any_of(std::wstring(1, delimiter)));
            return tokens;
        };

        std::vector<std::wstring> result = split(string, delimiter);
        bool is_creality_vendor = GUI::wxGetApp().preset_bundle->is_cx_vendor();
        if (is_creality_vendor && result.size() > 1)
        {
            if (wxString("cool_plate_temp_initial_layer") == result.at(1) 
                || wxString("eng_plate_temp_initial_layer") == result.at(1)
                || wxString("cool_plate_temp") == result.at(1)
                || wxString("eng_plate_temp") == result.at(1))
            {
                return false;
            }
        }
        ...
        {
            if (wxString("epoxy_resin_plate_temp_initial_layer") == result.at(1)
                || wxString("epoxy_resin_plate_temp") == result.at(1))
            {
                return false;
            }
        }
