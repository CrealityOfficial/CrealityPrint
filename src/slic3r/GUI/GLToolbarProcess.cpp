...
namespace ProcessBar {
	...
	float GLToolbar::get_main_size() const
	{
		...
		if (m_items.size()-num_hide_state > 1)
			size += ((float)(m_items.size()-num_hide_state) - 1.0f) * m_layout.gap_size;
			...
			if (0 <= item_id && item_id < (int)m_items.size()) {
				...
				if (item != nullptr && !item->is_separator() && !item->is_disabled() && (!check_hover || item->is_hovered())) {
					if ((type == GLToolbarItem::Right && item->is_right_toggable()) ||
						...
						// reset others state
						{
                            for (auto& sub : m_items) {
                                if (item != sub) {
                                    sub->set_state(GLToolbarItem::Normal);
								}
							}	
						}
	...
	GLToolbarItem* GLToolbar::get_item(const std::string& item_name, bool check_enabled)
	{
		if(check_enabled) {
			if (!m_enabled)
				return nullptr;
	...
	GLToolbarItem* GLToolbar::get_item(int item_id) 
	{
        if (0 <= item_id && item_id < m_items.size()) 
		{
	...
	{		
		...
		if (tex_id != 0 && tex_width > 0.0f && tex_height > 0.0f) {
			...
			if (m_layout.horizontal_orientation == Layout::HO_Left || m_layout.vertical_orientation == Layout::VO_Top)
				GLTexture::render_sub_texture(tex_id, left, internal_left, internal_top, top, { { 0, internal_bottom_uv }, { internal_right_uv, internal_bottom_uv }, { internal_right_uv, internal_top_uv }, { internal_left_uv, internal_top_uv } });
			else
	...
	void GLToolbar::reset_all_items_state_to_normal()
	{
		for (GLToolbarItem* item : m_items) 
		{
			item->set_state(GLToolbarItem::Normal);
		...
		void GLToolbar::on_params_panel_tab_changed(Slic3r::GUI::Tab* cur_tab)
	{
		if(!cur_tab)
			return;

		// if the current tab is a "Slic3r::GUI::TabPrintPart", hide the Support item,
        // because TabPrintPart NOT have a "Support" page
		Slic3r::GUI::TabPrintPart* partTab = dynamic_cast<Slic3r::GUI::TabPrintPart *> (cur_tab);
        if (partTab != nullptr)
        {
            ProcessBar::GLToolbarItem* support_item = get_item("Support", false);
            if (support_item) {
                support_item->set_visible(false);
			}
            ProcessBar::GLToolbarItem* multi_item = get_item("Multimaterial", false);
            if (multi_item) {
                multi_item->set_visible(true);
            }
        }

		 Slic3r::GUI::TabPrintLayer* layerTab = dynamic_cast<Slic3r::GUI::TabPrintLayer*>(cur_tab);
        if (layerTab != nullptr) {
            ProcessBar::GLToolbarItem* support_item = get_item("Support", false);
            if (support_item) {
                support_item->set_visible(false);
			...
			ProcessBar::GLToolbarItem* multi_item = get_item("Multimaterial", false);
            if (multi_item) {
                multi_item->set_visible(false);
            }
        }


		if (layerTab == nullptr && partTab == nullptr)
		{
            ProcessBar::GLToolbarItem* support_item = get_item("Support", false);
            if (support_item) {
                support_item->set_visible(true);
            }
            ProcessBar::GLToolbarItem* multi_item = get_item("Multimaterial", false);
            if (multi_item) {
                multi_item->set_visible(true);
	...
	std::string GLToolbar::get_process_toolbar_item_icon_file(std::string item_name, bool is_dark, bool is_modified)
    {
        std::string result("process_");
        if (item_name == "Frequent") {
            result += "frequent";
        } else if (item_name == "Quality") {
            result += "quality";
        } else if (item_name == "Strength") {
            result += "strength";
        } else if (item_name == "Speed") {
            result += "speed";
        } else if (item_name == "Support") {
            result += "support";
        } else if (item_name == "Others") {
            result += "others";
        } else if (item_name == "Multimaterial") {
            result += "multi_material";
        } else {
            return std::string();
		}
