...
void BooleanWarningManager::render_warning(const WarningItem& item, float width, ImTextureID warning_icon, ImTextureID error_icon, ImGuiWrapper* imgui, float icon_size)
{
    ...
    if (item.severity == WarningSeverity::Error)
        ...
        imgui->text_wrapped(_L(item.text), avail);
}
...
std::vector<WarningItem> BooleanWarningManager::get_inline_hints_for_state(MeshBooleanOperation mode, BooleanTargetMode target_mode, const VolumeListManager& volume_manager) const
{
    ...
    switch (mode) {
        case MeshBooleanOperation::Union:
            if (volume_manager.get_working_list().size() < 2)
                hints.push_back(WarningItem{ get_boolean_warning_by_mode(MeshBooleanOperation::Union, target_mode), WarningSeverity::Warning });
            break;
        case MeshBooleanOperation::Intersection:
            if (volume_manager.get_working_list().size() < 2)
                hints.push_back(WarningItem{ get_boolean_warning_by_mode(MeshBooleanOperation::Intersection, target_mode), WarningSeverity::Warning });
            break;
        case MeshBooleanOperation::Difference:
            if (volume_manager.get_list_a().empty() || volume_manager.get_list_b().empty())
                hints.push_back(WarningItem{ get_boolean_warning_by_mode(MeshBooleanOperation::Difference, target_mode), WarningSeverity::Warning });
            break;
        default: break;
    }
    return hints;
                                                ...
                                                const BooleanOperationSettings& settings)
{
    ...
    auto get_instance_matrix = [](const ModelObject* obj) -> Transform3d {
        if (obj && !obj->instances.empty() && obj->instances[0])
            return obj->instances[0]->get_matrix();
    ...
    for (ModelVolume* nv : settings.non_model_volumes_to_attach) {
        ...
        ModelVolume* attached = target_object->add_volume(std::move(local_mesh), nv->type());
...
BooleanOperationResult BooleanOperationEngine::perform_difference(const VolumeListManager& volume_manager,
    ...
    if (settings.target_mode == BooleanTargetMode::Object) {
            ...
            for (const auto &vol_info : kv.second) {
                ...
                if (!b_union_init) { b_union = bm; b_union_init = true; }
                    ...
                    try {
                        ...
                        if (b_union.empty()) { result.error_message = get_boolean_warning_by_mode(MeshBooleanOperation::Difference, settings.target_mode); return result; }
        ...
        if (!b_union_init) {
            result.error_message = get_boolean_warning_by_mode(MeshBooleanOperation::Difference, settings.target_mode);
            return result;
        }
...
std::string BooleanOperationEngine::validate_operation(MeshBooleanOperation type, BooleanTargetMode target_mode, const VolumeListManager& volume_manager) const {
    switch (type) {
        case MeshBooleanOperation::Union:
            if (!volume_manager.validate_for_union())
                return get_boolean_warning_by_mode(MeshBooleanOperation::Union, target_mode);
            break;
        case MeshBooleanOperation::Intersection:
            if (!volume_manager.validate_for_intersection())
                return get_boolean_warning_by_mode(MeshBooleanOperation::Intersection, target_mode);
            break;
        case MeshBooleanOperation::Difference:
            if (!volume_manager.validate_for_difference())
                return get_boolean_warning_by_mode(MeshBooleanOperation::Difference, target_mode);
            break;
        default:
...
ModelVolume* BooleanOperationEngine::create_result_volume(ModelObject* target_object,
    ...
    // ModelObject::add_volume() will center geometry and compensate by translating the volume.
    // We must preserve that compensation when copying the source volume transformation,
    // otherwise the resulting volume will shift (notably visible after Union).
    const Vec3d center_shift = local_mesh.bounding_box().center();

    // Create new volume with the local coordinate mesh, then copy metadata
    ModelVolumeType result_type = source_volume->type();
    ModelVolume* new_volume = target_object->add_volume(std::move(local_mesh), result_type);
    ...
    // Preserve the centered-geometry compensation by composing the source transform with a local translation.
    // If M is the source volume matrix and the mesh was translated by -center_shift, then we need M * T(center_shift).
    const Transform3d composed = source_volume->get_transformation().get_matrix() * Eigen::Translation3d(center_shift);
    new_volume->set_transformation(composed);
    return new_volume;
}
...
GLGizmoMeshBoolean::GLGizmoMeshBoolean(GLCanvas3D& parent,
    ...
    : GLGizmoBase(parent, icon_filename, sprite_id)
{
    ...
    m_ui->on_reset_operation = [this]() {
        ...
        const size_t cur_snapshot_time = plater->get_active_snapshot_time();
        // Reset should be a no-op before any boolean action in current session.
        if (cur_snapshot_time > m_last_snapshot_time)
            plater->undo_redo_to(m_last_snapshot_time);
...
void GLGizmoMeshBoolean::fire_boolean_analytics_event(MeshBooleanOperation mode, bool success, const std::string& error_message)
{
    AnalyticsEventPayload payload;
    payload.type = AnalyticsDataEventType::ANALYTICS_MODEL_BOOLEAN;
    nlohmann::json evtData;
    
    // Map operation mode to string
    switch (mode) {
        case MeshBooleanOperation::Union:
            evtData["type"] = "union";
            break;
        case MeshBooleanOperation::Intersection:
            evtData["type"] = "intersect";
            break;
        case MeshBooleanOperation::Difference:
            evtData["type"] = "difference";
            break;
    }
    
    // Set result: OK or error code/message
    if (success) {
        evtData["error_code"] = "OK";
    } else {
        std::string error_info = error_message.empty() ? 
                                 MeshBooleanWarnings::JOB_FAILED : error_message;
        evtData["error_code"] = error_info;
    }
...
bool GLGizmoMeshBoolean::is_on_same_plate(const Selection& selection) const
{
    ...
    for (const auto &kv : content) {
        ...
        for (int inst_id : kv.second) {
            ...
            const int plate_idx = plate_list.find_instance(obj_id, inst_id);
            if (plate_idx < 0) return false;

            if (target_plate < 0) {
                target_plate = plate_idx;
                target = plate_list.get_plate(target_plate);
                if (!target) return false;
            } else if (plate_idx != target_plate) {
                return false;
            }

            // Disallow boolean if any selected instance is not fully contained on the plate.
...
void GLGizmoMeshBoolean::on_render_input_window(float x, float y, float bottom_limit, bool force_update_pos)
{
    ...
    if (sel.get_content().empty() || m_target_mode == BooleanTargetMode::Unknown) {
        m_parent.get_gizmos_manager().open_gizmo(GLGizmosManager::EType::MeshBoolean);
        return;
    }
    ...
    // Shrink inner padding so content is closer to popup border.
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(12.0f, 8.0f) * currt_scale);
...
void GLGizmoMeshBoolean::render_selected_bounding_boxes()
{
    ...
    ColorRGB border_color_rgb { hover_rgba[0], hover_rgba[1], hover_rgba[2] };
    ...
    if (m_target_mode == BooleanTargetMode::Object) {
        ...
        for (unsigned int v_idx : m_volume_manager.get_selected_objects()) {
            ...
            if (merged.defined)
                const_cast<Selection&>(selection).render_bounding_box(merged, border_color_rgb, MeshBooleanConfig::CORNER_BOX_LINE_SCALE);
        }
    } else {
...
void GLGizmoMeshBoolean::handle_object_list_changed(const Selection& selection, bool& need_update)
{
    // When the sidebar object selection changes, re-evaluate whether this gizmo
    // should stay active and let the gizmo manager refresh its state.
    (void)selection;
    need_update = true;
}
...
BooleanOperationResult GLGizmoMeshBoolean::perform_boolean_operation_async(
    BooleanJobData& data, Job::Ctl& ctl)
{
    BooleanOperationResult result;
    result.success = false;
    ...
    switch (data.operation_mode) {
        ...
        case MeshBooleanOperation::Intersection: {
            ...
            if (data.volumes_a.size() < 2) {
                result.error_message = get_boolean_warning_by_mode(data.operation_mode, data.settings.target_mode);
                return result;
            }
        ...
        case MeshBooleanOperation::Difference: {
            ...
            if (data.volumes_a.empty() || data.volumes_b.empty()) {
                result.error_message = get_boolean_warning_by_mode(MeshBooleanOperation::Difference, data.settings.target_mode);
                return result;
            }

            // Object mode uses different algorithm than Part mode
            // Object: A_i - (B1 ?ȠB2 ?Ƞ... ?ȠBn) for each A_i
...
void GLGizmoMeshBoolean::execute_mesh_boolean()
{
    ...
    // Validate operation
    std::string validation_error = m_boolean_engine.validate_operation(m_operation_mode, m_target_mode, m_volume_manager);
    ...
    auto perform_current_operation = [&](MeshBooleanOperation mode) {
        ...
        fire_boolean_analytics_event(mode, result.success, result.error_message);
    };
    ...
    if (m_operation_mode == MeshBooleanOperation::Difference) {
        if (job_data.volumes_a.empty() || job_data.volumes_b.empty()) {
            m_warning_manager.add_warning(get_boolean_warning_by_mode(MeshBooleanOperation::Difference, m_target_mode));
            return;
        }
    } else if (job_data.volumes_a.size() < 2) {
        m_warning_manager.add_warning(get_boolean_warning_by_mode(m_operation_mode, m_target_mode));
        return;
    }
    ...
    boolean_job->set_process_callback(
        [this](Job::Ctl& ctl, BooleanJobData& data) {
    ...
    boolean_job->set_finalize_callback(
        [this](BooleanJobData& data) {
            ...
            // Fire analytics event
            fire_boolean_analytics_event(data.operation_mode, data.result.success, data.result.error_message);
    ...
    boolean_job->set_cancel_callback(
        [this](Job::Ctl& ctl, BooleanJobData& data) -> bool {
...
{
    // Enable per-volume color override in 3D view for A/B list distinction.
    m_parent.clear_all_volume_color_overrides();
    m_parent.set_use_volume_color_override(true);

    if (mode == MeshBooleanOperation::Difference)
        apply_a_b_list_color_overrides(mode);
    else
        apply_working_list_color_overrides(mode);
}
