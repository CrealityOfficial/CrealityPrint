...
struct MeshBooleanConfig {
    ...
    // ========================== ICON SIZES ==========================
    static constexpr float ICON_SIZE_DISPLAY = 16.0f;  // Display icons in lists
    static constexpr float ICON_SIZE_BUTTON = 15.0f;   // Interactive button icons
    static constexpr float ICON_SPACING = SPACING_LG;  // Spacing around icons
    static constexpr float ICON_PADDING = SPACING_XS;  // Padding inside icon areas

    // ========================== LIST DIMENSIONS ==========================
    static constexpr float LIST_WIDTH = 360.0f;        // Total list width
    ...
    static constexpr float LIST_ITEM_HEIGHT = 18.0f;   // Individual item height
    static constexpr float LIST_PADDING = 8.0f;       // Padding between lists
    ...
    // ========================== VISUAL STYLING ==========================
    static constexpr float ROUNDING_BUTTON = 4.0f;     // Button corner rounding
    static constexpr float ROUNDING_LIST = 4.0f;      // List container rounding
    static constexpr float ROUNDING_NONE = 0.0f;       // No rounding

    // ========================== SPACING DETAILS ==========================
    static constexpr float SPACING_TEXT = SPACING_XS;           // Text padding
    ...
    // ========================== UI COLORS ==========================
    static constexpr unsigned int COLOR_LIST_A = 0xFFFFCBB0;        // List A title (light blue)
    static constexpr unsigned int COLOR_LIST_B = 0xFFB5EBFC;        // List B title (green-yellow)
    static constexpr unsigned int COLOR_SELECTED_BG = 0xFFE6F6DC;   // Selected item background light
    static constexpr unsigned int COLOR_SELECTED_BG_DARK = 0xFF2C3B23; // Selected item background dark
    static constexpr unsigned int COLOR_HOVER_BORDER = 0xFF59C015;  // Hover border
...
public:
    ...
    std::string validate_operation(MeshBooleanOperation type, BooleanTargetMode target_mode, const VolumeListManager& volume_manager) const;
...
public:
    ...
    std::vector<WarningItem> get_inline_hints_for_state(MeshBooleanOperation mode, BooleanTargetMode target_mode, const VolumeListManager& volume_manager) const;
    // Validation helpers
...
public:
    ...
    GLGizmoMeshBoolean(GLCanvas3D& parent,
    ...
    // Event handler used by GLGizmosManager; not part of GLGizmoBase virtual API.
    bool gizmo_event(SLAGizmoEventType action, const Vec2d& mouse_position, bool shift_down, bool alt_down, bool control_down);

    // Sidebar object list selection change handler (called from GLCanvas3D).
    void handle_object_list_changed(const Selection& selection, bool& need_update);

    // Not overriding any GLGizmoBase method; helper used by manager
    std::string get_icon_filename(bool b_dark_mode) const;
    void apply_color_overrides_for_mode(MeshBooleanOperation mode);

protected:
    virtual bool on_init() override;
    virtual std::string on_get_name() const override;
    // Helper only for this gizmo; not present in base
    virtual std::string on_get_name_str() { return "Mesh Boolean"; }
    virtual bool on_is_activable() const override;
    virtual void on_render() override;
    // Base class does not declare on_render_for_picking; keep this as local helper
...
private:
    ...
    BooleanOperationResult perform_boolean_operation_async(BooleanJobData& data, Job::Ctl& ctl);
    ...
    // Helper method for analytics event
    void fire_boolean_analytics_event(MeshBooleanOperation mode, bool success, const std::string& error_message = "");
