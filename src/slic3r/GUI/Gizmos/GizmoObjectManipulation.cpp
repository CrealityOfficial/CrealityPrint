...
void GizmoObjectManipulation::do_render_move_window(ImGuiWrapper *imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    ...
    float space_size    = imgui_wrapper->get_style_scaling() * 14.0f;
    ...
    float caption_max   = std::max(position_size, World_size) + 1 * space_size;
    ...
    ImVec2 updown_button_size = ImVec2(15.0f * m_glcanvas.get_scale(), 12.0f * m_glcanvas.get_scale());
    float  unit_size          = imgui_wrapper->calc_text_size(MAX_SIZE).x + space_size + updown_button_size.x;
    int   index      = 1;
    int   index_unit = 1;
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(4.0f, (item_height - ImGui::GetFontSize()) * 0.5f)); // input wegit height 30.0f
    ...
    draw_input_double(label_values[0][0], &display_position[0], stride, Vec2d(unit_size, item_height));

    ImGui::SameLine(caption_max + unit_size + (++index) * space_size);
    
    draw_input_double(label_values[0][1], &display_position[1], stride, Vec2d(unit_size, item_height));

    ImGui::SameLine(caption_max + (++index_unit) * unit_size + (++index) * space_size);
    
    draw_input_double(label_values[0][2], &display_position[2], stride, Vec2d(unit_size, item_height));
...
void GizmoObjectManipulation::do_render_rotate_window(ImGuiWrapper *imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    ...
    draw_input_double(label_values[1][0], &rotation[0], 5.0f, Vec2d(unit_size, item_height));

    ImGui::SameLine(caption_max + unit_size + (++index) * space_size);
    
    draw_input_double(label_values[1][1], &rotation[1], 5.0f, Vec2d(unit_size, item_height));
    
    ImGui::SameLine(caption_max + (++index_unit) * unit_size + (++index) * space_size);
    
    draw_input_double(label_values[1][2], &rotation[2], 5.0f, Vec2d(unit_size, item_height));
...
void GizmoObjectManipulation::do_render_scale_input_window(ImGuiWrapper* imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    ...
    float space_size = imgui_wrapper->get_style_scaling() * 14;
    ...
    ImVec2 updown_button_size = ImVec2(15.0f, 12.0f);
    ...
    draw_input_double(label_scale_values[0][0], &scale[0], 1.0, Vec2d(unit_size, item_height));

    ImGui::SameLine(caption_max + unit_size + index * space_size);

    draw_input_double(label_scale_values[0][1], &scale[1], 1.0, Vec2d(unit_size, item_height));

    ImGui::SameLine(caption_max + (++index_unit) *unit_size + (++index) * space_size);
 
    draw_input_double(label_scale_values[0][2], &scale[2], 1.0, Vec2d(unit_size, item_height));
    ...
    draw_input_double(label_scale_values[1][0], &display_size[0], 1.0, Vec2d(unit_size, item_height));
    ImGui::SameLine(caption_max + unit_size + index * space_size);
    
    draw_input_double(label_scale_values[1][1], &display_size[1], 1.0, Vec2d(unit_size, item_height));
    ImGui::SameLine(caption_max + (++index_unit) *unit_size + (++index) * space_size);
    
    draw_input_double(label_scale_values[1][2], &display_size[2], 1.0, Vec2d(unit_size, item_height));
    ImGui::SameLine(caption_max + (++index_unit) *unit_size + (++index) * space_size);
    imgui_wrapper->text(this->m_new_unit_string);
...
void GizmoObjectManipulation::draw_input_double(const std::string& label, double* v, double stride, const Vec2d& size)
{
    m_glcanvas.draw_input_double(label, v, stride, size);
}
