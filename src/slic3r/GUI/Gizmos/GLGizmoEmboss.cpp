...
void GLGizmoEmboss::draw_font_list_line()
{
    ...
    if (exist_change_in_font || !exist_stored_style)
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_CREALITY, font_text);
    else
...
void GLGizmoEmboss::draw_model_type()
{
    ...
    if (ImGui::RadioButton(_u8L("Join").c_str(), type == part))
        ...
        m_imgui->tooltip(_u8L("Click to change text into object part.").c_str(), m_gui_cfg->max_tooltip_width);
    ImGui::SameLine();
    ...
    if (ImGui::RadioButton(_CTX_utf8(L_CONTEXT("Cut", "EmbossOperation"), "EmbossOperation").c_str(), type == negative))
        ...
        if (is_last_solid_part)
            m_imgui->tooltip(last_solid_part_hint.c_str(), m_gui_cfg->max_tooltip_width);
        else if (type != negative)
            m_imgui->tooltip(_u8L("Click to change part type into negative volume.").c_str(), m_gui_cfg->max_tooltip_width);
    }
    ...
    if (wxGetApp().plater()->printer_technology() != ptSLA && wxGetApp().get_mode() != ConfigOptionMode::comSimple) {
        ...
        if (ImGui::RadioButton(_u8L("Modifier").c_str(), type == modifier))
            ...
            if (is_last_solid_part)
                ...
                m_imgui->tooltip(_u8L("Click to change part type into modifier.").c_str(), m_gui_cfg->max_tooltip_width);
        }
    }
...
void GLGizmoEmboss::draw_style_save_button(bool is_modified)
{
    if (draw_button(m_icons, IconType::save, !is_modified)) {
        ...
        m_imgui->tooltip(tooltip.c_str(), m_gui_cfg->max_tooltip_width);
    }
}
...
void GLGizmoEmboss::draw_style_add_button()
{
    ...
    if (draw_button(m_icons, IconType::add, !can_add)) {
        ...
        if (!can_add) {
            m_imgui->tooltip(_u8L("Only valid font can be added to style.").c_str(), m_gui_cfg->max_tooltip_width);
        } else if (only_add_style) {
            m_imgui->tooltip(_u8L("Add style to my list.").c_str(), m_gui_cfg->max_tooltip_width);
        } else {
            m_imgui->tooltip(_u8L("Save as new style.").c_str(), m_gui_cfg->max_tooltip_width);
        }
    }
                               ...
                               Draw               draw) const
{
    ...
    if (changed || default_value == nullptr)
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_CREALITY, name);
    else
...
template<> bool imgui_input(const char* label, int* v, int step, int step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    const std::string &undo_tooltip, T step, T step_fast, const char *format, ImGuiInputTextFlags flags) const
{
    ...
    auto draw_offseted_input = [&offset = m_gui_cfg->input_offset, &width = m_gui_cfg->input_width,
                                &name, &value, &step, &step_fast, format, flags, this](){
        ...
        Vec2d size = Vec2d(width, ImGui::GetFrameHeight());
        ...
        const ImVec2 updown_button_size = ImVec2(15.0f * m_parent.get_scale(), height / 2.0f);
        ImGui::PushItemWidth(size.x() - updown_button_size.x);
        bool changed = imgui_input(("##" + name).c_str(), &value, (T)0, step_fast, format, flags);
        ImGui::PopItemWidth();
        {
            ImGui::SameLine();
            ImGui::BeginGroup();
            ...
            if (ImGui::ArrowButtonEX_CP(("##" + name + "##up").c_str(), ImGuiDir_Up, updown_button_size)) {
                value += step;
                changed |= true;
            }
            if (hover)
            ...
            if (ImGui::ArrowButtonEX_CP(("##" + name +"##down").c_str(), ImGuiDir_Down, updown_button_size)) {
                value -= step;
                changed |= true;
            }
...
template<typename T>
bool GLGizmoEmboss::input_with_arrow_btn(input_value<T>& value, const input_attri& attri)
{
	ImGui::SameLine(attri.offset);
...

    {
        ImDrawList* draw_list = ImGui::GetWindowDrawList();
        ImVec2      pos       = ImGui::GetCursorScreenPos();
        ImVec2      pos_max   = ImVec2(pos.x + attri.width + 4.0f, pos.y + height);
        draw_list->AddRect(pos, pos_max, IM_COL32(110, 110, 114, 255), 5.0f, ImDrawFlags_RoundCornersAll);
    }

    const ImVec2 updown_button_size = ImVec2(15.0f * m_parent.get_scale(), height / 2.0f);
    ImGui::PushItemWidth(attri.width - updown_button_size.x);

	//just draw a input text control
	const std::string label = "##" + attri.name;
    bool changed = imgui_input(label.c_str(), &value.value, (T) 0, (T) 0, attri.format.c_str(), attri.flags);
...

	{
		...
		{
			T step_value = value.value + value.step;
			if (step_value <= value.min)
			{
				step_value = value.min;
			}

			if (step_value >= value.max)
			{
				step_value = value.max;
			}

			//value being changed
			if (std::fabs(step_value - value.value) > std::numeric_limits<T>::epsilon())
			{
				changed |= true;
...
bool GLGizmoEmboss::rev_input(const std::optional<int>* default_value, std::optional<int>& value, int step, int min, int max, const input_attri& attri)
{
	//this lambda will draw the input with arrow button
	auto draw_input_optional_int = [&]() -> bool
	{
		int def_val = 0;
		if (default_value && default_value->has_value())
		{
			def_val = default_value->value();
		}

		//got a valid value
		if (value.has_value())
		{
			//construct the value data;
			struct input_value<int> v { *value, step, min, max };

			// using this value to draw the input text control and detect if the value being changed
			if (input_with_arrow_btn(v, attri))
			{
                if (*value == def_val)
                {
                    value = (default_value) ? *default_value : def_val;
                }
				return true;
			}
		}
		...
		{
			// using the default value to draw the input text control and detect if the value being changed
			int val = def_val;

			//construct the value data;
			struct input_value<int> v { val, step, min, max };
			if (input_with_arrow_btn(v, attri))
			{
				if (val != def_val) value = val;
				return true;
			}
		}
		return false;
	...
	//draw the revert button if the value has been changed
	float undo_offset = ImGui::GetStyle().WindowPadding.x;
	return revertible(attri.name, value, default_value, attri.undo_tooltip, undo_offset, draw_input_optional_int);
}

bool GLGizmoEmboss::rev_input(const std::optional<float>* default_value, std::optional<float>& value, float step, float min, float max, const input_attri& attri)
{
	//this lambda will draw the input with arrow button
	auto draw_input_optional_int = [&]() -> bool
	{
		...
		{
			//construct the value data;
			struct input_value<float> v { *value, step, min, max };

			// using this value to draw the input text control and detect if the value being changed
			if (input_with_arrow_btn(v, attri))
			{
				if (std::fabs(*value - def_val) <= std::numeric_limits<float>::epsilon())
				{
					value = (default_value) ? *default_value : def_val;
				}
				return true;
		...
		else
		{
			...
			//construct the value data;
			struct input_value<float> v { val, step, min, max };
			if (input_with_arrow_btn(v, attri))
			{
				if (std::fabs(val - def_val) > std::numeric_limits<float>::epsilon()) value = val;
				return true;
			}
		}
		return false;
...
bool GLGizmoEmboss::rev_input(float* default_value, float& value, float step, float min, float max, const input_attri& attri)
{
	auto draw_input_optional_int = [&]() -> bool
	{
		//construct the value data;
        ...
        return input_with_arrow_btn(v, attri);
	};
...
//         last_change = true;
    ...
    if (1)
    {
		...
		input_attri attri
		{
			m_gui_cfg->advanced_input_offset,
			m_gui_cfg->input_width,
			tr.char_gap,
			"%d " + _u8L("points"),
			_u8L("Distance between characters"),
			_u8L("Revert gap between characters")
		};

	    const std::optional<int>* def_char_gap = stored_style ? &stored_style->prop.char_gap : nullptr;

        bool ret = rev_input(def_char_gap, current_style.prop.char_gap, step, min, max, attri);
        if (ret)
        {
            const std::optional<int>& volume_char_gap = m_volume->text_configuration->style.prop.char_gap;
            if (!apply(current_style.prop.char_gap, limits.char_gap) || !volume_char_gap.has_value() || volume_char_gap != current_style.prop.char_gap)
            {
                // char gap is stored inside of imgui font atlas
                m_style_manager.clear_imgui_font();
                exist_change = true;
...
//     m_imgui->disabled_end(); // !is_multiline
	...
	if (1)
	{
		...
		input_attri attri
		{
			...
			tr.line_gap,
			"%d " + _u8L("points"),
			_u8L("Distance between lines"),
			_u8L("Revert gap between lines")
		};

		const std::optional<int>* def_line_gap = stored_style ? &stored_style->prop.line_gap : nullptr;

		bool ret = rev_input(def_line_gap, current_style.prop.line_gap, step, min, max, attri);
		if (ret)
		{
			const std::optional<int>& volume_line_gap = m_volume->text_configuration->style.prop.line_gap;
			if (!apply(current_style.prop.line_gap, limits.line_gap) || !volume_line_gap.has_value() || volume_line_gap != current_style.prop.line_gap) 
            {
				// line gap is planed to be stored inside of imgui font atlas
				m_style_manager.clear_imgui_font();
                if (font_prop.per_glyph)
...
//         last_change = true;
	...
	{
        int step = 30;
		int min = static_cast<int>(font_info.ascent * limits.boldness.gui.min);
		int max = static_cast<int>(font_info.ascent * limits.boldness.gui.max);
		...
		input_attri attri
		{
			...
			tr.boldness,
			"%.0f " + _u8L("points"),
			_u8L("Tiny / Wide glyphs"),
			_u8L("Undo boldness")
		};

		const std::optional<float>* def_boldness = stored_style ? &stored_style->prop.boldness : nullptr;

		bool ret = rev_input(def_boldness, current_style.prop.boldness, step, min, max, attri);
		if (ret)
		{
			const std::optional<float>& volume_boldness = m_volume->text_configuration->style.prop.boldness;
            if (!apply(current_style.prop.boldness, limits.boldness.values) || !volume_boldness.has_value() || volume_boldness != current_style.prop.boldness)
            {
                exist_change = true;
            }
		}
...
//         last_change = true;
    ...
    {
        float step = 0.01;
		float min = limits.skew.gui.min;
		float max = limits.skew.gui.max;
		...
		input_attri attri
		{
			...
			tr.skew_ration,
			"%.2f",
			_u8L("Italic strength ratio"),
			_u8L("Undo letter's skew")
		};

        const std::optional<float>* def_skew = stored_style ? &stored_style->prop.skew : nullptr;

        bool ret = rev_input(def_skew, current_style.prop.skew, step, min, max, attri);
        if (ret)
        {
			const std::optional<float>& volume_skew = m_volume->text_configuration->style.prop.skew;
            if (!apply(current_style.prop.skew, limits.skew.values) || !volume_skew.has_value() || volume_skew != current_style.prop.skew)
            {
                exist_change = true;
            }	
        }
...
// 	m_imgui->disabled_end();  // allowe_surface_distance
    ...
    if (1)
    {
        //if we are in inch unit
        bool use_inch = wxGetApp().app_config->get_bool("use_inches");

        //disable this control or not
		bool allowe_surface_distance = !use_surface && !m_volume->is_the_only_one_part();
        m_imgui->disabled_begin(!allowe_surface_distance);

		std::optional<float> &distance = current_style.distance;		
        const std::optional<float> *def_distance = stored_style ? &stored_style->distance : nullptr;

        float prev_distance = distance.value_or(.0f);

        float step = 0.01;
		float min = static_cast<float>(-2 * current_style.projection.depth);
		float max = static_cast<float>(2 * current_style.projection.depth);

// 		float offset = m_gui_cfg->advanced_input_offset;
// 		float width = m_gui_cfg->input_width;
// 		
//      const wxString tooltip = _L("Distance of the center of the text to the model surface.");
// 		const std::string undo_tooltip = _u8L("Undo translation");
		...
		if (use_inch) 
        {
			...
			step = step * GizmoObjectManipulation::mm_to_in;
			min *= GizmoObjectManipulation::mm_to_in;
			max *= GizmoObjectManipulation::mm_to_in;
			...
			input_attri attri
			{
				...
				"%.3f in",
				_u8L("Italic strength ratio"),
				_u8L("Distance of the center of the text to the model surface.")
			};

			//const std::optional<float>* def_skew = stored_style ? &stored_style->prop.skew : nullptr;

            bool ret = rev_input(def_distance, distance_inch, step, min, max, attri);
            //bool ret = rev_input_optional_float_with_arrow_btn(tr.from_surface, distance_inch, def_distance, step, min, max, offset, width, units_fmt, tooltip, undo_tooltip);
			//if (rev_slider(tr.from_surface, distance_inch, def_distance, undo_move_tooltip, min_distance, max_distance, "%.3f in", move_tooltip))
            if(ret)
            {
				if (distance_inch.has_value())
		...
		else 
        {
            ...
            //bool ret = rev_input_optional_float_with_arrow_btn(tr.from_surface, distance, def_distance, step, min, max, offset, width, units_fmt, tooltip, undo_tooltip);
            //if (rev_slider(tr.from_surface, distance, def_distance, undo_move_tooltip, min_distance, max_distance, "%.2f mm", move_tooltip))
            bool ret = rev_input(def_distance, distance, step, min, max, attri);
            if(ret)
            {
                is_moved = true;
            }
...
//     }
    ...
    const float row_rotation_y = ImGui::GetCursorPosY();
...
// 		float min = limits.angle.min;
// 		float max = limits.angle.max;
// 
// 		float offset = m_gui_cfg->advanced_input_offset;
// 		float width = m_gui_cfg->input_width;
// 
...
//         std::string units_fmt = u8"%.2f °";
        ...
        bool ret = rev_input(def_angle_deg, angle_deg, step, min, max, attri);
        //bool ret = rev_input_float_with_arrow_btn(tr.rotation, angle_deg, def_angle_deg, step, min, max, offset, width, units_fmt, tooltip, undo_tooltip);
        if(ret)
        {
			// convert back to radians and CCW
			double angle_rad = -angle_deg * M_PI / 180.0;
    ...
    if (!m_volume->is_the_only_one_part()) {
        ...
        const float bak_y = ImGui::GetCursorPosY();
    
        ImGui::SameLine(m_gui_cfg->lock_offset);
        ImGui::SetCursorPosY(row_rotation_y);
    ...
    if (ff.font_file->infos.size() > 1) {
        ...
        ImGui::SetNextItemWidth(m_gui_cfg->input_width);
        unsigned int selected = current_style.prop.collection_number.value_or(0);
        std::string tooltip = "";
        ImGuiWrapper::push_combo_style(m_parent.get_scale());
        if (ImGui::BBLBeginCombo("## Font collection", std::to_string(selected).c_str())) {
            for (unsigned int i = 0; i < ff.font_file->infos.size(); ++i) {
                ...
                if (ImGui::BBLSelectable(std::to_string(i).c_str(), is_selected)) {
                    if (i == 0) current_style.prop.collection_number.reset();
                    else current_style.prop.collection_number = i;
                    exist_change = true;
                    last_change = true;
                }
                ImGui::PopID();
