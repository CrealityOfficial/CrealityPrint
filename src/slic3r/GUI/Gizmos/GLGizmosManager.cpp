...
void GLGizmosManager::switch_gizmos_icon_filename()
{
    ...
    for (auto& gizmo : m_gizmos) {
        ...
        case(EType::Measure):
            ...
            break;
        case (EType::Hollowing): 
            gizmo->set_icon_filename(m_is_dark ? "toolbar_hollow_dark.svg" : "toolbar_hollow.svg");
            break;
        case (EType::Drill):
            gizmo->set_icon_filename(m_is_dark ? "toolbar_drill_dark.svg" : "toolbar_drill.svg"); 
            break;
        case (EType::BrimEars):
...
int GLGizmosManager::get_state_index(GLGizmosManager::EType type) const
{
    if (type == Undefined)
        return -1;
    ...
    int state = (m_highlight.first == idx ? (m_highlight.second ? 4 : 5) :
                    (m_current == idx) ? 1 :
                                ((m_hover == idx) ? 3 : (gizmo->is_activable() ? 0 : 2)));
...
bool GLGizmosManager::on_mouse(const wxMouseEvent &mouse_event)
{
    ...
    // MeshBoolean relies on gizmo_event() (not on_mouse()) for click selection.
    if (m_current == MeshBoolean) {
        ...
        if (mouse_event.RightUp() && !m_parent.is_mouse_dragging()) {
...
bool GLGizmosManager::on_key(wxKeyEvent& evt)
{
    ...
    if (evt.GetEventType() == wxEVT_KEY_UP) {
        ...
        if (m_current == MeshBoolean) {
            ...
            case WXK_DOWN:
                // MeshBoolean: disable arrow-key model translation.
                processed = true;
                break;
            default:
                break;
        ...
        if (!processed && m_current == Measure) {
...
void GLGizmosManager::do_render_overlay() const
{
    ...
    if (m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView) {
        ...
        //BBS: GUI refactor: GLToolbar&&Gizmo adjust
        float main_toolbar_width = (float) m_parent.get_main_toolbar_width() * 2;
...
//         top_x = top_x * inv_cnv_w;
        top_x = -cnv_w + main_toolbar_width + separator_width;
        top_x = top_x * inv_cnv_w;
    }
    float top_y = 1.0f;
    ...
    for (size_t idx : selectable_idxs) {
        ...
        //const int icon_idx = (m_highlight.first == idx ? (m_highlight.second ? 4 : 5) : (m_current == idx) ? 1 : ((m_hover == idx) ? 3 : (gizmo->is_activable()? 0 : 2)));
        const int   icon_idx = get_state_index((EType)idx);
...
//        sprite_size_px += 1;
    ...
    borders.push_back(path + "toolbar_border_disable.svg");
    borders.push_back(path + "toolbar_border_hover.svg");
    borders.push_back(path + "toolbar_border_press.svg");
...
{
    // On Wayland, GL init may be deferred and gizmos may not be populated yet.
    if (m_gizmos.empty())
