...
static const ColorRGBA SELECTED_COLOR = {0.08f, 0.75f, 0.35f, 1.0f};
static const ColorRGBA ERR_COLOR = {1.0f, 0.3f, 0.3f, 0.5f};
static const ColorRGBA HOVER_COLOR = {0.08f, 0.75f, 0.35f, 0.5f};
    ...
    for (size_t i = 0; i < cache_size; ++i) {
        ...
        if (hover) {
            ...
            if (size_t(m_hover_id) == i) // ignore hover state unless editing mode is active
                render_color = {0.08f, 0.75f, 0.35f, 1.f};
...
bool GLGizmoBrimEars::on_is_activable() const
{
    ...
    //if (!selection.is_single_full_instance() && !selection.is_single_full_object() && !selection.is_single_volume() &&
