...
//yy
#include "slic3r//GUI/MeshHollow.hpp"
...
//RENDER_AGAIN:
   ...
   /* if (current_mode >= quality_mode) {
    ...
    ImGui::AlignTextToFramePadding();
    m_imgui->text(m_desc.at("thickness"));
    ...
    std::string m_new_unit_string="mm";

    float  space_size         = m_imgui->get_style_scaling() * 14.0f;
    float  item_height        = ImGui::GetFrameHeight();
    ImVec2 updown_button_size = ImVec2(15.0f * m_parent.get_scale(), 10.0f * m_parent.get_scale());
    float  unit_size          = MAX(145,ImGuiWrapper::calc_text_size(MAX_SIZE).x +
                                ImGuiWrapper::calc_text_size(m_new_unit_string).x + space_size +
                                updown_button_size.x);

    float diam = m_new_hole_radius;
    double diam_double = (double)diam;
    
    draw_cp_input_double("##hole_diameter",&diam_double,0.1f,Vec2d(unit_size, item_height));
   ...
   // m_new_hole_radius = diam / 2.f;
    ...
    ImVec4 bt_bgc_light = ImVec4(255 / 255.0, 2555 / 255.0, 255 / 255.0, 1.0);
    ...
    const char * btn_title = (m_desc.at("Hollowing")).ToUTF8().data();
    ImVec2 title_size = ImGui::CalcTextSize(btn_title);
    ...
    if (ImGui::Button(btn_title)) {
      ...
      // mesh1.write_ascii("after.stl");
       ...
       // generate new volume
       ModelVolume* new_volume = curr_model_object->add_volume(std::move(mesh1));
       new_volume->name        = old_volume->name + " - " + "hollow";
...
void GLGizmoHollow::draw_cp_input_double(const std::string& label, double* v, double stride, const Vec2d& size, int disable)
{
    bool is_dark = wxGetApp().dark_mode();
    ...
    if (disable) {
        ImGui::PushStyleColor(ImGuiCol_Text, is_dark ? ImVec4(1.0f, 1.0f, 1.0f, 0.3f) : ImVec4{ 214 / 255.0f, 214 / 255.0f, 220 / 255.0f, 0.3f });
    }
    ...
    const ImVec2 updown_button_size = ImVec2(15.0f * m_parent.get_scale(), ImGui::GetFrameHeight()/2.0);
    const char*  text               = "mm";
...
bool GLGizmoHollow::on_is_activable() const
{
   ...
   // bool t1 = selection.is_from_single_instance();
   // bool t2 = wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology() != ptSLA;
