...
wxDECLARE_EVENT(EVT_GLTOOLBAR_PRINTER, SimpleEvent);
...
wxDECLARE_EVENT(EVT_GLVIEWTOOLBAR_BRIM_EARS, SimpleEvent);
wxDECLARE_EVENT(EVT_GLVIEWTOOLBAR_ASSEMBLE, SimpleEvent);
wxDECLARE_EVENT(EVT_GLVIEWTOOLBAR_AI_CLOUD_SERVICE, SimpleEvent);
...
public:
    ...
    void render(unsigned int tex_id, float left, float right, float bottom, float top, GLTexture::Quad_UVs uv) const;
...
struct BackgroundTexture
{
    struct Metadata
    {
        ...
        std::vector<std::string> icon_borders;
        Metadata();
    };
...
public:
    ...
    void render(const GLCanvas3D& parent, float scroll, GLToolbarItem::EType type = GLToolbarItem::Action);
