...
void Downloader::start_download(const std::string& full_url)
{
	...
	boost::regex re(R"(^(crealityprintlink|orcaslicer|prusaslicer|bambustudio|cura):\/\/open[\/]?\?file=)", boost::regbase::icase);
    ...
    if (is_creality_open(full_url)) {
        plater->request_model_download(from_u8(escaped_url));
    }
    else if (is_bambustudio_open(full_url) || (is_orca_open(full_url) && is_makerworld_link(full_url)))
