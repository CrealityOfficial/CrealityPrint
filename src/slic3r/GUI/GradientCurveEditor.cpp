...
// LAB DeltaE76 between two wxColours. Reimplemented locally because the helper
// in DeviceManager.cpp is `static` and not exported. The conversion helpers come
// from libslic3r/Utils/ColorSpaceConvert.hpp.
static float calc_color_distance(const wxColour& c1, const wxColour& c2)
{
    float lab[2][3];
    RGB2Lab(c1.Red() / 255.f, c1.Green() / 255.f, c1.Blue() / 255.f,
            &lab[0][0], &lab[0][1], &lab[0][2]);
    RGB2Lab(c2.Red() / 255.f, c2.Green() / 255.f, c2.Blue() / 255.f,
            &lab[1][0], &lab[1][1], &lab[1][2]);
    return DeltaE76(lab[0][0], lab[0][1], lab[0][2],
                    lab[1][0], lab[1][1], lab[1][2]);
}
} // namespace
...
void GradientCurveEditor::reset_to_linear(double y0, double y1)
{
    ...
    m_points.points.push_back(a0);
    m_points.points.push_back(a1);
    m_selected_curve = 0;
    Refresh();
    emit_changed();
}
...
void GradientCurveEditor::normalize_points()
{
    ...
    std::sort(m_points.points.begin(), m_points.points.end(),
              [](const GradientAnchor& a, const GradientAnchor& b) {
                  return a.x < b.x;
              });

    if (m_points.points.size() < 2) {
        GradientAnchor tail; tail.x = 1.0; tail.y = m_points.points.front().y;
        m_points.points.push_back(tail);
    }

    m_points.points.front().x = 0.0;
    m_points.points.back().x  = 1.0;
}
...
int GradientCurveEditor::hit_test(int px, int py) const
{
    ...
    for (size_t i = 0; i < m_points.points.size(); ++i) {
        ...
        const double vy = to_visual_y(m_selected_curve, m_points.points[i].y);
        const wxPoint p = data_to_px(m_points.points[i].x, vy);
        const int dx = px - p.x;
        const int dy = py - p.y;
...
int GradientCurveEditor::hit_test_curve(int px, int py, int* seg_out) const
{
    ...
    if (m_points.points.size() < 2) return -1;
    ...
    auto seg_for_x = [&](double cursor_x) -> int {
        for (size_t i = 1; i < m_points.points.size(); ++i) {
            if (cursor_x <= m_points.points[i].x)
                return static_cast<int>(i - 1);
        }
        return static_cast<int>(m_points.points.size() - 2);
    };
...
void GradientCurveEditor::on_paint(wxPaintEvent& /*evt*/)
{
    ...
    // Control points (selected curve only): white fill with light border.
    const int r = FromDIP(kPointRadius);
    dc.SetPen(wxPen(wxColour(0, 200, 200), 1));
...
void GradientCurveEditor::on_left_down(wxMouseEvent& evt)
{
    ...
    if (idx >= 0) {
        ...
        const bool had_tangent = std::isfinite(m_points.points[idx].m_in)
                              || std::isfinite(m_points.points[idx].m_out);
        m_points.points[idx].m_in  = std::numeric_limits<double>::quiet_NaN();
        m_points.points[idx].m_out = std::numeric_limits<double>::quiet_NaN();
        if (!HasCapture())
            CaptureMouse();
        Refresh();
        if (had_tangent)
    ...
    m_points.points.insert(m_points.points.begin() + insert_idx, a);
...
void GradientCurveEditor::on_right_down(wxMouseEvent& evt)
{
    const wxPoint pos = evt.GetPosition();
    const int idx = hit_test(pos.x, pos.y);
    if (idx > 0 && static_cast<size_t>(idx) + 1 < m_points.points.size()) {
        // Interior anchor on the selected curve -> delete it. Endpoints stay locked.
        m_points.points.erase(m_points.points.begin() + idx);
        Refresh();
        emit_changed();
        return;
    }
...
void GradientCurveEditor::on_motion(wxMouseEvent& evt)
{
    ...
    if (static_cast<size_t>(m_drag_idx) >= m_points.points.size())
        return;
    ...
    auto& p = m_points.points[m_drag_idx];
    const bool is_first = (m_drag_idx == 0);
    const bool is_last  = (static_cast<size_t>(m_drag_idx) + 1 == m_points.points.size());
    ...
    if (!is_first && !is_last) {
        const double xl = m_points.points[m_drag_idx - 1].x;
        const double xr = m_points.points[m_drag_idx + 1].x;
