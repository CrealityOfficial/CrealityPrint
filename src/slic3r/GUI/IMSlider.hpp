...
public:
    ...
    bool init_texture(bool onelayer);
    ...
    void SetLayersTimes(const std::vector<double> &layers_times, double total_time);
    ...
    void show_go_to_layer(bool show);

    bool render(int canvas_width, int canvas_height);

    //BBS update scroll value changed
    bool is_dirty();
    void set_as_dirty(bool dirty = true);
    bool is_need_post_tick_event();
    ...
    void set_animating(bool animate);
    void set_animate_tick(std::function<void(bool manual)> tick) { m_animate_tick = tick; }
...
private:
    ...
    double get_altarValue(int index);
    ...
    std::function<void(bool manual)> m_animate_tick;
};
