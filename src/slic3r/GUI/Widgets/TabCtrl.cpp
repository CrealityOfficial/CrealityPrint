                     ...
                     void * clientData)
{
    ...
    btn->SetTextColor(StateColor(
        std::make_pair(0xFFFFFF, (int) StateColor::NotChecked),
        std::make_pair(0xFFFFFF, (int) StateColor::Normal)));

    btn->SetBackgroundColor(StateColor(
        std::make_pair(*wxWHITE, (int)StateColor::NotChecked),
        std::make_pair(0x17CC5F, (int)StateColor::Normal),
        std::make_pair(0x17CC5F, (int)StateColor::NotHovered)));
    btn->SetCornerRadius(0);
    btn->SetPaddingSize({TAB_BUTTON_PADDING});
    btns.push_back(btn);
    if (btns.size() > 1)
        sizer->GetItem(sizer->GetItemCount() - 1)->SetMinSize({0, 0});
    //sizer->Add(btn, wxSizerFlags().Center().Border(wxALL, 0));
    sizer->Add(btn, 1, wxEXPAND | wxALL, 5);
