    ...
    , text_color(std::make_pair(0x909090, (int) StateColor::Disabled), std::make_pair(0x262E30, (int) StateColor::Normal))
{
    radius = 4;
    border_width     = 1;
    border_color     = StateColor(std::make_pair(0xDBDBDB, (int) StateColor::Disabled), std::make_pair(wxColour("#15BF59"), (int) StateColor::Hovered),
...
void SpinInput::updateTextValidator()
{
    if (!text_ctrl)
        return;

    wxTextValidator validator(min < 0 ? wxFILTER_INCLUDE_CHAR_LIST : wxFILTER_DIGITS);
    if (min < 0)
