...
void UnsavedChangesDialog::build(Preset::Type type, PresetCollection *dependent_presets, const std::string &new_selected_preset, const wxString &header)
{
   ...
   /* m_info_line = new wxStaticText(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(-1, 44), 0);
    ...
    auto add_btn = [this, m_sizer_button, btn_font, dependent_presets, btn_bg_green](Button **btn, int &btn_id, const std::string &icon_name, Action close_act, const wxString &label,
        ...
        *btn = new Button(this, _L(label));
        ...
        if (focus) {
            ...
            (*btn)->SetBorderColor(wxColour(255, 255, 255));
    ...
    if (ActionButtons::TRANSFER & m_buttons) {
        ...
        const Preset* new_selected = nullptr;
        if (dependent_presets && switched_presets)
            new_selected = type == dependent_presets->type() ?
                dependent_presets->find_preset(new_selected_preset) :
                switched_presets->find_preset(new_selected_preset);
        if (dependent_presets && switched_presets && new_selected && (type == dependent_presets->type() ?
            dependent_presets->get_edited_preset().printer_technology() == new_selected->printer_technology() :
            switched_presets->get_edited_preset().printer_technology() == new_selected->printer_technology()))
...
void DiffPresetDialog::create_info_lines()
{
    ...
    wxFont basic_font = m_top_info_line->GetFont();
    basic_font.SetPointSize(10);
    m_top_info_line->SetFont(basic_font);
...
void DiffPresetDialog::update_tree()
{
    ...
    for (auto preset_combos : m_preset_combos)
    {
        ...
        if (type == Preset::TYPE_PRINTER && left_pt == ptFFF &&
            ...
            std::string icon = "";
            if(category_icon_map.count("Extruder 1"))
                icon = category_icon_map.at("Extruder 1");
            m_tree->Append("extruders_count", type, "General", "Capabilities", local_label, left_val, right_val, icon);
        }
        ...
        for (const std::string& opt_key : dirty_options) {
            ...
            std::string icon = "";
            if(category_icon_map.count(option.category))
                icon = category_icon_map.at(option.category);
            m_tree->Append(opt_key, type, option.category_local, option.group_local, option.label_local,
