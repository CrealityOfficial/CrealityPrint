...
public:
    ...
    void calib_flowrate(int pass, float flowrate=1.0);
    void calib_temp(const Calib_Params& params);
    void calib_max_vol_speed(const Calib_Params& params);
    void calib_retraction(const Calib_Params& params);
    void calib_retraction_speed(const Calib_Params& params);
    void calib_limit_speed(const Calib_Params& params);
    void calib_speed_tower(const Calib_Params& params);
    void calib_jitter_speed(const Calib_Params& params);
    void calib_fan_speed(const Calib_Params& params);
    void calib_limit_acc(const Calib_Params& params);
    void calib_acc_tower(const Calib_Params& params);
    void calib_dec_acc(const Calib_Params& params);
    void calib_Arc_lerance();
    void calib_VFA(const Calib_Params& params);
    void set_calib_model(bool _value);
    ...
    void update_plate_thumbnail(wxCommandEvent& event);//Optimize it in the future
    ...
    // BBS: backup
    int export_3mf(const boost::filesystem::path& output_path = boost::filesystem::path(), SaveStrategy strategy = SaveStrategy::Default, int export_plate_idx = -1, Export3mfProgressFn proFn = nullptr, En3mfType type_3mf = En3mfType::From_Creality);
    ...
    std::map<int, int> m_bed_type_list_creality;
    ...
    ThumbnailData get_gcode_thumbnail();
    ...
    const CollapseBar::GLToolbar& get_collapse_toolbar() const;
    CollapseBar::GLToolbar& get_collapse_toolbar();

	const ProcessBar::GLToolbar& get_process_toolbar() const;
	ProcessBar::GLToolbar& get_process_toolbar();
    ...
    //Creality: set gcode bed shape
    void set_gcode_bed_shape(const Pointfs& shape, const Pointfs& exclude_area, const double printable_height, const Preset& preset) const;
    ...
    void select_bed_type(const std::string& bed_type_name);

    void show_send_localnet_printer_dlg(bool show = true);
    void create_send_to_printer_dlg();
...
private:
    ...
    void save_the_popup_successfully(const string& strPath);
