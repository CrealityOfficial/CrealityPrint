...
static const int PARTPLATE_ICON_GAP_TOP = 8;
static const int PARTPLATE_ICON_GAP_LEFT = 8;
...
static const int PARTPLATE_TEXT_OFFSET_Y = 8;
...
static ColorRGBA MODEL_SELECT_COLOR            = {0.607f, 0.607f, 0.6313f, 1.0f};
static ColorRGBA MODEL_UNSELECT_COLOR          = {0.8823f, 0.8823f, 0.9019f, 1.0f};
static ColorRGBA MODEL_SELECT_DARK_COLOR       = {0.2901f, 0.2901f, 0.3098, 1.0f};
static ColorRGBA MODEL_UNSELECT_DARK_COLOR     = {0.2235f, 0.2235f, 0.2314f, 1.0f};
...
{
    m_action_icon[e_at_unlock_triangle].reset();
    if (!init_model_from_poly(m_action_icon[e_at_unlock_triangle].model, poly, GROUND_Z))
...
void PartPlate::calc_gridlines(const ExPolygon& poly, const BoundingBox& pp_bbox) {
	...
	if (!init_model_from_lines(m_gridlines, gridlines, GROUND_Z_GRIDLINE+0.01))
		BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Unable to create bed grid lines\n";

	if (!init_model_from_lines(m_gridlines_bolder, gridlines_bolder, GROUND_Z_GRIDLINE + 0.01))
		BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Unable to create bed grid lines\n";
}
...
void PartPlate::calc_vertex_for_number(bool one_number, GLModel &buffer)
{
    buffer.reset();
	ExPolygon poly;
	...
	p(1) += PARTPLATE_TEXT_OFFSET_Y;
	poly.contour.append({ scale_(p(0) - (PARTPLATE_ICON_SIZE - offset_x)), scale_(p(1)) });
	poly.contour.append({ scale_(p(0) - offset_x), scale_(p(1)) });
	poly.contour.append({ scale_(p(0) - offset_x), scale_(p(1)+ PARTPLATE_ICON_SIZE)});
	poly.contour.append({ scale_(p(0) - (PARTPLATE_ICON_SIZE - offset_x)), scale_(p(1)+ PARTPLATE_ICON_SIZE) });

    if (!init_model_from_poly(buffer, poly, GROUND_Z))
		BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Unable to generate geometry buffers for icons\n";
}

int PartPlate::calc_vertex_for_plate_name_edit_icon(int w,PickingModel &model) {
    model.reset();
	auto    bed_ext = get_extents(m_shape);
	ExPolygon poly;
	int ypos  = bed_ext[3](1)+PARTPLATE_ICON_GAP_TOP;
	float     offset_x = 1;
	wxCoord sz = PARTPLATE_EDIT_PLATE_NAME_ICON_SIZE;
	poly.contour.append({scale_(w), scale_(ypos)});
    poly.contour.append({scale_(w + sz), scale_(ypos)});
    poly.contour.append({scale_(w + sz), scale_(ypos+sz)});
    poly.contour.append({scale_(w), scale_(ypos+sz)});
    if (!init_model_from_poly(model.model, poly, GROUND_Z))
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Unable to generate geometry buffers for icons\n";

    init_raycaster_from_model(model);
    return w + sz + PARTPLATE_ICON_GAP_LEFT;
}

void PartPlate::calc_vertex_for_name_background(int w, int h,GLModel& buffer) {
	ExPolygon poly;
    buffer.reset();
    auto      bed_ext = get_extents(m_shape);
    Vec2d p = bed_ext[3];
	p(0) -= PARTPLATE_ICON_GAP_LEFT;
    h += PARTPLATE_ICON_GAP_LEFT;
	w += PARTPLATE_ICON_GAP_LEFT;
    poly.contour.append({scale_(p(0)), scale_(p(1))});
    poly.contour.append({scale_(w + p(0)), scale_(p(1))});
    poly.contour.append({scale_(w + p(0)), scale_(p(1)+h)});
    poly.contour.append({scale_(p(0)), scale_(p(1)+h)});
    if (!init_model_from_poly(buffer, poly, GROUND_Z))
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Unable to generate geometry buffers for icons\n";
}
...
void PartPlate::calc_vertex_for_icons_background(GLModel &buffer)
{
    buffer.reset(); 
	...
	if (!init_model_from_poly(buffer, poly, GROUND_Z+0.0001))
		BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Unable to generate geometry buffers for icons\n";
}

void PartPlate::render_background(bool onlybody)
{
	// draw background
	glsafe(::glDepthMask(GL_FALSE));
	m_selected;
	//ColorRGBA color = GuiDispConfig().getGuiDispColor(GuiDispConfig::e_ct_inter);
    //m_action_icon[e_at_unlock_triangle].model.set_color(color);
   // m_action_icon[e_at_unlock_triangle].model.render();
...
void PartPlate::render_logo(bool bottom, int verder)
{
	if (verder == int(VendorType::Marlin_BBL))
	{
        ...
        for (int i = 0; i < m_partplate_list->cali_texture_info.parts.size(); ++i) {
            if (verder == int(VendorType::Creality)) {
                if (m_partplate_list->m_is_dark && i >= 2)
                    continue;
                if (!m_partplate_list->m_is_dark && i < 2)
                    continue;
            }
            auto& part = m_partplate_list->cali_texture_info.parts[i];
			// if(verder == int(VendorType::Marlin_BBL) && (m_width != 256))  //256 is X1C bed width
			// {
			// 	part.w = 224.0f * (float)m_width / 256.0f;
			// 	part.update_buffer();
			// }
            ...
            if (part.texture) {
                if (part.buffer && part.buffer->is_initialized()) {
					...
					std::string printer_model_key = full_cfg.option<ConfigOptionString>("printer_model")->value;

					if(boost::contains(printer_model_key.c_str(), "A1 mini"))
					{
						//nothing to do, invisible
					}else
                    	render_logo_texture(*(part.texture), *(part.buffer), bottom);
                }
            }
	...
	else if (verder == int(VendorType::Creality)) {
        ...
        if (curr_bed_type > btPC && curr_bed_type != btER && curr_bed_type != btDEF)
		{
            auto& part = m_partplate_list->bed_texture_info[curr_bed_type].parts[0];
            if (part.buffer && part.buffer->is_initialized()) {
                if (part.offset.x() != m_origin.x() || part.offset.y() != m_origin.y()) {
                    part.offset = Vec2d(m_origin.x(), m_origin.y());
                    part.update_buffer();
		...
		if (m_partplate_list->m_logo_texture.get_id() == 0 || m_partplate_list->m_logo_texture.get_source() != m_partplate_list->m_logo_texture_filename) {
			...
			if (boost::algorithm::iends_with(m_partplate_list->m_logo_texture_filename, ".svg")) {
				...
				// starts generating the main texture, compression will run asynchronously
				if (!m_partplate_list->m_logo_texture.load_from_png_file(m_partplate_list->m_logo_texture_filename, true, GLTexture::None, true)) {
					BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(": load logo texture from %1% failed!") % m_partplate_list->m_logo_texture_filename;
					return;
				}
			}
...
void PartPlate::render_exclude_area() {
...
void PartPlate::render_grid(bool bottom) {
	...
	if (bottom)
		...
		auto ct = m_selected ? DispConfig::e_ct_lineSel : DispConfig::e_ct_line;
		auto cc = DispConfig().getColor(ct);
		color.r(cc.x); color.g(cc.y); color.b(cc.z); color.a(cc.w);
	}
	...
	if (shader != nullptr) {
        ...
        if (!only_name) {
			...
			for(int i=0;i<e_at_unlock_triangle;++i)
			{
                ...
                if (i == e_at_lock) {
                    if (m_locked)
                        offset = DispConfig::e_tt_lock -DispConfig::e_tt_edit;
                } else if (i == e_at_set) {
					bool edited = m_config.keys().size() > 0;
                    if (edited) {
        ...
        if (!m_name.empty()) {
            ...
            render_icon_texture(m_plate_name_icon, m_partplate_list->m_is_dark ? m_name_texture_dark : m_name_texture);
        }
...
{
    for (int i = 0; i < e_at_count; ++i) {
        int id = picking_id_component(i);
		canvas.remove_raycasters_for_picking(SceneRaycaster::EType::Bed, id);
		canvas.add_raycaster_for_picking(SceneRaycaster::EType::Bed, id, *m_action_icon[i].mesh_raycaster);
	}	
}
...
void PartPlate::generate_plate_name_texture()
{
    ...
    if (!m_name.empty()){
        auto     text = from_u8(m_name);
        auto*    font = &Label::Head_32;
        if (!m_name_texture.generate_from_text_string(text.ToUTF8().data(), *font, *wxBLACK, {125, 125, 130, 0xff}))
            BOOST_LOG_TRIVIAL(error) << "PartPlate::generate_plate_name_texture(): generate_from_text_string() failed";
        if (!m_name_texture_dark.generate_from_text_string(text.ToUTF8().data(), *font, *wxBLACK, {203,203,203, 0xff}))
            BOOST_LOG_TRIVIAL(error) << "PartPlate::generate_plate_name_texture(): generate_from_text_string() failed";

        ExPolygon poly;
        float     offset_x = 1;
        wxCoord   w        = int(factor * (m_name_texture.get_width() * 16) / m_name_texture.get_height());
        Vec2d p   = bed_ext[3] + Vec2d(0, PARTPLATE_TEXT_OFFSET_Y + h * m_name_texture.m_original_height / m_name_texture.get_height());
        poly.contour.append({scale_(p(0) + offset_x), scale_(p(1) - h)});
        poly.contour.append({scale_(p(0) + w - offset_x), scale_(p(1) - h)});
        poly.contour.append({scale_(p(0) + w - offset_x), scale_(p(1))});
        poly.contour.append({scale_(p(0) + offset_x), scale_(p(1))});
    ...
    //calc_vertex_for_name_background(prew, h, m_left_top);

	auto canvas = this->m_partplate_list->m_plater->get_view3D_canvas3D();
    canvas->remove_raycasters_for_picking(SceneRaycaster::EType::Bed, picking_id_component(e_at_edit));
    canvas->add_raycaster_for_picking(SceneRaycaster::EType::Bed, picking_id_component(e_at_edit), *m_action_icon[e_at_edit].mesh_raycaster);
}
...
bool PartPlate::set_shape(const Pointfs& shape, const Pointfs& exclude_areas, Vec2d position, float height_to_lid, float height_to_rod)
{
	...
	if ((m_shape != new_shape) || (m_exclude_area != new_exclude_areas))
	{
        ...
        if (!init_model_from_poly(m_action_icon[e_at_unlock_triangle].model, poly, GROUND_Z))
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":Unable to create plate triangles\n";
        init_raycaster_from_model(m_action_icon[e_at_unlock_triangle]);
...
bool PartPlate::can_slice() 
{ 
	bool printable = false;
    ModelObjectPtrs objsInside = get_objects_on_this_plate();
    for (int i = 0, count = objsInside.size(); i < count && !printable; ++i) {
        ModelObject* obj = objsInside[i];
    ...
    return m_ready_for_slice && !m_apply_invalid && printable;
}
...
void PartPlateList::set_default_wipe_tower_pos_for_plate(int plate_idx)
{
	...
	//DynamicConfig&      prints_cfg   = wxGetApp().preset_bundle->prints.select_preset(plate_idx).config;
 //   DynamicConfig&      printers_cfg = wxGetApp().preset_bundle->printers.select_preset(plate_idx).config;
 //   ConfigOptionPoints* points2       = printers_cfg.opt<ConfigOptionPoints>("printable_area");

    const DynamicConfig&          ful_config = wxGetApp().preset_bundle->full_config();
...
void PartPlateList::set_verder_type(int tp) {
	is_load_bedtype_textures = false;
	is_load_cali_texture = false;
	m_verder = tp;
}

void PartPlateList::register_raycasters_for_picking(GLCanvas3D& canvas)
{
    for (auto plate : m_plate_list)
...
bool PartPlateList::is_are_the_slice_results_ready_to_export() const
{
	bool res = false;
...
bool PartPlateList::is_load_cali_texture = false;
 ...
 PartPlateList::BedTextureInfo::TexturePart::TexturePart(const TexturePart& part)
{
    this->x        = part.x;
    this->y        = part.y;
    this->w        = part.w;
 ...
 PartPlateList::BedTextureInfo::TexturePart::TexturePart(float xx, float yy, float ww, float hh, std::string file)
{
    x        = xx;
    y        = yy;
    w        = ww;
...
void PartPlateList::load_bedtype_textures()
{
	...
	auto  bed_ext = get_extents(m_shape);
	if (m_verder == VendorType::Marlin_BBL) {
	...
	else if (m_verder == VendorType::Creality){
		base_width = 220;
		base_height = 220;
        BedTextureInfo::TexturePart pc_part(-8.3, -22, 235, 250, "");
        bed_texture_info[btPC].parts.push_back(pc_part);
        bed_texture_info[btEP].parts.push_back(pc_part);
        bed_texture_info[btPEI].parts.push_back(pc_part);
        bed_texture_info[btPTE].parts.push_back(pc_part);
        bed_texture_info[btER].parts.push_back(pc_part);
	}
	...
	for (int i = 0; i < (unsigned int)btCount; ++i) {
		for (int j = 0; j < bed_texture_info[i].parts.size(); j++) {
			...
			if (boost::filesystem::exists(filename)) {
                auto& textu= PartPlateList::bed_texture_info[i].parts[j].texture;
				textu = new GLTexture();
				textu->load_from_png_svg_file(filename, true, false, true, logo_tex_size);
			}
		}
	}
...
void PartPlateList::load_cali_textures()
{
	if (PartPlateList::is_load_cali_texture) 
		return;
    cali_texture_info.reset();
    cali_texture_info.parts.clear();
	if (m_verder==VendorType::Marlin_BBL)
	{
        ...
        for (int j = 0; j < cali_texture_info.parts.size(); j++) {
            ...
            if (boost::filesystem::exists(filename)) {
                ...
                if (!PartPlateList::cali_texture_info.parts[j].texture->load_from_png_svg_file(filename, true, false, true, logo_tex_size)) {
    ...
    for (int j = 0; j < cali_texture_info.parts.size(); j++) {
        ...
        if (boost::filesystem::exists(filename)) {
            PartPlateList::cali_texture_info.parts[j].texture = new GLTexture();
			PartPlateList::cali_texture_info.parts[j].texture->load_from_png_svg_file(
				filename, true, false, true, 0);
        }
    }
