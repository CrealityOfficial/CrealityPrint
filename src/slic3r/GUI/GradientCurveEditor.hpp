...
// (MixedFilament::sample_gradient_curve) is shared with the slicing backend so what
...
{
public:
    // Underlying storage is the shared GradientCurve type so callers can
    // hand the editor the same curve that lives in a MixedFilament row.
    using PointList = GradientCurve;
