...
{
	return
		_L("The configuration may be generated by a newer version of CrealityPrint.") + " " +
...
std::string user_feedback_website() 
{
    ...
    // std::string strStartType = setting.value("server_type", "0").toString();
    // setting.endGroup();
	std::string strStartType = "0";
    // std::string version = PROJECT_VERSION_EXTRA;
	std::string version_type = get_vertion_type(); 
    if (strStartType == "0") {
      if (version_type != "Alpha") {
        website += "serverEnv=1";
	...
	// std::string versionNum = CrealityPrint_VERSION;
	Semver semver(SLIC3R_VERSION);
	std::string versionNum = "&version=" + std::to_string(semver.maj()) + "." + std::to_string(semver.min()) + "." + std::to_string(semver.patch());
	...
	hash.process_bytes(hostname.data(), hostname.size());
    hash.get_digest(digest);
    const unsigned char* charDigest = reinterpret_cast<const unsigned char*>(&digest);
    std::string result;
    boost::algorithm::hex(charDigest, charDigest + sizeof(md5::digest_type), std::back_inserter(result));

	website += "&deviceId=" + result;

    // std::string cloudId = CloudAccountService().lock()->getUserId();
