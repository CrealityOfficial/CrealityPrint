...
//-----------------------------------------------
    ...
    std::string addIncrementedNumberInParentheses(std::string input) {
        std::regex pattern("\\((\\d+)\\)$"); //
        std::smatch match;
        ...
        if (std::regex_search(input, match, pattern)) {
            ...
            numberCounts[number]++; //
            number++; //
            input = input.substr(0, input.length() - match.length()) + "(" + std::to_string(number) + ")"; //
        }
        else {
            //
            input += "(1)";
...
SavePresetDialog::Item::Item(Preset::Type type, const std::string &suffix, wxBoxSizer *sizer, SavePresetDialog *parent) : m_type(type), m_parent(parent)
{
    ...
    std::string tipText = m_parent->m_IsSave ? L("Save %s") : L("Save %s as");
    wxStaticText *label_top = new wxStaticText(m_parent, wxID_ANY, from_u8((boost::format(_utf8(tipText)) % into_u8(tab->title())).str()));
    ...
    const Preset* existing = m_presets->find_preset(preset_name, false);
    wxString presetName = from_u8(preset_name);
...
void SavePresetDialog::Item::update()
{
    ...
    if (m_valid_type == Valid && existing && m_preset_name != m_presets->get_selected_preset_name()) {
        ...
        {
            info_line = _L("The name cannot be duplicated.");
        }
    ...
    if (m_type == Preset::TYPE_PRINTER || m_type == Preset::TYPE_FILAMENT)
    {
        if (m_valid_type == Valid)
        {
            if (m_parent->m_confirm) {
                m_parent->m_confirm->Enable();
                m_parent->m_confirm->SetTextColor(wxColour("#FFFFFE"));
                m_parent->m_confirm->SetBackgroundColorNormal(wxColour(0x6E6E73));
            }
        }
        else {
            if (m_parent->m_confirm) {
                m_parent->m_confirm->Disable();
                m_parent->m_confirm->SetTextColor(wxColour(0x999999));
                m_parent->m_confirm->SetBackgroundColorNormal(wxColour(0x565658));
            }
        }
    }
...
SavePresetDialog::SavePresetDialog(wxWindow *parent, Preset::Type type, std::string suffix, bool isSave /*=false*/)
...
void SavePresetDialog::build(std::vector<Preset::Type> types, std::string suffix)
{
    ...
    StateColor btn_bg_grey(
        ...
        std::pair<wxColour, int>(wxColour(0x6E6E73), StateColor::Normal)
    );
