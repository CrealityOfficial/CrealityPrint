...
wxBoxSizer* WipingDialog::create_btn_sizer(long flags)
{
    ...
    StateColor ok_btn_bg(
        std::pair<wxColour, int>(wxColour(23, 204, 95), StateColor::Pressed),
...
,m_matrix(matrix), m_min_flush_volume(extra_flush_volume), m_max_flush_volume(Slic3r::g_max_flush_volume)
{
    ...
    {
        wxString message = _L("CrealityPrint would re-calculate your flushing volumes everytime the filaments color changed.");
    ...
    // BBS: for tunning flush volumes
    {
        ...
        wxFont m_min_flush_label_font = m_min_flush_label->GetFont();
        ...
        m_sizer_advanced->Add(m_type_flush_label, 0, wxEXPAND | wxLEFT, TEXT_BEG_PADDING);
        wxFont m_type_flush_label_font = m_type_flush_label->GetFont();
        m_type_flush_label_font.SetPointSize(10);
        m_type_flush_label->SetFont(m_type_flush_label_font);
        ...
        wxFont flush_multiplier_title_font = flush_multiplier_title->GetFont();
        flush_multiplier_title_font.SetPointSize(10);
        flush_multiplier_title->SetFont(flush_multiplier_title_font);
        ...
        param_sizer->AddSpacer(FromDIP(5));
        m_btn_reset = new ScalableButton(m_page_advanced, wxID_ANY, false ? "undo" : "undo", wxEmptyString,
                                           wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER, true, FromDIP(14));
        m_btn_reset->SetToolTip(_L("Reset"));
        m_btn_reset->Bind(wxEVT_LEFT_DOWN, ([this,on_apply_text_modify](auto& e) { on_set_sys_value(); on_apply_text_modify(e);}));
        param_sizer->Add(m_btn_reset, 0, wxALIGN_CENTER | wxALL, 0);
...
void WipingPanel::update_warning_texts()
{
    ...
    for (int i = 0; i < edit_boxes.size(); i++) {
        ...
        for (int j = 0; j < box_vec.size(); j++) {
            ...
            bool     isDiffType    = isDifferentFilament(i,j);
            wxString str = text_box->GetValue();
            int actual_volume = wxAtoi(str);
            bool     diffFilament = isDiffType && actual_volume < m_type_flush_volume;
            bool     flushScope    = actual_volume < m_min_flush_volume[i] || actual_volume > m_max_flush_volume;
            if (diffFilament)
    ...
    if (has_exception_flush && m_min_flush_label->GetForegroundColour() != g_warning_color) {
     ...
     if (has_diff_type_flush && m_type_flush_label->GetForegroundColour() != g_warning_color) {
        m_type_flush_label->SetForegroundColour(g_warning_color);
         m_type_flush_label->Refresh();
     } else if (!has_diff_type_flush && m_type_flush_label->GetForegroundColour() != g_text_color) {
         m_type_flush_label->SetForegroundColour(g_text_color);
         m_type_flush_label->Refresh();
    }
    wxString str = m_flush_multiplier_ebox->GetValue();
                ...
                if (is_to_support) {
                    ...
                    for (int i = 0; i < multi_colors[from_idx].size(); ++i) {
                        ...
                        for (int j = 0; j < multi_colors[to_idx].size(); ++j) {
                            ...
                            bool is_diff_filament  = isDifferentFilament(from_idx, to_idx);
                            if (is_diff_filament) {
                                    flushing_volume = std::max(flushing_volume, m_type_flush_volume);
                            }
                        }
                    }
