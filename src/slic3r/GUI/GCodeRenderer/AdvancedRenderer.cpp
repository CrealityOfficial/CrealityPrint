...
namespace
{
    float get_move_data_from_view_type(const Slic3r::GUI::EViewType type, const Slic3r::GCodeProcessorResult::MoveVertex& t_move)
    {
        switch (type)
        {
        ...
        //case Slic3r::GUI::EViewType::Summary:
    ...
    void add_segment(const Slic3r::GUI::Segment& t_seg, const std::vector<Slic3r::GUI::SegmentVertex>& segment_vertex, std::vector<float>& segment_index_list, uint32_t& prev_seg_index, bool flag = false) {
...
namespace Slic3r
{
    namespace GUI
    {
        ...
        //{
            ...
            void AdvancedRenderer::init(ConfigOptionMode mode, Slic3r::PresetBundle* preset_bundle, bool isgcode)
            {
            ...
            void AdvancedRenderer::update_sequential_view_current(unsigned int first, unsigned int last)
            {
				...
				//param first as moves slider lower value, but it is uncorrect!
				if (m_moves_slider->is_one_layer()) {
                    p_layer_manager->set_current_move_start(0);
				} else {
                    p_layer_manager->set_current_move_start(first);
				}
            ...
            void AdvancedRenderer::export_toolpaths_to_obj(const char* filename) const
            {
                ...
                if (EViewType::LayerTimeLog == m_view_type) {
                    range_type = Extrusions::Range::EType::Logarithmic;
                ...
                auto do_export = [&](const std::vector<float>& seg_list
                    ...
                    for (size_t i = 0; i < seg_count; ++i) {
                        ...
                        if (is_range_data_valid) {
                            if (range_type == Extrusions::Range::EType::Linear) {
            ...
            void AdvancedRenderer::render(int canvas_width, int canvas_height)
            {
                ...
                SequentialView *m_p_sequential_view = &m_sequential_view;
                if (m_p_sequential_view) {
					...
					if (!is_capture_mode)
					{
                        ...
                        if (m_gcode_result) {
                            ...
                            marker.update_curr_move(t_move_vertex);
                        }
            ...
            bool AdvancedRenderer::is_toolpath_move_type_visible(EMoveType type) const
            {
                const auto& p_layer_manager = get_layer_manager();
                if (p_layer_manager) {
                    return p_layer_manager->is_toolpath_move_type_visible(type);
                }
                return false;
            }

            void AdvancedRenderer::set_toolpath_move_type_visible(EMoveType type, bool visible)
            {
                const auto& p_layer_manager = get_layer_manager();
                if (p_layer_manager) {
                    p_layer_manager->set_toolpath_move_type_visible(type, visible);
                }
            }
            ...
            bool AdvancedRenderer::load_toolpaths(const GCodeProcessorResult& gcode_result, const BuildVolume& build_volume, const std::vector<BoundingBoxf3>& exclude_bounding_box)
            {
				...
				// set layers z range
                if (m_p_layer_manager && !m_p_layer_manager->empty())
                    m_layers_z_range = {0, static_cast<unsigned int>(m_p_layer_manager->size() - 1)};
            ...
            void AdvancedRenderer::load_layer_info(const GCodeProcessorResult& gcode_result, const BuildVolume& build_volume, const std::vector<BoundingBoxf3>& exclude_bounding_box)
            {
                ...
                for (size_t i = 0; i < t_move_count; ++i) {
                    ...
                    if (move.type == EMoveType::Extrude || move.type == EMoveType::Extrude_Alter) {
                        ...
                        if (move_id - last_travel_s_id > 1 && !p_layer_manager->empty()) {
                            auto& last_layer = p_layer_manager->get_layer(p_layer_manager->size() - 1);
                            last_layer.set_end(move_id);
                        }
                ...
                // layer is composite of extrude, push continuous traval/retract/unretract/wipe before extrude to the same layer
                {
                    ...
                    auto layer_size = m_p_layer_manager->size();
                    for (size_t i = 0; i < layer_size; ++i) {
                        
                        Layer& endpoint = m_p_layer_manager->get_layer(i);
                        if (i == 0)
                            endpoint.set_start( 0 );
				...
				move_ids_mapper.clear();
                move_ids_mapper.shrink_to_fit();
            ...
            void AdvancedRenderer::do_render_others(const std::vector<uint32_t>& layer_index_list, bool top_layer_only)
            {
                ...
                // draw opaque
                GLShaderProgram* p_shader = wxGetApp().get_shader("gcode");
                if (!p_shader) {
                    return;
                }
                ...
                for (size_t i = 0; i < layer_index_list.size(); ++i) {
					...
					if (m_view_type == EViewType::ColorPrint && t_layer.get_effects().size() > 0) {
						// make Pause_Print && Custom_GCode render as dark grey
                        p_shader->set_uniform("u_isRangeView_isRangeVaild_topLayerOnly_viewType",
                                             Vec4f(1.0f, float(data_range.max - data_range.min > 1e-6f), 1.0f,
                                                   float(m_view_type)));
                        p_shader->set_uniform("u_isTopLayer_hasCustomOptins", Vec2f(0.0f, 0.0f));
                    } else {
                        p_shader->set_uniform("u_isRangeView_isRangeVaild_topLayerOnly_viewType",
                                              Vec4f(1.0f, float(data_range.max - data_range.min > 1e-6f), float(top_layer_only),
                                                    float(m_view_type)));
                ...
                // Restore active texture unit to 0, preventing contamination of subsequent UI rendering (e.g., font textures).
            ...
            void AdvancedRenderer::do_render_options(const std::vector<uint32_t>& layer_index_list, bool top_layer_only)
            {
                ...
                // Restore active texture unit to 0, preventing contamination of subsequent UI rendering.
                glsafe(::glActiveTexture(GL_TEXTURE0));
                wxGetApp().unbind_shader();
            }
            ...
            BaseRenderer::Extrusions::Range AdvancedRenderer::get_range_according_to_view_type(EViewType type) const
            {
                Extrusions::Range t_range;
                t_range.min = 0;
                t_range.max = 0;
                t_range.count = 0;
                /*if (!m_extrusions) {
                ...
                {
                case Slic3r::GUI::EViewType::Height:
                    return m_extrusions.ranges.height;
                case Slic3r::GUI::EViewType::Width:
                    return m_extrusions.ranges.width;
                case Slic3r::GUI::EViewType::Feedrate:
                    return m_extrusions.ranges.feedrate;
                case Slic3r::GUI::EViewType::FanSpeed:
                    return m_extrusions.ranges.fan_speed;
                case Slic3r::GUI::EViewType::Temperature:
                    return m_extrusions.ranges.temperature;
                case Slic3r::GUI::EViewType::LayerTime:
                    return m_extrusions.ranges.layer_duration;
                case Slic3r::GUI::EViewType::LayerTimeLog:
					return m_extrusions.ranges.layer_duration_log;
                case Slic3r::GUI::EViewType::VolumetricRate:
                    return m_extrusions.ranges.volumetric_rate;
                case Slic3r::GUI::EViewType::Acceleration:
					return m_extrusions.ranges.acceleration;
                case Slic3r::GUI::EViewType::FeatureType:
                {
                    t_range.min = 0;
                ...
                case Slic3r::GUI::EViewType::FilamentId:
                {
                    t_range.min = 0.0f;
                    t_range.max = 256.0f;
                    return t_range;
            ...
            void AdvancedRenderer::bind_role_colors_texture(uint8_t stage)
            {
                ...
                if (!m_p_role_colors_texture) {
                    ...
                    std::memset(temp_data.data(), 0, temp_data.size() * sizeof(float));
                    for (size_t i = 0; i < color_count; ++i) {
                        for (int j = 0; j < Extrusion_Role_Colors[i].get_data().size(); ++j) {
            ...
            void AdvancedRenderer::bind_option_colors_texture(uint8_t stage)
            {
                ...
                if (!m_p_option_colors_texture) {
                    ...
                    std::memset(temp_data.data(), 0, temp_data.size() * sizeof(float));
                    for (size_t i = 0; i < color_count; ++i) {
                        for (int j = 0; j < Options_Colors[i].get_data().size(); ++j) {
            ...
            void AdvancedRenderer::bind_tool_colors_texture(uint8_t stage)
            {
                ...
                if (m_b_tool_colors_dirty) {
                    ...
                    for (size_t i = 0; i < color_count; ++i) {
                        ...
                        for (int j = 0; j < t_color.get_data().size(); ++j) {
            ...
            void AdvancedRenderer::initialize_triangular_prism()
            {
                ...
                t_geo.format.type = GLModel::Geometry::EPrimitiveType::Triangles;
            ...
            void Layer::update_visible_segment_list(const LayerManager& t_layer_manager, const std::vector<bool>& filament_visible_flags, uint32_t start_seg_index, uint32_t end_seg_index)
            {
                ...
                for (int i_seg = t_start_seg_index; i_seg <= t_end_seg_index; ++i_seg) {
                    ...
                    if (!t_layer_manager.is_toolpath_move_type_visible(t_seg.m_type)) {
                        continue;
                    }
                    ...
                    if (t_view_type == EViewType::ColorPrint &&
                        (t_seg.m_extruder_id < filament_visible_flags.size() && !filament_visible_flags[t_seg.m_extruder_id])) {
                        continue;
                    }
            ...
            uint32_t Layer::get_current_move_id(uint32_t seg_index) const
            {
                ...
                if (seg_index >= 1) {
                    const uint32_t vis_idx = seg_index - 1;
                    if (vis_idx < t_visibile_segments.size() && t_visibile_segments[vis_idx] < t_segments.size()) {
                ...
                {
                    if (!t_visibile_segments.empty() && t_visibile_segments[0] < t_segments.size()) {
            ...
            bool LayerManager::is_toolpath_move_type_visible(EMoveType type) const
            {
                return m_move_type_visible_flag & (1 << static_cast<int>(type));
            }

            void LayerManager::set_toolpath_move_type_visible(EMoveType type, bool visible)
            {
        ...
        namespace render
        {
            ...
            void BasicEffect::update_uniform(const GLShaderProgram * p_shader)
            {
            }
            ...
            const GLShaderProgram* ColorEffect::get_shader() const
            {
                const auto& p_shader = wxGetApp().get_shader("gcode_custom_effect");
                return p_shader;
            }

            void ColorEffect::update_uniform(const GLShaderProgram * p_shader)
            {
                if (!p_shader) {
                    return;
                }
