...
{
    switch (mode) {
    case Slic3r::GUI::GLTexture::ESamplerWrapMode::Repeat: return GL_REPEAT;
    case Slic3r::GUI::GLTexture::ESamplerWrapMode::MirrorRepeat: return GL_MIRRORED_REPEAT;
    case Slic3r::GUI::GLTexture::ESamplerWrapMode::Border: return GL_CLAMP_TO_BORDER;
...
bool GLTexture::load_from_png_svg_file(
    const std::string& filename, bool use_mipmaps, bool compress, bool apply_anisotropy, unsigned int max_size_px)
{
    ...
    if (boost::algorithm::iends_with(filename, ".png"))
        return load_from_png(filename, use_mipmaps, compress?ECompressionType::MultiThreaded:ECompressionType::None, apply_anisotropy);
    else if(boost::algorithm::iends_with(filename, ".svg"))
...
bool GLTexture::load_from_svg_files_as_sprites_array(const std::vector<std::string>& filenames,
    const std::vector<std::string>& border_filenames,
    ...
    bool                                     compress)
{
    ...
    //BBS
    std::vector<unsigned char> pressed_data(sprite_bytes, 0); // (gizmo) pressed
    std::vector<unsigned char> pressed_data_2(sprite_bytes, 0);   //process item press
    std::vector<unsigned char> hover_data(sprite_bytes, 0); // hover
    ...
    if (border_filenames.size() >= 4)
    {
        ...
        {
            float scale = (float)sprite_size_px / std::max(image->width, image->height);
            nsvgRasterize(rast, image, 1, 1, scale, border_disable.data(), sprite_size_px, sprite_size_px, sprite_stride);
            nsvgDelete(image);
        }

        image = nsvgParseFromFile(border_filenames.at(1).c_str(), "px", 96.0f);
        if (image != nullptr)
        {
            float scale = (float)sprite_size_px / std::max(image->width, image->height);
            nsvgRasterize(rast, image, 1, 1, scale, border_hover.data(), sprite_size_px, sprite_size_px, sprite_stride);
            nsvgDelete(image);
        }

        image = nsvgParseFromFile(border_filenames.at(2).c_str(), "px", 96.0f);
        if (image != nullptr)
        {
            float scale = (float)sprite_size_px / std::max(image->width, image->height);
    ...
    for (const std::string& filename : filenames) {
        ...
        // offset by 1 to leave the first pixel empty (both in x and y)
        nsvgRasterize(rast, image, 1, 1, scale,
                      sprite_data.data(), sprite_size_px, sprite_size_px,
                      sprite_stride);


        //Pressed_1
        ::memcpy((void*)pressed_data.data(), (const void*)border_press.data(), sprite_bytes);
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (sprite_data.data()[offset + 0] != 0 ||
                sprite_data.data()[offset + 1] != 0 ||
                sprite_data.data()[offset + 2] != 0)
            {
                unsigned char alpha = sprite_data[offset + 3];
                pressed_data.data()[offset + 0] = (pressed_color[0] * alpha + border_press[offset + 0] * (255 - alpha)) / 255;
                pressed_data.data()[offset + 1] = (pressed_color[1] * alpha + border_press[offset + 1] * (255 - alpha)) / 255;
                pressed_data.data()[offset + 2] = (pressed_color[2] * alpha + border_press[offset + 2] * (255 - alpha)) / 255;
                pressed_data.data()[offset + 3] = 255;
            }
        }
        

        // Pressed_2
        ::memcpy((void*) pressed_data_2.data(), (const void*) border_press.data(), sprite_bytes);
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (sprite_data.data()[offset + 0] != 0 || sprite_data.data()[offset + 1] != 0 || sprite_data.data()[offset + 2] != 0) {
                unsigned char alpha             = sprite_data[offset + 3];
                pressed_data_2.data()[offset + 0] = (sprite_data[offset + 0] * alpha + border_press[offset + 0] * (255 - alpha)) / 255;
                pressed_data_2.data()[offset + 1] = (sprite_data[offset + 1] * alpha + border_press[offset + 1] * (255 - alpha)) / 255;
                pressed_data_2.data()[offset + 2] = (sprite_data[offset + 2] * alpha + border_press[offset + 2] * (255 - alpha)) / 255;
                pressed_data_2.data()[offset + 3] = 255;
            }
        }
        
        //Hover
        ::memcpy((void*)hover_data.data(), (const void*)border_hover.data(), sprite_bytes);
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (sprite_data.data()[offset + 0] != 0 ||
                sprite_data.data()[offset + 1] != 0 ||
                sprite_data.data()[offset + 2] != 0)
            {
                hover_data.data()[offset + 0] = sprite_data[offset + 0];
                hover_data.data()[offset + 1] = sprite_data[offset + 1];
                hover_data.data()[offset + 2] = sprite_data[offset + 2];
                hover_data.data()[offset + 3] = sprite_data[offset + 3];
            }
        }
        ...
        for (int i = 0; i < sprite_n_pixels; ++i) {
            ...
            if (sprite_data.data()[offset + 0] != 0 ||
                ...
                sprite_data.data()[offset + 2] != 0)
            {
                sprite_white_only_data.data()[offset + 0] = sprite_data[offset + 0];
                sprite_white_only_data.data()[offset + 1] = sprite_data[offset + 1];
                sprite_white_only_data.data()[offset + 2] = sprite_data[offset + 2];
                sprite_white_only_data.data()[offset + 3] = sprite_data[offset + 3];
            }
        }

        //Disable
        ::memcpy((void*)sprite_gray_only_data.data(), (const void*)border_disable.data(), sprite_bytes);
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (sprite_data.data()[offset + 0] != 0 ||
                sprite_data.data()[offset + 1] != 0 ||
                ...
                sprite_gray_only_data.data()[offset + 1] = dark_mode ? disable_color_dark[1] : disable_color[1];
                sprite_gray_only_data.data()[offset + 2] = dark_mode ? disable_color_dark[2] : disable_color[2];
                sprite_gray_only_data.data()[offset + 3] = sprite_data.data()[offset + 3];
            }
        }
