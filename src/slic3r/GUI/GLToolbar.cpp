...
wxDEFINE_EVENT(EVT_GLTOOLBAR_FLUIDD_PRINT_MACHINE, SimpleEvent);
wxDEFINE_EVENT(EVT_GLTOOLBAR_SEND_TO_LOCAL_NET_PRINTER, SimpleEvent);
wxDEFINE_EVENT(EVT_GLTOOLBAR_UPLOAD_3MF, SimpleEvent);


wxDEFINE_EVENT(EVT_GLTOOLBAR_PRINTER, SimpleEvent);
...
{
    auto uv  = get_uvs_with_render_state(render_state, tex_width, tex_height, icon_size);
    GLTexture::render_sub_texture(tex_id, left, right, bottom, top, uv);
...
void GLToolbarItem::render(unsigned int tex_id, float left, float right, float bottom, float top, GLTexture::Quad_UVs uv) const
{
    GLTexture::render_sub_texture(tex_id, left, right, bottom, top, uv);
...
{
    auto uv = get_uvs(tex_width, tex_height, icon_size);
    GLTexture::render_sub_texture(tex_id, left, right, bottom, top, uv);
...
GLTexture::Quad_UVs GLToolbarItem::get_uvs_with_render_state(int          render_state,
                                                             unsigned int tex_width,
                                                             unsigned int tex_height,
                                                             unsigned int icon_size) const
{
...
bool GLToolbar::init(const BackgroundTexture::Metadata& background_texture)
{
    ...
    if (!background_texture.filename.empty())
        res = m_background_texture.texture.load_from_png_file(path + background_texture.filename, false, GLTexture::SingleThreaded, false);
...
void GLToolbar::render(const GLCanvas3D& parent, float scroll, GLToolbarItem::EType type)
{
    if (!m_enabled || m_items.empty())
        return;
...
float GLToolbar::get_width_horizontal() const
{
    ...
    if (m_items.size() > 1)
        size += ((float)nVisCnt - 1.0f) * m_layout.gap_size;
...
void GLToolbar::update_hover_state_horizontal(const Vec2d& mouse_pos, GLCanvas3D& parent)
{
    ...
    float left = m_layout.left + border + m_layout.scroll;
...
{
    const GLGizmosManager& gizmos = parent.get_gizmos_manager();
    ...
    render_background(left, top, m_horizontal_expand?1.0:right, bottom, border_w, border_h);
    ...
    for (const GLToolbarItem* item : m_items) {
        ...
        if (item->is_separator())
            ...
            if (!item->is_action_with_text_image()) {
                ...
                GLGizmosManager::EType type = gizmos.get_gizmo_from_name(filename); 
                GLGizmoBase* base = gizmos.get_gizmo(type);
                GLToolbarItem::EState idx  = (GLToolbarItem::EState) gizmos.get_state_index(type);
...
void GLToolbar::render_horizontal(const GLCanvas3D& parent, GLToolbarItem::EType type, float scroll) 
{
    const GLGizmosManager& gizmos = parent.get_gizmos_manager();
    ...
    const float bg_left = 2.0f * (m_layout.left) * inv_cnv_w;
    float top   = 2.0f * m_layout.top * inv_cnv_h;
    ...
    float left = 2.0f * (m_layout.left + scroll) * inv_cnv_w;
    left += border_w;
    top -= border_h;
        ...
        if (item->is_separator())
            ...
            if (!item->is_action_with_text_image() && !item->is_action_with_text()) {
                ...
                } else if (left > right) 
                {
                    ...
                    if (left < bg_left && left + icon_stride > bg_left) {
                        // render right part of the item
...
bool GLToolbar::generate_icons_texture()
{
    ...
    const std::vector<std::string>& icon_borders = m_background_texture.metadata.icon_borders;
    std::vector<std::string> borders;
    for (size_t i = 0; i < icon_borders.size(); i++) {
        
        const std::string& icon_border = icon_borders.at(i);
        if (!icon_border.empty())
            borders.push_back(path + icon_border);
    }
...
//        sprite_size_px += 1;
    ...
    bool res;
    if (borders.size() > 0) {    
        res = m_icons_texture.load_from_svg_files_as_sprites_array(filenames, borders, states, sprite_size_px, false);
    } else {
        res = m_icons_texture.load_from_svg_files_as_sprites_array(filenames, states, sprite_size_px, false);
    }
...
float GLToolbar::get_scaled_gap_size() { 
    return m_layout.gap_size * m_layout.scale; 
}

Vec2f GLToolbar::get_separator_position(int separator_idx)
{ 
    if (separator_idx < 0) 
    ...
    for (size_t i = 0; i < m_items.size(); i++) {
        ...
        if (item->is_separator() && left - gap_size > m_layout.left) {
            idx++;
            if (idx == separator_idx) {
                return Vec2f(left - gap_size, top);
            }
        }
