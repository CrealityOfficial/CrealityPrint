...
void BackgroundSlicingProcess::process_fff()
{
	...
	if (this->set_step_started(bspsGCodeFinalize)) {
		...
		{
			TempParamater tp;
			tp.is_bbl_printer = m_fff_print->is_BBL_printer();
			tp.extruderCount = m_fff_print->extruders(true).size();
			tp.plate_origin = m_fff_print->get_plate_origin();
			tp.plate_index = m_fff_print->get_plate_index();
			Calib_Params calib_params;
			calib_params.start = 0.0;
			calib_params.end = 0.0;
			calib_params.step = 0.0;
			calib_params.print_numbers = false;
			calib_params.bed_temp = 0.0;
			calib_params.acc_speed = 0.0;
			calib_params.print_speed = 0.0;
			calib_params.high_step = 0.0;
			calib_params.mode = CalibMode::Calib_None;
			ThumbnailsList thumbnails;
			std::string blpath = Slic3r::BLCompareTestFlow::BLPath();
			std::string result_path = Slic3r::BLCompareTestFlow::CompareResultPath();
 			int bl_type = Slic3r::BLCompareTestFlow::blType();
			wxString project_name = GUI::wxGetApp().mainframe->m_plater->get_project_name();
			std::string bl_name = "3mf_" + project_name.ToStdString();

			slice_bl_function(m_fff_print->model(), current_print()->full_print_config(), calib_params, thumbnails, tp, bl_type, bl_name, blpath, result_path);
		}
#endif
...
void export_metas_keys()
{
    ...
    {
        json                     j;
        std::vector<std::string> keys = Preset::machine_options();
...
void export_metas_impl()
{
    ...
    const ConfigDef*   _def = new_full_config.def();
    {
        ...
        save_nlohmann_json("my-text-fdm_machine_common.json", j);
    }
