...
AMSMaterialsSetting::AMSMaterialsSetting(wxWindow *parent, wxWindowID id)
    : DPIDialog(parent, id, _L("CFS Materials Setting"), wxDefaultPosition, wxDefaultSize, wxBORDER_NONE)
...
void AMSMaterialsSetting::create_panel_normal(wxWindow* parent)
{
    ...
    m_tip_readonly = new Label(parent, _L("Setting CFS slot information while printing is not supported"));
...
void AMSMaterialsSetting::enable_confirm_button(bool en)
{
    ...
    if (!m_is_third) {
        ...
        if ( !is_virtual_tray() ) {
            m_tip_readonly->SetLabelText(_L("Setting CFS slot information while printing is not supported"));
        }
        else {
    ...
    :PopupWindow(parent, wxBORDER_NONE)
{
    ...
    auto m_title_ams = new wxStaticText(m_def_color_box, wxID_ANY, _L("CFS"), wxDefaultPosition, wxDefaultSize, 0);
