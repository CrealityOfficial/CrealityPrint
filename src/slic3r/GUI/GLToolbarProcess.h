#ifndef GLTOOLBARPROCESS_H
#define GLTOOLBARPROCESS_H
...
namespace ProcessBar {
	...
	public:
		...
		void set_layout_dirty() { m_layout.dirty = true; }
		...
		GLToolbarItem* get_item(const std::string& item_name, bool check_enabled = true);
        GLToolbarItem* get_item(int item_id);
		//BBS: GUI refactor: GLToolbar
		...
		void reset_all_items_state_to_normal();

		void on_params_panel_tab_changed(Slic3r::GUI::Tab* cur_tab);

		static std::string get_process_toolbar_item_icon_file(std::string item_name, bool is_dark, bool is_modified);
