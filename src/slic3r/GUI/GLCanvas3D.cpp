...
static Slic3r::ColorRGBA DEFAULT_BG_LIGHT_COLOR_DARK = {0x36 / 255.0, 0x36 / 255.0, 0x38 / 255.0, 1.0f};
...
bool GLCanvas3D::LayersEditing::is_enabled() const { return m_enabled; }

void GLCanvas3D::LayersEditing::set_enabled(bool enabled) { m_enabled = is_allowed() && enabled; }
...
void GLCanvas3D::LayersEditing::render_variable_layer_height_dialog(const GLCanvas3D& canvas)
{
    ...
    float max_text_size = std::max(quality_speed_text_size.x, smooth_text_size.x);
    ...
    double adaptive_quality = m_adaptive_quality;
    bool   changed          = wxGetApp().plater()->get_view3D_canvas3D()->draw_input_double("##adaptive_input", &adaptive_quality, 0.05f,
                                                                                            Vec2d(input_box_width, 30.f));
    if (changed) {
        m_adaptive_quality = adaptive_quality;
    }
    ImGui::SameLine();

    bool   dark = wxGetApp().plater()->get_view3D_canvas3D()->get_dark_mode_status();
    ImVec4 normal_color(110 / 255.0, 110 / 255.0, 115 / 255.0, 1.0);

    auto render_round_button = [&dark, &normal_color](const bool hover, std::function<void()> btn_func) {
        if (btn_func == nullptr)
            return;
        ...
        ImGui::PushStyleColor(ImGuiCol_Button, dark ? normal_color : ImGui::GetStyleColorVec4(ImGuiCol_WindowBg));
        ImGui::PushStyleColor(ImGuiCol_Border,
                              dark ? ImGui::GetStyleColorVec4(ImGuiCol_Border) : ImVec4(214.0 / 255, 214.0 / 255, 220.0 / 255, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_Text, hover ? ImVec4(1, 1, 1, 1) : ImGui::GetStyleColorVec4(ImGuiCol_Text));
    ...
    static bool adaptive_hover = false;
    render_round_button(adaptive_hover, [&]() {
        if (ImGui::Button((apply_text + "##Adaptive").c_str()))
            wxPostEvent((wxEvtHandler*) canvas.get_wxglcanvas(),
                        Event<float>(EVT_GLCANVAS_ADAPTIVE_LAYER_HEIGHT_PROFILE, m_adaptive_quality));

        adaptive_hover = ImGui::IsItemHovered();
    });
    ...
    if (ImGui::BBLSliderScalar("##radius_slider", ImGuiDataType_S32, &radius, &v_min, &v_max)) {
        radius                 = std::clamp(radius, v_min, v_max);
    ...
    static bool smooth_hover = false;
    render_round_button(smooth_hover, [&]() {
        if (ImGui::Button((apply_text + "##Smooth").c_str()))
            wxPostEvent((wxEvtHandler*) canvas.get_wxglcanvas(),
                        HeightProfileSmoothEvent(EVT_GLCANVAS_SMOOTH_LAYER_HEIGHT_PROFILE, m_smooth_params));
        smooth_hover = ImGui::IsItemHovered();
    });
    ...
    static bool reset_hover = false;
    render_round_button(reset_hover, [&]() {
        if (imgui.button(_L("Reset")))
            wxPostEvent((wxEvtHandler*) canvas.get_wxglcanvas(), SimpleEvent(EVT_GLCANVAS_RESET_LAYER_HEIGHT_PROFILE));
        reset_hover = ImGui::IsItemHovered();
    });
...
Rect GLCanvas3D::LayersEditing::get_bar_rect_screen(const GLCanvas3D& canvas)
{
    ...
    float       bar_width = thickness_bar_width(canvas);
    float       margin    = canvas._layer_editing_profile_right_margin();
    return {w - bar_width - margin, 0.0f, w - margin, h};
}
...
void GLCanvas3D::LayersEditing::render_active_object_annotations(const GLCanvas3D& canvas)
{
    ...
    if (!m_profile.background.is_initialized() || m_profile.old_canvas_width != cnv_width || m_profile.old_canvas_height != cnv_height) {
        ...
        float margin = canvas._layer_editing_profile_right_margin();
        // vertices
        const float l = 1.0f - 2.0f * (thickness_bar_width(canvas) + margin) * cnv_inv_width;
        const float r = 1.0f - 2.0f * margin * cnv_inv_width;
        const float t = 1.0f;
        const float b = -1.0f;
...
void GLCanvas3D::LayersEditing::render_profile(const GLCanvas3D& canvas)
{
    ...
    const float scale_x = thickness_bar_width(canvas) / float(1.12 * m_slicing_parameters->max_layer_height);
    ...
    // Baseline
    if (!m_profile.baseline.is_initialized() || profile_geometry_changed || m_profile.old_layer_height_profile != m_layer_height_profile) {
        ...
        // vertices
        const float axis_x = 2.0f * ((cnv_width - thickness_bar_width(canvas) - canvas._layer_editing_profile_right_margin() +
                                      float(m_slicing_parameters->layer_height) * scale_x) *
                                         cnv_inv_width -
                                     0.5f);
        ...
        m_profile.baseline.set_color(ColorRGBA::BLACK());
    }

    if (!m_profile.profile.is_initialized() || profile_geometry_changed || m_profile.old_layer_height_profile != m_layer_height_profile) {
        ...
        for (unsigned int i = 0; i < (unsigned int) m_layer_height_profile.size(); i += 2) {
            init_data.add_vertex(Vec2f(2.0f * ((cnv_width - thickness_bar_width(canvas) - canvas._layer_editing_profile_right_margin() +
                                                float(m_layer_height_profile[i + 1]) * scale_x) *
                                                   cnv_inv_width -
                                               0.5f),
        ...
        m_profile.profile.set_color(ColorRGBA::BLUE());
    }
...
void GLCanvas3D::LayersEditing::adjust_layer_height_profile()
{
    ...
    Slic3r::adjust_layer_height_profile(*m_slicing_parameters, m_layer_height_profile, this->last_z, this->strength, this->band_width,
                                        this->last_action);
    m_layers_texture.valid = false;
}
...
void GLCanvas3D::LayersEditing::adaptive_layer_height_profile(GLCanvas3D& canvas, float quality_factor)
{
    ...
    // m_layer_height_profile =layer_height_overhang(*m_slicing_parameters,*m_model_object,0.2f);
...
GLCanvas3D::Mouse::Mouse()
    : dragging(false), position(DBL_MAX, DBL_MAX), scene_position(DBL_MAX, DBL_MAX, DBL_MAX), ignore_left_up(false), ignore_right_up(false)
{}
                                                        ...
                                                        const std::vector<std::pair<Polygon, float>>& height_polygons)
{
    ...
    if (!polygons.empty()) {
        if (m_render_fill) {
            ...
            m_fill.set_color({0.8f, 0.8f, 1.0f, 0.5f});
        }
    ...
    if (!height_polygons.empty()) {
        ...
        m_height_limit.set_color({0.8f, 0.8f, 1.0f, 0.5f});
    }
}
...
bool GLCanvas3D::init()
{
    ...
    // After a fresh canvas initialization the bed picking raycasters are empty.
    // Re-register them immediately so the plate action buttons work without additional refreshes.
...
void GLCanvas3D::on_change_color_mode(bool is_dark, bool reinit)
{
    ...
    // ParamsPanel
    wxGetApp().params_panel()->on_change_color_mode(is_dark);
...
const float GLCanvas3D::get_scale() const
{
    float sc = 1;
#if ENABLE_RETINA_GL
    sc = m_retina_helper->get_scale_factor();
#endif
#ifdef WIN32
...
Slic3r::GUI::GCodeViewer& GLCanvas3D::get_gcode_viewer() { return m_gcode_viewer; }

unsigned int GLCanvas3D::get_volumes_count() const { return (unsigned int) m_volumes.volumes.size(); }
...
void GLCanvas3D::set_model_object_visible(bool visible, const ModelObject* mo)
{
    if (!mo)
        return;
    ...
    for (GLVolume* vol : m_volumes.volumes) {
        if (vol->is_wipe_tower)
            continue;
        if (m_model->objects[vol->composite_id.object_id] == mo) {
            vol->is_active = visible;
        }
    }
}
...
void GLCanvas3D::set_process(BackgroundSlicingProcess* process) { m_process = process; }
...
void GLCanvas3D::set_clipping_plane(unsigned int id, const ClippingPlane& plane)
{
    if (id < 2) {
...
void GLCanvas3D::reset_clipping_planes_cache()
{
    m_sla_caps[0].triangles.clear();
    m_sla_caps[1].triangles.clear();
}

void GLCanvas3D::set_use_clipping_planes(bool use) { m_use_clipping_planes = use; }

bool GLCanvas3D::get_use_clipping_planes() const { return m_use_clipping_planes; }

const std::array<Slic3r::GUI::ClippingPlane, 2>& GLCanvas3D::get_clipping_planes() const { return m_clipping_planes; }

void GLCanvas3D::set_use_color_clip_plane(bool use) { m_volumes.set_use_color_clip_plane(use); }

void GLCanvas3D::set_color_clip_plane(const Vec3d& cp_normal, double offset) { m_volumes.set_color_clip_plane(cp_normal, offset); }

void GLCanvas3D::set_color_clip_plane_colors(const std::array<ColorRGBA, 2>& colors) { m_volumes.set_color_clip_plane_colors(colors); }
...
bool GLCanvas3D::is_layers_editing_enabled() const { return m_layers_editing.is_enabled(); }

bool GLCanvas3D::is_layers_editing_allowed() const { return m_layers_editing.is_allowed(); }
...
void GLCanvas3D::enable_picking(bool enable) { m_picking_enabled = enable; }

void GLCanvas3D::enable_moving(bool enable) { m_moving_enabled = enable; }
...
void GLCanvas3D::enable_dynamic_background(bool enable) { m_dynamic_background_enabled = enable; }

void GLCanvas3D::allow_multisample(bool allow) { m_multisample_allowed = allow; }
...
void GLCanvas3D::update_volumes_colors_by_config(const DynamicPrintConfig* config)
{
    if (config != nullptr)
        m_volumes.update_colors_by_config(config);
}
...
void GLCanvas3D::render(bool only_init)
{
    ...
    if (m_canvas_type == ECanvasType::CanvasAssembleView) {
        ...
        picking_color.set(2, outline_color.z);
    }
    ...
    if (m_canvas_type == ECanvasType::CanvasView3D) {
        ...
        if (m_extra_render_callbacks[static_cast<size_t>(ERenderEvent::ObjectCloneOptions)] ||
    ...
    _rebuild_postprocessing_pipeline(ogl_manager, write_to_framebuffer_name, OpenGLManager::s_back_frame, viewport[2], viewport[3]);
...
void GLCanvas3D::render_on_image(wxImage& image)
{
    ...
    for (int y = height - 1; y >= 0; y--) {
        for (int x = 0; x < width; x++) {
            ...
            data[i++] = _pixels[index];
            data[i++] = _pixels[index + 1];
            data[i++] = _pixels[index + 2];
        }
    }
...
void GLCanvas3D::render_calibration_thumbnail(ThumbnailData&          thumbnail_data,
                                              unsigned int            w,
                                              unsigned int            h,
                                              const ThumbnailsParams& thumbnail_params)
{
    // load current plate gcode
    m_gcode_viewer.render_calibration_thumbnail(thumbnail_data, w, h, thumbnail_params, wxGetApp().plater()->get_partplate_list(),
...
void GLCanvas3D::update_plate_thumbnails(bool force)
{
    if (force) {
        bool state                           = m_sel_plate_toolbar.is_render_finish;
        m_sel_plate_toolbar.is_render_finish = false;
        _update_imgui_select_plate_toolbar();
        m_sel_plate_toolbar.is_render_finish = state;
    } else {
        _update_imgui_select_plate_toolbar();
    }
}
...
void GLCanvas3D::delete_selected() { m_selection.erase(); }
...
void GLCanvas3D::set_gcode_options_visibility_from_flags(unsigned int flags) { m_gcode_viewer.set_options_visibility_from_flags(flags); }
...
void GLCanvas3D::reload_scene(bool refresh_immediately, bool force_full_scene_refresh)
{
    ...
    // reset cliper slider
    IMSlider* cliper = m_gcode_viewer.get_cliper_slider();
    if (cliper) {
        cliper->SetHigherValue(100);
        cliper->SetLowerValue(0);
    }
        ...
        if (wt && (purge_in_prime_tower == proj_purge_in_prime_tower)) {
            for (int plate_id = 0; plate_id < n_plates; plate_id++) {
                ...
                if (!current_print->is_step_done(psWipeTower) || !current_print->wipe_tower_data().wipe_tower_mesh_data) {
                    ...
                    float       width             = std::max(tower_brim_width, 0.f);
                    BoundingBox plate_bbox2d      = BoundingBox(scaled(Vec2f(0.f, 0.f)),
                                                                scaled(Vec2f(plate_bbox_x_max_local_coord, plate_bbox_y_max_local_coord)));
                    BoundingBox wipe_tower_bbox2d = BoundingBox(scaled(Vec2f(-width, -width)),
                                                                scaled(Vec2f(wipe_tower_size(0) + width, wipe_tower_size(1) + width)));
                    wipe_tower_bbox2d.rotate((M_PI / 180.) * a);
                    wipe_tower_bbox2d.translate(scaled(x), scaled(y));
                    Vec2f offset = WipeTower::move_box_inside_box(wipe_tower_bbox2d, plate_bbox2d, scaled(margin));
                    ...
                    int volume_idx_wipe_tower_new = m_volumes.load_wipe_tower_preview(1000 + plate_id, x + plate_origin(0),
                                                                                      y + plate_origin(1), (float) wipe_tower_size(0),
                    ...
                    if (!current_print->is_step_done(psSlicingFinished)) {
                        ...
                        BoundingBoxf3 plate_bbox   = wxGetApp().plater()->get_partplate_list().get_plate(plate_id)->get_build_volume();
                        ...
                        offset                     = WipeTower::move_box_inside_box(tower_bottom_bbox, plate_bbox2d, scaled(margin));
                        if (!is_approx(offset[0], 0.f)) {
                    ...
                    int volume_idx_wipe_tower_new =
                        m_volumes.load_real_wipe_tower_preview(1000 + plate_id, x + plate_origin(0) + offset[0],
                                                               y + plate_origin(1) + offset[1],
                                                               current_print->wipe_tower_data().wipe_tower_mesh_data->real_wipe_tower_mesh,
                                                               current_print->wipe_tower_data().wipe_tower_mesh_data->real_brim_mesh, true,
                                                               a, /*!print->is_step_done(psWipeTower)*/ true, m_initialized);
                    int volume_idx_wipe_tower_old = volume_idxs_wipe_tower_old[plate_id];
                    if (volume_idx_wipe_tower_old != -1)
                        map_glvolume_old_to_new[volume_idx_wipe_tower_old] = volume_idx_wipe_tower_new;
                }
    ...
    // Keep MeshBoolean per-volume override colors stable across scene reloads.
    // Scene reload remaps selection indices; do the same for override bookkeeping before updating extruder colors.
    m_volumes.remap_volume_color_overrides(map_glvolume_old_to_new);
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        if (!m_volumes.empty()) {
            ...
            post_event(Event<bool>(EVT_GLCANVAS_ENABLE_ACTION_BUTTONS, contained_min_one && !m_model->objects.empty() && !partlyOut));
        } else {
            _set_warning_notification(EWarning::ObjectOutside, false);
                                    ...
                                    bool                            only_gcode)
{
...
#if _DEBUG
    {
        ...
        if (wipe_x_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 3 wipe_tower_x : %1%.") % wipe_x_option->get_at(0);
        if (wipe_y_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 3 wipe_tower_y : %1%.") % wipe_y_option->get_at(0);
    }
#endif
    ...
    m_gcode_viewer.init(wxGetApp().get_mode(), wxGetApp().preset_bundle, true);
    m_gcode_viewer.load(gcode_result, *this->fff_print(), wxGetApp().plater()->build_volume(), exclude_bounding_box, wxGetApp().get_mode(),
                        only_gcode);
...
#if _DEBUG
    {
        ...
        if (wipe_x_option->size() > 0)
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(" 31 wipe_tower_x : %1%.") % wipe_x_option->get_at(0);
        if (wipe_y_option->size() > 0)
...
void GLCanvas3D::on_size(wxSizeEvent& evt)
{
    m_dirty    = true;
    m_inresize = true;
    _refresh_if_shown_on_screen(); // The canvas refreshes in real-time when the size of the form changes, otherwise you will see the parent form
    m_inresize = false;
}
...
void GLCanvas3D::on_char(wxKeyEvent& evt)
{
    ...
    if (keyCode == WXK_ESCAPE && (_deactivate_arrange_menu() || _deactivate_orient_menu() || unregister_all_extra_render_event()))
        return;
    ...
    if (obj_list->on_char(evt))
        return;

    if (m_gizmos.get_current_type() == GLGizmosManager::EType::MeshBoolean && (evt.GetModifiers() & ctrlMask) != 0) {
        switch (keyCode) {
#ifdef __APPLE__
        case 'a':
        case 'A':
    ...
    if ((evt.GetModifiers() & ctrlMask) != 0) {
        ...
        case 'a': {
            if ((evt.GetModifiers() & shiftMask) != 0) {
                ...
                if (wxGetApp().plater()->model().objects.size() > 0) {
...
//                    set_cursor(Standard);
                ...
                if (keyCode == WXK_SHIFT) {
                    ...
                    m_dirty = true;
                         ...
                         !wxGetApp().obj_list()->get_object_list_window_focus()) {
...
void GLCanvas3D::on_mouse(wxMouseEvent& evt)
{
    ...
    // on Windows, (left button down + shift) is used to move the bed;
    // on MacOS, (left button down + shift) is used to retangle select objects
    auto LeftIsDownWithoutKey = [](wxMouseEvent& evt) {
#ifdef __APPLE__
        return evt.LeftIsDown();
#else
    ...
    bool b1 = evt.Dragging();
    bool b2 = evt.LeftIsDown();
    bool b3 = m_picking_enabled;
    bool b4 = m_rectangle_selection.is_dragging();
    ...
    if (evt.Entering()) {
        ...
        // MeshBoolean uses double click for selection workflow; do not switch sidebar panels.
        if (m_gizmos.get_current_type() == GLGizmosManager::MeshBoolean)
            return;
    ...
    } else if (m_gizmos.get_current_type() != GLGizmosManager::MeshBoolean && isRectangleSelectable(evt) &&
               !m_rectangle_selection.is_dragging() && m_hover_volume_idxs.empty() && m_picking_enabled &&
               m_layers_editing.state != LayersEditing::Editing) // start rectangle selection.
    {
        auto t1 = evt.Dragging();
        auto t2 = evt.LeftIsDown();
        auto t3 = !m_rectangle_selection.is_dragging();
        auto t4 = m_hover_volume_idxs.empty();
        auto t5 = m_picking_enabled;
        auto t6 = m_layers_editing.state != LayersEditing::Editing;
        ...
        // BBS: add orient deactivate logic
        if (!m_gizmos.on_mouse(evt)) {
            // the arrange menu need to be visible when left click the scene
            if (/*_deactivate_arrange_menu() ||*/ _deactivate_orient_menu())
                return;
        }

        // move object operation is forbidden when render clone preview
        if (m_extra_render_callbacks[static_cast<size_t>(ERenderEvent::ObjectCloneOptions)] ||
            m_extra_render_callbacks[static_cast<size_t>(ERenderEvent::FillBedOptions)]) {
            return;
        }
    ...
    } else if (evt.Dragging() && LeftIsDownWithoutKey(evt) && m_mouse.drag.move_volume_idx != -1 &&
               m_layers_editing.state == LayersEditing::Unknown) {
        if (m_canvas_type != ECanvasType::CanvasAssembleView) {
    ...
    } else if (isRectangleSelectable(evt) && m_picking_enabled && m_rectangle_selection.is_dragging()) {
        ...
        // do not process the dragging if the left mouse was set down in another canvas
            ...
            // Right click rotation of the mouse
            ...
            if (m_mouse.is_start_position_2D_defined()) {
                ...
                camera.update_target_y_offset(orig - cur_pos);
    ...
    } else if ((evt.LeftUp() || evt.MiddleUp() || evt.RightUp()) || (m_camera_movement && !is_camera_rotate(evt) && !is_camera_pan(evt))) {
        m_mouse.position = pos.cast<double>();
        ...
        if (m_layers_editing.state != LayersEditing::Unknown) {
            ...
            post_event(SimpleEvent(EVT_GLCANVAS_MOUSE_DRAGGING_FINISHED));
                   ...
                   m_layers_editing.state != LayersEditing::Editing && !this->is_layers_editing_enabled()) { // stop rectangle selection.
            if (m_canvas_type != ECanvasType::CanvasAssembleView) {
            ...
            if ((!any_gizmo_active || !evt.CmdDown()) && m_picking_enabled)
                deselect_all();
        }
        // BBS Select plate in this 3D canvas.
            ...
            // MeshBoolean uses right click for selection toggling; suppress the global context menu while active.
            if (m_gizmos.get_current_type() == GLGizmosManager::MeshBoolean) {
                mouse_up_cleanup();
                return;
            }
            ...
            if (!m_hover_plate_idxs.empty() && (m_canvas_type == CanvasView3D) && !m_mouse.dragging) {
                ...
                /* Do not unselect when the right mouse button pops up
                if (m_hover_volume_idxs.empty())
                    deselect_all();
                    */
                render();
                                               ...
                                               int         plate_index /* = 0*/)
{
    PartPlateList& partplate_list = wxGetApp().plater()->get_partplate_list();
    PartPlate*     plate          = nullptr;
    plate                         = partplate_list.get_plate(plate_index);
    if (!plate) {
        plate = partplate_list.get_curr_plate();
    }

    BoundingBoxf3 build_volume = plate->get_build_volume();
    Vec2d         vmin(build_volume.min.x(), build_volume.min.y());
    Vec2d         vmax(build_volume.max.x(), build_volume.max.y());
    ...
    const float        min_x = start_point.x() - step.x() * int((start_point.x() - bbox.min.x()) / step.x());
    const float        min_y = start_point.y() - step.y() * int((start_point.y() - bbox.min.y()) / step.y());

    for (float x = min_x; x <= bbox.max.x() + EPSILON; x += step.x()) // Include boundary
        for (float y = min_y; y <= bbox.max.y() + EPSILON; y += step.y())
            cells.emplace_back(x, y);

    // Mark occupied cells -----------------------------------------------------
    std::unordered_set<size_t> occupied_indices;
    occupied_indices.clear();
    for (const auto& obj : m_model->objects) {
        if (obj->instances.empty())
            continue;

        // Generate base convex hull (using first instance as reference)
        const auto& inst0     = obj->instances.front();
        Polygon     base_hull = obj->convex_hull_2d(Geometry::assemble_transform({0.0, 0.0, inst0->get_offset().z()}, inst0->get_rotation(),
                                                                                 inst0->get_scaling_factor(), inst0->get_mirror()));
        if (base_hull.empty())
            continue;

        // Process all instances
        const double base_rot_z = inst0->get_rotation().z();
        for (const auto& inst : obj->instances) {
            // Calculate instance transformation
            Geometry::Transformation t;
            t.set_offset({scale_(inst->get_offset().x()), scale_(inst->get_offset().y()), 0.0});
            t.set_rotation(Z, inst->get_rotation().z() - base_rot_z);

            // Apply transformation to convex hull
            Polygon inst_hull = base_hull.transform(t.get_matrix());

            // Check if grid point is inside convex hull
            for (size_t i = 0; i < cells.size(); ++i) {
                ...
                BoundingBox hull_bbox     = inst_hull.bounding_box();
                Vec2d       hull_bbox_min = unscaled(Vec2d(hull_bbox.min.x(), hull_bbox.min.y()));
                Vec2d       hull_bbox_max = unscaled(Vec2d(hull_bbox.max.x(), hull_bbox.max.y()));
                BoundingBox hull_bbox2(Point(hull_bbox_min.cast<float>().x(), hull_bbox_min.cast<float>().y()),
                                       Point(hull_bbox_max.cast<float>().x(), hull_bbox_max.cast<float>().y()));
                if (bounding_box.overlap(hull_bbox2)) {
                    occupied_indices.insert(i);
                }

                // const Point p(scale_(cell.x()), scale_(cell.y()));

                // if (inst_hull.contains(p)) {
                //     occupied_indices.insert(i);
                // }
            }
        }
    ...
    for (size_t i = 0; i < cells.size(); ++i)
        if (!occupied_indices.count(i))
            cells_ret.push_back(cells[i]);

    // Sorting logic -----------------------------------------------------------
    // Vec2f sort_center = (start_point.x() < 0 || start_point.y() < 0)
    //    ? Vec2f(bbox.center().x(), bbox.center().y())
    //    : start_point;
    Vec2f sort_center = start_point; // always use the model-object bbox'center as sort_center

    std::sort(cells_ret.begin(), cells_ret.end(),
              [&sort_center](const Vec2f& a, const Vec2f& b) { return (a - sort_center).squaredNorm() < (b - sort_center).squaredNorm(); });
...
Vec2f GLCanvas3D::get_nearest_empty_cell(const Vec2f start_point,
                                         ...
                                         int         plate_index /* = 0*/)
{
    std::vector<Vec2f> empty_cells = get_empty_cells(start_point, bbox_size, extra_offset, plate_index);
...
bool GLCanvas3D::is_bbox_overlap_with_any_object(const BoundingBox& check_bbox)
{
    ...
    for (const auto& obj : m_model->objects) {
        ...
        for (const auto& inst : obj->instances) {
            ...
            if (check_bbox.overlap(hull_bbox2)) {
                return true;
            }
        }
    }
...
std::vector<Vec2f> GLCanvas3D::get_expand_cells(const Vec2f&         start_point,
                                                int                  max_cells,
                                                const Vec2f          bbox_size /*    = {10, 10*/,
                                                float                extra_offset /* = 1.0f*/,
                                                ...
                                                bool                 ignore_others /* = false*/)
{
    ...
    while (all_empty_cells.size() < max_cells) {
        ...
        if (plate_bbox) {
            ...
            expanded_min_y = plate_bbox->min.y() - expansion_step * step.y();
...
void GLCanvas3D::export_toolpaths_to_obj(const char* filename) const { m_gcode_viewer.export_toolpaths_to_obj(filename); }
...
bool GLCanvas3D::draw_input_double(const std::string& label, double* v, double stride, const Vec2d& size)
{
...

    {
        ...
        ImVec2      pos_max   = ImVec2(pos.x + size.x() + 4.0f, pos.y + height);
        draw_list->AddRect(pos, pos_max, m_is_dark ? IM_COL32(110, 110, 114, 255) : IM_COL32(214, 214, 220, 255), 5.0f,
                           ImDrawFlags_RoundCornersAll);
    }

    const ImVec2 updown_button_size = ImVec2(15.0f * get_scale(), height / 2.0f);
    ImGui::PushItemWidth(size.x() - updown_button_size.x);
    bool drag_input = ImGui::InputDouble(label.c_str(), v, 0.0f, 0.0f, "%.2f");
    ImGui::PopItemWidth();
    {
        ImGui::SameLine();
        ImGui::BeginGroup();
        ...
        if (ImGui::ArrowButtonEX_CP((label + "##up").c_str(), ImGuiDir_Up, updown_button_size)) {
            *v += stride;
            drag_input |= true;
        }
        if (hover)
        ...
        if (ImGui::ArrowButtonEX_CP((label + "##down").c_str(), ImGuiDir_Down, updown_button_size)) {
            *v -= stride;
            drag_input |= true;
        }
...
bool GLCanvas3D::draw_input_int(const std::string& label, int* v, int stride, const int* p_min, const int* p_max, const Vec2d& size)
{
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4());
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4());
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4());
    ...
    ImGui::PushItemWidth(size.x() - updown_button_size.x);
    // bool drag_input = ImGui::InputDouble(label.c_str(), v, 0.0f, 0.0f, "%.2f");
    bool drag_input = ImGui::BBLDragScalar(label.c_str(), ImGuiDataType_S32, v, stride, p_min, p_max);
    ImGui::PopItemWidth();
    {
        ImGui::SameLine();
        ImGui::BeginGroup();
        ...
        if (ImGui::ArrowButtonEX_CP((label + "##up").c_str(), ImGuiDir_Up, updown_button_size)) {
            if (p_max == nullptr || (*v + stride <= *p_max)) {
                *v += stride;
                drag_input |= true;
            }
        }
        ...
        if (ImGui::ArrowButtonEX_CP((label + "##down").c_str(), ImGuiDir_Down, updown_button_size)) {
            if (p_min == nullptr || (*v - stride >= *p_min)) {
                *v -= stride;
                drag_input |= true;
            }
        }
...
bool GLCanvas3D::draw_input_int_v2(const std::string& label, int* v, int stride, const int* p_min, const int* p_max, const Vec2d& size)
{
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4());
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4());
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4());
...
bool GLCanvas3D::_render_arrange_menu(float left, float right, float bottom, float top)
{
    ...
    if (!is_init) {
        ...
        // imgui->set_next_window_pos(x, m_main_toolbar.get_height(), ImGuiCond_Always, 0.0f, 0.0f);
        imgui->set_draggable_window_pos(x, m_main_toolbar.get_height(), ImGuiCond_Always, 0.f, 0.0f);
#else
        const float x = canvas_w - m_main_toolbar.get_width();
        const float y = 0.5f * canvas_h - top * float(wxGetApp().plater()->get_camera().get_zoom());
        // imgui->set_next_window_pos(x, y, ImGuiCond_Always, 1.0f, 0.0f);
    ...
    double dist = settings.distance;

    ImGui::SameLine(0.f);
    draw_input_double("##spacing_input", &dist, 0.10f, Vec2d(70.f * get_scale(), 30.f));
    settings.distance = (float) dist;

    if (settings.distance != before_dist) {
    ...
    bool disable_arrange_selected_button = true;
    ...
    // adjust style
    if (disable_arrange_selected_button) {
        ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true);
        ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
    }

    if (imgui->button(_L("Arrange Selected"))) {
        wxGetApp().plater()->set_prepare_state(Job::PREPARE_STATE_EXTRA);
        wxGetApp().plater()->arrange();
    }
...
bool GLCanvas3D::_render_object_clone_options(float left, float right, float bottom, float top)
{
    ImGuiWrapper* imgui = wxGetApp().imgui();
    ...
    if (imgui->button(_L("Confirm"))) {
        ...
        m_selection.release_clone_preview_info();
        this->unregister_extra_render_event(ERenderEvent::ObjectCloneOptions);
    }
...
bool GLCanvas3D::_render_fill_bed_options(float left, float right, float bottom, float top)
{
    ImGuiWrapper* imgui = wxGetApp().imgui();
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(8.0f, (30.0f * get_scale() - ImGui::GetFontSize()) / 2.0)); // use for titlebar
    imgui->begin(_L("Fill in bed options"), ImGuiWrapper::TOOLBAR_WINDOW_FLAGS);
    ImGui::PopStyleVar(1);
    ...
    ImGui::SameLine(0.f);
    draw_input_double("##spacing_input", &dist, 0.05f, Vec2d(70.f * get_scale(), 30.f));
    settings.distance = (float) dist;
    ...
    if (settings.distance != before_dist) {
        ...
        if (settings.checkerboard_layout) {
            m_selection.calculate_checkerboard_preview_offsets(settings.distance);
        }
    }
    ...
    if (settings.checkerboard_layout) { // do not allow "Auto rotate for arrangement"if checkerboard_layout is enabled
        imgui->disabled_begin(true);
        settings_out.enable_rotation = false;
    }
    ...
    // Align to Y axis. Only enable this option when auto rotation not enabled
    {
        if (settings_out.enable_rotation || settings.checkerboard_layout) { // do not allow align to Y axis if rotation is enabled
            imgui->disabled_begin(true);
            settings_out.align_to_y_axis = false;
        }
        ...
        if (settings_out.enable_rotation || settings.checkerboard_layout) {
            imgui->disabled_end();
        }
    }

    {
        ...
        if (imgui->bbl_checkbox(_L("Checkerboard Layout"), settings.checkerboard_layout)) {
            settings_out.checkerboard_layout = settings.checkerboard_layout;
            settings_changed                 = true;
            if (settings_out.checkerboard_layout) {
                m_selection.calculate_checkerboard_preview_offsets(settings.distance);
            }

            m_selection.toggle_selection_visible(!settings_out.checkerboard_layout);
        }
    }
    ...
    if (imgui->button(_L("Confirm"))) {
        m_selection.toggle_selection_visible(true);
        if (settings.checkerboard_layout) {
            m_selection.paste_objects_from_checkerboard_layout();
        } else {
            wxGetApp().plater()->fill_bed_with_instances();
        }

        this->unregister_extra_render_event(ERenderEvent::FillBedOptions);
        m_selection.clear_clone_offsets();
    }
...
void GLCanvas3D::_render_extra_popup()
{
    ...
    for (const auto& callback : m_extra_render_callbacks) {
        if (callback) {
            callback();
            break; // only one extra render is allowed at the same time
        }
    }
}

void GLCanvas3D::triger_extra_render_event(ERenderEvent render_event)
{
    size_t index = static_cast<size_t>(render_event);

    if (index >= m_extra_render_callbacks.size()) {
        return;
    }
    ...
    for (size_t i = 0; i < m_extra_render_callbacks.size(); ++i) {
        if (i != index && m_extra_render_callbacks[i]) {
            ...
            unregister_extra_render_event(static_cast<ERenderEvent>(i));
        }
    }

    switch (render_event) {
    case ERenderEvent::FillBedOptions: {
        m_extra_render_callbacks[index] = [this]() { this->_render_fill_bed_options(0, 0, 100, 100); };
        ArrangeSettings& settings       = get_arrange_settings();
        settings.checkerboard_layout    = true; // set default value to true each time open the fill bed ui
        m_selection.calculate_checkerboard_preview_offsets(settings.distance);
        m_selection.toggle_selection_visible(false);
        m_main_toolbar.on_set_virtual_item("virtual-fill-bed-item");
...
void GLCanvas3D::unregister_extra_render_event(ERenderEvent event)
{
    ...
    m_extra_render_callbacks[index] = nullptr;
    m_main_toolbar.on_set_virtual_item("");
}

bool GLCanvas3D::unregister_all_extra_render_event()
{
    bool has_callback = false;
    for (size_t i = 0; i < m_extra_render_callbacks.size(); i++) {
        if (m_extra_render_callbacks[i]) {
            has_callback                = true;
            m_extra_render_callbacks[i] = nullptr;
        }
    }
...
Transform3d GLCanvas3D::get_preview_extra_transform()
{
    Transform3d transform = Transform3d::Identity();
    // cr30 need inverse transform
    if (wxGetApp().preset_bundle->machine_is_belt()) {
        DynamicPrintConfig& proj_cfg = wxGetApp().preset_bundle->project_config;
...
void GLCanvas3D::_render_silhouette_effect()
{
...

    {
        ...
        p_gaussian_blur33_shader->start_using();
        ...
        p_gaussian_blur33_shader->stop_using();
    ...
    if (ogl_manager.is_fxaa_enabled()) {
        ...
        if (p_main_frame_buffer) {
            ...
            if (p_main_frame_buffer->is_texture_valid(output_texture_id)) {
                GLShaderProgram* p_fxaa_shader = ogl_manager.get_shader("fxaa");
                if (p_fxaa_shader) {
                    p_fxaa_shader->start_using();
...
#define FACE_UNSELECT_DATK 145 / 255.f, 149 / 255.f, 153 / 255.f
#define FACE_UNSELECT_UNDARK 247 / 255.f, 248 / 255.f, 250 / 255.f
...
void GLCanvas3D::_render_3d_navigator()
{
    ...
    const float manipulate_ypos = icon_ygap;

    const float icon_xpos = manipulate_xpos + (manipulate_size - home_icon_size.x) * 0.5f;
    const float icon_ypos = manipulate_ypos + manipulate_size + 10.0f; // small margin below cube
    ...
    style.Colors[ImGuizmo::COLOR::TEXT]        = m_is_dark ? ImVec4(TEXT_UNSELECT_DATK, 1.f) : ImVec4(TEXT_UNSELECT_UNDARK, 1.0f);
    style.Colors[ImGuizmo::COLOR::FACE]        = m_is_dark ? ImVec4(FACE_UNSELECT_DATK, 1.f) : ImVec4(FACE_UNSELECT_UNDARK, 1);
    style.Colors[ImGuizmo::COLOR::TEXT_SELECT] = ImVec4(TEXT_SELECT_COLOR, 1.f);
    style.Colors[ImGuizmo::COLOR::FACE_SELECT] = ImVec4(FACE_SELECT_COLOR, 1.f);
    ...
    strcpy(style.FaceLabels[ImGuizmo::FACES::FACE_FRONT], _u8L("FRONT").c_str());
    strcpy(style.FaceLabels[ImGuizmo::FACES::FACE_BACK], _u8L("BACK").c_str());
    strcpy(style.FaceLabels[ImGuizmo::FACES::FACE_TOP], _u8L("TOP").c_str());
    strcpy(style.FaceLabels[ImGuizmo::FACES::FACE_BOTTOM], _u8L("BOTTOM").c_str());
    ...
    const bool dirty = ImGuizmo::ViewManipulate(cameraView, cameraProjection, ImGuizmo::OPERATION::ROTATE, ImGuizmo::MODE::WORLD,
                                                identityMatrix, camDistance, ImVec2(manipulate_xpos, manipulate_ypos),
                                                ImVec2(manipulate_size, manipulate_size), 0x00101010);
    imgui.pop_bold_font();
    ...
    cfg.bgalpha = 0;
    cfg.padding = {0, 0};
    // config.prepareWindow(DispConfig::e_wt_reset, {xpos,ypos});
    ImVec2 bias = config.getWindowBias(DispConfig::e_wt_reset);
    ImGui::SetNextWindowPos({icon_xpos, icon_ypos}, ImGuiCond_Always, bias);
    ImGui::SetNextWindowSize(home_icon_size);

    config.processWindows(
        "home_icon",
        [&]() {
            // auto size = config.getWindowSize(DispConfig::e_wt_reset, sc);
            if (ImGui::ImageButton3(config.getTextureId(DispConfig::e_tt_home, false), config.getTextureId(DispConfig::e_tt_home, true),
                                    home_icon_size)) {
                camera.select_view("topfront"); // plate
            ...
            if (ImGui::IsItemHovered()) {
                ...
                float  icon_cy  = 0.5f * (icon_min.y + icon_max.y);
                                           ...
                                           const std::string&        view_type)
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format(
                                   ...
                                   plate_idx % visible_volumes.size() % shader % use_top_view % for_picking;
...
void GLCanvas3D::_switch_toolbars_icon_filename()
{
    ...
    // m_separator_toolbar.init(background_data); separator is diffrence from other toolbar

    // process toolbar
    {
        auto&                                   processToolBar = wxGetApp().plater()->get_process_toolbar();
        ProcessBar::BackgroundTexture::Metadata bk_data;
        bk_data.filename = m_is_dark ? "process_toolbar_background_dark.png" : "process_toolbar_background.png";
        bk_data.left     = 16;
        bk_data.top      = 16;
        bk_data.right    = 16;
        bk_data.bottom   = 16;
        processToolBar.init(bk_data);

        _update_process_toolbar_item_icons();
    }
    ...
    // main toolbar
    {
        ...
        // item = m_main_toolbar.get_item("Printer");
        // if(nullptr != item)
        //   item->set_icon_filename(m_is_dark ? "printer_dark_default.svg" : "printer_light_default.svg");
        ...
        item = m_main_toolbar.get_item("assembly_view");
        if (nullptr != item)
        ...
        for (size_t i = GLGizmosManager::EType::Move; i < GLGizmosManager::EType::Undefined; i++) {
            ...
            if (gizmo) {
                std::string    name = gizmo->get_name(false);
                GLToolbarItem* item = m_main_toolbar.get_item(name);
                if (item) {
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    m_main_toolbar.set_horizontal_expand(true);
    ...
    m_main_toolbar.set_border(10.0f);
    m_main_toolbar.set_separator_size(1.0f);
    m_main_toolbar.set_gap_size(10.0f);
    // m_main_toolbar.set_icons_size(40.0f);
...
#if 0
    item.name                 = "Printer";
    item.icon_filename        = m_is_dark ? "printer_dark_default.svg" : "printer_light_default.svg";
    item.tooltip              = _utf8(L("Printer")) + " [" + GUI::shortkey_ctrl_prefix() + "I]";
    item.sprite_id            = 0;
    item.left.action_callback = [this]() {
        if (m_canvas != nullptr)
            wxPostEvent(m_canvas, SimpleEvent(EVT_GLTOOLBAR_PRINTER));
    };
    ...
    item.enabling_callback = [this]() -> bool {
        GLToolbarItem* arrange_item = m_main_toolbar.get_item("arrange");
        if (arrange_item && arrange_item->is_visible()) {
            if (wxGetApp().plater()->model().objects.empty()) {
                return false;
            }

            return true; // need to be always displayed when press the "layout" button
        }
    ...
    //
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::Flatten);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            item.sprite_id++;
            item.left.action_callback = [this]() { m_gizmos.open_gizmo(GLGizmosManager::EType::Flatten); };
            item.enabling_callback    = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
        }
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::Hollowing);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::Drill);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::MeshBoolean);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            item.sprite_id++;
            item.left.action_callback = [this]() { m_gizmos.open_gizmo(GLGizmosManager::EType::MeshBoolean); };
            item.enabling_callback    = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
        }
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::Measure);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            item.sprite_id++;
            item.left.action_callback = [this]() { m_gizmos.open_gizmo(GLGizmosManager::EType::Measure); };
            item.enabling_callback    = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
        }
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::FdmSupports);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            item.sprite_id++;
            item.left.action_callback = [this]() { m_gizmos.open_gizmo(GLGizmosManager::EType::FdmSupports); };
            item.enabling_callback    = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
        }
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::Seam);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            item.sprite_id++;
            item.left.action_callback = [this]() { m_gizmos.open_gizmo(GLGizmosManager::EType::Seam); };
            item.enabling_callback    = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
        }
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::FuzzySkin);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            ...
            item.enabling_callback    = [obj]() -> bool { return obj->is_activable(); };
            item.left.action_callback = [this]() { m_gizmos.open_gizmo(GLGizmosManager::EType::FuzzySkin); };
            // item.visibility_callback  = [obj]() -> bool { return obj->is_activable(); };
            item.visibility_callback = [obj]() -> bool { return true; };
            if (!m_main_toolbar.add_item(item))
                return false;
...

    {
        ...
        if (obj) {
            ...
            item.left.action_callback = [this]() {
                if (m_canvas != nullptr) {
                    wxPostEvent(m_canvas, SimpleEvent(EVT_GLVIEWTOOLBAR_BRIM_EARS));
                }
            };
            item.left.render_callback = GLToolbarItem::Default_Render_Callback;
            item.visible              = true;
            item.visibility_callback  = [this]() -> bool { return true; };
            item.enabling_callback    = [obj, this]() -> bool {
                bool is_activable = obj->is_activable();

                int brim_ears_index = m_main_toolbar.get_item_id(obj->get_name(false));
                ...
                //        if (volume && volume->emboss_shape.has_value()) {
                //            return true;
                //        }
                //    }
                //}
    ...
    {
        GLGizmoBase* obj = m_gizmos.get_gizmo(GLGizmosManager::EType::Emboss);
        if (obj) {
            item.name          = obj->get_name(false);
            item.icon_filename = obj->get_icon_filename();
            item.tooltip       = obj->get_name(true);
            item.sprite_id++;
            item.left.action_callback = [this]() {
                GLGizmoBase* gizmo_emboss = m_gizmos.get_gizmo(GLGizmosManager::EType::Emboss);
                if (gizmo_emboss) {
    ...
    {
        item.name          = "ai_cloud_service";
        item.icon_filename = m_is_dark ? "ai_cloud_service_dark.svg" : "ai_cloud_service.svg";
        item.tooltip       = _utf8(L("AI cloud service")) + "\n\n" + _utf8(L("AI Recommendation Support generation"));
        item.sprite_id++;
        item.left.toggable        = false;
        item.left.action_callback = [this]() {
            if (m_canvas != nullptr) {
                m_gizmos.reset_all_states();
                wxPostEvent(m_canvas, SimpleEvent(EVT_GLVIEWTOOLBAR_AI_CLOUD_SERVICE));
            }
        };
        item.left.render_callback = GLToolbarItem::Default_Render_Callback;
        item.visible              = true;
        item.visibility_callback  = [this]() -> bool { return true; };
        item.enabling_callback    = [this]() -> bool {
            if (wxGetApp().plater()->has_assmeble_view()) {
                return true;
            }
    ...
    GLToolbarItem::Data virtual_item;

    {
        // add virtual item (object-clone), when the clone gui is rendered, the other GLToolbarItem state need to set to  Disabled
        virtual_item.name                 = "virtual-clone-item";
        virtual_item.icon_filename        = "";
        virtual_item.tooltip              = "";
        ...
        if (!m_main_toolbar.add_item(virtual_item))
            return false;
    }

    {
        // add virtual item (object-fill-in-bed), when the object-fill-in-bed gui is rendered, the other GLToolbarItem state need to set to Disabled
        virtual_item.name                 = "virtual-fill-bed-item";
        virtual_item.icon_filename        = "";
        virtual_item.tooltip              = "";
        virtual_item.visible              = false;
        virtual_item.sprite_id            = -1;
...
bool GLCanvas3D::_init_select_plate_toolbar()
{
    ...
    result                = result &&
             item->image_texture_transparent.load_from_svg_file(path + "im_all_plates_stats_transparent.svg", false, false, false, 128);
...
bool GLCanvas3D::_init_assemble_view_toolbar()
{
    ...
    */
    ...
    m_assemble_view_toolbar.set_horizontal_expand(true);
...
#ifdef __WXGTK__
    font_size *= 1.25f; // previously 1.5f; shrink text sizes across ImGui widgets
#else
    font_size *= 1.5f;
#endif
#endif
...
#else
    // Further compact ImGui style spacing on Linux/GTK by slightly reducing style scaling
    float style_scale = m_canvas->GetContentScaleFactor();
#ifdef __WXGTK__
    style_scale *= 0.90f; // reduce paddings, combo item spacing, etc.
#endif
    imgui->set_scaling(font_size, style_scale, 1.0f);
#endif
...
BoundingBoxf3 GLCanvas3D::_max_bounding_box(bool include_gizmos, bool include_bed_model, bool include_plates) const
{
    ...
    // for visual purpose, to visualize the plate number
    bb.max.y() += 20.0;
...
void GLCanvas3D::_render_objects(GLVolumeCollection::ERenderType type, bool with_outline)
{
    ...
    if ((GUI::ERenderPipelineStage::Silhouette == render_pipeline_stage) || shader != nullptr) {
        ...
        case GLVolumeCollection::ERenderType::Opaque: {
            ...
            if (dynamic_cast<GLGizmoPainterBase*>(gm.get_current()) == nullptr) {
                if (m_picking_enabled && m_layers_editing.is_enabled() && (m_layers_editing.last_object_id != -1) &&
                    (m_layers_editing.object_max_z() > 0.0f) && GUI::ERenderPipelineStage::Silhouette != render_pipeline_stage) {
                    int           object_id = m_layers_editing.last_object_id;
                    const Camera& camera    = wxGetApp().plater()->get_camera();
                    m_volumes.render(render_pipeline_stage, type, false, camera.get_view_matrix(), camera.get_projection_matrix(),
                    ...
                    const Camera& camera = wxGetApp().plater()->get_camera();
                    m_volumes.render(
                        render_pipeline_stage, type, m_picking_enabled, camera.get_view_matrix(), camera.get_projection_matrix(),
        ...
        case GLVolumeCollection::ERenderType::Transparent: {
            ...
            // BBS:add assemble view related logic
            m_volumes.render(
                render_pipeline_stage, type, false, camera.get_view_matrix(), camera.get_projection_matrix(),
                [this, canvas_type](const GLVolume& volume) {
                    if (canvas_type == ECanvasType::CanvasAssembleView) {
...
void GLCanvas3D::render_hollow_anime(float value)
{
    // yy
    IMSlider* cliper_slide = m_gcode_viewer.get_cliper_slider();

    float max_value = cliper_slide->GetMaxValue();
    float min_value = cliper_slide->GetMinValue();

    cliper_slide->SetHigherValue(value / 100.0f * max_value);

    // set_use_clipping_planes(true);
    // auto pl = get_clipping_planes();
    //
    //    // pl[0].set_offset(-cliper_slide->GetLowerValueD()/(i*1.0f));
    //     pl[1].set_offset(cliper_slide->GetHigherValueD()*(value/100.f));
...
void GLCanvas3D::_render_cliper(int canvas_width, int canvas_height)
{
    ...
    m_volumes.getVolumeZrange(zmin, zmax);
    if (zmax > zmin) {
        cliper_slide->SetSliderAlternateValues({zmin, zmax});
        cliper_slide->render(canvas_width, canvas_height);
        if (cliper_slide->is_need_post_tick_event()) {
        ...
        if (cliper_slide->is_dirty()) {
            ...
            set_use_clipping_planes(true);
            auto pl = get_clipping_planes();
            pl[0].set_offset(-cliper_slide->GetLowerValueD());
            pl[1].set_offset(cliper_slide->GetHigherValueD());
            set_clipping_plane(0, pl[0]);
            set_clipping_plane(1, pl[1]);
        }
    } else {
        // set_use_clipping_planes(false);
        // reset_clipping_planes_cache();
    }
}

void GLCanvas3D::_render_slice_control() const
{
    auto   canvas_w = float(get_canvas_size().get_width());
    auto   canvas_h = float(get_canvas_size().get_height());
    ImVec2 windows  = ImVec2(canvas_w - _right_leaning_widget_margin(), canvas_h);

    float scale = get_scale();

    DispConfig::ButtonConfig bigcfg;
    bigcfg.border = 0;
    bigcfg.bg     = DispConfig::e_ct_btnBg;
    bigcfg.fg     = DispConfig::e_ct_btnBgSel;
    bigcfg.size   = ImVec2(150 * scale, 28 * scale);

    auto smallcfg = bigcfg;
    smallcfg.fg   = DispConfig::e_ct_btnBgSelWhite;
    smallcfg.size = ImVec2(28 * scale, 28 * scale);
    auto config   = DispConfig();

    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0 * scale, 6 * scale));
    ImGuiWrapper& imgui = *wxGetApp().imgui();
    imgui.push_bold_font();

    DispConfig::WindowConfig wcfg;
    wcfg.padding = ImVec2(10 * scale, 10 * scale);
    ...
    config.processWindows(
        ...
        [&]() {
            ...
            int en = s_sst == MainFrame::eSliceAll && !wxGetApp().mainframe->plater()->only_gcode_mode() ? 2 : (enslice ? 2 : 1);
            if (en == 2) {
...
#if ENABLE_RENDER_SELECTION_CENTER
void GLCanvas3D::_render_selection_center() { m_selection.render_center(m_gizmos.is_dragging()); }
#endif // ENABLE_RENDER_SELECTION_CENTER

void GLCanvas3D::_check_and_update_toolbar_icon_scale()
{
    // Update collapse toolbar
    CollapseBar::GLToolbar& collapse_toolbar = wxGetApp().plater()->get_collapse_toolbar();
    collapse_toolbar.set_enabled(wxGetApp().plater()->get_sidebar_docking_state() != Sidebar::None);
    bool toolbar_enabled = wxGetApp().plater()->get_sidebar_docking_state() != Sidebar::None &&
                           false == wxGetApp().params_panel()->current_tab_is_plate_settings() &&
                           false == wxGetApp().obj_list()->is_connectors_item_selected();
    wxGetApp().plater()->get_process_toolbar().set_enabled(toolbar_enabled);
    ...
    float top_tb_width =
        m_main_toolbar
            .get_width(); // +m_gizmos.get_scaled_total_width() + m_assemble_view_toolbar.get_width() + m_separator_toolbar.get_width();
...
void GLCanvas3D::_render_main_toolbar()
{
    ...
    const float obj_list_panel_right = get_main_toolbar_offset();
    float       start_x              = obj_list_panel_right;
    float       start_y              = 0.0f;

    float space_width   = cnv_size.get_width() - obj_list_panel_right;
    float toolbar_width = m_main_toolbar.get_width();

    float cur = m_main_toolbar.get_scroll();

    if (toolbar_width > space_width) {
        // left: display left arrow button
        const float  toolbar_height    = m_main_toolbar.get_height();
        const ImVec2 arrow_button_size = ImVec2(floorf(0.421f * toolbar_height), toolbar_height);
        space_width -= 2.0f * arrow_button_size.x; // consider left & right arrow buttons
        const float stride     = m_main_toolbar.get_scaled_icon_size();
        const float min_scroll = 0.0f;
        const float max_scroll = toolbar_width - space_width;

        ImGui::SetNextWindowPos(ImVec2(start_x, start_y), ImGuiCond_Always);
        ImGui::SetNextWindowSize(arrow_button_size);
        ...
        ImU32 left_arrow_button_color = m_is_dark ? IM_COL32(0, 0, 0, 0.5 * 255) : IM_COL32(0, 0, 0, 0.1 * 255);

        ImU32         right_arrow_button_color = left_arrow_button_color;
        ImU32         arrow_color              = m_is_dark ? IM_COL32(251, 251, 251, 255) : IM_COL32(65, 65, 65, 255);
        ImGuiContext& context                  = *GImGui;
        ImGuiWrapper& imgui                    = *wxGetApp().imgui();

        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2());

        if (ImGui::Begin("##scroll_buttn_left", nullptr, flags)) {
            ImDrawList*  draw_list         = ImGui::GetWindowDrawList();
            ...
            if (ImGui::IsMouseHoveringRect(left_arrow_button.Min, left_arrow_button.Max)) {
                // left_arrow_button_color = IM_COL32(0, 0, 0, 255);
                imgui.set_requires_extra_frame();
                if (context.IO.MouseDownDuration[ImGuiMouseButton_Left] > 0.5) {
                    m_main_toolbar.set_scroll(-min_scroll);
                }
                if (context.IO.MouseClicked[ImGuiMouseButton_Left]) {
                    float cur = m_main_toolbar.get_scroll();
                    float x   = std::clamp(cur + stride, -max_scroll, -min_scroll);
                    m_main_toolbar.set_scroll(x);
                }
            }
        ...
        // middel:main toolbar
        {
            if (cur < 0.0f && cur + toolbar_width < space_width) {
                m_main_toolbar.set_scroll(std::min(space_width - toolbar_width, 0.0f));
            }
            const float left = -0.5f * cnv_size.get_width() + start_x + arrow_button_size.x;
            m_main_toolbar.set_position(top, left);
            m_main_toolbar.set_limit_width(space_width);
            m_main_toolbar.render(*this, m_main_toolbar.get_scroll());
        }

        // right arrow button
        ImGui::SetNextWindowPos(ImVec2(cnv_size.get_width(), start_y), ImGuiCond_Always, ImVec2(1, 0));
        ImGui::SetNextWindowSize(arrow_button_size);
        if (ImGui::Begin("##scroll_buttn_right", nullptr, flags)) {
            ImDrawList*  draw_list          = ImGui::GetWindowDrawList();
            ...
            if (ImGui::IsMouseHoveringRect(right_arrow_button.Min, right_arrow_button.Max)) {
                // right_arrow_button_color = IM_COL32(0, 0, 0, 255);
                imgui.set_requires_extra_frame();
                if (context.IO.MouseDownDuration[ImGuiMouseButton_Left] > 0.50) {
                    m_main_toolbar.set_scroll(-max_scroll);
                }
                if (context.IO.MouseClicked[ImGuiMouseButton_Left]) {
                    float cur = m_main_toolbar.get_scroll();
                    float x   = std::clamp(cur - stride, -max_scroll, -min_scroll);
        ...
        m_main_toolbar.set_scroll(0.0f);
        m_main_toolbar.set_limit_width(-1.0f);
        const float left = -0.5f * cnv_size.get_width() + start_x;
        m_main_toolbar.set_position(top + start_y, left);
        m_main_toolbar.render(*this, m_main_toolbar.get_scroll());
    }
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    m_sel_plate_toolbar.set_icon_size(96 * f_scale, 96 * f_scale);
    ...
    bool          show_scroll = item_count * (button_height + frame_padding * 2.0f + button_margin) - button_margin + 22.0f * f_scale >
                               window_height_max ?
                                    true :
                                    false;
    ...
    ImVec4 button_hover  = ImVec4(0.5f, 0.5f, 0.5, 1.0f);
    ...
    imgui.begin(_L("Select Plate"), window_flags);
    ...
    if (show_scroll) {
        ...
        float  track_h      = win_size.y - 2.0f * track_margin;
        ...
        if (scroll_max > 0.0f && track_h > 0.0f) {
            ...
            if (ImGui::IsMouseHoveringRect(bar_min, bar_max) && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
                dragging = true;
            if (dragging) {
    ...
    if (m_sel_plate_toolbar.show_stats_item) {
        ImVec2 stats_size = ImVec2(96.0f * f_scale, 71.0f * f_scale);
        ...
        if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::UNSLICED) {
            ImVec2 size    = ImVec2(stats_w, stats_h);
            ...
            ImVec2 size           = ImVec2(stats_w, stats_h * all_plates_stats_item->percent / 100.0f);
            ImVec2 rect_start_pos = ImVec2(start_pos.x, start_pos.y + size.y);
            ImVec2 rect_end_pos   = ImVec2(start_pos.x + stats_w, start_pos.y + stats_h);
        ...
        ImVec2 text_size_all   = ImGui::CalcTextSize(("All Plates"));
        ImVec2 text_size_stats = ImGui::CalcTextSize(("Stats"));
        float  total_width     = text_size_all.x + text_size_stats.x + 6.0f * f_scale; // spacer
        float  start_x         = start_pos.x + (stats_w - total_width) / 2.0f;
...
void GLCanvas3D::_render_separator_toolbar_right() const
{
    ...
    const float left = -0.5f * cnv_size.get_width() + m_main_toolbar.get_width() + gizmo_width + separator_width / 2;
...
void GLCanvas3D::_render_separators() const
{
    if (!m_separator_toolbar.is_enabled() || !m_main_toolbar.is_enabled())
        return;

    int idx = 0;
    while (1) {
        Vec2f pos = m_main_toolbar.get_separator_position(idx);
        if (pos == Vec2f(FLT_MAX, FLT_MAX)) {
            break;
        } else {
            m_separator_toolbar.set_position(pos.y(), pos.x());
            m_separator_toolbar.render(*this, GLToolbarItem::SeparatorLine);
        }
        idx++;
    }
...
void GLCanvas3D::_render_process_toolbar() const
{
    auto&      plater              = *wxGetApp().plater();
    const auto sidebar_docking_dir = plater.get_sidebar_docking_state();
    if (sidebar_docking_dir == Sidebar::None) {
...
void GLCanvas3D::_render_paint_toolbar() const
{
    ...
    float f_scale = 1.0f;
#endif
    float em_unit = wxGetApp().em_unit() / 10.0f;
...
void GLCanvas3D::_render_assemble_info() const
{
    ...
    imgui->set_next_window_pos(margin, canvas_h - margin, ImGuiCond_Always, 0.0f, 1.0f);
...
void GLCanvas3D::_render_selection_sidebar_hints() { m_selection.render_sidebar_hints(m_sidebar_field, m_gizmos.get_uniform_scaling()); }
...
void GLCanvas3D::_start_timer() { m_timer.Start(100, wxTIMER_CONTINUOUS); }

void GLCanvas3D::_stop_timer() { m_timer.Stop(); }
                                              ...
                                              const std::vector<CustomGCode::Item>& color_print_values)
{
    ...
    struct Ctxt
    {
        ...
        const size_t color_print_color_idx_by_layer_idx_and_extruder(const size_t layer_idx, const int extruder) const
        {
            ...
            auto it = std::find_if(color_print_values->begin(), color_print_values->end(),
                                   [print_z](const CustomGCode::Item& code) { return fabs(code.print_z - print_z) < EPSILON; });
            if (it != color_print_values->end()) {
                CustomGCode::Type type = it->type;
                // pause print or custom Gcode
                if (type == CustomGCode::PausePrint || (type != CustomGCode::ColorChange && type != CustomGCode::ToolChange))
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, ctxt.layers.size(), grain_size), [&ctxt, &new_volume, is_selected_separate_extruder,
        ...
        auto select_geometry = [&ctxt, &geometries](size_t layer_idx, int extruder, int feature) -> GLModel::Geometry& {
            return geometries[ctxt.color_by_color_print() ? ctxt.color_print_color_idx_by_layer_idx_and_extruder(layer_idx, extruder) :
                              ctxt.color_by_tool()        ? std::min<int>(ctxt.number_tools() - 1, std::max<int>(extruder - 1, 0)) :
                                                            feature];
        };
        ...
        for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer) {
            ...
            if (is_selected_separate_extruder) {
                ...
                for (const LayerRegion* layerm : layer->regions()) {
                    ...
                    if (cfg.wall_filament.value == m_selected_extruder || cfg.sparse_infill_filament.value == m_selected_extruder ||
    ...
    BOOST_LOG_TRIVIAL(debug) << "Loading print object toolpaths in parallel - finalizing results" << m_volumes.log_memory_info()
                             << log_memory_info();
    // Remove empty volumes from the newly added volumes.
    {
        ...
        m_volumes.volumes.erase(std::remove(m_volumes.volumes.begin() + volumes_cnt_initial, m_volumes.volumes.end(), nullptr),
...
void GLCanvas3D::_load_wipe_tower_toolpaths(const BuildVolume& build_volume, const std::vector<std::string>& str_tool_colors)
{
    ...
    struct Ctxt
    {
        ...
        const std::vector<WipeTower::ToolChangeResult>& tool_change(size_t idx)
        {
            ...
            return priming.empty() ? ((idx == tool_changes.size()) ? final : tool_changes[idx]) :
                                     ((idx == 0)                       ? priming :
                                      (idx == tool_changes.size() + 1) ? final :
                                                                         tool_changes[idx - 1]);
        }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, n_items, grain_size), [&ctxt, &new_volume](const tbb::blocked_range<size_t>& range) {
                ...
                for (size_t i = 1; i < extrusions.extrusions.size();) {
                    ...
                    if (ctxt.color_by_tool())
                        for (; j < extrusions.extrusions.size() && extrusions.extrusions[j].tool == e.tool &&
                               extrusions.extrusions[j].width > 0.f;
                             ++j)
                            ;
                    else
    ...
    BOOST_LOG_TRIVIAL(debug) << "Loading wipe tower toolpaths in parallel - finalizing results" << m_volumes.log_memory_info()
                             << log_memory_info();
    // Remove empty volumes from the newly added volumes.
    {
        for (auto ptr_it = m_volumes.volumes.begin() + volumes_cnt_initial; ptr_it != m_volumes.volumes.end(); ++ptr_it)
...
void GLCanvas3D::_update_sla_shells_outside_state() { check_volumes_outside_state(); }
...
{
    enum ErrorType { PLATER_WARNING, PLATER_ERROR, SLICING_SERIOUS_WARNING, SLICING_ERROR };
    std::string        text;
    ...
    case EWarning::SlaSupportsOutside:
        text  = _u8L("SLA supports outside the print area were detected.");
...
bool GLCanvas3D::_deactivate_collapse_toolbar_items() { return false; }
...
void GLCanvas3D::GizmoHighlighter::set_timer_owner(wxEvtHandler* owner, int timerid /* = wxID_ANY*/) { m_timer.SetOwner(owner, timerid); }
...
void GLCanvas3D::_update_process_toolbar_item_icons() const
{
    auto&                  plater  = *wxGetApp().plater();
    ProcessBar::GLToolbar& toolbar = plater.get_process_toolbar();
    ...
    if (tab_print != nullptr) {
        unsigned int cnt = toolbar.get_items_count();
        for (size_t i = 0; i < cnt; i++) {
            ProcessBar::GLToolbarItem* item      = toolbar.get_item(i);
            bool                       isPressed = item->is_pressed();
            if (item && !item->is_separator()) {
                const std::string& name      = item->get_name();
                const std::string& icon_file = item->get_icon_filename();

                bool is_modified = tab_print->get_page_is_modified_values(L(name));

                std::string new_icon_file = ProcessBar::GLToolbar::get_process_toolbar_item_icon_file(name, isPressed ? true : m_is_dark,
                                                                                                      is_modified);

                if (new_icon_file != icon_file) {
                    item->set_icon_filename(new_icon_file);
                    toolbar.set_icon_dirty();
                }
            }
        }
    }
...
void GLCanvas3D::_render_printer_preset_and_obj_list()
{
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        float rd = m_canvas_type == ECanvasType::CanvasView3D ? 0.0f : 5.0;
        ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, rd);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, is_dark ? 0.0f : 1.0f);

        ImVec4 bgc       = {34.0f / 255.0f, 34.0f / 255.0f, 34.0f / 255.0f, 0.6f};
        ImVec4 bgc_light = {1.0f, 1.f, 1.f, 0.6f};
        ImGui::PushStyleColor(ImGuiCol_WindowBg, is_dark ? bgc : bgc_light);
        ImGui::PushStyleColor(ImGuiCol_FrameBg, is_dark ? bgc : bgc_light);
        ImGui::PushStyleColor(ImGuiCol_PopupBg, is_dark ? ImVec4(0.0, 0.0, 0.0, 0.9) : ImVec4(1.0, 1.0, 1.0, 1.0f));
        // ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.0, 0.0, 0.0, 0.9));
...
bool GLCanvas3D::_render_global_objects_switch_button()
{
    float view_scale    = wxGetApp().plater()->get_current_canvas3D()->get_scale();
    bool  enable_object = wxGetApp().params_panel()->get_switch_of_object();

    ImVec4 border_color = m_is_dark ? ImVec4(110.0f / 255.0f, 110.0f / 255.0f, 114.0f / 255.0f, 1.0f) :
                                      ImVec4(170.0f / 255.0f, 170.0f / 255.0f, 176.0f / 255.0f, 1.0f);
    ...
    std::string global  = _u8L("Global");
    std::string objects = _u8L("Objects");

    ImVec2 global_size  = ImGui::CalcTextSize(global.c_str(), NULL, true);
    ImVec2 objects_size = ImGui::CalcTextSize(objects.c_str(), NULL, true);

    float rounding = 4.0f;
    float pading   = style.FramePadding.x;
    // float pading_y = style.FramePadding.y;

    ImVec2 p_min = window->DC.CursorPos;
    ImVec2 p_max = ImVec2(p_min.x + global_size.x + objects_size.x + pading * 4.0, p_min.y + 20.0f * view_scale);

    // background with white border
    ImGui::RenderFrame(p_min, p_max, IM_COL32(0, 0, 0, 0), true, rounding);

    ImGui::PopStyleVar(1);

    // selected tab
    if (enable_object == false) {
        ImVec2 p_max2 = ImVec2(p_min.x + global_size.x + pading * 2.0, p_max.y);
        window->DrawList->AddRectFilled(p_min, p_max2, ImGui::GetColorU32(ImGuiWrapper::COL_CREALITY), rounding,
                                        ImDrawFlags_RoundCornersLeft);
    } else {
        ImVec2 p_min2 = ImVec2(p_min.x + global_size.x + pading * 2.0, p_min.y);
        window->DrawList->AddRectFilled(p_min2, p_max, ImGui::GetColorU32(ImGuiWrapper::COL_CREALITY), rounding,
    ...
    // switch button has diffrent text color
    ImVec4 text_color = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
    if (enable_object == true) {
        text_color = m_is_dark ? ImVec4(201.0f / 255.0f, 201.0f / 255.0f, 201.0f / 255.0f, 1.0f) : ImVec4(0.2, 0.2, 0.2, 1.0);
    }
    ImGui::PushStyleColor(ImGuiCol_Text, text_color);

    window->DC.CursorPos = ImVec2(p_min.x, p_min.y + 2.0);
    ...
    window->DC.CursorPos = ImVec2(p_min.x + global_size.x + pading * 2, p_min.y + 2.0);
...
float GLCanvas3D::_right_leaning_widget_margin() const
{
    return is_layers_editing_enabled() ? GLCanvas3D::LayersEditing::THICKNESS_BAR_WIDTH + _layer_editing_profile_right_margin() : 0.0f;
}

float GLCanvas3D::_layer_editing_profile_right_margin() const { return wxGetApp().plater()->get_process_toolbar().get_width() + 1.0f; }

void GLCanvas3D::set_left_panel_fold(ECanvasType type, bool fold) {}
...
void GLVolumeCollection::set_use_volume_color_override(bool use)
{
    if (!use) {
        clear_all_volume_color_overrides();
        return;
    }
    m_use_volume_color_override = true;
}

void GLVolumeCollection::set_volume_color_override(unsigned int volume_idx, const std::array<float, 4>& color)
{
    if (volume_idx >= volumes.size())
        return;
    GLVolume* v = volumes[volume_idx];
    if (v == nullptr)
        return;
    ...
    if (it == m_volume_color_overrides.end()) {
        ...
        it->second.override_color = override_color;
    }

    v->color               = override_color;
    v->force_native_color  = true;
    v->force_neutral_color = false;
}

void GLVolumeCollection::set_volumes_color_override(const std::vector<unsigned int>& volume_indices, const std::array<float, 4>& color)
{
    m_use_volume_color_override = true;
    for (unsigned int idx : volume_indices)
        set_volume_color_override(idx, color);
}

void GLVolumeCollection::clear_volume_color_override(unsigned int volume_idx)
{
    auto it = m_volume_color_overrides.find(volume_idx);
    if (it == m_volume_color_overrides.end())
        return;
    ...
    if (volume_idx < volumes.size()) {
        if (GLVolume* v = volumes[volume_idx]) {
            v->color               = it->second.original_color;
            v->force_native_color  = it->second.original_force_native_color;
            v->force_neutral_color = it->second.original_force_neutral_color;
        }
    }

    m_volume_color_overrides.erase(it);
    if (m_volume_color_overrides.empty())
        m_use_volume_color_override = false;
}

void GLVolumeCollection::clear_all_volume_color_overrides()
{
    for (const auto& kv : m_volume_color_overrides) {
        unsigned int                    idx = kv.first;
        const VolumeColorOverrideState& st  = kv.second;
        if (idx >= volumes.size())
            continue;
        GLVolume* v = volumes[idx];
        if (v == nullptr)
            continue;
        v->color               = st.original_color;
        v->force_native_color  = st.original_force_native_color;
        v->force_neutral_color = st.original_force_neutral_color;
    }
...
void GLVolumeCollection::remap_volume_color_overrides(const std::vector<size_t>& old_to_new)
{
    if (!m_use_volume_color_override || m_volume_color_overrides.empty())
        return;

    std::unordered_map<unsigned int, VolumeColorOverrideState> new_overrides;
    ...
    for (const auto& kv : m_volume_color_overrides) {
        ...
        const size_t new_idx_sz = old_to_new[old_idx];
        if (new_idx_sz == size_t(-1) || new_idx_sz >= volumes.size())
            continue;

        GLVolume* v = volumes[new_idx_sz];
        if (v == nullptr)
            continue;

        VolumeColorOverrideState st = old_state;
        // After a scene reload, base colors/flags may have changed. Capture current values as "original",
        // then re-apply the boolean override color to keep the gizmo visualization stable.
        st.original_color               = v->color;
        st.original_force_native_color  = v->force_native_color;
        st.original_force_neutral_color = v->force_neutral_color;

        v->color               = st.override_color;
        v->force_native_color  = true;
        v->force_neutral_color = false;
