...
void GLCanvas3D::LayersEditing::render_variable_layer_height_dialog(const GLCanvas3D& canvas) {
    ...
    bool changed = wxGetApp().plater()->get_view3D_canvas3D()->draw_input_double("##adaptive_input", &adaptive_quality, 0.05f, Vec2d(input_box_width, 30.f));
    if (changed) {
    ...
    bool radius_changed = wxGetApp().plater()->get_view3D_canvas3D()->draw_input_int("##radius_input", &radius, 1, &v_min,
                                                                                     &v_max, Vec2d(input_box_width, 30.f));
    if (radius_changed)
    {
        radius = std::clamp(radius, v_min, v_max);
        m_smooth_params.radius = (unsigned int) radius;
...
void GLCanvas3D::LayersEditing::render_profile(const GLCanvas3D& canvas)
{
    ...
    if (!m_profile.baseline.is_initialized() || m_profile.old_layer_height_profile != m_layer_height_profile) {
        ...
        // vertices
        const float axis_x = 2.0f * ((cnv_width - THICKNESS_BAR_WIDTH - canvas._layer_editing_profile_right_margin() + float(m_slicing_parameters->layer_height) * scale_x) * cnv_inv_width - 0.5f);
    ...
    if (!m_profile.profile.is_initialized() || m_profile.old_layer_height_profile != m_layer_height_profile) {
        ...
        for (unsigned int i = 0; i < (unsigned int)m_layer_height_profile.size(); i += 2) {
            init_data.add_vertex(Vec2f(2.0f * ((cnv_width - THICKNESS_BAR_WIDTH - canvas._layer_editing_profile_right_margin() + float(m_layer_height_profile[i + 1]) * scale_x) * cnv_inv_width - 0.5f),
...
Slic3r::GUI::GCodeViewer& GLCanvas3D::get_gcode_viewer()
{
...
bool GLCanvas3D::get_use_clipping_planes() const
{
    return m_use_clipping_planes;
}

const std::array<Slic3r::GUI::ClippingPlane, 2>& GLCanvas3D::get_clipping_planes() const
{
    return m_clipping_planes;
}

void GLCanvas3D::set_use_color_clip_plane(bool use)
{
    m_volumes.set_use_color_clip_plane(use);
}

void GLCanvas3D::set_color_clip_plane(const Vec3d& cp_normal, double offset)
{
    m_volumes.set_color_clip_plane(cp_normal, offset);
}

void GLCanvas3D::set_color_clip_plane_colors(const std::array<ColorRGBA, 2>& colors)
{
...
void GLCanvas3D::render(bool only_init)
{
    ...
    /* preview render */
        ...
        {
            _render_bed(camera.get_view_matrix(), camera.get_projection_matrix(), !camera.is_looking_downward(), show_axes);
            _render_platelist(camera.get_view_matrix(), camera.get_projection_matrix(), !camera.is_looking_downward(), only_current, true, hover_id, false);
        }
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        wxGetApp().plater()->get_notification_manager()
            ->render_notifications(*this, get_overlay_window_width());
...
//#endif
    ...
    else if (evt.Dragging() && evt.LeftIsDown() && !m_rectangle_selection.is_dragging() &&m_hover_volume_idxs.empty() && m_picking_enabled && m_layers_editing.state != LayersEditing::Editing)// start rectangle selection.
    {
		...
		{
			m_rectangle_selection.start_dragging(m_mouse.position, evt.AltDown() ? GLSelectionRectangle::Deselect : GLSelectionRectangle::Select);
			m_dirty = true;
		}
    }
        ...
        if (layer_editing_object_idx != -1 && m_layers_editing.bar_rect_contains(*this, pos(0), pos(1))) {
            ...
            if (evt.LeftDown() && m_moving && m_hover_volume_idxs.empty() && !m_rectangle_selection.is_dragging() && m_picking_enabled && m_layers_editing.state != LayersEditing::Editing) {
...
bool GLCanvas3D::draw_input_double(const std::string& label, double* v, double stride, const Vec2d& size)
{
...

    {
        ...
        draw_list->AddRect(pos, pos_max, IM_COL32(110, 110, 114, 255), 5.0f, ImDrawFlags_RoundCornersAll);
    }
...
//     strcpy(style.AxisLabels[ImGuizmo::Axis::Axis_Z], "x");
    ...
    m_view_manipulate_rect = ImVec4(manipulate_xpos, manipulate_ypos,
                                    manipulate_size, manipulate_size);

    ImGuiWrapper& imgui = *wxGetApp().imgui();
    imgui.push_bold_font();
    const bool dirty = ImGuizmo::ViewManipulate(
    ...
    config.processWindows("home_icon", [&]() {
        //auto size = config.getWindowSize(DispConfig::e_wt_reset, sc);
        if (ImGui::ImageButton3(
            config.getTextureId(DispConfig::e_tt_home,false), config.getTextureId(DispConfig::e_tt_home, true),
                                    home_icon_size)) {
        ...
        if (ImGui::IsItemHovered())
            imgui.tooltip(_utf8("Angle reset").c_str(), 120);
        }, cfg);
}
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    //move gizmo
    {
        ...
        if (obj) 
        {
            ...
            item.sprite_id ++;
            item.left.action_callback = [this]() { 
                m_gizmos.open_gizmo(GLGizmosManager::EType::Move);
            };
            item.enabling_callback    = [obj]() -> bool { 
                return obj->is_activable();
            };
            if (!m_main_toolbar.add_item(item))
                return false;
        }
    ...
    //rotate gizmo
    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() { 
                m_gizmos.open_gizmo(GLGizmosManager::EType::Rotate);
            };
    ...
    //
    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() {
                m_gizmos.open_gizmo(GLGizmosManager::EType::Flatten);
            };
            item.enabling_callback = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
    ...
    //Scale
    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() { 
                m_gizmos.open_gizmo(GLGizmosManager::EType::Scale);
            };
            item.enabling_callback = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
...

    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() {
                m_gizmos.open_gizmo(GLGizmosManager::EType::Cut);
            };
            item.enabling_callback = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
...

    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() { 
                m_gizmos.open_gizmo(GLGizmosManager::EType::Measure);
            };
            item.enabling_callback = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
...

    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() { 
                m_gizmos.open_gizmo(GLGizmosManager::EType::FdmSupports);
            };
            item.enabling_callback = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
...

    {
        ...
        if (obj) {
            ...
            item.sprite_id++;
            item.left.action_callback = [this]() {
                m_gizmos.open_gizmo(GLGizmosManager::EType::Seam);
            };
            item.enabling_callback = [obj]() -> bool { return obj->is_activable(); };
            if (!m_main_toolbar.add_item(item))
                return false;
...
bool GLCanvas3D::_update_imgui_select_plate_toolbar()
{
    ...
    if (!m_sel_plate_toolbar.is_enabled() 
        || m_sel_plate_toolbar.is_render_finish) 
        return false;
...
bool GLCanvas3D::_init_process_toolbar()
{
	return wxGetApp().plater()->init_process_toolbar();
}
...
void GLCanvas3D::_render_platelist(const Transform3d& view_matrix, const Transform3d& projection_matrix, bool bottom, bool only_current, bool only_body, int hover_id,
    bool show_logo)
{
...
void GLCanvas3D::_render_cliper(int canvas_width, int canvas_height){
    ...
    GLGizmosManager::EType gizmo_type = m_gizmos.get_current_type();
    if ((gizmo_type == GLGizmosManager::FdmSupports) || (gizmo_type == GLGizmosManager::Seam) || (gizmo_type == GLGizmosManager::MmuSegmentation))
        return;
...
void GLCanvas3D::_render_slice_control() const
{
    ...
    bigcfg.size   = ImVec2(150 * scale, 35 * scale);

    auto smallcfg = bigcfg;
    smallcfg.fg = DispConfig::e_ct_btnBgSelWhite;
    smallcfg.size = ImVec2(35 * scale, 35 * scale);
    auto config = DispConfig();
    ...
    config.processWindows("Print", [&]() {
        ...
        if (config.normalButton(s_print_string[s_pst], bigcfg, enprint ? 2 : 1)) {
             if (wxGetApp().mainframe) {
                 ...
                 if (s_pst == 1)pst = MainFrame::eExportSlicedFile;
                 else if (s_pst == 2)pst = MainFrame::eExportAllSlicedFile;
                 else if (s_pst == 3)pst = MainFrame::eUploadGcode;
...
void GLCanvas3D::_render_gcode(int canvas_width, int canvas_height)
{
    ...
    if (moves_slider->is_dirty()) {
        ...
        m_gcode_viewer.update_sequential_view_current(
            (moves_slider->GetLowerValueD() - 1.0)
...
void GLCanvas3D::_check_and_update_toolbar_icon_scale()
{
    ...
    //BBS: GUI refactor: GLToolbar
#if BBS_TOOLBAR_ON_TOP
    float top_tb_width = m_main_toolbar.get_width();// +m_gizmos.get_scaled_total_width() + m_assemble_view_toolbar.get_width() + m_separator_toolbar.get_width();
    int   items_cnt = m_main_toolbar.get_visible_items_cnt() + m_gizmos.get_selectable_icons_cnt() + m_assemble_view_toolbar.get_visible_items_cnt() + m_separator_toolbar.get_visible_items_cnt();
...
void GLCanvas3D::_render_main_toolbar()
{
    ...
    float start_x = get_main_toolbar_offset();
    float start_y = 0.0f;
    //To the right
    ...
    float space_width = cnv_size.get_width() - get_main_toolbar_offset();
    ...
    if (toolbar_width > space_width) {
        ...
        const ImVec2     arrow_button_size  = ImVec2(0.421f * toolbar_height, toolbar_height);
    ...
    if (tab_print != nullptr) {
        ...
        for (size_t i = 0; i < cnt; i++) {
            ...
            if (item && !item->is_separator()) {
                ...
                std::string new_icon_file = ProcessBar::GLToolbar::get_process_toolbar_item_icon_file(name, m_is_dark, is_modified);
...
float GLCanvas3D::_layer_editing_profile_right_margin() const 
{
    return wxGetApp().plater()->get_process_toolbar().get_width() + 1.0f;
}
