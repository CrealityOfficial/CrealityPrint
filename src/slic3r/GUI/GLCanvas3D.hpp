...
#include "libslic3r/OverhangOptimization.hpp"
...
{
public:
    RenderTimerEvent(wxEventType type, wxTimer& timer) : wxEvent(timer.GetId(), type), m_timer(&timer) { SetEventObject(timer.GetOwner()); }
...
{
public:
    ToolbarHighlighterTimerEvent(wxEventType type, wxTimer& timer) : wxEvent(timer.GetId(), type), m_timer(&timer)
    {
...
{
public:
    GizmoHighlighterTimerEvent(wxEventType type, wxTimer& timer) : wxEvent(timer.GetId(), type), m_timer(&timer)
    {
        SetEventObject(timer.GetOwner());
    }
    int      GetInterval() const { return m_timer->GetInterval(); }
...
class GLCanvas3D
{
    ...
    {
    public:
        enum EState : unsigned char { Unknown, Editing, Completed, Num_States };
    ...
    public:
        ...
        void set_normal_angle(float angle_in_deg) const { m_volumes.set_slope_normal_z(-::cos(Geometry::deg2rad(90.0f - angle_in_deg))); }
    };
...
private:
    ...
    ArrangeSettings m_arrange_settings_fff, m_arrange_settings_sla, m_arrange_settings_fff_seq_print;

    PrinterTechnology current_printer_technology() const;

    ImVec2 m_printer_objects_panel_pos;
    ImVec2 m_printer_objects_panel_size;

    ImVec4 m_view_manipulate_rect;
...
public:
    ...
    GLToolbar&       getToolBar() { return m_main_toolbar; };
    wxRect     getSlicerBtnRec() { return m_SlicerBtnRec; };
    wxRect           getSenderBtnRec() { return m_SenderBtnRec; };
    ...
    {
        return (print_seq == PrintSequence::ByObject) ? m_arrange_settings_fff_seq_print : m_arrange_settings_fff;
    }
...
public:
    ...
    std::shared_ptr<SceneRaycasterItem> add_raycaster_for_picking(SceneRaycaster::EType type,
                                                                  int                   id,
    ...
    void remove_raycasters_for_picking(SceneRaycaster::EType type, int id) { m_scene_raycaster.remove_raycasters(type, id); }
    ...
    bool                           is_all_plates_selected()
    {
        return m_sel_plate_toolbar.m_all_plates_stats_item && m_sel_plate_toolbar.m_all_plates_stats_item->selected;
    }
    ...
    void toggle_model_objects_visibility(bool               visible,
    ...
    const std::array<ClippingPlane, 2>& get_clipping_planes() const;
    void                                reset_clipping_planes_cache();

    void set_use_clipping_planes(bool use);
    bool get_use_clipping_planes() const;
    ...
    void render_on_image(wxImage& image);
    bool is_rendering_enabled() { return m_enable_render; }
    void enable_render(bool enabled) { m_enable_render = enabled; }
    ...
    void update_plate_thumbnails(bool force = false);
    ...
    // yy
    void render_hollow_anime(float value);
    ...
    ArrangeSettings get_arrange_settings() const
    {
        ...
        if (&settings == &m_arrange_settings_fff_seq_print) {
            ret.distance = std::max(ret.distance, float(min_object_distance(*m_config)));
        }
    ...
    void set_sequential_print_clearance_visible(bool visible) { m_sequential_print_clearance.set_visible(visible); }

    void set_sequential_print_clearance_render_fill(bool render_fill) { m_sequential_print_clearance.set_render_fill(render_fill); }
    ...
    ImVec2 get_printer_objects_panel_pos() { return m_printer_objects_panel_pos; }
...
private:
    ...
    // BBS: add part plate related logic
    void _render_platelist(const Transform3d& view_matrix,
    ...
    void _render_cliper(int canvas_width, int canvas_height);
    ...
    void _render_separators() const;
    void _render_collapse_toolbar() const;
    void _render_process_toolbar() const;
    void _update_process_toolbar_item_icons() const;
    // BBS
    // void _render_view_toolbar() const;
    void _render_paint_toolbar() const;
    void _render_slice_control() const;
    ...
    void render_card_popup_if_open(
        ...
        const std::function<void()>& content_fn       // ֻ���ڲ����ݣ����� Begin/End��
    );
    // ==== easy print mode related ====
    ...
    void _load_print_object_toolpaths(const PrintObject&                    print_object,
    ...
    void _render_printer_preset_and_obj_list();
    bool _render_global_objects_switch_button();
