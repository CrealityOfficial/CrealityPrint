...
static constexpr const char* DEFAULT_VIRTUAL_FILAMENT_BASIC_TYPE = "Hyper PLA";
static constexpr const char* DEFAULT_VIRTUAL_FILAMENT_SHORT_TYPE = "PLA";
static constexpr const char* DEFAULT_VIRTUAL_FILAMENT_NAME       = "Hyper PLA";
...
public:
        ...
        , m_display_numbers(std::move(display_numbers))
    {
        ...
        int       pop_w   = std::max(FromDIP(213), popup_width);
        ...
        // The action row sits between the mixed sections and "New Physical
        // Filaments", so both labels must live inside m_content and be added to
        // `outer` (the scrolled sizer) rather than to top_sizer.
        // wxStaticText cannot paint a border, so each label is wrapped in a panel
        // that draws the rounded border itself.
        const wxColour action_bd = dark_or(wxColour(0xA6, 0xAC, 0xB4), wxColour(0x50, 0x50, 0x52));
        auto make_action_box = [&]() {
            auto* box = new wxPanel(m_content, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
            box->SetBackgroundColour(pop_bg);
            box->SetBackgroundStyle(wxBG_STYLE_PAINT);
            box->Bind(wxEVT_PAINT, [box, pop_bg, action_bd](wxPaintEvent&) {
                wxAutoBufferedPaintDC dc(box);
                const wxSize sz = box->GetClientSize();
                dc.SetBrush(wxBrush(pop_bg));
                dc.SetPen(*wxTRANSPARENT_PEN);
                dc.DrawRectangle(0, 0, sz.x, sz.y);
                dc.SetBrush(*wxTRANSPARENT_BRUSH);
                dc.SetPen(wxPen(action_bd, 1));
                dc.DrawRoundedRectangle(0, 0, sz.x, sz.y, box->FromDIP(4));
            });
            return box;
        };

        wxPanel* decompose_box = make_action_box();
        wxPanel* add_box       = make_action_box();

        auto* decompose_label = new wxStaticText(decompose_box, wxID_ANY, _L("Decompose Color"));
        auto* add_label = new wxStaticText(add_box, wxID_ANY, _L("+ Add Material"));
        //wxFont af = add_label->GetFont();
        //af.SetPointSize(10);
        //add_label->SetFont(af);
        add_label->SetFont(::Label::Body_12);
        //decompose_label->SetFont(af);
        decompose_label->SetFont(::Label::Body_12);
        ...
        const wxCursor action_cursor(add_enabled ? wxCURSOR_HAND : wxCURSOR_ARROW);
        add_label->SetCursor(action_cursor);
        decompose_label->SetCursor(action_cursor);
        add_box->SetCursor(action_cursor);
        decompose_box->SetCursor(action_cursor);

        // Center each label inside its bordered box.
        auto center_in_box = [&](wxPanel* box, wxStaticText* label) {
            auto* box_sizer = new wxBoxSizer(wxVERTICAL);
            box_sizer->Add(label, 0, wxALIGN_CENTER_HORIZONTAL | wxTOP | wxBOTTOM, FromDIP(5));
            box->SetSizerAndFit(box_sizer);
        };
        center_in_box(decompose_box, decompose_label);
        center_in_box(add_box, add_label);

        if (!add_enabled) {
            const wxString add_tip = wxString::Format(
                _L("The project supports up to %d filaments. Extra filaments will be discarded."),
                (int)16/*EnforcerBlockerType::ExtruderMax*/);
            add_label->SetToolTip(add_tip);
            add_box->SetToolTip(add_tip);
        }
        auto on_decompose_click = [this](wxMouseEvent&) {
        ...
        auto on_add_click = [this](wxMouseEvent&) {
            if (m_can_add_filament && !m_can_add_filament()) {
                return;
            }
        ...
        // Bind on the wrapper too, so clicks on the padding inside the border work.
        decompose_label->Bind(wxEVT_LEFT_DOWN, on_decompose_click);
        decompose_box->Bind(wxEVT_LEFT_DOWN, on_decompose_click);
        add_label->Bind(wxEVT_LEFT_DOWN, on_add_click);
        add_box->Bind(wxEVT_LEFT_DOWN, on_add_click);

        // Height of the action row block, excluded from the row-count clamp below.
        int action_row_h = 0;
        auto add_action_row = [&]() {
            auto* top_line = new StaticLine(m_content);
            top_line->SetLineColour(texture_import_separator_colour());
            outer->Add(top_line, 0, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, pad);

            // Both actions share one horizontal row, each in its own bordered box.
            auto* action_sizer = new wxBoxSizer(wxHORIZONTAL);
            action_sizer->Add(decompose_box, 1, wxEXPAND);
            action_sizer->AddSpacer(FromDIP(6));
            action_sizer->Add(add_box, 1, wxEXPAND);
            outer->Add(action_sizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxTOP | wxBOTTOM, pad);

            action_row_h = top_line->GetMinSize().y + pad
                         + action_sizer->GetMinSize().y + 2 * pad;
        };

        // Display order requested by UX: project filaments, mixed filaments, the
        // action row, then "New Physical Filaments" last.
        // NOTE: this no longer matches compute_display_numbers(), which still
        // assigns IDs in post-apply sidebar order (ExistingPhysical -> NewPhysical
        // -> ExistingMixed -> NewMixed). Each entry keeps its correct number, but
        // the numbers no longer ascend monotonically down the popup.
        ...
        // The action row (two labels side by side) can be wider than a list row,
        // so widen before pinning m_content's size, otherwise the labels clip.
        pop_w = std::max(pop_w, outer->GetMinSize().x);
        // outer now also contains the action row, so the row-count clamp must
        // only shrink the list portion. The action row's height is always kept,
        // otherwise the pinned max height cuts off whatever sits directly above
        // it (the mixed-filament sections).
        const int content_h = outer->GetMinSize().y;
        int list_h = content_h;
        if (m_colors_rgba.size() > max_visible_rows) {
            const int overflow = ((int)m_colors_rgba.size() - max_visible_rows) * row_h;
            list_h = std::max(action_row_h + row_h, content_h - overflow);
        }
...
private:
    ...
    wxPanel* create_item_row(size_t idx, int row_h)
    {
        ...
        const int sq     = row->FromDIP(20);
        const int sq_r   = row->FromDIP(0);
...
void TexturePreviewCanvas::ensure_gl_ready()
{
    ...
    if (version == 0) {
        BOOST_LOG_TRIVIAL(error) << "TexturePreviewCanvas: failed to init GLAD OpenGL loader";
        return;
    }
...
void TexturePreviewCanvas::render()
{
    ...
    if (is_dark())
        glClearColor(0.192f, 0.192f, 0.192f, 1.0f);
    else
...
{
    SetSize(wxSize(FromDIP(892), FromDIP(692)));
...
void TextureImportDialog::build_ui()
{
    ...
    top_sizer->Add(left_sizer, 3, wxEXPAND | wxALL, FromDIP(0));

    wxBoxSizer* right_sizer = new wxBoxSizer(wxVERTICAL);
    build_params_panel(this, right_sizer);
    build_mapping_panel(this, right_sizer);
    top_sizer->Add(right_sizer, 2, wxEXPAND | wxLEFT, FromDIP(8));
    main_sizer->Add(top_sizer, 4, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, FromDIP(8));

    wxBoxSizer* bottom_sizer = new wxBoxSizer(wxVERTICAL);
    build_bottom_buttons(bottom_sizer);
    main_sizer->Add(bottom_sizer, 0, wxEXPAND | wxALL, FromDIP(8));
...
{
    wxColour preview_bg = dark_or(wxColour(238, 238, 238), wxColour(0x31, 0x31, 0x31));
    ...
    m_btn_view_original   = new Button(m_tab_panel, _L("Original Texture"));
    ...
    auto update_preview_overlay_buttons = [this, preview_container]() {
        if (m_tab_panel) {
            m_tab_panel->Fit();
            // m_tab_panel is a manually positioned overlay (not managed by a sizer),
            // so align it to the right edge of the preview area instead of relying
            // on a stretch spacer inside its own sizer.
            const int margin = FromDIP(8);
            const int x = std::max(margin, preview_container->GetClientSize().x - m_tab_panel->GetSize().x - margin);
            m_tab_panel->SetPosition(wxPoint(x, margin));
            m_tab_panel->Raise();
        }
    };
...
{
    wxSizer* outer_sizer = sizer;
    wxColour panel_bg = dark_or(wxColour(255, 255, 255), wxColour(0x31, 0x31, 0x31));
    wxColour panel_bd = dark_or(wxColour(0xD0, 0xD4, 0xDE), wxColour(0x50, 0x50, 0x52));

    wxPanel* panel = new wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
    wxSize mapping_panel_size(FromDIP(390), FromDIP(152));
    panel->SetMinSize(mapping_panel_size);
    panel->SetMaxSize(mapping_panel_size);
    panel->SetBackgroundColour(panel_bg);
    panel->SetBackgroundStyle(wxBG_STYLE_PAINT);
    panel->Bind(wxEVT_SIZE, [panel](wxSizeEvent& evt) {
        evt.Skip();
        panel->Refresh();
    });
    panel->Bind(wxEVT_PAINT, [panel_bg, panel_bd](wxPaintEvent& evt) {
        auto* p = static_cast<wxPanel*>(evt.GetEventObject());
        wxAutoBufferedPaintDC dc(p);
        wxSize sz = p->GetClientSize();
        dc.SetPen(*wxTRANSPARENT_PEN);
        dc.SetBrush(wxBrush(panel_bg));
        dc.DrawRectangle(0, 0, sz.x, sz.y);
        dc.SetPen(wxPen(panel_bd, 1));
        dc.DrawRoundedRectangle(0, 0, sz.x, sz.y, 4);
    });
    ...
    lbl_colors->SetFont(::Label::Body_14);
    color_header_sizer->Add(lbl_colors, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
    wxStaticText* lbl_tip = new wxStaticText(parent, wxID_ANY, "(<=64)");
    lbl_tip->SetForegroundColour(wxColour(0xFF, 0x6F, 0x00));
    lbl_tip->SetFont(::Label::Body_12);
    color_header_sizer->Add(lbl_tip, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
...

    {
        ...
        for (auto* btn : {m_btn_color_4, m_btn_color_8, m_btn_color_16}) {
            ...
            btn->SetMinSize(wxSize(FromDIP(30), FromDIP(28)));
    ...
    color_header_sizer->Add(m_btn_color_4,  0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
    color_header_sizer->Add(m_btn_color_8,  0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
    ...
    m_color_spin->SetFont(::Label::Body_14);

    //color_slider_sizer->Add(m_color_slider, 1, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(4));
    color_header_sizer->Add(m_color_spin, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, FromDIP(10));
    //sizer->Add(color_slider_sizer, 0, wxEXPAND | wxBOTTOM, FromDIP(8));
    sizer->Add(color_header_sizer, 0, wxEXPAND | wxBOTTOM, FromDIP(14));
    ...
    lbl_smooth->SetFont(::Label::Body_14);
    smooth_sizer->Add(lbl_smooth, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
    lbl_tip = new wxStaticText(parent, wxID_ANY, "(<=10)");
    lbl_tip->SetForegroundColour(wxColour(0xFF, 0x6F, 0x00));
    lbl_tip->SetFont(::Label::Body_12);
    smooth_sizer->Add(lbl_tip, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(8));
    ...
    sizer->Add(smooth_sizer, 0, wxEXPAND | wxBOTTOM, FromDIP(14));
...

    {
        ...
        m_btn_color_auto->SetCornerRadius(FromDIP(4));
        m_btn_color_auto->SetMinSize(wxSize(FromDIP(173), FromDIP(32)));
        ...
        m_btn_apply->SetCornerRadius(FromDIP(4));
        m_btn_apply->SetMinSize(wxSize(FromDIP(173), FromDIP(32)));
    ...
    apply_sizer->Add(m_btn_color_auto, 0, wxRIGHT, FromDIP(12));
    apply_sizer->Add(m_btn_apply, 0);
    apply_sizer->AddStretchSpacer();
    sizer->Add(apply_sizer, 0, wxEXPAND | wxALIGN_RIGHT | wxBOTTOM, FromDIP(2));
    ...
    m_hint_label->Hide();
    sizer->Add(m_hint_label, 0, wxBOTTOM, FromDIP(2));

    auto* panel_sizer = new wxBoxSizer(wxVERTICAL);
    panel_sizer->Add(content_sizer, 1, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, FromDIP(12));
    panel->SetSizer(panel_sizer);
    outer_sizer->Add(panel, 0, wxEXPAND | wxBOTTOM, FromDIP(8));
}
...
void TextureImportDialog::build_mapping_panel(wxWindow* parent, wxSizer* sizer)
{
    ...
    wxPanel* panel = new wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
    wxSize mapping_panel_size(FromDIP(390), -1);
    panel->SetMinSize(mapping_panel_size);
    panel->SetMaxSize(mapping_panel_size);
    panel->SetBackgroundColour(panel_bg);
    panel->SetBackgroundStyle(wxBG_STYLE_PAINT);
    ...
    lbl_mapping->SetFont(::Label::Body_14);
    ...
    m_combo_auto_mix = new ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                  wxSize(FromDIP(178), FromDIP(28)), 0, nullptr, wxCB_READONLY);
    m_combo_auto_mix->SetToolTip(_L("Choose the one-click auto-mix mode for texture color import"));
    wxFont btn_font = m_combo_auto_mix->GetFont();
    btn_font.SetPointSize(m_auto_mix_font_point_size);
    m_combo_auto_mix->SetFont(::Label::Body_12);
    m_combo_auto_mix->Append(auto_mix_mode_label(TextureAutoMixMode::CMYW));
    m_combo_auto_mix->Append(auto_mix_mode_label(TextureAutoMixMode::RYBW));
    m_combo_auto_mix->SetSelection(m_auto_mix_mode == TextureAutoMixMode::CMYW ? 0 : 1);
    {
        StateColor btn_bg(
            std::pair<wxColour, int>(dark_or(wxColour(245, 245, 245), wxColour(0x3C, 0x3C, 0x42)), StateColor::Pressed),
            std::pair<wxColour, int>(dark_or(wxColour(248, 248, 248), wxColour(0x35, 0x35, 0x3A)), StateColor::Hovered),
        ...
        m_combo_auto_mix->SetBackgroundColor(btn_bg);
        m_combo_auto_mix->SetBorderColor(btn_bd);
        m_combo_auto_mix->SetLabelColor(btn_text);
        m_combo_auto_mix->SetTextColor(btn_text);
        m_combo_auto_mix->SetCornerRadius(FromDIP(5));
        auto& drop = m_combo_auto_mix->GetDropDown();
        drop.SetFont(::Label::Body_12);
        drop.SetCornerRadius(FromDIP(5));
        drop.SetUseContentWidth(true, true);
        drop.SetMaxVisibleItems(2);
        drop.SetBackgroundColour(dark_or(*wxWHITE, wxColour(0x2D, 0x2D, 0x31)));
        drop.SetBorderColor(StateColor(
            std::pair<wxColour, int>(dark_or(wxColour(206, 206, 206), wxColour(0x54, 0x54, 0x5B)), StateColor::Normal)));
        drop.SetTextColor(StateColor(std::pair<wxColour, int>(texture_import_text_colour(), StateColor::Normal)));
        drop.SetSelectorBorderColor(StateColor(
            std::pair<wxColour, int>(wxColour(0x15, 0xBF, 0x59), StateColor::Hovered),
            std::pair<wxColour, int>(dark_or(*wxWHITE, wxColour(0x2D, 0x2D, 0x31)), StateColor::Normal)));
        drop.SetSelectorBackgroundColor(StateColor(
            std::pair<wxColour, int>(wxColour(0x1F, 0xCA, 0x63), StateColor::Checked),
            std::pair<wxColour, int>(dark_or(*wxWHITE, wxColour(0x2D, 0x2D, 0x31)), StateColor::Normal)));
    }
    m_combo_auto_mix->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& evt) {
        if (evt.GetSelection() == 0)
            set_auto_mix_mode(TextureAutoMixMode::CMYW);
        else if (evt.GetSelection() == 1)
            set_auto_mix_mode(TextureAutoMixMode::RYBW);
        evt.Skip();
    });
    header_sizer->Add(m_combo_auto_mix, 0, wxALIGN_CENTER_VERTICAL);
    ...
    m_mapping_scroll->SetBackgroundColour(panel_bg);
    ...
    m_drop_warning_label =
        new wxStaticText(parent, wxID_ANY,
                         wxString::Format(_L("The project supports up to %d filaments. Extra filaments will be discarded."),
                                          (int) max_filament_count()));
    m_drop_warning_label->SetForegroundColour(wxColour(0xFF, 0x6F, 0x00));
    m_drop_warning_label->SetFont(texture_import_section_title_font(parent));
    m_drop_warning_label->Hide();
    sizer->Add(m_drop_warning_label, 0, wxALIGN_LEFT | wxBOTTOM, FromDIP(4));

    auto* panel_sizer = new wxBoxSizer(wxVERTICAL);
    panel_sizer->Add(content_sizer, 1, wxEXPAND | wxALL, FromDIP(12));
    panel->SetSizer(panel_sizer);
    outer_sizer->Add(panel, 1, wxEXPAND);
}

void TextureImportDialog::build_bottom_buttons(wxBoxSizer* sizer)
{
    ...
    m_btn_skip->SetCornerRadius(FromDIP(5));
    ...
    m_btn_ok->SetCornerRadius(FromDIP(5));
...
void TextureImportDialog::update_ui_for_state()
{
    ...
    // The computation is guarded by a modal progress dialog. Keep the spin
    // controls in their current visual state to avoid a disable/enable repaint
    // becoming visible when Apply starts and finishes.
    ...
    if (m_combo_auto_mix)
        m_combo_auto_mix->Enable(!computing);
    if (m_btn_mix_reset)
    ...
    // Only the hint label's visibility affects layout here; everything else just
    // toggles enable state or button colours. Track it so the dialog is only
    // re-laid-out when it actually changes, instead of on every state switch
    // (which made the spin controls flicker on every Apply click).
    const bool hint_was_shown = m_hint_label && m_hint_label->IsShown();
    ...
    // Re-layout only when the hint label appeared/disappeared, and scope it to
    // that label's parent. A full dialog repaint on each state change was
    // enough to make the spin controls visibly flash during Apply.
    const bool hint_is_shown = m_hint_label && m_hint_label->IsShown();
    if (hint_is_shown != hint_was_shown) {
        wxWindow* hint_parent = m_hint_label->GetParent();
        if (hint_parent) {
            hint_parent->Layout();
        } else {
            Layout();
...
void TextureImportDialog::on_computation_complete(wxCommandEvent&)
{
    ...
    const bool multicolor_was_shown = m_btn_view_multicolor->IsShown();
    if (!multicolor_was_shown)
        m_btn_view_multicolor->Show();
    if (!multicolor_was_shown && m_tab_panel) {
        m_tab_panel->GetSizer()->Layout();
        m_tab_panel->Fit();
        // Keep the overlay right-aligned after its width changed.
        const int margin = FromDIP(8);
        const int x = std::max(margin, m_tab_panel->GetParent()->GetClientSize().x - m_tab_panel->GetSize().x - margin);
        m_tab_panel->SetPosition(wxPoint(x, margin));
    }
...
{
    return static_cast<size_t>(64/*EnforcerBlockerType::ExtruderMax*/);
}
...
void TextureImportDialog::set_auto_mix_mode(TextureAutoMixMode mode)
{
    ...
    if (m_combo_auto_mix) {
        m_combo_auto_mix->SetSelection(mode == TextureAutoMixMode::CMYW ? 0 : 1);
    }
...
bool TextureImportDialog::add_decomposed_mixed_filament(size_t row_index)
{
    ...
    for (const auto& entry : m_filament_entries) {
        ...
        std::string ftype = filament_type_for_color_decompose(preset);
        // filament_type_for_color_decompose() only special-cases official Bambu
        // Basic presets; everything else falls back to the raw filament_type, so
        // "HP-TPU" collapses to "TPU" and "Hyper PLA" to "PLA". The dialog shows
        // these strings in its "Project Filament List" group, so prefer the
        // filament's own display name and only keep the generic type as fallback.
        if (!entry.name.empty())
            ftype = entry.name;
        else if (ftype.empty())
            ftype = kDecomposePlaShortType;
        physical_types.push_back(ftype);
...
void TextureImportDialog::rebuild_mapping_rows()
{
    ...
    // dark_or(light, dark): #EFF0F6 in light mode, #2B2B2B in dark mode.
    const wxColour src_bg     = dark_or(wxColour("#EFF0F6"), wxColour("#2B2B2B"));
    ...
    for (size_t ci = 0; ci < m_current_matches.size(); ++ci) {
        ...
        const int src_w = FromDIP(106);
        const int target_min_w = FromDIP(236);
        ...
        row.source_panel->SetBackgroundColour(src_bg);

        row.source_panel->Bind(wxEVT_PAINT, [this, ci, src_wx_color, src_bg, hex_fg](wxPaintEvent& e) {
            auto* p = static_cast<wxPanel*>(e.GetEventObject());
            wxAutoBufferedPaintDC dc(p);
            wxSize sz = p->GetClientSize();

            // Clear with the row background so the rounded corners blend in.
            dc.SetBrush(wxBrush(p->GetParent()->GetBackgroundColour()));
            dc.SetPen(*wxTRANSPARENT_PEN);
            dc.DrawRectangle(0, 0, sz.x, sz.y);

            // Filled card, no border.
            dc.SetBrush(wxBrush(src_bg));
            dc.SetPen(*wxTRANSPARENT_PEN);
            int r = p->FromDIP(8);
        ...
        row.target_panel->Bind(wxEVT_PAINT, [this, ci, get_target_wxcolor, get_filament_label,
            ...
            if (fil_idx >= 0 && fil_idx < (int)m_filament_entries.size() &&
                ...
                int x = p->FromDIP(6);
                const int sw = p->FromDIP(20);
                const int sw_r = p->FromDIP(4);
            ...
            // Numbered color square 20x20, rounded 4px
            int sq = p->FromDIP(20);
            int sq_x = p->FromDIP(8);
            int sq_y = (sz.y - sq) / 2;
            int sq_r = p->FromDIP(4);
...
void TextureImportDialog::set_color_count_value(int value, bool update_spin)
{
    ...
    //m_color_slider->SetValue(m_param_color_count);
    if (update_spin && m_color_spin->GetValue() != m_param_color_count)
        m_color_spin->SetValue(m_param_color_count);
    update_color_count_preset_buttons();
...
void TextureImportDialog::set_smooth_value(int value, bool update_spin)
{
    ...
    //m_smooth_slider->SetValue(m_param_smooth);
    if (update_spin && m_smooth_spin->GetValue() != m_param_smooth)
        m_smooth_spin->SetValue(m_param_smooth);
    update_confirm_button_state();
}
...
void TextureImportDialog::update_drop_warning_visibility()
{
    ...
    if (wxWindow* parent = m_drop_warning_label->GetParent())
        parent->Layout();
    else
        Layout();
}
...
void TextureImportDialog::update_confirm_button_state()
{
    ...
    // Same rationale as update_ui_for_state(): only the hint label changes the
    // layout, so avoid a full-dialog Layout() on every call.
    const bool hint_was_shown = m_hint_label && m_hint_label->IsShown();
    auto relayout_hint_if_changed = [this, hint_was_shown]() {
        const bool hint_is_shown = m_hint_label && m_hint_label->IsShown();
        if (hint_is_shown == hint_was_shown)
            return;
        if (wxWindow* hint_parent = m_hint_label->GetParent()) {
            hint_parent->Layout();
        } else {
            Layout();
        }
...
void TextureImportDialog::on_dpi_changed(const wxRect&)
{
    ...
    if (m_btn_color_auto) {
        m_btn_color_auto->SetCornerRadius(FromDIP(5));
        m_btn_color_auto->SetMinSize(wxSize(FromDIP(60), FromDIP(28)));
    }
    if (m_btn_apply) {
        m_btn_apply->SetCornerRadius(FromDIP(5));
        m_btn_apply->SetMinSize(wxSize(FromDIP(60), FromDIP(28)));
    }
    if (m_combo_auto_mix) {
        m_combo_auto_mix->SetCornerRadius(FromDIP(5));
        m_combo_auto_mix->SetMinSize(wxSize(FromDIP(178), FromDIP(28)));
    }
    if (m_btn_mix_reset)
        m_btn_mix_reset->SetPaddingSize(wxSize(FromDIP(2), FromDIP(2)));
