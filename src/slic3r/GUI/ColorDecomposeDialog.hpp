...
private:
    ...
    // Last valid (non-title) selection index in m_type_combo. Used to roll
    // back the visual selection when the user clicks on a group-header item
    // ("Project Filament List" / "Standard Mode Recommendations") which is
    // intentionally not a selectable option.
    int                         m_last_valid_sel{-1};
