...
bool ImGui::BBLButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (hovered || g.ActiveId == id|| pressed) {
    ...
    PopStyleColor(push_color_count);
...
bool ImGui::ArrowButtonEX_CP(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{ 
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (dir == ImGuiDir_Up || dir == ImGuiDir_Down) {
        float  width   = size.x * 0.7;
        float  height  = width * 0.5f;
        ImVec2 center  = bb.Min + size * 0.5;
        ImVec2 new_min = center - ImVec2(width * 0.5, height * 0.5);
        ImVec2 new_max = center + ImVec2(width * 0.5, height * 0.5);

        CPRenderArrow(window->DrawList, new_min, new_max, text_col, dir);
    } else {
        //to be continue...
        CPRenderArrow(window->DrawList, bb.Min, bb.Max, text_col, dir);
    }
...
{ 
    ...
    if (has_window_size_constraint) {
        ...
        if (flags & ImGuiComboFlags_HeightRegular)
            ...
            popup_max_height_in_items = 20;
        SetNextWindowSizeConstraints(ImVec2(w - arrow_size * 2, 0.0f),
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive) {
            ...
            if (flags & ImGuiComboFlags_PopupAlignLeft)
                ...
                popup_window->AutoPosLastDirection = ImGuiDir_Down; // "Below, Toward Right (default)"
            ImRect r_outer = GetWindowAllowedExtentRect(popup_window);
            ImVec2 pos     = FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer,
...
{
    ImGui::PushStyleColor(ImGuiCol_BorderActive, RGBA_CREALITY);
...
bool ImGui::BBLSliderScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max, const char *format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        //window->DrawList->AddCircleFilled(p1 - ImVec2(0.0f, 2.0f), 8.50f, GetColorU32(ImGuiCol_SliderGrabActive) * 0.7);
        window->DrawList->AddCircleFilled(p1 - ImVec2(0.0f, 2.0f), 7.0f, GetColorU32(ImGuiCol_SliderGrabActive));
...
bool ImGui::CPSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // RenderTextClipped(text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    RenderTextClippedEx(window->DrawList, text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    if (flags & ImGuiSelectableFlags_Disabled)
        PopStyleColor();
